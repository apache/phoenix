...
/**
 * A default implementation of the Statistics tracker that helps to collect stats like min key, max
 * key and guideposts.
 */
public class DefaultStatisticsCollector implements StatisticsCollector {
  ...
  final Map<ImmutableBytesPtr, Pair<Long, GuidePostsInfoBuilder>> guidePostsInfoWriterMap =
    Maps.newHashMap();
  ...
  public DefaultStatisticsCollector(Configuration configuration, Region region, String tableName,
    byte[] family, byte[] gp_width_bytes, byte[] gp_per_region_bytes, StatisticsWriter statsWriter,
    Table htable) {
  ...
  public void init() throws IOException {
    ...
    LOGGER.info("Initialization complete for " + this.getClass()
      + " statistics collector for table " + tableName);
  }

  /**
   * Determine the GPW for statistics collection for the table. The order of priority from highest
   * to lowest is as follows 1. Value provided in UPDATE STATISTICS SQL statement (N/A for MR jobs)
   * 2. GPW column in SYSTEM.CATALOG for the table is not null Inherits the value from base table
   * for views and indexes (PHOENIX-4332) 3. Value from global configuration parameters from
   * hbase-site.xml GPW of 0 disables the stats collection. If stats were previously collected, this
   * task would attempt to delete entries from SYSTEM.STATS table. Not reading '0' from
   * SYSTEM.CATALOG would mean the fall back to global value which is defaulted to
   * DEFAULT_STATS_GUIDEPOST_PER_REGION
   */
    ...
    if (guidePostPerRegionBytes != null || guidePostWidthBytes != null) {
      ...
      if (guidepostWidth >= 0) {
        ...
        this.guidePostDepth = StatisticsUtil.getGuidePostDepth(
          configuration.getInt(QueryServices.STATS_GUIDEPOST_PER_REGION_ATTRIB,
            QueryServicesOptions.DEFAULT_STATS_GUIDEPOST_PER_REGION),
          configuration.getLongBytes(QueryServices.STATS_GUIDEPOST_WIDTH_BYTES_ATTRIB,
  ...
  private long getGuidePostDepthFromSystemCatalog() throws IOException, SQLException {
    try {
      ...
      if (!result.isEmpty()) {
        ...
        /*
         * The table we are collecting stats for is potentially a base table, or local index or a
         * global index. For view indexes, we rely on the the guide post width column in the parent
         * data table's metadata which we already tried retrieving above.
         */
        try (Connection conn = QueryUtil.getConnectionOnServer(configuration)) {
          ...
          if (table.getType() == PTableType.INDEX && IndexUtil.isGlobalIndex(table)) {
            /*
             * For global indexes, we need to get the parentName first and then fetch guide post
             * width configured for the parent table.
             */
  ...
  private void getGuidePostDepthFromStatement() {
    ...
    if (guidePostPerRegionBytes != null) {
      guidepostPerRegion =
        PInteger.INSTANCE.getCodec().decodeInt(guidePostPerRegionBytes, 0, SortOrder.getDefault());
    }
    if (guidePostWidthBytes != null) {
      guidepostWidth =
        PLong.INSTANCE.getCodec().decodeInt(guidePostWidthBytes, 0, SortOrder.getDefault());
    }
  ...
  private void writeStatistics(final Region region, boolean delete, List<Mutation> mutations,
    long currentTime, Scan scan) throws IOException {
    ...
    if (cachedGuidePosts == null) {
      ...
      for (Store store : region.getStores()) {
        ...
        if (!guidePostsInfoWriterMap.containsKey(cfKey)) {
          Pair<Long, GuidePostsInfoBuilder> emptyGps =
            new Pair<Long, GuidePostsInfoBuilder>(0l, new GuidePostsInfoBuilder());
  ...
  public void collectStatistics(final List<Cell> results) {
    ...
    for (Cell cell : results) {
      ...
      if (byteCount >= guidePostDepth) {
        if (
          gps.getSecond().addGuidePostOnCollection(row, byteCount, gps.getSecond().getRowCount())
        ) {
          gps.setFirst(0l);
  ...
  public InternalScanner createCompactionScanner(RegionCoprocessorEnvironment env, Store store,
    ...
    LOGGER.info(
      "StatisticsScanner created for table: " + tableName + " CF: " + store.getColumnFamilyName());
