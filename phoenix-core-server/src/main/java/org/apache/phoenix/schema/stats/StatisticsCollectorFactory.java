...
/**
 * Provides new {@link StatisticsCollector} instances based on configuration settings for a table
 * (or system-wide configuration of statistics).
 */
public class StatisticsCollectorFactory {
  ...
  public static StatisticsCollector createStatisticsCollector(RegionCoprocessorEnvironment env,
    ...
    return createStatisticsCollector(env, tableName, clientTimeStamp, null, guidepostWidthBytes,
      guidepostsPerRegionBytes);
  }

  public static StatisticsCollector createStatisticsCollector(RegionCoprocessorEnvironment env,
    String tableName, long clientTimeStamp, byte[] storeName) throws IOException {
    return createStatisticsCollector(env, tableName, clientTimeStamp, storeName, null, null);
  }

  public static StatisticsCollector createStatisticsCollector(RegionCoprocessorEnvironment env,
    String tableName, long clientTimeStamp, byte[] storeName, byte[] guidepostWidthBytes,
    byte[] guidepostsPerRegionBytes) throws IOException {
    if (statisticsEnabled(env)) {
      StatisticsWriter statsWriter = StatisticsWriter.newWriter(env, tableName, clientTimeStamp);
      Table table = ConnectionFactory.getConnection(ConnectionType.DEFAULT_SERVER_CONNECTION, env)
        .getTable(SchemaUtil.getPhysicalTableName(PhoenixDatabaseMetaData.SYSTEM_CATALOG_NAME_BYTES,
   ...
   * RegionCoprocessorEnvironment for now to allow setting this on a per-table basis, although it
   * could be moved to the general table metadata in the future if there is a realistic use case for
   * that.
   */
