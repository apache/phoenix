...
public class Transform extends TransformClient {
  ...
  private static void deleteTransformingNewTableLink(PhoenixConnection connection, String tenantId,
    ...
    // Constrain COLUMN_NAME IS NULL so the predicate resolves to the single table-level link row
    // (a link row carries no COLUMN_NAME) instead of range-scanning every column row of the table.
    // COLUMN_NAME is a PK column between TABLE_NAME and COLUMN_FAMILY, so leaving it unbound forces
    // a range scan that matches COLUMN_FAMILY/LINK_TYPE as non-PK filters. An unset PK VARCHAR
    // matches IS NULL, so this is a point delete, not a silent no-op.
    String deleteLink = "DELETE FROM " + PhoenixDatabaseMetaData.SYSTEM_CATALOG_NAME + " WHERE "
      ...
      + PhoenixDatabaseMetaData.TABLE_NAME + " = ? AND " + PhoenixDatabaseMetaData.COLUMN_NAME
      + " IS NULL AND " + PhoenixDatabaseMetaData.COLUMN_FAMILY + " = ? AND "
      + PhoenixDatabaseMetaData.LINK_TYPE + " = ?";
  ...
  public static void updateTransformRecord(PhoenixConnection connection,
    ...
    updateTransformRecord(connection, transformRecord, newStatus, pendingPartialPassUntilTs,
      transformRecord.getCutoverTs());
  }

  public static void updateTransformRecord(PhoenixConnection connection,
    SystemTransformRecord transformRecord, PTable.TransformStatus newStatus,
    Long pendingPartialPassUntilTs, Long cutoverTs) throws SQLException {
