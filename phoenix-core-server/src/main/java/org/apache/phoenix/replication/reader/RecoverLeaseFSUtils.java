...
public class RecoverLeaseFSUtils {
  ...
  /**
   * The lease recovery timeout in milliseconds. Default is 15 minutes. It's huge, but the idea is
   * that if we have a major issue, HDFS usually needs 10 minutes before marking the nodes as dead.
  ...
  /**
   * The first pause before retrying the lease recovery. This setting should be a little above what
  ...
  /**
   * The dfs timeout during lease recovery in milliseconds. This should be set to how long it'll
   * take for us to timeout against primary datanode if it is dead. We set it to 64 seconds, 4
   * seconds than the default READ_TIMEOUT in HDFS, the default value for
   * DFS_CLIENT_SOCKET_TIMEOUT_KEY. If recovery is still failing after this timeout, then further
   * recovery will take liner backoff with this base, to avoid endless preemptions when this value
