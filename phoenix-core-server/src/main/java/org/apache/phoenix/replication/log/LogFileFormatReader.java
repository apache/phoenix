...
public class LogFileFormatReader implements Closeable {
  ...
  public LogFile.Record next() throws IOException {
    while (true) {
      ...
      try {
        ...
        String message = String.format("Encountered corrupt block at offset %d for path: %s",
  ...
  private ByteBuffer readNextBlock() throws IOException {
    while (currentPosition < getEndOfDataOffset()) {
      ...
      try {
        ...
        try {
          input.readFully(payloadBuffer.array(), payloadBuffer.arrayOffset(), payloadSize);
          ...
          if (expectedChecksum != actualChecksum) {
            throw new IOException("Checksum mismatch for block at offset " + blockStartOffset
  ...
  private ByteBuffer decompressBlock(ByteBuffer compressedBuffer, LogFile.BlockHeader header)
    ...
    try {
      ...
      int decompressedSize = decompressor.decompress(decompressedBuffer.array(),
  ...
  private boolean resyncReader(long offset) throws IOException {
    ...
    while (nextOffset < endOfDataOffset) {
      ...
      try {
        ...
        if (blockHeader.getUncompressedDataSize() < 0 || blockHeader.getCompressedDataSize() < 0) {
        ...
        LOG.warn("Found magic bytes at offset {} but hit EOF trying to read header", startPos);
        return false;
      } catch (IOException | IllegalArgumentException e) {
        // Header was invalid (bad magic, version, compression, sizes, etc.)
        LOG.warn("Found magic bytes at offset {} but header validation failed: {}," + " continuing",
  ...
  private void validateReadCounts() {
    ...
    if (trailer.getBlockCount() != context.getBlocksRead()) {
      LOG.warn("Trailer block count mismatch! expected=" + trailer.getBlockCount() + ", actual="
        + context.getBlocksRead());
    }
    if (trailer.getRecordCount() != context.getRecordsRead()) {
      LOG.warn("Trailer record count mismatch! expected=" + trailer.getRecordCount() + ", actual="
