...
/**
 * Handles the low-level writing of headers, blocks, and trailers for Log files. Manages buffering,
 * compression, checksums, and writing to the underlying stream.
 */
  ...
  public void closeBlock() throws IOException {
    ...
    if (ourCompression != Compression.Algorithm.NONE) {
      ...
      try {
        ...
        compressBuf.clear();
        writeLen =
          compressor.compress(compressBuf.array(), compressBuf.arrayOffset(), compressBuffNeeded);
