...
public class ReplicationLogProcessor implements Closeable {
  ...
  /**
   * The default batch size for reading the replication log file. Assuming each log record to be 10
   * KB (un-compressed) and allow at max 64 MB of in-memory records to be processed
   */
  public static final int DEFAULT_REPLICATION_STANDBY_LOG_REPLAY_BATCH_SIZE = 6400;

  /**
   * The maximum total size of mutations to process in single batch while reading replication log
   * file
   */
  ...
  protected ReplicationLogProcessor(final Configuration conf, final String haGroupName) {
    ...
    this.executorService = Executors.newFixedThreadPool(threadPoolSize, new ThreadFactoryBuilder()
      .setNameFormat("Phoenix-Replication-Log-Processor-" + haGroupName + "-%d").build());
  }

  /**
   * Decorate the Configuration object to make replication more receptive to delays by reducing the
  ...
  /**
   * Creates a LogFileReader for the specified file path. Validates that the file exists and
  ...
  protected Optional<LogFileReader> createLogFileReader(FileSystem fs, Path filePath)
    ...
    // Skip empty files unconditionally before any lease state inspection
    if (fs.getFileStatus(filePath).getLen() <= 0) {
      // Since writer appends the header in eager fashion (at time of creating the file), empty file
      // denotes header was not appended and RS crashed just after creating the file, which implies
      // there was surely NO data to read and we can safely return.
  ...
  protected void closeReader(LogFileReader logFileReader) {
    ...
    try {
      ...
      LOG.error("Error while closing LogFileReader: {}", logFileReader, exception);
    }
  }

  protected void processReplicationLogBatch(Map<TableName, List<Mutation>> tableMutationMap)
    throws IOException {
    ...
    while (attempt <= batchRetryCount && !currentOperations.isEmpty()) {
      if (attempt > 0) {
        LOG.warn("Retrying failed batch operations, attempt {} of {}", attempt, batchRetryCount);
      }
   ...
   */
  protected ApplyMutationBatchResult applyMutations(Map<TableName, List<Mutation>> tableMutationMap)
    throws IOException {
    ...
    for (Map.Entry<TableName, List<Mutation>> entry : tableMutationMap.entrySet()) {
      ...
      AsyncTable<?> table = getAsyncConnection().getTable(tableName, executorService);
  ...
  /**
   * Return the {@link AsyncConnection} which is used for applying mutations. It ensures to create a
    ...
    if (existingAsyncConnection == null || existingAsyncConnection.isClosed()) {
      synchronized (this) {
        ...
        if (existingAsyncConnection == null || existingAsyncConnection.isClosed()) {
           ...
           */
          existingAsyncConnection = FutureUtils.get(ConnectionFactory.createAsyncConnection(conf));
