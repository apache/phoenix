...
public interface MetricsReplicationLogTracker extends BaseSource {
  ...
  String MARK_FILE_IN_PROGRESS_RENAME_FAILED_COUNT_DESC =
    "Number of mark-file-in-progress attempts whose claim rename returned false; typically the "
      + "claim was lost to another process. Counts only the boolean rename() == false outcome -- an "
      + "IOException thrown during the rename is not counted here (that path only logs an error). A "
      + "strict subset of the request count.";
  ...
  /**
   * Increments the counter for mark file in progress requests. This counter tracks the number of
   ...
   * any other {@code rename() == false} outcome is also counted. An IOException thrown during the
   * rename is not counted here (that path only logs an error and returns empty). A strict subset of
   ...
   * requests made to mark files completed.
   */
  void incrementMarkFileCompletedRequestCount();

  /**
   * Increments the counter for mark file failed requests. This counter tracks the number of
   * requests made to mark files failed.
   */
  void incrementMarkFileFailedRequestCount();

  /**
   * Increments the counter for mark file completed request failures. This counter tracks the number
   * of requests made to mark files completed that failed.
   */
