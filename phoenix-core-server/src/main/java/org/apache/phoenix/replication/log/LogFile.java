...
/**
 * Defines the structure and constants for Phoenix Replication Log files. Provides interfaces for
 * reading and writing these logs.
 */
public interface LogFile {
  ...
  interface BlockHeader {
    ...
    /**
     * Gets the size of the data payload after compression. If compression is NONE, this will be the
  ...
  interface Trailer {
    /**
     * Gets the major version of the log file format stored in the trailer. Useful for validation
     * when reading from the end of the file.
     * @return The major version number.
     */
    int getMajorVersion();
    ...
    /**
     * Gets the minor version of the log file format stored in the trailer. Useful for validation
     * when reading from the end of the file.
     * @return The minor version number.
     */
    int getMinorVersion();
    ...
    /**
     * Reads the trailer fields from the provided DataInput stream. Assumes the stream is positioned
  ...
  interface Record {
    ...
    /**
     * Gets the total serialized length of this record in bytes, including any length prefixes used
     * by the codec. This value should be set by the codec after writing or reading.
  ...
  interface Writer extends Closeable {
    ...
    /**
     * Returns the current length of the log file being written, in bytes. This typically reflects
     * the position of the underlying output stream.
  ...
  interface Reader extends Closeable, Iterable<Record> {
    /**
     * Initializes the reader with necessary context. This typically involves opening the file and
     * reading the header.
     * @param context The context containing initialization parameters.
    ...
    /**
     * Returns the Trailer information read from the log file during initialization. If the trailer
     * was missing or corrupt, this may return null.
  ...
  interface Codec {
    ...
    interface Decoder {
      /**
       * Moves the decoder to the next record in the buffer. If a non-null {@code reuse} object is
       * provided and is compatible, it will be populated with the data instead of creating a new
       * object internally.
       * @return true if a record was successfully read, false if the end of the stream is reached.
       * @throws IOException if an error occurs during reading.
       */
      boolean advance() throws IOException;

      /**
       * Returns the current record, which might be the object passed in the last successful call to
       * {@code advance(reuse)} or an internal object if reuse was null. Throws
       * IllegalStateException if advance() has not been called successfully first.
       */
      Record current();
