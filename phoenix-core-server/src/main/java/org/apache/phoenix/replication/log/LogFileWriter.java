...
/**
 * Writer for Phoenix Replication Log files. Manages writing the header, blocks (via
 * LogFormatWriter), and trailer.
 */
  ...
  public long getLength() throws IOException {
    if (closed) {
      ...
      if (context.getFileSystem().exists(context.getFilePath())) {
        ...
        throw new FileNotFoundException("LogFile not found at path " + context.getFilePath());
      }
    }
    if (writer == null) {
