 ...
 * - SYNC: Normal synchronized processing where both lastRoundProcessed and lastRoundInSync
 *   advance together
 ...
 *
 * Key features:
 * - Uses getFirstRoundToProcess() to start replay from lastRoundInSync (not just from
 *   lastRoundProcessed)
...
public class ReplicationLogDiscoveryReplay extends ReplicationLogDiscovery {
     ...
     * - Initializes lastRoundProcessed from minimum of: in-progress files, new files, or
     *   current time
     * - Initializes lastRoundInSync from minimum of: lastSyncStateTimeInMs (from HA Store)
     *   or minimum timestamp from IN and IN PROGRESS files
    ...
    protected void initializeLastRoundProcessed() throws IOException {
        ...
        if (HAGroupStoreRecord.HAGroupState.DEGRADED_STANDBY.equals(haGroupStoreRecord.getHAGroupState())) {
            ...
            this.lastRoundInSync = replicationLogTracker
                    .getReplicationShardDirectoryManager()
                    .getReplicationRoundFromEndTime(Math.min(
                            haGroupStoreRecord.getLastSyncStateTimeInMs(),
     ...
     * Unlike getNextRoundToProcess() which uses lastRoundProcessed, this method uses
     * lastRoundInSync
     ...
     *
     * @return Optional containing the first round to process, or empty if not enough time
     *         has passed
     */
     ...
     * all replication logs up to the current time have been processed before transitioning the cluster
    ...
    protected boolean shouldTriggerFailover() throws IOException {
        ...
        if (!isInDirectoryEmpty) {
            LOG.debug("Failover not triggered. New files exist from next round to current timestamp round.");
            return false;
        }
