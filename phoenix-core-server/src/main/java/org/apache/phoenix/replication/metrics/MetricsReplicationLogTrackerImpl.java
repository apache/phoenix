...
public class MetricsReplicationLogTrackerImpl extends BaseSourceImpl
  ...
  public MetricsReplicationLogTrackerImpl(String metricsName, String metricsDescription,
    ...
    markFileCompletedRequestFailedCount = getMetricsRegistry().newCounter(
      MARK_FILE_COMPLETED_REQUEST_FAILED_COUNT, MARK_FILE_COMPLETED_REQUEST_FAILED_COUNT_DESC, 0L);
    markFileInProgressTime = getMetricsRegistry().newHistogram(MARK_FILE_IN_PROGRESS_TIME,
      MARK_FILE_IN_PROGRESS_TIME_DESC);
    markFileCompletedTime =
      getMetricsRegistry().newHistogram(MARK_FILE_COMPLETED_TIME, MARK_FILE_COMPLETED_TIME_DESC);
    markFileFailedTime =
      getMetricsRegistry().newHistogram(MARK_FILE_FAILED_TIME, MARK_FILE_FAILED_TIME_DESC);
  }
