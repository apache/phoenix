...
public class CsvBulkLoadTool extends AbstractBulkLoadTool {
  ...
  static final Option DELIMITER_OPT =
    new Option("d", "delimiter", true, "Input delimiter, defaults to comma");
  static final Option QUOTE_OPT = new Option("q", "quote", true,
    "Supply a custom phrase delimiter, defaults to double quote character");
  static final Option ESCAPE_OPT =
    new Option("e", "escape", true, "Supply a custom escape character, default is a backslash");
  static final Option ARRAY_DELIMITER_OPT =
    new Option("a", "array-delimiter", true, "Array element delimiter (optional)");
  static final Option binaryEncodingOption =
  ...
  protected void configureOptions(CommandLine cmdLine, List<ColumnInfo> importColumns,
    ...
    if (cmdLine.hasOption(DELIMITER_OPT.getOpt())) {
      String delimString =
        StringEscapeUtils.unescapeJava(cmdLine.getOptionValue(DELIMITER_OPT.getOpt()));
    ...
    if (cmdLine.hasOption(QUOTE_OPT.getOpt())) {
      String quoteString =
        StringEscapeUtils.unescapeJava(cmdLine.getOptionValue(QUOTE_OPT.getOpt()));
