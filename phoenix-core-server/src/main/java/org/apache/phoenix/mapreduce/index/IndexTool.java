...
public class IndexTool extends Configured implements Tool {
  ...
  public enum IndexDisableLoggingType {
    ...
    public static IndexDisableLoggingType fromValue(String value) {
      ...
      throw new IllegalStateException(
        "Invalid value: " + value + " for " + IndexDisableLoggingType.class);
    }
  ...
  private static final Option SPLIT_INDEX_OPTION = new Option("sp", "split", true,
    "Split the index table before building, to have the same # of regions as the data table.  "
      ...
      + "Takes an optional argument specifying the sampling rate," + "otherwise defaults to "
      + DEFAULT_SPLIT_SAMPLING_RATE);

  private static final int DEFAULT_AUTOSPLIT_NUM_REGIONS = 20;

  private static final Option AUTO_SPLIT_INDEX_OPTION = new Option("spa", "autosplit", true,
  ...
  private static final Option DELETE_ALL_AND_REBUILD_OPTION =
    new Option("deleteall", "delete-all-and-rebuild", false,
  ...
  private static final Option START_TIME_OPTION =
    new Option("st", "start-time", true, "Start time for indextool rebuild or verify");
  private static final Option END_TIME_OPTION =
    new Option("et", "end-time", true, "End time for indextool rebuild or verify");

  private static final Option RETRY_VERIFY_OPTION = new Option("rv", "retry-verify", true,
    "Max scan ts of the last rebuild/verify that needs to be retried incrementally");

  private static final Option DISABLE_LOGGING_OPTION = new Option("dl", "disable-logging", true,
    "Disable logging of failed verification rows for BEFORE, " + "AFTER, or BOTH verify jobs");
  ...
  public CommandLine parseOptions(String[] args) {
    ...
    if (!cmdLine.hasOption(DATA_TABLE_OPTION.getOpt())) {
      throw new IllegalStateException(
        DATA_TABLE_OPTION.getLongOpt() + " is a mandatory " + "parameter");
    }
    ...
    if (
      ...
      throw new IllegalStateException(
        "Index name should not be passed with " + PARTIAL_REBUILD_OPTION.getLongOpt());
    }
    ...
    if (
      ...
      throw new IllegalStateException(
        "Index name should be passed unless it is a partial rebuild.");
    }
    ...
    if (
      ...
      throw new IllegalStateException(DELETE_ALL_AND_REBUILD_OPTION.getLongOpt()
        + " is not compatible with " + PARTIAL_REBUILD_OPTION.getLongOpt());
    }

    boolean splitIndex = cmdLine.hasOption(AUTO_SPLIT_INDEX_OPTION.getOpt())
    ...
    if (splitIndex && cmdLine.hasOption(PARTIAL_REBUILD_OPTION.getOpt())) {
      throw new IllegalStateException(
        "Cannot split index for a partial rebuild, as the index table is dropped");
    }
    if (loggingDisabledMismatchesVerifyOption(cmdLine)) {
      throw new IllegalStateException("Can't disable index verification logging when no "
        ...
        + "DisableLoggingType: [" + cmdLine.getOptionValue(DISABLE_LOGGING_OPTION.getOpt()) + "]");
    }
    return cmdLine;
  }
  ...
  private boolean loggingDisabledMismatchesVerifyOption(CommandLine cmdLine) {
    ...
    if (
      (verifyType.equals(IndexVerifyType.BEFORE) || verifyType.equals(IndexVerifyType.ONLY))
    ...
    if (
      (verifyType.equals(IndexVerifyType.AFTER))
        && loggingDisableType.equals(IndexDisableLoggingType.BEFORE)
    ) {
      return true;
    }
    if (
      loggingDisableType.equals(IndexDisableLoggingType.BOTH)
  ...
  class JobFactory {
    ...
    public Job getJob() throws Exception {
      if (isPartialBuild) {
        ...
        if (useSnapshot || (!isLocalIndexBuild && pDataTable.isTransactional())) {
          ...
          // It is safe not to set CURRENT_SCN_VALUE for server side rebuilds, in order to make sure
          // that
          ...
          // However, PHOENIX-5732 introduces endTime parameter to be passed optionally for
          // IndexTool.
          // When endTime is passed for local and non-tx global indexes, we'll override the
          // CURRENT_SCN_VALUE.
          if (endTime != null) {
    ...
    private Job configureJobForPartialBuild() throws Exception {
      ...
      if (minDisableTimestamp == 0) {
        ...
        // TODO probably we can initiate the job by ourself or can skip them while making the list
        // for partial build with a warning
      }

      long maxTimestamp = getMaxRebuildAsyncDate(schemaName, disableIndexes);

      // serialize index maintaienr in job conf with Base64 TODO: Need to find better way to
      // serialize them in conf.
      ...
      for (PTable index : disabledPIndexes) {
        maintainers
          .add(index.getIndexMaintainer(pDataTable, connection.unwrap(PhoenixConnection.class)));
      }
      ...
      final String jobName =
        String.format("Phoenix Indexes build for " + pDataTable.getName().toString());
      ...
      TableMapReduceUtil.initTableMapperJob(physicalTableName, scan,
        PhoenixIndexPartialBuildMapper.class, null, null, job);
    ...
    private long getMaxRebuildAsyncDate(String schemaName, List<String> disableIndexes)
      throws SQLException {
      ...
      String query = String.format(
        "SELECT MAX(" + ASYNC_REBUILD_TIMESTAMP + "), " + "MAX(" + INDEX_DISABLE_TIMESTAMP
          + ") FROM " + SYSTEM_CATALOG_NAME + " (" + ASYNC_REBUILD_TIMESTAMP + " BIGINT) WHERE "
          + TABLE_SCHEM + " %s  AND " + TABLE_NAME + " IN ( %s )",
      ...
      try (PreparedStatement selSyscat = connection.prepareStatement(query)) {
        ...
        // Do check if table is disabled again after user invoked async rebuilding during the run of
        // the job
    ...
    private Job configureJobForAsyncIndex() throws Exception {
      ...
      if (!useSnapshot) {
        ...
        try {
          ...
          snapshotName = new StringBuilder("INDEXTOOL-").append(pDataTable.getName().getString())
            .append("-Snapshot-").append(System.currentTimeMillis()).toString();
    ...
    private Job configureJobForServerBuildIndex() throws Exception {
      ...
      PhoenixMapReduceUtil.setInput(job, PhoenixServerBuildIndexDBWritable.class,
        PhoenixServerBuildIndexInputFormat.class, dataTableWithSchema, "");
  ...
  public int populateIndexToolAttributes(CommandLine cmdLine) throws Exception {
    ...
    if (verify) {
      ...
      if (disableLogging) {
        disableLoggingType = IndexDisableLoggingType
          .fromValue(cmdLine.getOptionValue(DISABLE_LOGGING_OPTION.getOpt()));
      }
    }
  ...
  public int validateLastVerifyTime() throws Exception {
    ...
    if (
      lastVerifyTime.compareTo(currentTime) > 0 || lastVerifyTime == 0L
  ...
  public boolean isValidLastVerifyTime(Long lastVerifyTime) throws Exception {
    try (Connection conn = getConnection(configuration);
      ...
      s.setRowPrefixFilter(Bytes.toBytes(String.format("%s%s%s", lastVerifyTime, ROW_KEY_SEPARATOR,
        SchemaUtil.getPhysicalHBaseTableName(qSchemaName,
          SchemaUtil.normalizeIdentifier(indexTable), isNamespaceMapped))));
    ...
    if (IndexType.LOCAL.equals(indexType)) {
      ...
      if (useSnapshot) {
        throw new IllegalArgumentException(String.format(
          "%s is a local index. snapshots are not supported for local indexes.", qIndexTable));
      }
      try (org.apache.hadoop.hbase.client.Connection hConn =
        ...
        RegionLocator regionLocator =
          hConn.getRegionLocator(TableName.valueOf(pIndexTable.getPhysicalName().getBytes()));
  ...
  private void preSplitIndexTable(CommandLine cmdLine, Connection connection)
    ...
    if (
      !isSalted
        && (IndexType.GLOBAL.equals(indexType) || IndexType.UNCOVERED_GLOBAL.equals(indexType))
        && (autosplit || splitIndex)
    ) {
      ...
      double samplingRate =
        rateOpt == null ? DEFAULT_SPLIT_SAMPLING_RATE : Double.parseDouble(rateOpt);
      ...
      splitIndexTable(connection.unwrap(PhoenixConnection.class), autosplit, autosplitNumRegions,
        samplingRate);
    }
  }
  ...
  private void deleteBeforeRebuild(Connection conn) throws SQLException, IOException {
    ...
    if (isLocalIndexBuild) {
      throw new IllegalArgumentException(String.format(
        "%s is a local index.  delete-all-and-rebuild is not supported for local indexes",
        qIndexTable));
    } else {
  ...
  private void splitIndexTable(PhoenixConnection pConnection, boolean autosplit,
    ...
    final String dataSampleQuery =
      QueryUtil.constructSelectStatement(qTableSample, dataColNames, null, Hint.NO_INDEX, true);
    ...
    try (
      final PhoenixResultSet rs =
        pConnection.createStatement().executeQuery(dataSampleQuery).unwrap(PhoenixResultSet.class);
      ...
      // do the split
      // to get the splits, we just need the right bound of every histogram bucket, excluding the
      // last
  ...
  private org.apache.hadoop.hbase.client.Connection
    getTemporaryHConnection(PhoenixConnection pConnection) throws SQLException, IOException {
  ...
  public static ValueGetter getIndexValueGetter(final PhoenixResultSet rs,
    ...
    return new AbstractValueGetter() {
      ...
      public ImmutableBytesWritable getLatestValue(ColumnReference ref, long ts)
        throws IOException {
        try {
          String fullColumnName = SchemaUtil.getEscapedFullColumnName(
            SchemaUtil.getColumnDisplayName(ref.getFamily(), ref.getQualifier()));
