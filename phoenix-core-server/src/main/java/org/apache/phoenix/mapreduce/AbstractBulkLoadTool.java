...
public abstract class AbstractBulkLoadTool extends Configured implements Tool {
  ...
  static final Option ZK_QUORUM_OPT =
    new Option("z", "zookeeper", true, "Supply zookeeper connection details (optional)");
  static final Option INPUT_PATH_OPT =
    new Option("i", "input", true, "Input path(s) (comma-separated, mandatory)");
  static final Option OUTPUT_PATH_OPT =
    new Option("o", "output", true, "Output path for temporary HFiles (optional)");
  static final Option SCHEMA_NAME_OPT =
    new Option("s", "schema", true, "Phoenix schema name (optional)");
  static final Option TABLE_NAME_OPT =
    new Option("t", "table", true, "Phoenix table name (mandatory)");
  static final Option INDEX_TABLE_NAME_OPT = new Option("it", "index-table", true,
    "Phoenix index table name when just loading this particualar index table");
  static final Option IMPORT_COLUMNS_OPT =
    new Option("c", "import-columns", true, "Comma-separated list of columns to be imported");
  static final Option IGNORE_ERRORS_OPT =
    new Option("g", "ignore-errors", false, "Ignore input errors");
  static final Option HELP_OPT = new Option("h", "help", false, "Show this help and quit");
  static final Option SKIP_HEADER_OPT =
  ...
  private int loadData(Configuration conf, CommandLine cmdLine) throws Exception {
    ...
    if (cmdLine.hasOption(ZK_QUORUM_OPT.getOpt())) {
      ...
      ConnectionInfo info =
        ConnectionInfo.create(PhoenixRuntime.JDBC_PROTOCOL_ZK + ":" + zkQuorum, conf, null, null);
    ...
    try (Connection conn = QueryUtil.getConnection(conf)) {
      ...
      tablesToBeLoaded
        .add(new TargetTableRef(qualifiedTableName, table.getPhysicalName().getString()));
      boolean hasGlobalIndexes = false;
      for (PTable index : table.getIndexes()) {
        if (index.getIndexType() == IndexType.LOCAL) {
          hasLocalIndexes = qualifiedIndexTableName == null
            ? true
            : index.getTableName().getString().equals(qualifiedIndexTableName);
      ...
      if (
        hasGlobalIndexes && tableNotEmpty && !cmdLine.hasOption(ENABLE_CORRUPT_INDEXES.getOpt())
      ) {
      ...
      if (qualifiedIndexTableName != null) {
        ...
        if (targetIndexRef == null) {
          throw new IllegalStateException(
            "Bulk Loader error: index table " + qualifiedIndexTableName + " doesn't exist");
        }
  ...
  /**
   * Submits the jobs to the cluster. Loads the HFiles onto the respective tables.
   */
  ...
  /**
   * Perform any required validation on the table being bulk loaded into: - ensure no column family
   * names start with '_', as they'd be ignored leading to problems.
   * @throws java.sql.SQLException
   */
  private void validateTable(Connection conn, String schemaName, String tableName)
    throws SQLException {
    ...
    while (rs.next()) {
      ...
      if (familyName != null && familyName.startsWith("_")) {
        if (QueryConstants.DEFAULT_COLUMN_FAMILY.equals(familyName)) {
          ...
          throw new IllegalStateException(
            "Bulk Loader error: Column family name " + "must not start with '_': " + familyName);
        }
      }
    }
    rs.close();
