...
public class IndexVerificationOutputRepository implements AutoCloseable {
  ...
  private static final Logger LOGGER =
    LoggerFactory.getLogger(IndexVerificationOutputRepository.class);
  ...
  public static byte[] generateOutputTableRowKey(long ts, byte[] indexTableName,
    byte[] dataRowKey) {
  ...
  private static byte[] generatePartialOutputTableRowKey(long ts, byte[] indexTableName) {
    ...
    // The row key for the output table : timestamp | index table name | data row key
    partialRowKey =
      new byte[keyPrefix.length + ROW_KEY_SEPARATOR_BYTE.length + indexTableName.length];
    Bytes.putBytes(partialRowKey, targetOffset, keyPrefix, 0, keyPrefix.length);
    targetOffset += keyPrefix.length;
    Bytes.putBytes(partialRowKey, targetOffset, ROW_KEY_SEPARATOR_BYTE, 0,
  ...
  public void logToIndexToolOutputTable(byte[] dataRowKey, byte[] indexRowKey, long dataRowTs,
    long indexRowTs, String errorMsg, byte[] expectedValue, byte[] actualValue, long scanMaxTs,
    byte[] tableName, boolean isBeforeRebuild, IndexVerificationErrorType errorType)
    throws IOException {
    if (shouldLogOutput(isBeforeRebuild, errorType)) {
      byte[] rowKey =
        generateOutputTableRowKey(scanMaxTs, indexTable.getName().toBytes(), dataRowKey);
  ...
  public boolean shouldLogOutput(boolean isBeforeRebuild, IndexVerificationErrorType errorType) {
    return shouldLogOutputForVerifyType(isBeforeRebuild) && shouldLogOutputForErrorType(errorType);
  }
  ...
  private boolean shouldLogOutputForVerifyType(boolean isBeforeRebuild) {
    ...
    if (
      isBeforeRebuild && (disableLoggingVerifyType.equals(IndexTool.IndexDisableLoggingType.AFTER))
    ) {
      return true;
    }
    if (
      !isBeforeRebuild && disableLoggingVerifyType.equals(IndexTool.IndexDisableLoggingType.BEFORE)
    ) {
      return true;
    }
    return false;
  ...
  private boolean shouldLogOutputForErrorType(IndexVerificationErrorType errorType) {
    if (
      errorType != null && (errorType.equals(IndexVerificationErrorType.BEYOND_MAX_LOOKBACK_INVALID)
  ...
  public static byte[] getErrorMessageBytes(String errorMsg, byte[] expectedValue,
    byte[] actualValue) {
    byte[] errorMessageBytes;
    errorMessageBytes =
      new byte[errorMsg.length() + expectedValue.length + actualValue.length + TOTAL_PREFIX_LENGTH];
  ...
  public static IndexVerificationOutputRow getOutputRowFromResult(Result result) {
    ...
    builder.setDataTableName(
      Bytes.toString(result.getValue(OUTPUT_TABLE_COLUMN_FAMILY, DATA_TABLE_NAME_BYTES)));
    builder
      .setIndexTableRowKey(result.getValue(OUTPUT_TABLE_COLUMN_FAMILY, INDEX_TABLE_ROW_KEY_BYTES));
    builder.setDataTableRowTimestamp(Long
      .parseLong(Bytes.toString(result.getValue(OUTPUT_TABLE_COLUMN_FAMILY, DATA_TABLE_TS_BYTES))));
    builder.setIndexTableRowTimestamp(Long.parseLong(
      Bytes.toString(result.getValue(OUTPUT_TABLE_COLUMN_FAMILY, INDEX_TABLE_TS_BYTES))));
    builder.setErrorMessage(
      Bytes.toString(result.getValue(OUTPUT_TABLE_COLUMN_FAMILY, ERROR_MESSAGE_BYTES)));
    ...
    try {
      errorType = IndexVerificationErrorType
