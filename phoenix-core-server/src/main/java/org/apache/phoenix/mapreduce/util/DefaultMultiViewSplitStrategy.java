...
public class DefaultMultiViewSplitStrategy implements MultiViewSplitStrategy {
  ...
  public List<InputSplit> generateSplits(List<ViewInfoWritable> views,
    ...
    for (int i = 0; i < numberOfMappers; i++) {
      pSplits.add(new PhoenixMultiViewInputSplit(
        views.subList(i * numViewsInSplit, getUpperBound(numViewsInSplit, i, views.size()))));
    }
  ...
  /*
   * Calculate number of mappers are needed based on split policy and number of views on the cluster
   */
  ...
  /*
   * Calculate the upper bound for each mapper. For example, given split policy is 10 cleanup jobs
   * per mapper, and the total view size at the cluster is 12. The first mapper will take from [0 -
   * 10), this method will return 10 as upper bound The second mapper will take from [10 - 12), this
   * method will return 12 as upper bound.
   */
