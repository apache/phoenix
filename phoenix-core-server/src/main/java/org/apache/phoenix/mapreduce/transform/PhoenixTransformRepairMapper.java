...
public class PhoenixTransformRepairMapper extends TableMapper<ImmutableBytesWritable, IntWritable> {
  ...
  protected void setup(final Context context) throws IOException, InterruptedException {
    ...
    try {
      ...
      maintainers =
        new ImmutableBytesPtr(PhoenixConfigurationUtil.getIndexMaintainers(configuration));
  ...
  protected void map(ImmutableBytesWritable row, Result value, Context context)
    ...
    try (Table oldHTable = connection.getQueryServices().getTable(Bytes.toBytes(oldTableName))) {
      for (Cell cell : value.rawCells()) {
        ...
        // partial rebuild
        buildNewTableScan.setAttribute(BaseScannerRegionObserverConstants.UNGROUPED_AGG,
          TRUE_BYTES);
        buildNewTableScan.setAttribute(PhoenixIndexCodec.INDEX_PROTO_MD, maintainers.get());
        buildNewTableScan.setAttribute(BaseScannerRegionObserverConstants.REBUILD_INDEXES,
          TRUE_BYTES);
        buildNewTableScan.setAttribute(
          BaseScannerRegionObserverConstants.SKIP_REGION_BOUNDARY_CHECK, Bytes.toBytes(true));
        IndexMaintainer transformMaintainer =
          TransformMaintainer.deserialize(maintainers.get()).get(0);
        ...
        long code = PLong.INSTANCE.getCodec().decodeLong(new ImmutableBytesWritable(scanVal),
        ...
        context
          ...
          .setValue(extraRowsInNewTable.size());
