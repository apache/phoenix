...
public class IndexScrutinyTool extends Configured implements Tool {
  ...
  private static final Option TIMESTAMP = new Option("t", "time", true,
  ...
  public static final Option SOURCE_TABLE_OPTION = new Option("src", "source", true,
  ...
  private static final Option OUTPUT_FORMAT_OPTION = new Option("of", "output-format", true,
  ...
  private static final Option OUTPUT_MAX = new Option("om", "output-max", true,
    "Max number of invalid rows to output per mapper.  Defaults to 1M");
  ...
  private static class JobFactory {
    ...
    public JobFactory(Connection connection, Configuration configuration, long batchSize,
      ...
      scrutinyExecuteTime = EnvironmentEdgeManager.currentTimeMillis(); // time at which scrutiny
                                                                        // was run.
      // Same for
      // all jobs created from this factory
      PhoenixConfigurationUtil.setScrutinyExecuteTimestamp(configuration, scrutinyExecuteTime);
    ...
    public Job createSubmittableJob(String schemaName, String indexTable, String dataTable,
      ...
      final String selectQuery = QueryUtil.constructSelectStatement(qSourceTable, sourceColumnNames,
        null, Hint.NO_INDEX, true);
      ...
      if (outputInvalidRows && OutputFormat.TABLE.equals(outputFormat)) {
        String upsertStmt = IndexScrutinyTableOutput.constructOutputTableUpsert(sourceDynamicCols,
      ...
      final String jobName = String.format(INDEX_JOB_NAME_TEMPLATE, qSourceTable,
      ...
      Path outputPath = getOutputPath(configuration, basePath,
        SourceTable.DATA_TABLE_SOURCE.equals(sourceTable) ? pdataTable : pindexTable);

      return configureSubmittableJob(job, outputPath, mapperClass);
    }

    private Job configureSubmittableJob(Job job, Path outputPath,
      Class<? extends IndexScrutinyMapper> mapperClass) throws Exception {
    ...
    Path getOutputPath(final Configuration configuration, String basePath, PTable table)
      ...
      if (basePath != null) {
        outputPath =
          CsvBulkImportUtil.getOutputPath(new Path(basePath), table.getPhysicalName().getString());
  ...
  public static void createScrutinyToolTables(Connection conn) throws Exception {
    ...
    conn.createStatement().execute(IndexScrutinyTableOutput.OUTPUT_TABLE_BEYOND_LOOKBACK_DDL);
