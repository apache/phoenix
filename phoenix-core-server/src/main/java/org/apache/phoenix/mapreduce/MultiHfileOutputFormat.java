...
/**
 * The MultiHfileOutputFormat class simplifies writing HFiles for multiple tables. It has been
 * adapted from {#link HFileOutputFormat2} but differs from the fact it creates HFiles for multiple
 * tables.
 */
public class MultiHfileOutputFormat extends FileOutputFormat<TableRowkeyPair, Cell> {
  ...
  static <V extends Cell> RecordWriter<TableRowkeyPair, V> createRecordWriter(
    ...
    final long maxsize =
      conf.getLongBytes(HConstants.HREGION_MAX_FILESIZE, HConstants.DEFAULT_MAX_FILE_SIZE);
    ...
    return new RecordWriter<TableRowkeyPair, V>() {
      ...
      private WriterLength getNewWriter(final String tableName, byte[] family, Configuration conf)
        ...
        final Map<byte[], DataBlockEncoding> datablockEncodingMap =
          createFamilyDataBlockEncodingMap(conf, tableName);
        ...
        HFileContextBuilder contextBuilder = new HFileContextBuilder().withCompression(compression)
          ...
          .withBytesPerCheckSum(CompatUtil.getBytesPerChecksum(conf)).withBlockSize(blockSize)
        ...
        StoreFileWriter.Builder storeFileWriterBuilder =
          new StoreFileWriter.Builder(conf, new CacheConfig(tempConf), fs).withOutputDir(familydir)
            .withBloomType(bloomType).withFileContext(hFileContext);
  ...
  /**
   * Returns the set of configurations that have been configured for the table during job
   * initialization.
   */
  private static Map<String, String> getTableConfigurations(Configuration conf,
    final String tableName) {
  ...
  static Map<byte[], DataBlockEncoding> createFamilyDataBlockEncodingMap(Configuration conf,
    ...
    Map<byte[], DataBlockEncoding> encoderMap =
      new TreeMap<byte[], DataBlockEncoding>(Bytes.BYTES_COMPARATOR);
    Map<String, String> tableConfigs = getTableConfigurations(conf, tableName);
    if (tableConfigs == null) {
      return encoderMap;
    }
   ...
   */
  private static Map<byte[], String> createFamilyConfValueMap(Map<String, String> configs,
    String confName) {
  ...
  private static void writePartitions(Configuration conf, Path partitionsPath,
    ...
    SequenceFile.Writer writer = SequenceFile.createWriter(fs, conf, partitionsPath,
      TableRowkeyPair.class, NullWritable.class);
   ...
   * @param conf  to persist serialized values into on failure to read column family descriptors
   */
  ...
  static String configureCompression(TableDescriptor tableDescriptor)
    ...
    for (ColumnFamilyDescriptor familyDescriptor : families) {
      ...
      compressionConfigValue
        .append(URLEncoder.encode(familyDescriptor.getCompressionType().getName(), "UTF-8"));
    }
    return compressionConfigValue.toString();
  }
   ...
   * @param conf            to persist serialized values into on failure to read column family
   *                        descriptors
   */
  @VisibleForTesting
   ...
   * @param conf  to persist serialized values into on failure to read column family descriptors
   */
  static String configureDataBlockEncoding(TableDescriptor tableDescriptor)
    throws UnsupportedEncodingException {
    ...
    for (ColumnFamilyDescriptor familyDescriptor : families) {
      ...
      dataBlockEncodingConfigValue
        .append(URLEncoder.encode(familyDescriptor.getNameAsString(), "UTF-8"));
      dataBlockEncodingConfigValue.append('=');
  ...
  public static void configureIncrementalLoad(Job job, List<TargetTableRef> tablesToBeLoaded)
    throws IOException {
    ...
    for (TargetTableRef table : tablesToBeLoaded) {
      ...
      try (Connection hbaseConn = ConnectionFactory.createConnection(conf);) {
        Set<TableRowkeyPair> startKeys =
          getRegionStartKeys(tableName, hbaseConn.getRegionLocator(TableName.valueOf(tableName)));
        tablesStartKeys.addAll(startKeys);
        TableDescriptor tableDescriptor =
          hbaseConn.getTable(TableName.valueOf(tableName)).getDescriptor();
    ...
    LOGGER.info(
      "Configuring " + tablesStartKeys.size() + " reduce partitions to match current region count");
   ...
   */
  private static Set<TableRowkeyPair> getRegionStartKeys(String tableName, RegionLocator table)
    throws IOException {
