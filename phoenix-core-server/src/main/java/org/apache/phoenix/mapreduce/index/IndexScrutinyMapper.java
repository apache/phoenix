...
public class IndexScrutinyMapper extends Mapper<NullWritable, PhoenixIndexDBWritable, Text, Text> {
  ...
  protected void setup(final Context context) throws IOException, InterruptedException {
    ...
    try {
      ...
      // equivalent
      // name)
      targetTableQuery = QueryUtil.constructSelectStatement(qTargetTable,
        columnNames.getCastedTargetColNames(), targetPksCsv, Hint.NO_INDEX, false) + " IN ";
      ...
      Long maxLookbackAge = pdataTable.getMaxLookbackAge();
      maxLookbackAgeMillis = MetaDataUtil.getMaxLookbackAge(configuration, maxLookbackAge);
  ...
  protected void processBatch(Context context)
    ...
    // table)
    String inClause =
      QueryUtil.constructParameterizedInClause(numTargetPkCols, currentBatchValues.size());
      ...
      if (outputInvalidRows) {
        for (Pair<Long, List<Object>> sourceRowWithoutTargetRow : targetPkToSourceValues.values()) {
          ...
          if (OutputFormat.FILE.equals(outputFormat)) {
            context.write(new Text(Arrays.toString(valuesWithoutTarget.toArray())),
  ...
  private int getTableTTL(Configuration configuration) throws SQLException, IOException {
    ...
    if (
      pSourceTable.getType() == PTableType.INDEX
    ...
    String physicalTable =
      getSourceTableName(pSourceTable, SchemaUtil.isNamespaceMappingEnabled(null, cqsi.getProps()));
    if (
      configuration.getBoolean(QueryServices.PHOENIX_TABLE_TTL_ENABLED,
        QueryServicesOptions.DEFAULT_PHOENIX_TABLE_TTL_ENABLED)
    ) {
      return pSourceTable.getTTL() == TTL_NOT_DEFINED ? DEFAULT_TTL : pSourceTable.getTTL();
    } else {
      TableDescriptor tableDesc;
      try (Admin admin = cqsi.getAdmin()) {
        tableDesc = admin.getDescriptor(TableName.valueOf(physicalTable));
      }
  ...
  public static String getSourceTableName(PTable pSourceTable, boolean isNamespaceEnabled) {
    ...
    if (pSourceTable.getType() == PTableType.VIEW || MetaDataUtil.isViewIndex(sourcePhysicalName)) {
      ...
      physicalTable =
        SchemaUtil.getPhysicalHBaseTableName(schema, table, isNamespaceEnabled).toString();
    }
    return physicalTable;
  }

  protected Map<String, Pair<Long, List<Object>>>
    buildTargetStatement(PreparedStatement targetStatement) throws SQLException {
    Map<String, Pair<Long, List<Object>>> targetPkToSourceValues =
  ...
  private void outputInvalidRow(Context context, List<Object> sourceValues,
    List<Object> targetValues, long sourceTS, long targetTS)
    throws SQLException, IOException, InterruptedException {
    if (OutputFormat.FILE.equals(outputFormat)) {
  ...
  protected void writeToOutputTable(Context context, List<Object> sourceValues,
    List<Object> targetValues, long sourceTS, long targetTS) throws SQLException {
    if (
      context.getCounter(PhoenixScrutinyJobCounters.INVALID_ROW_COUNT).getValue() > outputMaxRows
    ) {
      return;
    }
    int index = 1;
