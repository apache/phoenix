...
public class GroupedAggregateRegionObserver extends BaseScannerRegionObserver
  ...
  /**
   * Replaces the RegionScanner s with a RegionScanner that groups by the key formed by the list of
   * expressions from the scan and returns the aggregated rows of each group. For example, given the
   * following original rows in the RegionScanner: KEY COL1 row1 a row2 b row3 a row4 a the
   * following rows will be returned for COUNT(*): KEY COUNT a 3 b 1 The client is required to do a
   * sort and a final aggregation, since multiple rows with the same key may be returned from
   * different regions.
   */
  @Override
  protected RegionScanner doPostScannerOpen(ObserverContext<RegionCoprocessorEnvironment> c,
    ...
    byte[] expressionBytes =
      scan.getAttribute(BaseScannerRegionObserverConstants.UNORDERED_GROUP_BY_EXPRESSIONS);

    if (expressionBytes == null) {
      expressionBytes =
        scan.getAttribute(BaseScannerRegionObserverConstants.KEY_ORDERED_GROUP_BY_EXPRESSIONS);
      keyOrdered = true;
    }
    int offset = 0;
    ...
    try (MemoryChunk em = tenantCache.getMemoryManager().allocate(0)) {
      ServerAggregators aggregators = ServerAggregators.deserialize(
        scan.getAttribute(BaseScannerRegionObserverConstants.AGGREGATORS),
        c.getEnvironment().getConfiguration(), em);
      ...
      if (ScanUtil.isLocalOrUncoveredGlobalIndex(scan) || (j == null && p != null)) {
        if (dataColumns != null) {
          tupleProjector = IndexUtil.getTupleProjector(scan, dataColumns);
          viewConstants = IndexUtil.deserializeViewConstantsFromScan(scan);
        }
        ImmutableBytesPtr tempPtr = new ImmutableBytesPtr();
        innerScanner = getWrappedScanner(c, innerScanner, offset, scan, dataColumns, tupleProjector,
          c.getEnvironment().getRegion(), indexMaintainers == null ? null : indexMaintainers.get(0),
          viewConstants, p, tempPtr, useQualifierAsIndex);
      }
      ...
      if (keyOrdered) { // Optimize by taking advantage that the rows are
        ...
        return new OrderedGroupByRegionScanner(c, scan, innerScanner, expressions, aggregators,
          limit, pageSizeMs);
      } else { // Otherwse, collect them all up in an in memory map
        return new UnorderedGroupByRegionScanner(c, scan, innerScanner, expressions, aggregators,
          limit, pageSizeMs);
      }
    }
  }
  ...
  /**
   * Cache for distinct values and their aggregations which is completely in-memory (as opposed to
   * spilling to disk). Used when GROUPBY_SPILLABLE_ATTRIB is set to false. The memory usage is
   * tracked at a coursed grain and will throw and abort if too much is used.
   * @since 3.0.0
   */
  private static final class InMemoryGroupByCache implements GroupByCache {
    ...
    InMemoryGroupByCache(RegionCoprocessorEnvironment env, ImmutableBytesPtr tenantId,
      byte[] customAnnotations, ServerAggregators aggregators, int estDistVals,
      ...
      if (rowAggregators == null) {
        ...
        if (LOGGER.isDebugEnabled()) {
          LOGGER.debug(LogUtil.addCustomAnnotations(
            "Adding new aggregate bucket for row key "
    ...
    public RegionScanner getScanner(final RegionScanner s) {
      ...
      long estSize =
        sizeOfUnorderedGroupByMap(aggregateMap.size(), aggregators.getEstimatedByteSize());
      chunk.resize(estSize);
      ...
      for (Map.Entry<ImmutableBytesPtr, Aggregator[]> entry : aggregateMap.entrySet()) {
        ...
        if (!isIncompatibleClient) {
          ...
          aggResults.add(PhoenixKeyValueUtil.newKeyValue(aggregateGroupValPtr.get(),
  ...
  private static final class GroupByCacheFactory {
    ...
    GroupByCache newCache(RegionCoprocessorEnvironment env, ImmutableBytesPtr tenantId,
      byte[] customAnnotations, ServerAggregators aggregators, int estDistVals,
      boolean isIncompatibleClient) {
  ...
  /**
   * Used for an aggregate query in which the key order does not necessarily match the group by key
   * order. In this case, we must collect all distinct groups within a region into a map,
  ...
  private static class UnorderedGroupByRegionScanner extends BaseRegionScanner {
    ...
    private UnorderedGroupByRegionScanner(final ObserverContext<RegionCoprocessorEnvironment> c,
      ...
      this.scan = scan;
      scanStartRowKey = ServerUtil.getScanStartRowKeyFromScanOrRegionBoundaries(scan, region);
      ...
      Configuration conf = env.getConfiguration();
      int estDistVals = conf.getInt(GROUPBY_ESTIMATED_DISTINCT_VALUES_ATTRIB,
        DEFAULT_GROUPBY_ESTIMATED_DISTINCT_VALUES);
      byte[] estDistValsBytes =
        scan.getAttribute(BaseScannerRegionObserverConstants.ESTIMATED_DISTINCT_VALUES);
      ...
      useQualifierAsIndex = EncodedColumnsUtil
        .useQualifierAsIndex(EncodedColumnsUtil.getMinMaxQualifiersFromScan(scan));
      ...
      groupByCache = GroupByCacheFactory.INSTANCE.newCache(env, ScanUtil.getTenantId(scan),
        ScanUtil.getCustomAnnotations(scan), aggregators, estDistVals, isIncompatibleClient);
      if (LOGGER.isDebugEnabled()) {
        LOGGER
          .debug(
            LogUtil.addCustomAnnotations(
              "Grouped aggregation over unordered rows with scan " + scan + ", group by "
                + expressions + ", aggregators " + aggregators,
              ScanUtil.getCustomAnnotations(scan)));
        LOGGER.debug(LogUtil.addCustomAnnotations("Spillable groupby enabled: " + spillableEnabled,
          ScanUtil.getCustomAnnotations(scan)));
      }
    }
     ...
     * @return always true, because some rows are likely to exist as we are returning dummy result
     *         to the client.
     */
      ...
      if (scanStartRowKey.length > 0 && !ScanUtil.isLocalIndex(scan)) {
        if (hasRegionMoved()) {
          ...
          if (scanStartRowKey.length > 1 && Bytes.compareTo(lastByte, ByteUtil.ZERO_BYTE) == 0) {
            ...
            System.arraycopy(scanStartRowKey, 0, prevKey, 0, prevKey.length);
    ...
    /**
     * Return true if the region has moved in the middle of an ongoing scan operation, resulting in
     * scanner reset. Based on the return value of this function, we need to either scan the region
     * as if we are scanning for the first time or we need to scan the region considering that we
     * have already returned some rows back to client and we need to resume from the last row that
  ...
  /**
   * Used for an aggregate query in which the key order match the group by key order. In this case,
   * we can do the aggregation as we scan, by detecting when the group by key changes.
   */
  private static class OrderedGroupByRegionScanner extends BaseRegionScanner {
    ...
    private OrderedGroupByRegionScanner(final ObserverContext<RegionCoprocessorEnvironment> c,
      ...
      encodingScheme = EncodedColumnsUtil.getQualifierEncodingScheme(scan);
      initStartRowKey = ServerUtil.getScanStartRowKeyFromScanOrRegionBoundaries(scan, region);
              ...
              if (!aggBoundary) {
                ...
                if (LOGGER.isDebugEnabled()) {
                  LOGGER.debug(LogUtil.addCustomAnnotations("Row passed filters: " + kvs
                    + ", aggregated values: " + Arrays.asList(rowAggregators),
                    ScanUtil.getCustomAnnotations(scan)));
                }
                currentKey = key;
                if (result.size() > 0) {
      ...
      try {
        ...
        if (currentKey != null) {
          if (!isIncompatibleClient) {
            ...
            System.arraycopy(currentKey.get(), currentKey.getOffset(), aggregateGroupValueBytes, 0,
              aggregateGroupValueBytes.length);
            byte[] finalValue =
              ByteUtil.concat(PInteger.INSTANCE.toBytes(aggregateGroupValueBytes.length),
                aggregateGroupValueBytes, aggregateArrayBytes);
            Cell keyValue =
              PhoenixKeyValueUtil.newKeyValue(currentKeyRowKey.get(), currentKeyRowKey.getOffset(),
                currentKeyRowKey.getLength(), GROUPED_AGGREGATOR_VALUE_BYTES,
                GROUPED_AGGREGATOR_VALUE_BYTES, AGG_TIMESTAMP, finalValue, 0, finalValue.length);
            results.add(keyValue);
          } else {
            byte[] value = aggregators.toBytes(rowAggregators);
            Cell keyValue = PhoenixKeyValueUtil.newKeyValue(currentKey.get(),
              currentKey.getOffset(), currentKey.getLength(), SINGLE_COLUMN_FAMILY, SINGLE_COLUMN,
        ...
        LOGGER.error("Ordered group-by scanner next encountered some issue for" + " region {}",
          region.getRegionInfo().getRegionNameAsString(), e);
        if (e instanceof IOException) {
          throw e;
        } else {
    ...
    /**
     * Add dummy cell to the result list based on either the previous rowkey returned to the client
     * or the start rowkey and start rowkey include params.
        ...
        if (includeInitStartRowKey && initStartRowKey.length > 0) {
          ...
          if (
            ...
            if (initStartRowKey.length < (HConstants.MAX_ROW_LENGTH - 1 - regionLookupInMetaLen)) {
              prevKey = ByteUtil.previousKeyWithLength(
  ...
  protected boolean isRegionObserverFor(Scan scan) {
    return scan.getAttribute(BaseScannerRegionObserverConstants.UNORDERED_GROUP_BY_EXPRESSIONS)
        != null
      || scan.getAttribute(BaseScannerRegionObserverConstants.KEY_ORDERED_GROUP_BY_EXPRESSIONS)
          != null;
  }
