...
/**
 * Coprocessor for task related operations. This coprocessor would only be registered to SYSTEM.TASK
 * table
 */
...
public class TaskRegionObserver implements RegionObserver, RegionCoprocessor {
  ...
  protected ScheduledThreadPoolExecutor executor =
    new ScheduledThreadPoolExecutor(TaskType.values().length);
  ...
  private static Map<TaskType,
    String> classMap = ImmutableMap.<TaskType, String> builder()
  ...
  public static class SelfHealingTask extends TimerTask {
    ...
    public void run() {
      ...
      try {
        // TODO: Not sufficient info available when namespaces are enabled and index rebuild task
        // are run
        // getConnectionOnServer can fail when namespaces are enabled but SYSTEM namespace not
        // available
        connForTask =
        ...
        for (Task.TaskRecord taskRecord : taskRecords) {
          try {
            ...
            if (
              result == null || taskRecord.getStatus().equals(PTable.TaskStatus.RETRY.toString())
            ) {
              // reread task record. There might be async setting of task status
              taskRecord = Task
                .queryTaskTable(connForTask, taskRecord.getTimeStamp(), taskRecord.getSchemaName(),
                  taskRecord.getTableName(), taskType, taskRecord.getTenantId(), null)
                .get(0);
              if (
                taskRecord.getStatus() != null
              ...
              ServerTask.addTask(new SystemTaskParams.SystemTaskParamsBuilder().setConn(connForTask)
                ...
                .setTaskStatus(PTable.TaskStatus.STARTED.toString()).setData(taskRecord.getData())
                .setPriority(taskRecord.getPriority()).setStartTs(taskRecord.getTimeStamp())
                .setEndTs(null).setAccessCheckEnabled(true).build());
            ...
            LOGGER.warn("Exception while running self healingtask. "
              + "It will be retried in the next system task table scan : " + " taskType : "
              + taskRecord.getTaskType().name() + taskRecord.getSchemaName() + "."
              + taskRecord.getTableName() + " with tenant id "
              + (taskRecord.getTenantId() == null ? " IS NULL" : taskRecord.getTenantId())
    ...
    public static void setEndTaskStatus(PhoenixConnection connForTask, Task.TaskRecord taskRecord,
      String taskStatus) throws IOException, SQLException {
      ...
      ServerTask.addTask(new SystemTaskParams.SystemTaskParamsBuilder().setConn(connForTask)
        .setTaskType(taskRecord.getTaskType()).setTenantId(taskRecord.getTenantId())
        .setSchemaName(taskRecord.getSchemaName()).setTableName(taskRecord.getTableName())
        .setTaskStatus(taskStatus).setData(data).setPriority(taskRecord.getPriority())
        .setStartTs(taskRecord.getTimeStamp()).setEndTs(endTs).setAccessCheckEnabled(false)
        .build());
    }
  }
}
