...
public class CompactionScanner implements InternalScanner {
  ...
  public CompactionScanner(RegionCoprocessorEnvironment env, Store store,
    ...
    LOGGER.info("Starting CompactionScanner for table " + tableName + " store " + columnFamilyName
      + (this.major ? " major " : " not major ") + "compaction ttl "
      + ttlTracker.getRowContext().getTTL() + "ms " + "max lookback " + this.maxLookbackInMillis
      + "ms");
  ...
  private TTLTracker createTTLTrackerFor(RegionCoprocessorEnvironment env, Store store,
    ...
    try (PhoenixConnection serverConnection =
      ...
      return isPartitioned
        ? new PartitionedTableTTLTracker(baseTable, isSalted, isSharedIndex, isLongViewIndexEnabled,
          viewTTLTenantViewsPerScanLimit)
  ...
  /*
   * private void printRow(List<Cell> result, String title, boolean sort) { List<Cell> row; if
   * (sort) { row = new ArrayList<>(result); Collections.sort(row, CellTimeComparator.COMPARATOR); }
   * else { row = result; } System.out.println("---- " + title + " ----"); System.out.println((major
   * ? "Major " : "Not major ") + "compaction time: " + compactionTime);
   ...
   * System.out.println("TTL in ms: " + ttlInMillis); boolean maxLookbackLine = false; boolean
   * ttlLine = false; for (Cell cell : row) { if (!maxLookbackLine && cell.getTimestamp() <
   * maxLookbackWindowStart) { System.out.println("-----> Max lookback window start time: " +
   * maxLookbackWindowStart); maxLookbackLine = true; } else if (!ttlLine && cell.getTimestamp() <
   * ttlWindowStart) { System.out.println("-----> TTL window start time: " + ttlWindowStart);
   * ttlLine = true; } System.out.println(cell); } }
   */
  ...
  public boolean next(List<Cell> result) throws IOException {
    ...
    if (!result.isEmpty()) {
      // printRow(result, "Input for " + tableName + " " + columnFamilyName, true); // This is for
      // debugging
      phoenixLevelRowCompactor.compact(result, false);
      outputCellCount += result.size();
      // printRow(result, "Output for " + tableName + " " + columnFamilyName, true); // This is for
      // debugging
    }
    return hasMore;
  }
              ...
              if (isSharedIndex) {
                try (Connection tableConnection =
                  ...
                  for (PTable index : pTable.getIndexes()) {
                    ...
                    tableTTLInfoList
                      .add(new TableTTLInfo(pTable.getPhysicalName().getBytes(), tenantIdBytes,
                        index.getTableName().getBytes(), viewIndexIdBytes, index.getTTL()));
                  }

                }
              } else {
                tableTTLInfoList.add(new TableTTLInfo(physicalTableName.getBytes(), tenantIdBytes,
                  fullTableName.getBytes(), rowKeyMatcher, viewTTL));
              }
            }
          }
        }
  ...
  private class TableTTLTrackerForFlushesAndMinor implements TTLTracker {
    ...
    public TableTTLTrackerForFlushesAndMinor(String tableName) {
      ...
      ttl = DEFAULT_TTL;
      LOGGER.info(
        String.format("TableTTLTrackerForFlushesAndMinor params:- " + "(table-name=%s, ttl=%d)",
          tableName, ttl * 1000));
    }
  ...
  private class NonPartitionedTableTTLTracker implements TTLTracker {
    ...
    public NonPartitionedTableTTLTracker(PTable pTable, Store store) {
      ...
      LOGGER.info(String.format(
        "NonPartitionedTableTTLTracker params:- " + "(physical-name=%s, ttl=%d, isSystemTable=%s)",
  ...
  private class PartitionedTableTTLTracker implements TTLTracker {
    ...
    public PartitionedTableTTLTracker(PTable table, boolean isSalted, boolean isSharedIndex,
      boolean isLongViewIndexEnabled, int viewTTLTenantViewsPerScanLimit) throws IOException {
        ...
        if (LOGGER.isTraceEnabled()) {
          LOGGER.trace(String.format(
            "visiting row-key = %s, region = %s, " + "table-ttl-info=%s, "
              ...
              + "ttl = %d, matched-offset = %d, " + "pk-pos = %d, pk-pos-list = %s",
            ...
            matched ? Bytes.toStringBinary(tableTTLInfo.getMatchPattern()) : "NULL", rowTTLInSecs,
  ...
  class PhoenixLevelRowCompactor {
     ...
     * name and column qualifier. The cells within the max lookback window except the once at the
     * lower edge of the max lookback window (the last row of the max lookback window) are retained
     * immediately. This method also returned the remaining cells (outside the max lookback window)
     * of the empty colum
     */
    private void getLastRowVersionInMaxLookbackWindow(List<Cell> result, List<Cell> lastRowVersion,
      ...
      for (Cell cell : result) {
        ...
        if (currentColumnCell == null || !CellUtil.matchingColumn(cell, currentColumnCell)) {
          currentColumnCell = cell;
          isEmptyColumn = ScanUtil.isEmptyColumn(cell, emptyCF, emptyCQ);
          if (
            (cell.getType() != Cell.Type.Delete && cell.getType() != Cell.Type.DeleteColumn)
