...
/**
 * Client provides last DDL timestamp of tables/views/indexes included in read/write operation This
 * verifies that client has the latest version of LAST_DDL_TIMESTAMP version. If client's provided
 * LAST_DDL_TIMESTAMP is less than what is present in SYSTEM.CATALOG then it throws
...
public class VerifyLastDDLTimestamp {
   ...
   * @throws SQLException StaleMetadataCacheException if client provided timestamp is stale.
   */
  public static void verifyLastDDLTimestamp(ServerMetadataCache cache, byte[] tenantID,
    ...
    if (clientLastDDLTimestamp < lastDDLTimestamp) {
      LOGGER.error(
        "Stale metadata for LAST_DDL_TIMESTAMP for tenantID: {}, schema: {},"
        ...
        clientLastDDLTimestamp, lastDDLTimestamp);
      String fullTableName = SchemaUtil.getTableName(schemaName, tableName);
      throw new StaleMetadataCacheException(
        "Stale metadata cache for table name: " + fullTableName);
    }
  }
}
