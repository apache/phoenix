    ...
    try (PhoenixConnection conn =
      ...
      if (
        ...
        if (partialTransform != null) {
          ...
          // Fix unverified rows. Running partial transform will make the transform status go back
          // to started
          long startFromTs = 0;
          if (systemTransformRecord.getTransformLastStateTs() != null) {
            startFromTs = systemTransformRecord.getTransformLastStateTs().getTime() - 1;
          }
          TransformTool.runTransformTool(systemTransformRecord, conf, true, startFromTs, null, true,
            false);
