...
public class TransformMonitorTask extends BaseTask {
      ...
      if (
        ...
        if (partialTransform != null) {
          ...
          Transform.updateTransformRecord(conn, systemTransformRecord,
            PTable.TransformStatus.PENDING_PARTIAL_PASS, waitUntilTs);
        } else {
        ...
        if (waitUntilTs != null && EnvironmentEdgeManager.currentTimeMillis() < waitUntilTs) {
          // Still inside the client cache-refresh window; re-poll on the next scan.
          LOGGER.info(
        ...
        if (PTable.TransformType.isPartialTransform(systemTransformRecord.getTransformType())) {
          ...
          if (job != null && !job.isComplete()) {
            ...
            if (systemTransformRecord.getTransformRetryCount() < maxRetryCount) {
              // Retry the partial pass. This is a retry, so the retry count must strictly increase
              // (TransformTool's STARTED transition increments it) rather than staying pinned;
              // otherwise the retries-exhausted -> FAILED transition below is unreachable and a
              // deterministically-failing partial pass resubmits forever.
              kickPartialPass(conn, conf, systemTransformRecord, tableName, true);
            } else {
          ...
          // Transient window: the record is PARTIAL_PASS_RUNNING but still carries the full
          // transform type because the kick has not yet flipped it to the partial variant. Do not
          // act on the stale full-pass job id; the next monitor scan re-evaluates once the kick has
          // committed the partial-type marker.
  ...
  private void kickPartialPass(PhoenixConnection conn, Configuration conf,
    ...
    long startFromTs = 0;
    if (partialRecord.getTransformLastStateTs() != null) {
      startFromTs = partialRecord.getTransformLastStateTs().getTime() - 1;
    }
