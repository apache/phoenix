...
public class PhoenixTTLRegionObserver extends BaseScannerRegionObserver
  ...
  protected RegionScanner doPostScannerOpen(final ObserverContext<RegionCoprocessorEnvironment> c,
    ...
    LOG.trace(String.format(
      "********** PHOENIX-TTL: PhoenixTTLRegionObserver::postScannerOpen TTL for table = "
        + "[%s], scan = [%s], TTL = %d ***************, " + "numMaskExpiredRequestCount=%d, "
        + "numDeleteExpiredRequestCount=%d",
      s.getRegionInfo().getTable().getNameAsString(), scan.toJSON(Integer.MAX_VALUE),
      ScanUtil.getTTL(scan), metricSource.getMaskExpiredRequestCount(),
  ...
  private static class PhoenixTTLRegionScanner extends BaseRegionScanner {
    ...
    public PhoenixTTLRegionScanner(RegionCoprocessorEnvironment env, Scan scan,
      ...
      scanTableName =
        scan.getAttribute(BaseScannerRegionObserverConstants.PHOENIX_TTL_SCAN_TABLE_NAME);
