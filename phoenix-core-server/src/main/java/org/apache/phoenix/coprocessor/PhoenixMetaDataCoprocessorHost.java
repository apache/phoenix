...
public class PhoenixMetaDataCoprocessorHost extends CoprocessorHost<PhoenixCoprocessor,
  PhoenixMetaDataCoprocessorHost.PhoenixMetaDataControllerEnvironment> {
  ...
  private abstract class PhoenixObserverOperation
    extends ObserverOperationWithoutResult<MetaDataEndpointObserver> {
  ...
  private boolean execOperation(final PhoenixObserverOperation ctx) throws IOException {
  ...
  @Override
  protected void handleCoprocessorThrowable(final PhoenixMetaDataControllerEnvironment env,
  ...
  public static class PhoenixMetaDataControllerEnvironment extends
    BaseEnvironment<PhoenixCoprocessor> implements CoprocessorEnvironment<PhoenixCoprocessor> {

    private RegionCoprocessorEnvironment env;

    PhoenixMetaDataControllerEnvironment(RegionCoprocessorEnvironment env,
      PhoenixCoprocessor instance, int priority, int sequence, Configuration conf) {
  ...
  void preCreateTable(final String tenantId, final String tableName,
    final TableName physicalTableName, final TableName parentPhysicalTableName,
    final PTableType tableType, final Set<byte[]> familySet, final Set<TableName> indexes)
    throws IOException {
    execOperation(new PhoenixObserverOperation() {
      @Override
      public void call(MetaDataEndpointObserver observer) throws IOException {
        observer.preCreateTable(this, tenantId, tableName, physicalTableName,
          parentPhysicalTableName, tableType, familySet, indexes);
      }
    });
  }
  ...
  void preDropTable(final String tenantId, final String tableName,
    final TableName physicalTableName, final TableName parentPhysicalTableName,
    final PTableType tableType, final List<PTable> indexes) throws IOException {
    execOperation(new PhoenixObserverOperation() {
      @Override
      public void call(MetaDataEndpointObserver observer) throws IOException {
        observer.preDropTable(this, tenantId, tableName, physicalTableName, parentPhysicalTableName,
          tableType, indexes);
      }
    });
  }

  void preAlterTable(final String tenantId, final String tableName,
    final TableName physicalTableName, final TableName parentPhysicalTableName,
    final PTableType type) throws IOException {
    execOperation(new PhoenixObserverOperation() {
      @Override
      public void call(MetaDataEndpointObserver observer) throws IOException {
        observer.preAlterTable(this, tenantId, tableName, physicalTableName,
          parentPhysicalTableName, type);
      }
    });
  }
  ...
  void preIndexUpdate(final String tenantId, final String indexName,
    final TableName physicalTableName, final TableName parentPhysicalTableName,
    final PIndexState newState) throws IOException {
    execOperation(new PhoenixObserverOperation() {
      @Override
      public void call(MetaDataEndpointObserver observer) throws IOException {
        observer.preIndexUpdate(this, tenantId, indexName, physicalTableName,
          parentPhysicalTableName, newState);
      }
    });
  }
  ...
  @Override
  public PhoenixMetaDataControllerEnvironment createEnvironment(PhoenixCoprocessor instance,
