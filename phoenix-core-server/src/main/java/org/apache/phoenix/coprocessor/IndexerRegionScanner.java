...
public class IndexerRegionScanner extends GlobalIndexRegionScanner {
  ...
  private boolean verifySingleIndexRow(Result indexRow, final Put dataRow,
    ...
    Put indexPut = indexMaintainer.buildUpdateMutation(GenericKeyValueBuilder.INSTANCE, valueGetter,
      new ImmutableBytesWritable(dataRow.getRow()), ts, null, null, false,
    ...
    for (List<Cell> cells : indexPut.getFamilyCellMap().values()) {
      ...
      for (Cell expectedCell : cells) {
        ...
        if (!CellUtil.matchingValue(actualCell, expectedCell) || actualCell.getTimestamp() != ts) {
          if (
            isTimestampBeyondMaxLookBack(maxLookBackInMills, currentTime, actualCell.getTimestamp())
          ) {
  ...
  private void verifyIndexRows(List<KeyRange> keys, Map<byte[], Put> perTaskDataKeyToDataPutMap,
    ...
    // Check if any expected rows from index(which we didn't get) are beyond max look back and have
    // been compacted away
    if (!perTaskDataKeyToDataPutMap.isEmpty()) {
  ...
  private void addVerifyTask(TaskBatch<Boolean> tasks, final List<KeyRange> keys,
    final Map<byte[], Put> perTaskDataKeyToDataPutMap,
      ...
      public Boolean call() throws Exception {
        try {
          if (Thread.currentThread().isInterrupted()) {
            exceptionMessage =
              "Pool closed, not attempting to verify index rows! " + indexHTable.getName();
            throw new IOException(exceptionMessage);
          }
  ...
  private void parallelizeIndexVerify(
    IndexToolVerificationResult.PhaseResult verificationPhaseResult) throws IOException {
    ...
    List<IndexToolVerificationResult.PhaseResult> verificationPhaseResultList =
    ...
    IndexToolVerificationResult.PhaseResult perTaskVerificationPhaseResult =
  ...
  private void setMutationAttributes(Mutation m, byte[] uuidValue) {
    ScanUtil.annotateMutationWithMetadataAttributes(tenantId, schemaName, logicalTableName,
      tableType, lastDdlTimestamp, m);
  ...
  private byte[] commitIfReady(byte[] uuidValue,
    ...
    if (
      ...
      ungroupedAggregateRegionObserver.commitBatchWithRetries(region, mutationList,
  ...
  public boolean next(List<Cell> results) throws IOException {
    ...
    try {
        ...
        do {
          ...
          if (!row.isEmpty()) {
            ...
            if (!verify) {
              ...
              indexKeyToDataPutMap.put(indexMaintainer.getIndexRowKey(put, encodedRegionName), put);
            }
            rowCount++;
      ...
      LOGGER.error(
        String.format("IOException during rebuilding: %s", Throwables.getStackTraceAsString(e)));
      throw e;
    } finally {
