 ...
 * <h3>Purpose</h3> This scanner extends {@link UncoveredGlobalIndexRegionScanner} to handle CDC
 * index queries where the CDC index doesn't contain all the columns needed to satisfy the query. It
 * bridges the gap between CDC index rows and the original data table to reconstruct complete CDC
 * events.
 * <h3>CDC Event Processing</h3> The scanner processes two types of CDC events:
 * <ul>
 * <li><b>Regular CDC Events:</b> Requires data table scan to build CDC event JSON from
 * current/historical row state</li>
 * <li><b>Pre-Image CDC Events:</b> Contains embedded CDC data (e.g., TTL delete events) that can be
 * returned directly without data table scan</li>
 * </ul>
 * <h3>CDC Event Structure</h3> The scanner produces CDC events in JSON format containing:
 * <ul>
...
public class CDCGlobalIndexRegionScanner extends UncoveredGlobalIndexRegionScanner {
  ...
  protected boolean getNextCoveredIndexRow(List<Cell> result) throws IOException {
    if (indexRowIterator.hasNext()) {
      ...
      ImmutableBytesPtr dataRowKey = new ImmutableBytesPtr(indexToDataRowKeyMap.get(indexRowKey));
      ...
      Expression[] expressions =
        dataTableProjector != null ? dataTableProjector.getExpressions() : null;
      boolean isSingleCell = dataTableProjector != null;
      byte[] emptyCQ = EncodedColumnsUtil
        .getEmptyKeyValueInfo(cdcDataTableInfo.getQualifierEncodingScheme()).getFirst();
      ...
      try {
        if (dataRow != null) {
          ...
          cellLoop: for (Cell cell : dataRow.rawCells()) {
            ...
            if (cell.getType() == Cell.Type.DeleteFamily) {
              if (changeTS == cell.getTimestamp()) {
                ...
                changeTS > cell.getTimestamp() && changeBuilder.getLastDeletedTimestamp() == 0L
              ) {
          ...
          if (changeBuilder.isNonEmptyEvent()) {
            ...
            if (cdcRow != null && tupleProjector != null) {
              if (indexCell.getType() == Cell.Type.DeleteFamily) {
                ...
                result
                  .add(CellBuilderFactory.create(CellBuilderType.SHALLOW_COPY).setRow(indexRowKey)
                    .setFamily(ImmutableBytesPtr.cloneCellFamilyIfNecessary(indexCell))
                    .setQualifier(indexMaintainer.getEmptyKeyValueQualifier())
                    .setTimestamp(indexCell.getTimestamp()).setType(Cell.Type.Put)
  ...
  private Result getCDCImage(byte[] indexRowKey, Cell firstCell) throws JsonProcessingException {
    byte[] value =
      JacksonUtil.getObjectWriter(HashMap.class).writeValueAsBytes(changeBuilder.buildCDCEvent());
  ...
  private Result createCDCResult(byte[] indexRowKey, Cell firstCell, long timestamp, byte[] value) {
    ...
    return Result.create(Collections.singletonList(
      builder.setRow(indexRowKey).setFamily(ImmutableBytesPtr.cloneCellFamilyIfNecessary(firstCell))
        .setQualifier(cdcDataTableInfo.getCdcJsonColQualBytes()).setTimestamp(timestamp)
        .setValue(value).setType(Cell.Type.Put).build()));
  }
  ...
  /**
   * Handles CDC events that already contain pre-image data, avoiding data table scan. Supports both
   * the new CDC_IMAGE_CQ column and traditional CDC JSON column.
   ...
   */
  private boolean handlePreImageCDCEvent(List<Cell> indexRow, byte[] indexRowKey, Cell indexCell,
    ...
    for (Cell cell : indexRow) {
      if (
        Bytes.equals(cell.getQualifierArray(), cell.getQualifierOffset(), cell.getQualifierLength(),
          QueryConstants.CDC_IMAGE_CQ_BYTES, 0, QueryConstants.CDC_IMAGE_CQ_BYTES.length)
      ) {
        cdcDataCell = cell;
        break;
      }
   ...
   * @throws InvalidProtocolBufferException if the stored proto bytes are malformed.
   */
  private boolean handleIdxMutationsCDCEvent(List<Cell> indexRow, byte[] indexRowKey,
    Cell indexCell, List<Cell> result) throws InvalidProtocolBufferException {
    ...
    for (Cell cell : indexRow) {
      if (
        ...
        preMutations = IndexMutationsProtos.IndexMutations.parseFrom(rawBytes);
        preCell = cell;
      } else if (
        Bytes.equals(cell.getQualifierArray(), cell.getQualifierOffset(), cell.getQualifierLength(),
        ...
        byte[] rawBytes = CellUtil.cloneValue(cell);
        postMutations = IndexMutationsProtos.IndexMutations.parseFrom(rawBytes);
      }
    }
