...
public class CDCGlobalIndexRegionScanner extends UncoveredGlobalIndexRegionScanner {
              ...
              if (firstIndexCell.getType() == Cell.Type.DeleteFamily) {
                ...
                result
                  .add(CellBuilderFactory.create(CellBuilderType.SHALLOW_COPY).setRow(indexRowKey)
                    .setFamily(ImmutableBytesPtr.cloneCellFamilyIfNecessary(firstIndexCell))
                    .setQualifier(indexMaintainer.getEmptyKeyValueQualifier())
                    .setTimestamp(firstIndexCell.getTimestamp()).setType(Cell.Type.Put)
  ...
  private Result getCDCImage(byte[] indexRowKey, Cell firstCell) throws JsonProcessingException {
    ...
    Result cdcRow = Result.create(Arrays.asList(
      builder.setRow(indexRowKey).setFamily(ImmutableBytesPtr.cloneCellFamilyIfNecessary(firstCell))
        ...
        .setTimestamp(changeBuilder.getChangeTimestamp()).setValue(value).setType(Cell.Type.Put)
        .build()));
    return cdcRow;
  }
