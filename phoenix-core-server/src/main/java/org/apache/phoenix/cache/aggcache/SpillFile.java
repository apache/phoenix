...
/**
 * This class abstracts a SpillFile It is a accessible on a per page basis For every SpillFile
 * object a single spill file is always created. Additional overflow files are dynamically created
 * in case the page index requested is not covered by the spillFiles allocated so far
 */
public class SpillFile implements Closeable {
  ...
  /**
   * Create a new SpillFile using the Java TempFile creation function. SpillFile is access in pages.
   */
  ...
  private TempFile createTempFile() throws IOException {
    ...
    File tempFile = File.createTempFile(UUID.randomUUID().toString(), null, spillFilesDirectory);
    if (LOGGER.isDebugEnabled()) {
