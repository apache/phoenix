...
public class NonTxIndexBuilder extends BaseIndexBuilder {
  ...
  public Collection<Pair<Mutation, byte[]>> getIndexUpdate(Mutation mutation,
    IndexMetaData indexMetaData, LocalHBaseState localHBaseState) throws IOException {
   ...
   * <p>
   * Adds all the updates in the {@link Mutation} to the state, as a side-effect. current state of
   * the row for the mutation. mutation to batch
   * @param indexMetaData TODO index updates into which to add new updates. Modified as a
   *                      side-effect.
   */
  private void batchMutationAndAddUpdates(IndexUpdateManager manager, LocalTableState state,
    Mutation m, IndexMetaData indexMetaData) throws IOException {
    // The cells of a mutation are broken up into time stamp batches prior to this call (in
    // Indexer).
   ...
   * pending update and building the index update based on the new row state. map to update with new
   * index elements timestamp-based batch of edits local state to update and pass to the codec
  ...
  private boolean addMutationsForBatch(IndexUpdateManager updateMap, Batch batch,
    LocalTableState state, IndexMetaData indexMetaData) throws IOException {
  ...
  private long addUpdateForGivenTimestamp(long ts, LocalTableState state,
    IndexUpdateManager updateMap, IndexMetaData indexMetaData) throws IOException {
   ...
   * batch is back in time). to update with index mutations current state of the table
   * @param indexMetaData TODO to apply to the current state
   ...
   * {@link IndexCodec#getIndexDeletes(TableState, IndexMetaData, byte[], byte[])} and then add them
   * to the update map.
   * <p>
  ...
  protected void addDeleteUpdatesToMap(IndexUpdateManager updateMap, LocalTableState state, long ts,
    ...
    Iterable<IndexUpdate> cleanup = codec.getIndexDeletes(state, indexMetaData,
      env.getRegionInfo().getStartKey(), env.getRegionInfo().getEndKey());
    if (cleanup != null) {
