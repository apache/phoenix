 ...
 * <p>
 * On failure to #write(Multimap), we return a MultiIndexWriteFailureException that contains the
 * list of {@link HTableInterfaceReference} that didn't complete successfully.
 * <p>
...
public class TrackingParallelWriterIndexCommitter implements IndexCommitter {
  ...
  public void write(Multimap<HTableInterfaceReference, Mutation> toWrite,
    final boolean allowLocalUpdates, final int clientVersion) throws IOException {
    ...
    for (Entry<HTableInterfaceReference, Collection<Mutation>> entry : entries) {
      ...
      tasks.add(new Task<Boolean>() {
        ...
        private void throwFailureIfDone() throws SingleIndexWriteFailureException {
          if (
            stopped.isStopped() || (env != null && (env.getConnection() == null
              || env.getConnection().isClosed() || env.getConnection().isAborted()))
    ...
    if (failedTables.size() > 0) {
      ...
      if (disableIndexOnFailure) {
        ...
        exception = new MultiIndexWriteFailureException(Collections.unmodifiableList(failedTables),
          false, cause);
        throw wrapInDoNotRetryIOException("At least one index write failed after retries",
          exception, EnvironmentEdgeManager.currentTimeMillis());
      }
    }
    return;
  }
