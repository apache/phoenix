...
public abstract class AbstractParallelWriterIndexCommitter implements IndexCommitter {
  ...
  public static final String NUM_CONCURRENT_INDEX_WRITER_THREADS_CONF_KEY =
    "phoenix.index.writer.threads.max";
  ...
  static {
    Configuration.addDeprecation("index.writer.threads.max",
      NUM_CONCURRENT_INDEX_WRITER_THREADS_CONF_KEY);
    Configuration.addDeprecation("index.writer.threads.keepalivetime",
      INDEX_WRITER_KEEP_ALIVE_TIME_CONF_KEY);
  }
  ...
  public void setup(IndexWriter parent, RegionCoprocessorEnvironment env, String name,
    ...
    setup(IndexWriterUtils.getDefaultDelegateHTableFactory(env),
      ThreadPoolManager.getExecutor(new ThreadPoolBuilder(name, conf)
   ...
   * Exposed for TESTING
   */
  public void setup(HTableFactory factory, ExecutorService pool, Stoppable stop,
        ...
        public Void call() throws Exception {
          ...
          try {
            ...
            if (disableIndexOnFailure) {
              factory = clientVersion < MetaDataProtocol.MIN_CLIENT_RETRY_INDEX_WRITES
                ? retryingFactory
                : noRetriesFactory;
            } else {
   ...
   * {@link Stoppable} passed in the constructor. the reason for stopping
   */
  @Override
