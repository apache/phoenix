...
public class IndexWriterUtils {
   ...
   * IndexWriterUtils#NUM_CONCURRENT_INDEX_WRITER_THREADS_CONF_KEY) has a single HTable. However,
   * each table is backed by a threadpool to manage the updates to that table. this specifies the
   * number of threads to allow in each of those tables. Generally, you shouldn't need to change
   * this, unless you have a small number of indexes to which most of the writes go. Defaults to:
   * {@value #DEFAULT_NUM_PER_TABLE_THREADS}.
   * <p>
   ...
   * than HBase's rpc timeout (default of 60 sec) or else the client will retry before receiving the
   * response
   */
  @Deprecated
  ...
  /**
   * Retry server-server index write rpc only once, and let the client retry the data write instead
   * to avoid tying up the handler
   */
  ...
  public static class CoprocessorHConnectionTableFactory implements HTableFactory {
    ...
    CoprocessorHConnectionTableFactory(RegionCoprocessorEnvironment env,
