...
public class IndexCDCConsumer implements Runnable {
  ...
  private long processCDCBatch(String partitionId, String ownerPartitionId,
    ...
    try (PhoenixConnection conn =
      ...
      List<Pair<Long, BsonDocument>> batchMutations = new ArrayList<>();
  ...
  private void executeIndexMutations(String partitionId,
    List<Pair<Long, BsonDocument>> batchMutations, String ownerPartitionId,
    ...
    if (!batchMutations.isEmpty()) {
      ...
      for (Pair<Long, BsonDocument> batchMutation : batchMutations) {
        BsonDocument mutationsDoc = batchMutation.getSecond();
        BsonArray tables = mutationsDoc.getArray("tables");
        BsonArray mutations = mutationsDoc.getArray("mutations");
        ...
        for (int i = 0; i < tables.size(); i++) {
          BsonValue tableValue = tables.get(i);
          BsonValue mutationValue = mutations.get(i);
          byte[] indexTableName = tableValue.asBinary().getData();
          byte[] mutationBytes = mutationValue.asBinary().getData();
      ...
      if (!indexUpdates.isEmpty()) {
        ...
        LOG.debug(
          ...
          indexUpdates.size(), dataTableName, partitionId, ownerPartitionId,
          lastProcessedTimestamp);
      }
    }
  }
  ...
  private static Pair<Long, Boolean> getMutationsAndTimestamp(PreparedStatement ps,
    long initialLastTimestamp, List<Pair<Long, BsonDocument>> batchMutations) throws SQLException {
    ...
    try (ResultSet rs = ps.executeQuery()) {
      while (rs.next()) {
        ...
        String cdcJson = rs.getString(2);
        if (cdcJson != null && !cdcJson.isEmpty()) {
          BsonDocument mutationsDoc = BsonDocument.parse(cdcJson);
          if (mutationsDoc.containsKey("tables") && mutationsDoc.containsKey("mutations")) {
            batchMutations.add(Pair.newPair(lastTimestamp, mutationsDoc));
          }
        }
      }
    }
