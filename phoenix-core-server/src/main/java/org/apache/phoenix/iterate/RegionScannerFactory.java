...
public abstract class RegionScannerFactory {
  ...
  /**
   * Returns a processed region scanner based on the query conditions. Thie functionality is
   * abstracted out of the non-aggregate region observer class for better usage
   * @param scan input scan
   * @param s    input region scanner
   */
  public abstract RegionScanner getRegionScanner(final Scan scan, final RegionScanner s)
    throws Throwable;

  /**
   * Return wrapped scanner that catches unexpected exceptions (i.e. Phoenix bugs) and re-throws as
   * DoNotRetryIOException to prevent needless retrying hanging the query for 30 seconds.
   * Unfortunately, until HBASE-7481 gets fixed, there's no way to do the same from a custom filter.
  ...
  public RegionScanner getWrappedScanner(final RegionCoprocessorEnvironment env,
    ...
    final Expression[] serverParsedFuncRefs, final int offset, final Scan scan,
    final ColumnReference[] dataColumns, final TupleProjector tupleProjector,
    final Region dataRegion, final IndexMaintainer indexMaintainer, PhoenixTransactionContext tx,
    final byte[][] viewConstants, final KeyValueSchema kvSchema, final ValueBitSet kvSchemaBitSet,
    final TupleProjector projector, final ImmutableBytesWritable ptr,
    final boolean useQualifierAsListIndex) throws IOException {
    return new RegionScanner() {
...

      {
        ...
        if (ScanUtil.isLocalOrUncoveredGlobalIndex(scan)) {
          byte[] expBytes = scan.getAttribute(BaseScannerRegionObserverConstants.INDEX_FILTER);
          if (expBytes == null) {
            // For older clients
            expBytes = scan.getAttribute(BaseScannerRegionObserverConstants.LOCAL_INDEX_FILTER);
          }
          if (expBytes != null) {
            try {
          ...
          if (limitBytes == null) {
            // For older clients
            limitBytes = scan.getAttribute(BaseScannerRegionObserverConstants.LOCAL_INDEX_LIMIT);
          }
          if (limitBytes != null) {
            extraLimit = Bytes.toLong(limitBytes);
          }
          if (
            ScanUtil.isLocalOrUncoveredGlobalIndex(scan) && (tupleProjector != null
      ...
      public boolean nextRaw(List result, ScannerContext scannerContext) throws IOException {
        try {
          ...
          if ((ScanUtil.isLocalOrUncoveredGlobalIndex(scan)) && !ScanUtil.isAnalyzeTable(scan)) {
            ...
            if (extraWhere != null) {
              Tuple merged = useQualifierAsListIndex
          ...
          if (
            serverParsedFuncRefs != null && serverParsedFuncRefs.length > 0
              && serverParsedKVRefs.size() > 0
          ) {
       ...
       * @return A pair, whose first part is a combined projected value tuple containing the known
       *         column values along with resolved dynamic column values and whose second part is
       *         the serialized list of dynamic column PColumns. In case dynamic columns are not to
       *         be exposed or are not present, this returns the original tuple and an empty byte
       *         array.
      ...
      private Pair<Tuple, byte[]> getTupleWithDynColsIfRequired(List<Cell> result, Tuple tuple)
        ...
        if (Bytes.equals(scan.getAttribute(WILDCARD_SCAN_INCLUDES_DYNAMIC_COLUMNS), TRUE_BYTES)) {
          ...
          TupleProjector dynColTupleProj =
            TupleProjector.getDynamicColumnsTupleProjector(result, dynCols, dynColCells);
          if (dynColTupleProj != null) {
            Tuple toProject = useQualifierAsListIndex
              ? new PositionBasedResultTuple(dynColCells)
              : new ResultTuple(Result.create(dynColCells));
            Tuple dynColsProjectedTuple =
              dynColTupleProj.projectResults(toProject, useNewValueColumnQualifier);
            ...
            Tuple mergedTuple =
              TupleProjector.mergeProjectedValue((TupleProjector.ProjectedValueTuple) tuple,
                destBitSet, dynColsProjectedTuple, dynColTupleProj.getValueBitSet(),
                projector.getSchema().getFieldCount(), useNewValueColumnQualifier);
      ...
      /**
       * When there is a merge in progress while scanning local indexes we might get the key values
       * less than scan start row. In that case we need to scan until get the row key more or equal
       * to scan start key. TODO try to fix this case in LocalIndexStoreFileScanner when there is a
       * merge.
       */
      private boolean scanTillScanStartRow(final RegionScanner s, List<Cell> result,
        ...
        for (KeyValueColumnExpression kvExp : serverParsedKVRefs) {
          if (kvExp.evaluate(tuple, ptr)) {
            ...
            while (itr.hasNext()) {
              Cell kv = itr.next();
              if (
                Bytes.equals(kvExp.getColumnFamily(), 0, kvExp.getColumnFamily().length,
