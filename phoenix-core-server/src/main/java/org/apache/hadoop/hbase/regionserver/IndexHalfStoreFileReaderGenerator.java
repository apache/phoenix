...
public class IndexHalfStoreFileReaderGenerator implements RegionObserver, RegionCoprocessor {
  ...
  public StoreFileReader preStoreFileReaderOpen(ObserverContext<RegionCoprocessorEnvironment> ctx,
    ...
    if (reader == null && r != null) {
      ...
      try (PhoenixConnection conn =
        ...
        Scan scan = MetaTableAccessor.getScanForTableName(hbaseConn.getConfiguration(), tableName);
        SingleColumnValueFilter scvf = null;
        if (Reference.isTopFileRegion(r.getFileRegion())) {
          scvf = new SingleColumnValueFilter(HConstants.CATALOG_FAMILY, HConstants.SPLITB_QUALIFIER,
            CompareOperator.EQUAL, RegionInfoUtil.toByteArray(region.getRegionInfo()));
          scvf.setFilterIfMissing(true);
        } else {
          scvf = new SingleColumnValueFilter(HConstants.CATALOG_FAMILY, HConstants.SPLITA_QUALIFIER,
            CompareOperator.EQUAL, RegionInfoUtil.toByteArray(region.getRegionInfo()));
          scvf.setFilterIfMissing(true);
        }
        if (scvf != null) {
        ...
        if (result == null || result.isEmpty()) {
          ...
          if (Bytes.compareTo(mergeRegions.get(0).getStartKey(), splitRow) == 0) {
            if (
              ...
              regionStartKeyInHFile = mergeRegions.get(0).getStartKey().length == 0
        ...
        for (PTable index : indexes) {
          if (index.getIndexType() == IndexType.LOCAL) {
            ...
            indexMaintainers.put(new ImmutableBytesWritable(
              index.getviewIndexIdType().toBytes(index.getViewIndexId())), indexMaintainer);
          }
        }
   ...
   * @return StoreScanner for new Local Index data for a passed store and Null if repair is not
   *         possible
   */
  private InternalScanner getRepairScanner(RegionCoprocessorEnvironment env, Store store)
    throws IOException {
  ...
  private boolean isLocalIndexStore(Store store) {
    return store.getColumnFamilyDescriptor().getNameAsString()
