...
public class LocalIndexStoreFileScanner extends CompatLocalIndexStoreFileScanner {
  ...
  public LocalIndexStoreFileScanner(IndexHalfStoreFileReader reader, boolean cacheBlocks,
    ...
    this.reader = reader;
    this.changeBottomKeys = this.reader.getRegionInfo().getStartKey().length == 0
  ...
  private Cell getChangedKey(Cell next, boolean changeBottomKeys) {
    ...
    KeyValue changedKv = new KeyValue(changedKey, 0, changedKey.length, next.getFamilyArray(),
      next.getFamilyOffset(), next.getFamilyLength(), next.getQualifierArray(),
      next.getQualifierOffset(), next.getQualifierLength(), next.getTimestamp(),
      Type.codeToType(next.getTypeByte()), next.getValueArray(), next.getValueOffset(),
      next.getValueLength(), next.getTagsArray(), next.getTagsOffset(), next.getTagsLength());
    return changedKv;
  }
  ...
  public boolean requestSeek(Cell kv, boolean forward, boolean useBloom) throws IOException {
    ...
    if (requestSeek) {
      Cell peek = super.peek();
      if (
        Bytes.compareTo(kv.getRowArray(), kv.getRowOffset(), kv.getRowLength(), peek.getRowArray(),
          peek.getRowOffset(), peek.getRowLength()) == 0
      ) {
  ...
  private boolean isSatisfiedMidKeyCondition(Cell kv) {
    ImmutableBytesWritable rowKey = new ImmutableBytesWritable(kv.getRowArray(),
      kv.getRowOffset() + reader.getOffset(), kv.getRowLength() - reader.getOffset());
    Entry<ImmutableBytesWritable, IndexMaintainer> entry =
      reader.getIndexMaintainers().entrySet().iterator().next();
   ...
   * But in the actual HFiles, the key will be with the start key of the old parent region. In order
   * to make the real seek in the HFiles, we need to build the old key. The logic here is just
   * replace daughter region start key with parent region start key in the key part.
   */
  private KeyValue getKeyPresentInHFiles(Cell keyValue) {
    ...
    System.arraycopy(keyValue.getRowArray(), keyValue.getRowOffset() + reader.getSplitRow().length,
      replacedKey, reader.getOffset() + ROW_LENGTH_SIZE, rowLength - reader.getSplitRow().length);
    System.arraycopy(keyValue.getRowArray(), rowOffset + rowLength, replacedKey,
      reader.getOffset() + keyValue.getRowLength() - reader.getSplitRow().length + ROW_LENGTH_SIZE,
      keyValue.getRowArray().length - (rowOffset + rowLength));
  ...
  public boolean seekOrReseek(Cell cell, boolean isSeek) throws IOException {
    ...
    if (reader.isTop()) {
      ...
      if (
        !isSeek && reader.getRegionInfo().getStartKey().length == 0
  ...
  private byte[] getNewRowkeyByRegionStartKeyReplacedWithSplitKey(Cell kv,
    ...
    System.arraycopy(
      changeBottomKeys ? new byte[reader.getSplitRow().length] : reader.getSplitRow(), 0,
