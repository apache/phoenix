...
public class TenantLoadEventGeneratorFactory
  ...
  @Override
  public LoadEventGenerator<TenantOperationInfo> newLoadEventGenerator(PhoenixUtil phoenixUtil,
    DataModel model, Scenario scenario, Properties properties) {
  ...
  @Override
  public LoadEventGenerator<TenantOperationInfo> newLoadEventGenerator(PhoenixUtil phoenixUtil,
    DataModel model, Scenario scenario, List<PherfWorkHandler> workHandlers,
    Properties properties) {
    GeneratorType type = GeneratorType.valueOf(scenario.getGeneratorName());
    switch (type) {
      case WEIGHTED:
        return new WeightedRandomLoadEventGenerator(phoenixUtil, model, scenario, workHandlers,
          properties);
      case UNIFORM:
        return new UniformDistributionLoadEventGenerator(phoenixUtil, model, scenario, workHandlers,
          properties);
      case SEQUENTIAL:
        return new SequentialLoadEventGenerator(phoenixUtil, model, scenario, workHandlers,
          properties);
      default:
        throw new IllegalArgumentException("Unknown generator type");
    }
