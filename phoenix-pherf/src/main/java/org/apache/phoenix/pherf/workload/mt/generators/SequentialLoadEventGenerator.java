...
/**
 * A load generator that generates tenant operation events in the order specified in the scenario
 * file. The scenario file can also specify on how many iterations to be executed per operation,
...
public class SequentialLoadEventGenerator extends BaseLoadEventGenerator {
  ...
  private static class SequentialSampler {
    ...
    public SequentialSampler(List<Operation> operationList, DataModel model, Scenario scenario,
    ...
    public TenantOperationInfo nextSample() {
      ...
      String paddedTenantId =
        Strings.padStart(formattedTenantId, loadProfile.getTenantIdLength(), 'x');
  ...
  public SequentialLoadEventGenerator(PhoenixUtil phoenixUtil, DataModel model, Scenario scenario,
    ...
    this.sampler =
      new SequentialSampler(operationFactory.getOperations(), model, scenario, properties);
  ...
  public List<PherfWorkHandler> getWorkHandlers(Properties properties) {
    ...
    if (properties.containsKey(PherfConstants.HANDLERS_PER_SCENARIO_PROP_KEY)) {
      this.numHandlers =
        Integer.parseInt((String) properties.get(PherfConstants.HANDLERS_PER_SCENARIO_PROP_KEY));
    }

    if (properties.containsKey(PherfConstants.NUM_SEQUENTIAL_ITERATIONS_PROP_KEY)) {
      this.numIterations = Integer
        .parseInt((String) properties.get(PherfConstants.NUM_SEQUENTIAL_ITERATIONS_PROP_KEY));
    }

    if (properties.containsKey(PherfConstants.NUM_SEQUENTIAL_EXECUTION_TYPE_PROP_KEY)) {
      this.executionType = ExecutionType
        .valueOf((String) properties.get(PherfConstants.NUM_SEQUENTIAL_EXECUTION_TYPE_PROP_KEY));
    ...
    for (int i = 0; i < numHandlers; i++) {
      ...
      workers.add(new RendezvousingWorkHandler(operationFactory, handlerId, rendezvousPoints));
    }
    return workers;
  }
