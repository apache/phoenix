...
/**
 * This class starts JMX stats for the configured monitors. Monitors should be configured in
 * MonitorDetails Enum. Each stat implements {@link org.apache.phoenix.pherf.jmx.monitors.Monitor}.
 * For the duration of any Pherf run, when the configured
 * {@link org.apache.phoenix.pherf.PherfConstants#MONITOR_FREQUENCY} is reached a snapshot of each
 * monitor is taken and dumped out to a log file.
 */
          ...
          while (!shouldStop()) {
            ...
            synchronized (resultHandler) {
              for (MonitorDetails monitorDetails : MONITOR_DETAILS_LIST) {
                ...
                try {
                  StandardMBean bean =
                    new StandardMBean(monitorDetails.getMonitor(), Monitor.class);
                  ...
                  Result result = new Result(ResultFileDetails.CSV,
