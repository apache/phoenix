    ...
    return new Function<TenantOperationInfo, OperationStats>() {
      ...
      public OperationStats apply(final TenantOperationInfo input) {
        ...
        final String tenantId =
          isTenantGroupGlobal || upsert.isUseGlobalConnection() ? null : input.getTenantId();
        final String opName = String.format("%s:%s:%s:%s:%s", scenarioName, tableName, opGroup,
        ...
        try (Connection connection = phoenixUtil.getConnection(tenantId)) {
          ...
          try {
            ...
            for (long i = rowCount; i > 0; i--) {
              stmt = phoenixUtil.buildStatement(rulesApplier, scenario, upsert.getColumn(), stmt,
                simpleDateFormat);
              if (useBatchApi) {
                stmt.addBatch();
              } else {
