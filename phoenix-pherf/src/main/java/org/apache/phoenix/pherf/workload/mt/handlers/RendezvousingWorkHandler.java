...
/**
 * A handler {@link WorkHandler} for simple orchestrations using the supplied rendezvous points The
 * handler will wait for the rendezvous to happen before executing the operations {@link Operation}
 * The handlers as in the basic {@link TenantOperationWorkHandler} will pick up the operation events
 * as and when they become available on the {@link com.lmax.disruptor.RingBuffer} when published by
 * the workload generator {@link MultiTenantWorkload}
 */
...
public class RendezvousingWorkHandler
  ...
  public RendezvousingWorkHandler(TenantOperationFactory operationFactory, String handlerId,
  ...
  public void onEvent(TenantOperationEvent event) throws Exception {
    ...
    boolean startRendezvousEnabled =
      rendezvousPoints.containsKey(PherfConstants.MT_HANDLER_START_RENDEZVOUS_PROP_KEY);
    ...
    boolean resultsRendezvousEnabled =
      rendezvousPoints.containsKey(PherfConstants.MT_HANDLER_RESULTS_RENDEZVOUS_PROP_KEY);
  ...
  public void onStart() {
    ...
    LOGGER.info(String.format("TenantOperationWorkHandler started for %s:%s", scenario.getName(),
  ...
  public void onShutdown() {
    Scenario scenario = operationFactory.getScenario();
    LOGGER.info(String.format("TenantOperationWorkHandler stopped for %s:%s", scenario.getName(),
      scenario.getTableName()));
  }
