...
public class Query {
  ...
  /**
   * DDL is executed only once. If tenantId is specified then DDL is executed with tenant specific
   * connection.
   */
  @XmlAttribute
  ...
  public void setHint(String queryHint) {
    if (null != queryHint) {
      this.statement =
        this.statement.toUpperCase().replace("SELECT ", "SELECT /*+ " + queryHint + "*/ ");
    }
  }
