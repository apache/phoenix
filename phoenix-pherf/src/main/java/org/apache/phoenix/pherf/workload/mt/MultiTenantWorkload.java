...
/**
 * This class creates workload for tenant based load profiles. It uses @see
 * {@link TenantOperationFactory} in conjunction with
 * @see {@link LoadEventGenerator} to generate the load events. It then publishes these events onto
 *      a RingBuffer based queue. The @see {@link TenantOperationWorkHandler} drains the events from
 *      the queue and executes them. Reference for RingBuffer based queue
...
public class MultiTenantWorkload implements Workload {
  ...
  public MultiTenantWorkload(PhoenixUtil phoenixUtil, DataModel model, Scenario scenario,
    ...
    this.generator =
      evtGeneratorFactory.newLoadEventGenerator(phoenixUtil, model, scenario, properties);
  }

  public MultiTenantWorkload(PhoenixUtil phoenixUtil, DataModel model, Scenario scenario,
    List<PherfWorkHandler> workHandlers, Properties properties) throws Exception {
    this.generator = evtGeneratorFactory.newLoadEventGenerator(phoenixUtil, model, scenario,
      workHandlers, properties);
  }
