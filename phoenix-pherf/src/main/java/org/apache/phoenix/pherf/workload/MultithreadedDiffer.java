...
class MultithreadedDiffer implements Callable<Void> {
  ...
  private void diffQuery() throws Exception {
    ...
    getThreadTime().getRunTimesInMs()
      .add(new RunTime(verifyResult == true ? PherfConstants.DIFF_PASS : PherfConstants.DIFF_FAIL,
        explainPlan, startDate, -1L, elapsedTime, !(elapsedTime >= executionDurationInMs)));
  }
  ...
  public Void call() throws Exception {
    ...
    for (long i = numberOfExecutions; (i > 0
      && ((EnvironmentEdgeManager.currentTimeMillis() - start) < executionDurationInMs)); i--) {
      try {
        diffQuery();
      } catch (Exception e) {
        e.printStackTrace();
