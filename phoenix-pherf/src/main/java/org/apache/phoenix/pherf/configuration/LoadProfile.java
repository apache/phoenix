...
public class LoadProfile {
  ...
  /**
   * Holds the format to be used when generating tenantIds. TenantId format should typically have 2
   * parts - 1. string fmt - that hold the tenant group id. 2. int fmt - that holds a random number
   * between 1 and max tenants for e.g DEFAULT_TENANT_ID_FMT = "T%s%08d"; When the Tenant Group is
   * configured to use a global connection, for now this is modelled as a special tenant whose id
   * will translate to "TGLOBAL00000001" since the group id => "GLOBAL" and num tenants = 1. For now
   * this is a hack/temporary workaround. TODO : Ideally it needs to be built into the framework and
   * injected during event generation.
   */
