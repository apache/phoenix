...
public class WriteWorkload implements Workload {
   ...
   */
  public WriteWorkload(PhoenixUtil phoenixUtil, XMLConfigParser parser, Properties properties,
    ...
    String bSize = (writeParams == null) || (writeParams.getBatchSize() == Long.MIN_VALUE)
  ...
  private synchronized void exec(DataLoadTimeSummary dataLoadTimeSummary,
    ...
    pUtil.executeScenarioDdl(scenario.getPreScenarioDdls(), scenario.getTenantId(),
      dataLoadTimeSummary);

    // Write data
    List<Future<Info>> writeBatches = getBatches(dataLoadThreadTime, scenario);
    waitForBatches(dataLoadTimeSummary, scenario, EnvironmentEdgeManager.currentTimeMillis(),
      writeBatches);
    ...
    pUtil.executeScenarioDdl(scenario.getPostScenarioDdls(), scenario.getTenantId(),
      dataLoadTimeSummary);
  }
  ...
  private List<Future<Info>> getBatches(DataLoadThreadTime dataLoadThreadTime, Scenario scenario)
    ...
    for (int i = 0; i < getThreadPoolSize(); i++) {
      List<Column> phxMetaCols = pUtil.getColumnsFromPhoenix(scenario.getSchemaName(),
      ...
      Future<Info> write = upsertData(scenario, phxMetaCols, scenario.getTableName(),
        threadRowCount, dataLoadThreadTime, this.useBatchApi);
  ...
  private void waitForBatches(DataLoadTimeSummary dataLoadTimeSummary, Scenario scenario,
    ...
    LOGGER
      .info("Writes completed with total row count (" + sumRows + ") with total elapsed time of ("
        + testDuration + ") ms and total CPU execution time of (" + sumDuration + ") ms");
  ...
  public Future<Info> upsertData(final Scenario scenario, final List<Column> columns,
    final String tableName, final int rowCount, final DataLoadThreadTime dataLoadThreadTime,
    final boolean useBatchApi) {
    Future<Info> future = pool.submit(new Callable<Info>() {
      ...
      public Info call() throws Exception {
        ...
        try {
          ...
          long maxDuration = (WriteWorkload.this.writeParams == null)
            ? Long.MAX_VALUE
            : WriteWorkload.this.writeParams.getExecutionDurationInMs();

          int logPerNRows = PherfConstants.LOG_PER_NROWS;
          String customizedLogPerNRows =
            connection.getClientInfo().getProperty(PherfConstants.LOG_PER_NROWS_NAME);
          ...
          for (long i = rowCount; (i > 0)
            && ((EnvironmentEdgeManager.currentTimeMillis() - logStartTime) < maxDuration); i--) {
            ...
            if ((i % getBatchSize()) == 0) {
              if (useBatchApi) {
                ...
                for (int x = 0; x < results.length; x++) {
                  ...
                  if (result < 1) {
                    final String msg =
                      "Failed to write update in batch (update count=" + result + ")";
              ...
              LOGGER.info("Writer (" + Thread.currentThread().getName()
                + ") committed Batch. Total " + getBatchSize() + " rows for this thread ("
                + this.hashCode() + ") in (" + duration + ") Ms");

              if (i % logPerNRows == 0 && i != 0) {
                dataLoadThreadTime.add(tableName, Thread.currentThread().getName(), i,
