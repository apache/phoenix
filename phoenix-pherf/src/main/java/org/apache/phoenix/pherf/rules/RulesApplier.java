...
public class RulesApplier {
  ...
  private ConcurrentHashMap<String, RuleBasedDataGenerator> columnRuleBasedDataGeneratorMap =
   ...
   * @param scenario      {@link org.apache.phoenix.pherf.configuration.Scenario} We are getting
   *                      data for
   * @param phxMetaColumn {@link org.apache.phoenix.pherf.configuration.Column} From Phoenix
   *                      MetaData that are generating data for. It defines the type we are trying
   *                      to match.
   */
  public DataValue getDataForRule(Scenario scenario, Column phxMetaColumn) throws Exception {
    // TODO Make a Set of Rules that have already been applied so that so we don't generate for
    // every value

    List<Scenario> scenarios = dataModel != null ? dataModel.getScenarios() : parser.getScenarios();
    DataValue value = null;
    if (scenarios.contains(scenario)) {
      LOGGER.debug("We found a correct Scenario" + scenario.getName() + "column "
        + phxMetaColumn.getName() + " " + phxMetaColumn.getType());

      Map<DataTypeMapping, List> overrideRuleMap = this.getCachedScenarioOverrides(scenario);

      if (overrideRuleMap != null) {
        List<Column> overrideRuleList =
          this.getCachedScenarioOverrides(scenario).get(phxMetaColumn.getType());
      ...
      if (ruleList != null && ruleList.contains(phxMetaColumn)) {
        ...
        LOGGER.warn(String.format(
          "Attempted to apply rule to data, " + "but could not find a rule to match type %s on %s",
    ...
    switch (column.getType()) {
      ...
      case UNSIGNED_LONG:
        if ((column.getDataValues() != null) && (column.getDataValues().size() > 0)) {
          ...
          if (
            column.getType() == DataTypeMapping.UNSIGNED_LONG
          ) Preconditions.checkArgument((minLong > 0) && (maxLong > 0),
            "min and max values need to be set in configuration for unsigned_longs "
              + column.getName());
      ...
      case TIMESTAMP:
        if ((column.getDataValues() != null) && (column.getDataValues().size() > 0)) {
          ...
          Preconditions.checkArgument((minYear > 0) && (maxYear > 0),
            "min and max values need to be set in configuration for date/timestamps "
              + column.getName());
  ...
  /**
   * Given an int chance [0-100] inclusive, this method will return true if a winner is selected,
   * otherwise false.
  ...
  private DataValue pickDataValueFromList(final DataValue valueRule) throws Exception {
    ...
    Preconditions.checkArgument(
      (retValue.getMinValue() != null) || (retValue.getMaxValue() != null),
      "Both min/maxValue tags must be set if value tag is not used");
    Preconditions.checkArgument((retValue.getType() == DataTypeMapping.DATE),
      "Currently on DATE is supported for ranged random values");
  ...
  /**
   * Top level {@link java.util.List} {@link java.util.Map}. This will likely only have one entry
   * until we have multiple files.
   * <p/>
   * <p/>
   * Each Map entry in the List is: {@link java.util.Map} of
   * {@link org.apache.phoenix.pherf.configuration.DataTypeMapping} --> List of
   * {@link org.apache.phoenix.pherf.configuration.Column Build the initial Map with all the general
   * rules. These are contained in:
   * <datamode><datamapping><column>...</column></datamapping></datamode>
   * <p/>
   * <p/>
   * Unsupported until V2 Build the overrides by appending them to the list of rules that match the
   * column type
   */
  private void populateModelList() {
    ...
    List<DataModel> models =
      dataModel != null ? Lists.newArrayList(dataModel) : parser.getDataModels();
  ...
  private Column getColumnForRule(List<Column> ruleList, Column phxMetaColumn) {
    ...
    for (Column columnRule : ruleList) {
      ...
      if (columnRule.isUserDefined() && !columnRule.getName().equals(phxMetaColumn.getName())) {
        continue;
      }
