...
public class RuleGeneratorTest {
  ...
  // Test to check the current date is generated correctly between the timestamps at column level
  // and datavalue level
  @Test
  public void testCurrentDateGenerator() throws Exception {
    ...
    for (Column dataMapping : model.getDataMappingColumns()) {
      if (
        (dataMapping.getType() == DataTypeMapping.DATE) && (dataMapping.getUseCurrentDate() == true)
  ...
  public void testSequentialDataSequence() throws Exception {
    ...
    for (Column column : model.getDataMappingColumns()) {
      ...
      if (
        !DataTypeMapping.INTEGER.equals(column.getType()) && sequence == DataSequence.SEQUENTIAL
      ) {
        targetColumn = column;
        break;
      }
   ...
   */
  private void assertRandomDateValue(Column dataMapping, RulesApplier rulesApplier)
    throws Exception {
  ...
  /**
   * This method will test {@link org.apache.phoenix.pherf.configuration.DataSequence} SEQUENTIAL It
   * ensures values returned always increase uniquely. RulesApplier will be accessed by multiple
   * writer so we must ensure increment is thread safe.
   */
  private void assertMultiThreadedIncrementValue(final Column column,
    final RulesApplier rulesApplier) throws Exception {
