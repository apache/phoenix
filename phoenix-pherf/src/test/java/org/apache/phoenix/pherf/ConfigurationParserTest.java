...
public class ConfigurationParserTest extends ResultBaseTest {
  ...
  public void testConfigReader() {
    try {
      ...
      for (Column column : override.getColumn()) {
        assertTrue("Could not lookup Column (" + column.getName() + ") in DataMapping columns: "
          + dataMappingColumns, dataMappingColumns.contains(column));
      }
  ...
  public void testWorkloadWithLoadProfile() throws Exception {
    ...
    assertEquals("Number of properties(size) not as expected: ", 2, props.size());
    TenantLoadEventGeneratorFactory.GeneratorType type =
      TenantLoadEventGeneratorFactory.GeneratorType
        .valueOf(testScenarioWithLoadProfile.getGeneratorName());
    assertEquals("Unknown generator type: ", TenantLoadEventGeneratorFactory.GeneratorType.UNIFORM,
      type);

    LoadProfile loadProfile = testScenarioWithLoadProfile.getLoadProfile();
    assertEquals("batch size not as expected: ", 1, loadProfile.getBatchSize());
    assertEquals("num operations not as expected: ", 1000, loadProfile.getNumOperations());
    assertEquals("tenant group size is not as expected: ", 3,
      loadProfile.getTenantDistribution().size());
    assertEquals("operation group size is not as expected: ", 5,
      loadProfile.getOpDistribution().size());
    assertEquals("UDFs size is not as expected  ", 1, testScenarioWithLoadProfile.getUdfs().size());
    assertNotNull("UDFs clazzName cannot be null ",
      testScenarioWithLoadProfile.getUdfs().get(0).getClazzName());
    assertEquals("UDFs args size is not as expected  ", 2,
      testScenarioWithLoadProfile.getUdfs().get(0).getArgs().size());
    assertEquals("UpsertSet size is not as expected ", 1,
      testScenarioWithLoadProfile.getUpserts().size());
    assertEquals("#Column within the first upsert is not as expected ", 7,
      testScenarioWithLoadProfile.getUpserts().get(0).getColumn().size());
    assertEquals("QuerySet size is not as expected ", 1,
      testScenarioWithLoadProfile.getQuerySet().size());
    assertEquals("#Queries within the first querySet is not as expected ", 2,
    ...
    assertEquals("batch size not as expected: ", 1, loadProfileWithGlobalConn.getBatchSize());
    assertEquals("num operations not as expected: ", 1000,
      loadProfileWithGlobalConn.getNumOperations());
    assertEquals("tenant group size is not as expected: ", 1,
      loadProfileWithGlobalConn.getTenantDistribution().size());
    assertEquals("global tenant is not as expected: ", 1,
      loadProfileWithGlobalConn.getTenantDistribution().get(0).getNumTenants());
    assertEquals("global tenant id is not as expected: ", "GLOBAL",
      loadProfileWithGlobalConn.getTenantDistribution().get(0).getId());
    assertEquals("global tenant weight is not as expected: ", 100,
      loadProfileWithGlobalConn.getTenantDistribution().get(0).getWeight());
    assertEquals("operation group size is not as expected: ", 1,
      loadProfileWithGlobalConn.getOpDistribution().size());
    assertEquals("UpsertSet size is not as expected ", 1,
      testScenarioWithGlobalConn.getUpserts().size());
    assertEquals("#Column within the first upsert is not as expected ", 7,
      testScenarioWithGlobalConn.getUpserts().get(0).getColumn().size());
    assertEquals("Upsert operation not using global connection as expected ", true,
  ...
  private void assertDateValue(List<Column> dataMappingColumns) {
    for (Column dataMapping : dataMappingColumns) {
      if (
        (dataMapping.getType() == DataTypeMapping.DATE)
  ...
  private String writeXML() {
    ...
    try {
      ...
      preScenarioDdls.add(new Ddl("CREATE INDEX IF NOT EXISTS ? ON FHA (NEWVAL_NUMBER) ASYNC",
        "FHAIDX_NEWVAL_NUMBER"));
      preScenarioDdls.add(new Ddl("CREATE LOCAL INDEX IF NOT EXISTS ? ON FHA (NEWVAL_NUMBER)",
        "FHAIDX_NEWVAL_NUMBER"));
