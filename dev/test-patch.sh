...
# checking patched attached to JIRA in "Patch Available" status.
...
PATCH_DIR=/tmp
BASEDIR=$(pwd)
BRANCH_NAME="4.x-HBase-0.98"

. $BASEDIR/dev/test-patch.properties
...
checkoutBranch() {
  ...
  echo ""
  if [[ $JENKINS == "true" ]] ; then
    if [[ "$BRANCH_NAME" != "4.x-HBase-0.98" ]]; then
...
setup () {
  ...
  echo "======================================================================"
  echo "======================================================================"
  echo " Pre-build 4.x-HBase-0.98 to verify stability and javac warnings"
  echo "======================================================================"
  echo "======================================================================"
  echo ""
  echo ""
...
checkJavacWarnings () {
  ...
  if [[ -f $PATCH_DIR/patchJavacWarnings.txt ]] ; then
    ...
    {color:red}-1 javac{color}.  The applied patch generated $patchJavacWarnings javac compiler warnings (more than the 4.x-HBase-0.98's current $trunkJavacWarnings warnings)."
        return 1
      fi
    fi
  fi
...
checkCheckstyleErrors() {
  ...
  if [[ -f $PATCH_DIR/trunkCheckstyle.xml ]] ; then
    ...
    if [[ $patchCheckstyleErrors -gt $trunkCheckstyleErrors ]] ; then
                ...
                {color:red}-1 checkstyle{color}.  The applied patch generated $patchCheckstyleErrors checkstyle errors (more than the 4.x-HBase-0.98's current $trunkCheckstyleErrors errors)."
        return 1
    fi
    echo "There were $patchCheckstyleErrors checkstyle errors in this patch compared to $trunkCheckstyleErrors on 4.x-HBase-0.98."
  fi
  JIRA_COMMENT_FOOTER="Checkstyle Errors: $BUILD_URL/artifact/patchprocess/checkstyle-aggregate.html
...
checkReleaseAuditWarnings () {
  ...
  if [[ -f $PATCH_DIR/patchReleaseAuditWarnings.txt ]] ; then
    ...
    {color:red}-1 release audit{color}.  The applied patch generated $patchReleaseAuditWarnings release audit warnings (more than the 4.x-HBase-0.98's current $OK_RELEASEAUDIT_WARNINGS warnings)."
...
checkLineLengths () {
  ...
  lines=`cat $PATCH_DIR/patch | grep "^+" | grep -v "^@@" | grep -v "^+++" | grep -v "import" | grep -v "org.apache.thrift." | grep -v "com.google.protobuf." | grep -v "phoenix.*.generated" | awk -v len="$MAX_LINE_LENGTH_PATCH" 'length ($0) > len' | head -n 10`
  ll=`echo "$lines" | wc -l`
  if [[ "$ll" -gt "1" ]]; then
    JIRA_COMMENT="$JIRA_COMMENT
