...
public class PhoenixStorageHandlerUtil {
	...
	public static String[] getConstantValues(IndexSearchCondition condition, String comparisonOp) {
		...
		if (comparisonOp.endsWith("UDFOPEqual") || comparisonOp.endsWith("UDFOPNotEqual")) {
			// key = 1  2016-02-29 Added by JeongMin Ju : Adding not equal
	...
	private static String reverseDNS(InetAddress ipAddress) throws NamingException,
		...
		if (hostName == null) {
			...
			try {
				...
				// from the
				// name service. Also, in case of ipv6, we need to use the InetAddress since
				// resolving
...
//		Preconditions.checkArgument(columnNames.length == columnTypes.length, "length of column
// name(" + Lists.newArrayList(columnNames) + ") & column type(" + Lists.newArrayList
// (columnTypes) + ") is different");
