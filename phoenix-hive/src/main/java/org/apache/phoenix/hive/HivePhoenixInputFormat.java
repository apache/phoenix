...
public class HivePhoenixInputFormat<T extends DBWritable> extends PhoenixInputFormat<T> implements InputFormat<NullWritable, T> {
    ...
    private QueryPlan getQueryPlan() throws IOException {
        try {
            LOG.debug("PhoenixInputFormat getQueryPlan statement "
                    + this.configuration.get(PhoenixConfigurationUtil.SELECT_STATEMENT));
