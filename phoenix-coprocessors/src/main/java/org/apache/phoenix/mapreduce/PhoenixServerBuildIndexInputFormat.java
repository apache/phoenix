...
import static org.apache.phoenix.mapreduce.util.PhoenixToolsUtil.getDisableLoggingVerifyType;
...
import static org.apache.phoenix.mapreduce.util.PhoenixToolsUtil.getIndexToolSourceTable;
...
public class PhoenixServerBuildIndexInputFormat<T extends DBWritable> extends PhoenixInputFormat {
    ...
    protected QueryPlan getQueryPlan(final JobContext context, final Configuration configuration)
        ...
        try (final Connection connection = ConnectionUtil.getInputConnection(configuration, overridingProps)) {
            ...
            if (txnScnValue != null) {
                scan.setAttribute(BaseScannerRegionObserverConstants.TX_SCN, Bytes.toBytes(Long.valueOf(txnScnValue)));
            }
            return queryPlan;
