...
import static org.apache.phoenix.hbase.index.IndexRegionObserverConstants.removeEmptyColumn;
...
public abstract class GlobalIndexRegionScanner extends BaseRegionScanner {
    ...
    protected void submitTasks(TaskBatch<Boolean> tasks) throws IOException{
        ...
        for (Boolean result : resultsAndFutures.getFirst()) {
            if (result == null) {
                Throwable cause = ClientUtil.getExceptionFromFailedFuture(resultsAndFutures.getSecond().get(index));
                // there was a failure
