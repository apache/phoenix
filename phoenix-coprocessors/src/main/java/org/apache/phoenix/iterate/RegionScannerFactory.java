    ...
    return new RegionScanner() {
...

      {
          ...
          if (ScanUtil.isLocalIndex(scan)) {
              byte[] expBytes = scan.getAttribute(BaseScannerRegionObserverConstants.LOCAL_INDEX_FILTER);
              if (expBytes != null) {
                  try {
              ...
              byte[] limitBytes = scan.getAttribute(BaseScannerRegionObserverConstants.LOCAL_INDEX_LIMIT);
              if (limitBytes != null) {
      ...
      public boolean nextRaw(List<Cell> result) throws IOException {
        try {
          ...
          if (ScanUtil.isLocalIndex(scan) && !ScanUtil.isAnalyzeTable(scan)) {
             ...
             */
            ServerIndexUtil.wrapResultUsingOffset(env, result, offset, dataColumns,
