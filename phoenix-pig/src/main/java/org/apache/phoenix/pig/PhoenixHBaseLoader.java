...
public final class PhoenixHBaseLoader extends LoadFunc implements LoadMetadata {
    ...
    private void initializePhoenixPigConfiguration(final String location, final Configuration configuration) throws PigException {
        ...
        ConfigurationUtil.setInputClass(this.config, PhoenixPigDBWritable.class);
        ...
        try {
            if (location.startsWith(PHOENIX_TABLE_NAME_SCHEME)) {
				...
				ConfigurationUtil.setSchemaType(this.config, SchemaType.TABLE);
                ...
                ConfigurationUtil.setInputQuery(this.config, this.selectQuery);
				ConfigurationUtil.setSchemaType(this.config, SchemaType.QUERY);
            }
            ...
            ConfigurationUtil.setInputTableName(this.config, this.tableName);
            if(!isEmpty(selectedColumns)) {
                ConfigurationUtil.setSelectColumnNames(this.config, selectedColumns);   
            }
    ...
    @Override
    public InputFormat<NullWritable,? extends DBWritable> getInputFormat() throws IOException {
        if(inputFormat == null) {
    ...
    public Tuple getNext() throws IOException {
        try {
            ...
            final PhoenixPigDBWritable phoenixRecord = reader.getCurrentValue();
