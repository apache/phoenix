 ...
 * org.apache.hadoop.io.FastByteComparisons which was borrowed and slightly modified from Guava's
 * {@link UnsignedBytes} class to be able to compare arrays that start at non-zero offsets. The only
 * difference is that we sort a smaller length bytes as *larger* than longer length bytes when all
...
public class DescVarLengthFastByteComparisons {
  ...
  private interface Comparer<T> {
    abstract public int compareTo(T buffer1, int offset1, int length1, T buffer2, int offset2,
      int length2);
  }
  ...
  private static class LexicographicalComparerHolder {
    ...
    private enum UnsafeComparer implements Comparer<byte[]> {
      ...
      /**
       * Lexicographically compare two arrays. left operand right operand Where to start comparing
       * in the left buffer Where to start comparing in the right buffer How much to compare from
       * the left buffer How much to compare from the right buffer
