...
public class ClientAggregatePlan extends ClientProcessingPlan {
  ...
  public ClientAggregatePlan(StatementContext context, FilterableStatement statement,
    TableRef table, RowProjector projector, Integer limit, Integer offset, Expression where,
    OrderBy orderBy, GroupBy groupBy, Expression having, QueryPlan delegate) {
    ...
    this.serverAggregators =
      ServerAggregators.deserialize(context.getScan().getAttribute(AGGREGATORS),
  ...
  public ResultIterator iterator(ParallelScanGrouper scanGrouper, Scan scan) throws SQLException {
    ...
    if (groupBy.isEmpty()) {
      aggResultIterator = new ClientUngroupedAggregatingResultIterator(
        LookAheadResultIterator.wrap(iterator), serverAggregators);
      aggResultIterator = new UngroupedAggregatingResultIterator(
      ...
      if (groupBy.isOrderPreserving()) {
        aggResultIterator = new ClientGroupedAggregatingResultIterator(
          LookAheadResultIterator.wrap(iterator), serverAggregators, keyExpressions);
      } else {
        ...
        for (Expression keyExpression : keyExpressions) {
          /**
           * Sort the result tuples by the GroupBy expressions. If some GroupBy expression is
           * SortOrder.DESC, then sorted results on that expression are DESC, not ASC. for
           * ClientAggregatePlan,the orderBy should not be OrderBy.REV_ROW_KEY_ORDER_BY, which is
           * different from {@link AggregatePlan.OrderingResultIteratorFactory#newIterator}
           **/
          keyExpressionOrderBy.add(
            OrderByExpression.createByCheckIfOrderByReverse(keyExpression, false, true, false));
        }

        if (useHashAgg) {
          // Pass in orderBy to apply any sort that has been optimized away
          aggResultIterator = new ClientHashAggregatingResultIterator(context, iterator,
            serverAggregators, keyExpressions, orderBy);
        } else {
          iterator = new OrderedResultIterator(iterator, keyExpressionOrderBy, spoolingEnabled,
  ...
  private static class ClientGroupedAggregatingResultIterator
    ...
    public ClientGroupedAggregatingResultIterator(PeekingResultIterator iterator,
    ...
    public String toString() {
      return "ClientGroupedAggregatingResultIterator [resultIterator=" + resultIterator
        + ", aggregators=" + aggregators + ", groupByExpressions=" + groupByExpressions + "]";
    }
  }
  ...
  private static class ClientUngroupedAggregatingResultIterator
    ...
    public ClientUngroupedAggregatingResultIterator(PeekingResultIterator iterator,
    ...
    @Override
    public String toString() {
      return "ClientUngroupedAggregatingResultIterator [resultIterator=" + resultIterator
        + ", aggregators=" + aggregators + "]";
    }
  }
  ...
  private OrderBy convertActualOutputOrderBy(OrderBy orderBy, GroupBy groupBy,
    ...
    if (
      this.useHashAgg && !groupBy.isEmpty() && !groupBy.isOrderPreserving()
