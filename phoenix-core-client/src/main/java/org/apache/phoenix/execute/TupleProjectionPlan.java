...
public class TupleProjectionPlan extends DelegateQueryPlan {
  ...
  /**
   * Map the expressions in the actualOutputOrderBys of targetQueryPlan to
   * {@link ProjectedColumnExpression}.
   */
  private List<OrderBy> convertInputOrderBys(QueryPlan targetQueryPlan) throws SQLException {
    ...
    for (OrderBy inputOrderBy : inputOrderBys) {
      OrderBy newOrderBy = this.convertSingleInputOrderBy(targetQueryPlan,
        selectColumnExpressionToIndex, selectColumnExpressions, inputOrderBy);
  ...
  private OrderBy convertSingleInputOrderBy(QueryPlan targetQueryPlan,
    Map<Expression, Integer> selectColumnExpressionToIndex, Expression[] selectColumnExpressions,
    ...
    OrderPreservingTracker orderPreservingTracker = new OrderPreservingTracker(
      targetQueryPlan.getContext(), GroupBy.EMPTY_GROUP_BY, Ordering.UNORDERED,
      selectColumnExpressions.length, Collections.singletonList(inputOrderBy), null, null);
    for (Expression selectColumnExpression : selectColumnExpressions) {
    ...
    List<OrderByExpression> newOrderByExpressions =
      new ArrayList<OrderByExpression>(orderPreservingTrackInfos.size());
    for (Info orderPreservingTrackInfo : orderPreservingTrackInfos) {
      ...
      assert index != null;
      ProjectedColumnExpression projectedValueColumnExpression =
        this.getProjectedValueColumnExpression(index);
      OrderByExpression newOrderByExpression =
        OrderByExpression.createByCheckIfOrderByReverse(projectedValueColumnExpression,
          orderPreservingTrackInfo.isNullsLast(), orderPreservingTrackInfo.isAscending(), false);
      newOrderByExpressions.add(newOrderByExpression);
    }
    return new OrderBy(newOrderByExpressions);
  }

  private ProjectedColumnExpression getProjectedValueColumnExpression(int columnIndex)
    throws SQLException {
