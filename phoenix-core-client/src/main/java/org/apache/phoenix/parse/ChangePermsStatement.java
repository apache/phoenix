...
/**
 * See PHOENIX-672, Use GRANT/REVOKE statements to assign or remove permissions for a user OR group
 * on a table OR namespace Permissions are managed by HBase using hbase:acl table, Allowed
 * permissions are RWXCA
 */
public class ChangePermsStatement implements BindableStatement {
  ...
  public ChangePermsStatement(String permsString, boolean isSchemaName, TableName tableName,
    String schemaName, boolean isGroupName, LiteralParseNode ugNode, boolean isGrantStatement) {
    // PHOENIX-672 HBase API doesn't allow to revoke specific permissions, hence this parameter will
    // be ignored here.
    // To comply with SQL standards, we may support the user given permissions to revoke specific
    // permissions in future.
    // GRANT permissions statement requires this parameter and the parsing will fail if it is not
    // specified in SQL
    if (permsString != null) {
