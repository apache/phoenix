...
public class HintNode {
  ...
  private static final String SPLIT_REGEXP = "\\s+|((?<=\\" + PREFIX + ")|(?=\\" + PREFIX
    + "))|((?<=\\" + SUFFIX + ")|(?=\\" + SUFFIX + "))";
  ...
  public enum Hint {
    ...
    /**
     * Prevents the usage of indexes, forcing usage of the data table for a query.
     */
    NO_INDEX,
    /**
     * Hint of the form {@code INDEX(<table_name> <index_name>...) } to suggest usage of the index
     * if possible. The first usable index in the list of indexes will be choosen. Table and index
     * names may be surrounded by double quotes if they are case sensitive.
     */
    INDEX,
    /**
     * All things being equal, use the data table instead of the index table when optimizing.
     */
    USE_DATA_OVER_INDEX_TABLE,
    /**
     * All things being equal, use the index table instead of the data table when optimizing.
     */
    ...
    NO_STAR_JOIN,
    /**
     * Avoid using the no seek optimization. When there are many columns which are not selected
     * coming in between 2 selected columns and/or versions of columns, this should be used.
     */
    SEEK_TO_COLUMN,
    /**
     * Avoid seeks to select specified columns. When there are very less number of columns which are
     * not selected in between 2 selected columns this will be give better performance.
     */
    NO_SEEK_TO_COLUMN,
    /**
    ...
    FORWARD_SCAN,
    /**
     * Prefer a hash aggregate over a sort plus streaming aggregate. Issue
   ...
   * @param hint the hint
   * @return the value specified in parenthesis following the hint or null if the hint is not
   *         present.
   */
