...
public class SelectStatement implements FilterableStatement {
  public static final SelectStatement SELECT_STAR = new SelectStatement(null, null, false,
    Arrays.asList(), null, Collections.<ParseNode> emptyList(), null,
    Collections.<OrderByNode> emptyList(), null, null, 0, false, false,
  ...
  public static final SelectStatement SELECT_ONE = new SelectStatement(null, null, false,
    Collections.<AliasedNode> singletonList(new AliasedNode(null, LiteralParseNode.ONE)), null,
    Collections.<ParseNode> emptyList(), null, Collections.<OrderByNode> emptyList(), null, null, 0,
    false, false, Collections.<SelectStatement> emptyList(), new HashMap<String, UDFParseNode>(1));
  public static final SelectStatement COUNT_ONE = new SelectStatement(null, null, false,
    Collections.<AliasedNode> singletonList(new AliasedNode(null,
      new AggregateFunctionParseNode(CountAggregateFunction.NORMALIZED_NAME, LiteralParseNode.STAR,
        new BuiltInFunctionInfo(CountAggregateFunction.class,
          CountAggregateFunction.class.getAnnotation(BuiltInFunction.class))))),
    null, Collections.<ParseNode> emptyList(), null, Collections.<OrderByNode> emptyList(), null,
  ...
  public static SelectStatement create(SelectStatement select, HintNode hint) {
    ...
    return new SelectStatement(select.getFrom(), hint, select.isDistinct(), select.getSelect(),
  ...
  public static SelectStatement create(SelectStatement select, TableNode tableNode,
    List<AliasedNode> selects) {
    return new SelectStatement(tableNode, select.getHint(), select.isDistinct(), selects,
      select.getWhere(), select.getGroupBy(), select.getHaving(), select.getOrderBy(),
      select.getLimit(), select.getOffset(), select.getBindCount(), select.isAggregate(),
      select.hasSequence(), select.getSelects(), select.getUdfParseNodes());
  }
  ...
  public SelectStatement combine(ParseNode where) {
    ...
    return new SelectStatement(this.getFrom(), this.getHint(), this.isDistinct(), this.getSelect(),
      where, this.getGroupBy(), this.getHaving(), this.getOrderBy(), this.getLimit(),
      this.getOffset(), this.getBindCount(), this.isAggregate(), this.hasSequence(), this.selects,
      this.udfParseNodes);
  }

  public static SelectStatement create(SelectStatement select, List<AliasedNode> selects) {
    return new SelectStatement(select.getFrom(), select.getHint(), select.isDistinct(), selects,
      select.getWhere(), select.getGroupBy(), select.getHaving(), select.getOrderBy(),
      select.getLimit(), select.getOffset(), select.getBindCount(), select.isAggregate(),
      select.hasSequence(), select.getSelects(), select.getUdfParseNodes());
  }
  ...
  public static SelectStatement create(SelectStatement select, List<OrderByNode> orderBy,
    LimitNode limit, OffsetNode offset, boolean isAggregate) {
    return new SelectStatement(select.getFrom(), select.getHint(), select.isDistinct(),
      select.getSelect(), select.getWhere(), select.getGroupBy(), select.getHaving(), orderBy,
      limit, offset, select.getBindCount(), isAggregate, select.hasSequence(), select.getSelects(),
  ...
  protected SelectStatement(TableNode from, HintNode hint, boolean isDistinct,
    ...
    this.isAggregate =
      isAggregate || groupBy.size() != countConstants(groupBy) || this.having != null;
  ...
  /**
   * This method should not be called during the early stage of the plan preparation phase since
   * fromTable might not be ConcreteTableNode at that time(e.g., JoinTableNode). By the time
   * getTableSamplingRate method is called, each select statements should have exactly one
  ...
  public SelectStatement getInnerSelectStatement() {
    if (fromTable == null || !(fromTable instanceof DerivedTableNode)) return null;
  ...
  public boolean haveGroupBy() {
    return this.getGroupBy() != null && this.getGroupBy().size() > 0 || !this.isAggregate()
      && this.isDistinct() && this.getSelect() != null && this.getSelect().size() > 0;
  }
