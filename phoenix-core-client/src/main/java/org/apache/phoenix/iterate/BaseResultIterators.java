...
public abstract class BaseResultIterators extends ExplainTable implements ResultIterators {
  ...
  static final Function<HRegionLocation, KeyRange> TO_KEY_RANGE =
    new Function<HRegionLocation, KeyRange>() {
      @Override
      public KeyRange apply(HRegionLocation region) {
        return KeyRange.getKeyRange(region.getRegion().getStartKey(),
  ...
  protected boolean useStats() {
    /*
     * Don't use guide posts: 1) If we're collecting stats, as in this case we need to scan entire
     * regions worth of data to track where to put the guide posts. 2) If the query is going to be
     * executed serially.
     */
  ...
  private static void initializeScan(QueryPlan plan, Integer perScanLimit, Integer offset,
    ...
    boolean wildcardIncludesDynamicCols =
    ...
    if (table.hasConditionalTTL()) {
      CompiledConditionalTTLExpression ttlExpr =
        (CompiledConditionalTTLExpression) table.getCompiledTTLExpression(context.getConnection());
    ...
    if (context.getConnection().isDescVarLengthRowKeyUpgrade()) {
      ...
      scan.setAttribute(BaseScannerRegionObserverConstants.UPGRADE_DESC_ROW_KEY,
        UngroupedAggregateRegionObserverHelper.serialize(table));
    } else {
      ...
      if (keyOnlyFilter && !context.hasRowSizeFunction()) {
        ...
        if (table.getEncodingScheme() == NON_ENCODED_QUALIFIERS) {
          ...
          if (Bytes.compareTo(ecf, 0, ecf.length, firstFamily, 0, firstFamily.length) == 0) {
            ScanUtil.andFilterAtBeginning(scan, new FirstKeyOnlyFilter());
          } else {
            ScanUtil.andFilterAtBeginning(scan, new EmptyColumnOnlyFilter(ecf, ecq));
          }
      ...
      scan.setAttribute(BaseScannerRegionObserverConstants.QUALIFIER_ENCODING_SCHEME,
        new byte[] { table.getEncodingScheme().getSerializedMetadataValue() });
      scan.setAttribute(BaseScannerRegionObserverConstants.IMMUTABLE_STORAGE_ENCODING_SCHEME,
        new byte[] { table.getImmutableStorageScheme().getSerializedMetadataValue() });
      // we use this flag on the server side to determine which value column qualifier to use in the
      // key value we return from server.
      scan.setAttribute(BaseScannerRegionObserverConstants.USE_NEW_VALUE_COLUMN_QUALIFIER,
        Bytes.toBytes(true));
    ...
    if (
      ...
      for (Entry<byte[], NavigableSet<byte[]>> entry : familyMap.entrySet()) {
        if (entry.getValue() != null) {
          ...
          if (
            table.getType() == INDEX && table.getIndexType() == LOCAL
          ...
          if (!keyOnlyFilter) {
            Pair<Integer, Integer> qualifierRangeForFamily =
              EncodedColumnsUtil.setQualifiersForColumnsInFamily(table, family, qualifierSet);
            familyMap.put(familyBytes, qualifierSet);
            if (qualifierRangeForFamily != null) {
              adjustQualifierRange(qualifierRangeForFamily.getFirst(), minMaxQualifiers);
              adjustQualifierRange(qualifierRangeForFamily.getSecond(), minMaxQualifiers);
            }
          }
        }
      }
  ...
  private static void adjustQualifierRange(Integer qualifier,
  ...
  private static void optimizeProjection(StatementContext context, Scan scan, PTable table,
    ...
    BitSet trackedColumnsBitset =
      isPossibleToUseEncodedCQFilter(encodingScheme, storageScheme) && !hasDynamicColumns(table)
        ? new BitSet(10)
        : null;
    ...
    if (statement.getHint().hasHint(Hint.SEEK_TO_COLUMN)) {
       ...
       * set the column qualifiers of the column family in the scan and not just project the entire
       * column family.
       */
      if (statement.getHint().hasHint(Hint.NO_SEEK_TO_COLUMN)) {
        ...
        // When only a single column family is referenced, there are no hints, and HBase server
        // version
        // is less than when the fix for HBASE-13109 went in (0.98.12), then we prevent seeking to a
        // column.
        preventSeekToColumn =
          referencedCfCount == 1 && hbaseServerVersion < MIN_SEEK_TO_COLUMN_VERSION;
      }
    }
    ...
    for (Pair<byte[], byte[]> whereCol : context.getWhereConditionColumns()) {
      ...
      if (preventSeekToColumn) {
        ...
        if (familyMap.containsKey(family)) {
          // where column's CF is present. If there are some specific columns added against this CF,
          // we
          // need to ensure this where column also getting added in it.
          // If the select was like select cf1.*, then that itself will select the whole CF. So no
          // need to
          // specifically add the where column. Adding that will remove the cf1.* stuff and only
          // this
    ...
    if (!columnsTracker.isEmpty()) {
      if (preventSeekToColumn) {
        for (ImmutableBytesPtr f : columnsTracker.keySet()) {
          ...
          // We don't want the ExplicitColumnTracker to be used. Instead we have the
          // ColumnProjectionFilter
      ...
      if (!statement.isAggregate() && filteredColumnNotInProjection) {
        ScanUtil.andFilterAtEnd(scan,
          trackedColumnsBitset != null
            ? new EncodedQualifiersColumnProjectionFilter(SchemaUtil.getEmptyColumnFamily(table),
              trackedColumnsBitset, conditionOnlyCfs, table.getEncodingScheme())
            : new ColumnProjectionFilter(SchemaUtil.getEmptyColumnFamily(table), columnsTracker,
              conditionOnlyCfs,
  ...
  public BaseResultIterators(QueryPlan plan, Integer perScanLimit, Integer offset,
    ParallelScanGrouper scanGrouper, Scan scan, Map<ImmutableBytesPtr, ServerCache> caches,
    QueryPlan dataPlan) throws SQLException {
    super(plan.getContext(), plan.getTableRef(), plan.getGroupBy(), plan.getOrderBy(),
      plan.getStatement().getHint(), QueryUtil.getOffsetLimit(plan.getLimit(), plan.getOffset()),
      offset);
    this.plan = plan;
    this.scan = scan;
    this.caches = caches;
    ...
    List<KeyRange> splitRanges =
      Lists.newArrayListWithExpectedSize(scans.size() * ESTIMATED_GUIDEPOSTS_PER_REGION);
  ...
  private static void updateEstimates(GuidePostsInfo gps, int guideIndex,
    GuidePostEstimate estimate) {
    estimate.rowsEstimate += gps.getRowCounts()[guideIndex];
    estimate.bytesEstimate += gps.getByteCounts()[guideIndex];
    /*
     * It is possible that the timestamp of guideposts could be different. So we report the time at
     * which stats information was collected as the minimum of timestamp of the guideposts that we
     * will be going over.
     */
   ...
   * scans when there is split/merges while scanning a chunk. In this case we need not go by all the
   * regions or guideposts.
   */
  private ScansWithRegionLocations getParallelScans(Scan scan) throws SQLException {
    ...
    if (scan.getStopRow().length > 0) {
      stopIndex = Math.min(stopIndex,
        regionIndex + getIndexContainingExclusive(regionBoundaries.subList(regionIndex, stopIndex),
          scan.getStopRow()));
    }
    ...
    while (regionIndex <= stopIndex) {
      ...
      if (ScanUtil.isLocalIndex(scan)) {
        ...
        if (
          scan.getStopRow().length == 0 || (regionInfo.getEndKey().length != 0
  ...
  private int computeColumnsInCommon() {
    PTable dataTable;
    if ((dataTable = dataPlan.getTableRef().getTable()).getBucketNum() != null) { // unable to
                                                                                  // compute prefix
                                                                                  // range for
                                                                                  // salted data
                                                                                  // table
    ...
    for (int i = 1 + nColumnsInCommon; i < nIndexPKColumns; i++) {
      ...
      if (
        indexColumn.getDataType() == dataColumn.getDataType()
  ...
  // public for testing
  public static ScanRanges computePrefixScanRanges(ScanRanges dataScanRanges,
    int nColumnsInCommon) {
    ...
    ScanRanges commonScanRanges =
      ScanRanges.create(dataScanRanges.getSchema(), cnf, slotSpan, null, useSkipScan, -1);
   ...
   */
  public static KeyRange clipRange(RowKeySchema schema, int fieldIndex, int rangeSpan,
    KeyRange range) {
  ...
  private static boolean clipKeyRangeBytes(RowKeySchema schema, int fieldIndex, int rangeSpan,
    byte[] rowKey, ImmutableBytesWritable ptr, boolean trimTrailingNulls) {
    int position = 0;
    ...
    do {
      ...
      if (field.getDataType().isFixedWidth()) {
        ...
        byte[] sepBytes =
          SchemaUtil.getSeparatorBytes(field.getDataType(), true, isNull, field.getSortOrder());
  ...
  /**
   * Compute the list of parallel scans to run for a given query. The inner scans may be
   * concatenated together directly, while the other ones may need to be merge sorted, depending on
   * the query. Also computes an estimated bytes scanned, rows scanned, and last update time of
   * statistics. To compute correctly, we need to handle a couple of edge cases: 1) if a guidepost
   * is equal to the start key of the scan. 2) If a guidepost is equal to the end region key. In
   * both cases, we set a flag (delayAddingEst) which indicates that the previous gp should be use
   * in our stats calculation. The normal case is that a gp is encountered which is in the scan
   * range in which case it is simply added to our calculation. For the last update time, we use the
   * min timestamp of the gp that are in range of the scans that will be issued. If we find no gp in
   * the range, we use the gp in the first or last region of the scan. If we encounter a region with
   * no gp, then we return a null value as an indication that we don't know with certainty when the
   * stats were updated last. This handles the case of a split occurring for a large ingest with
   * stats never having been calculated for the new region.
  ...
  private ScansWithRegionLocations getParallelScans(byte[] startKey, byte[] stopKey)
    ...
    if (!isLocalIndex && scanRanges.isPointLookup() && !scanRanges.useSkipScanFilter()) {
      ...
      generateEstimates(scanRanges, table, GuidePostsInfo.NO_GUIDEPOST,
        GuidePostsInfo.NO_GUIDEPOST.isEmptyGuidePost(), parallelScans, estimates, Long.MAX_VALUE,
        false);
    ...
    if (isLocalIndex) {
      ...
      if (dataPlan != null && dataPlan.getTableRef().getTable().getType() != PTableType.INDEX) { // Sanity
                                                                                                 // check
    ...
    try {
      ...
      if (gpsSize > 0) {
        ...
        firstRegionStartKey =
          new ImmutableBytesWritable(regionLocations.get(regionIndex).getRegion().getStartKey());
        try {
          int c;
          // Continue walking guideposts until we get past the currentKey
          while (
            (c = currentKey.compareTo(currentGuidePost = PrefixByteCodec.decode(decoder, input)))
                >= 0
          ) {
      ...
      while (regionIndex <= stopIndex) {
        ...
        if (isLocalIndex) {
          if (dataPlan != null && dataPlan.getTableRef().getTable().getType() != PTableType.INDEX) { // Sanity
            ...
            if (
              !dataScanRanges.intersectRegion(regionInfo.getStartKey(), regionInfo.getEndKey(),
                false)
            ) {
          ...
          if (columnsInCommon > 0 && prefixScanRanges.useSkipScanFilter()) {
            ...
            clipKeyRangeBytes(prefixScanRanges.getSchema(), 0, columnsInCommon, regionStartKey, ptr,
              false);
        ...
        while (
          ...
          List<Scan> newScans =
            scanRanges.intersectScan(scan, currentKeyBytes, currentGuidePostBytes, keyOffset,
              splitPostfix, getTable().getBucketNum(), gpsComparedToEndKey == 0);
          if (useStatsForParallelization) {
            for (int newScanIdx = 0; newScanIdx < newScans.size(); newScanIdx++) {
              ...
              parallelScanCollector.addNewScan(plan, newScan, gpsComparedToEndKey == 0 && lastOfNew,
                regionLocation);
            }
          }
          if (newScans.size() > 0) {
        ...
        List<Scan> newScans = scanRanges.intersectScan(scan, currentKeyBytes, endKey, keyOffset,
        ...
        gpsAvailableForAllRegions &= (gpsInThisRegion && everNotDelayed) || // GP in this region
          (regionIndex == startRegionIndex && gpsForFirstRegion) || // GP in first region (before
                                                                    // start key)
          (gpsAfterStopKey = (regionIndex == stopIndex && intersectWithGuidePosts && // GP in last
                                                                                     // region
                                                                                     // (after stop
                                                                                     // key)
      ...
      generateEstimates(scanRanges, table, gps, emptyGuidePost,
        parallelScanCollector.getParallelScans(), estimates, fallbackTs, gpsAvailableForAllRegions);
    } finally {
      if (stream != null) Closeables.closeQuietly(stream);
    }
    sampleScans(parallelScanCollector.getParallelScans(),
      this.plan.getStatement().getTableSamplingRate());
  ...
  private void generateEstimates(ScanRanges scanRanges, PTable table, GuidePostsInfo gps,
    ...
    if (scanRanges.isPointLookup() || pageLimit != null) {
      ...
      if (scanRanges.isPointLookup() && pageLimit != null) {
        this.estimatedRows =
          Long.valueOf(Math.min(scanRanges.getPointLookupCount(), pageLimit * parallelFactor));
  ...
  /**
   * Return row count limit of PageFilter if exists and there is no where clause filter.
   */
  ...
  private static Long computeMinTimestamp(boolean gpsAvailableForAllRegions,
    GuidePostEstimate estimates, long fallbackTs) {
  ...
  /**
   * Loop through List<List<Scan>> parallelScans object, rolling dice on each scan based on
   * startRowKey. All FilterableStatement should have tableSamplingRate. In case it is delete
   * statement, an unsupported message is raised. In case it is null tableSamplingRate, 100%
   * sampling rate will be applied by default.
   */
  ...
  public List<PeekingResultIterator> getIterators() throws SQLException {
    ...
    return getIterators(scans, services, isLocalIndex, allIterators, iterators, isReverse,
      maxQueryEndTime, splits.size(), previousScan,
  ...
  private List<PeekingResultIterator> getIterators(List<List<Scan>> scan,
    ...
    Queue<PeekingResultIterator> allIterators, List<PeekingResultIterator> iterators,
    boolean isReverse, long maxQueryEndTime, int splitSize, ScanWrapper previousScan,
    ...
    try {
      ...
      for (List<Pair<Scan, Future<PeekingResultIterator>>> future : reverseIfNecessary(futures,
        isReverse)) {
        ...
        while (scanPairItr.hasNext()) {
          ...
          try {
            ...
            if (
              isLocalIndex && previousScan != null && previousScan.getScan() != null
                && (((!isReverse
                  && Bytes.compareTo(scanPair.getFirst().getAttribute(SCAN_ACTUAL_START_ROW),
                    previousScan.getScan().getStopRow()) < 0)
                  || (isReverse && previousScan.getScan().getStopRow().length > 0
                    && Bytes.compareTo(scanPair.getFirst().getAttribute(SCAN_ACTUAL_START_ROW),
                      previousScan.getScan().getStopRow()) > 0)
            ...
            LOGGER.warn(
              "Getting iterators at BaseResultIterators encountered error " + "for table {}",
              TableName.valueOf(physicalTableName), e);
            try { // Rethrow as SQLException
              throw ClientUtil.parseServerException(e);
            } catch (StaleRegionBoundaryCacheException | HashJoinCacheNotFoundException e2) {
              // Catch only to try to recover from region boundary cache being out of date
              if (!clearedCache) { // Clear cache once so that we rejigger job based on new
                                   // boundaries
              ...
              if (e2 instanceof HashJoinCacheNotFoundException) {
                ...
                if (cache.getCachePtr() != null) {
                  if (
                    !hashCacheClient.addHashCacheToServer(startKey, cache,
      ...
      toThrow = new SQLExceptionInfo.Builder(OPERATION_TIMED_OUT)
        .setMessage(". Query couldn't be completed in the allotted time: " + queryTimeOut + " ms")
  ...
  private List<PeekingResultIterator> recreateIterators(ConnectionQueryServices services,
    ...
    ScanWrapper previousScan, boolean clearedCache, List<PeekingResultIterator> concatIterators,
    Iterator<Pair<Scan, Future<PeekingResultIterator>>> scanPairItr,
  ...
  private void addIterator(List<PeekingResultIterator> parentIterators,
    List<PeekingResultIterator> childIterators) throws SQLException {
    if (!childIterators.isEmpty()) {
      if (plan.useRoundRobinIterator()) {
        /*
         * When using a round robin iterator we shouldn't concatenate the iterators together. This
         * is because a round robin iterator should be calling next() on these iterators directly
         * after selecting them in a round robin fashion. This helps take advantage of loading the
         * underlying scanners' caches in parallel as well as preventing errors arising out of
         * scanner lease expirations.
         */
  ...
  protected static final class ScanLocator {
    ...
    public ScanLocator(Scan scan, int outerListIndex, int innerListIndex, boolean isFirstScan,
      boolean isLastScan) {
   ...
   * @param planSteps                    Add generated plan in list of planSteps. This argument is
   *                                     used to provide planSteps as whole statement consisting of
   *                                     list of Strings.
   * @param explainPlanAttributesBuilder Add generated plan in attributes object. Having an API to
   *                                     provide planSteps as an object is easier while comparing
  ...
  private void explainUtil(List<String> planSteps,
    ...
    boolean displayChunkCount = context.getConnection().getQueryServices().getProps().getBoolean(
      QueryServices.EXPLAIN_CHUNK_COUNT_ATTRIB, QueryServicesOptions.DEFAULT_EXPLAIN_CHUNK_COUNT);
    StringBuilder buf = new StringBuilder();
    buf.append("CLIENT ");
    if (displayChunkCount) {
      boolean displayRowCount = context.getConnection().getQueryServices().getProps().getBoolean(
        QueryServices.EXPLAIN_ROW_COUNT_ATTRIB, QueryServicesOptions.DEFAULT_EXPLAIN_ROW_COUNT);
    ...
    if (this.plan instanceof ScanPlan) {
      ...
      if (scanPlan.getRowOffset().isPresent()) {
        String rowOffset = Hex.encodeHexString(scanPlan.getRowOffset().get());
