...
import org.apache.phoenix.trace.stub.HTraceConfiguration;
...
import org.apache.phoenix.trace.stub.ProbabilitySampler;
import org.apache.phoenix.trace.stub.TraceCallable;
import org.apache.phoenix.trace.stub.TraceRunnable;
...
public class Tracing {
  ...
  public static enum Frequency {
    ...
    ALWAYS("always", CREATE_ALWAYS),
    PROBABILITY("probability", CREATE_PROBABILITY);
    ...
    new Function<ConfigurationAdapter, Sampler<?>>() {
      ...
      public Sampler<?> apply(ConfigurationAdapter conf) {
        ...
        items.put(ProbabilitySampler.SAMPLER_FRACTION_CONF_KEY,
          conf.get(QueryServices.TRACING_PROBABILITY_THRESHOLD_ATTRIB,
  ...
  public static Sampler<?> getConfiguredSampler(PhoenixConnection connection) {
    String tracelevel = connection.getQueryServices().getProps()
      .get(QueryServices.TRACING_FREQ_ATTRIB, QueryServicesOptions.DEFAULT_TRACING_FREQ);
    return getSampler(tracelevel,
      new ConfigurationAdapter.ConnectionConfigurationAdapter(connection));
  }

  public static Sampler<?> getConfiguredSampler(Configuration conf) {
    String tracelevel =
      conf.get(QueryServices.TRACING_FREQ_ATTRIB, QueryServicesOptions.DEFAULT_TRACING_FREQ);
    return getSampler(tracelevel, new ConfigurationAdapter.HadoopConfigConfigurationAdapter(conf));
  }
  ...
  /**
   * Start a span with the currently configured sampling frequency. Creates a new 'current' span on
   * this thread - the previous 'current' span will be replaced with this newly created span.
   * <p>
  ...
  /**
   * Helper to automatically start and complete tracing on the given method, used in conjuction with
   * {@link CallRunner#run}.
   * <p>
   * This will always attempt start a new span (which will always start, unless the {@link Sampler}
   * says it shouldn't be traced). If you are just looking for flexible tracing that only turns on
   * if the current thread/query is already tracing, use {@link #wrap(Callable, String)} or
  ...
  public synchronized static void addTraceMetricsSource() {
    try {
      ...
      if (!initialized && options.isTracingEnabled()) {
        ...
        TraceWriter traceWriter = new TraceWriter(options.getTableName(),
