...
/**
 * Filter that evaluates WHERE clause expression, used in the case where there are references to
 * multiple column qualifiers over multiple column families. Also there same qualifier names in
 * different families.
 */
public class MultiCFCQKeyValueComparisonFilter extends MultiKeyValueComparisonFilter {
  ...
  public MultiCFCQKeyValueComparisonFilter(Expression expression, boolean allCFs,
    byte[] essentialCF) {
  ...
  private static class ImmutablePairBytesPtr {
    ...
    private ImmutablePairBytesPtr(byte[] bytes1, int offset1, int length1, byte[] bytes2,
    ...
    public void set(byte[] bytes1, int offset1, int length1, byte[] bytes2, int offset2,
      int length2) {
      this.bytes1 = bytes1;
    ...
    public boolean equals(Object obj) {
      ...
      if (
        Bytes.compareTo(this.bytes2, this.offset2, this.length2, that.bytes2, that.offset2,
          that.length2) != 0
      ) return false;
      if (
        Bytes.compareTo(this.bytes1, this.offset1, this.length1, that.bytes1, that.offset1,
          that.length1) != 0
      ) return false;
      return true;
    }
  ...
  public static MultiCFCQKeyValueComparisonFilter parseFrom(final byte[] pbBytes)
    throws DeserializationException {
    try {
      return (MultiCFCQKeyValueComparisonFilter) Writables.getWritable(pbBytes,
        new MultiCFCQKeyValueComparisonFilter());
    } catch (IOException e) {
