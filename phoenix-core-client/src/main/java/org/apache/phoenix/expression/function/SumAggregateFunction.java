 ...
 * @since 0.1
 */
@BuiltInFunction(name = SumAggregateFunction.NAME, nodeClass = SumAggregateParseNode.class,
...
public class SumAggregateFunction extends DelegateConstantToCountAggregateFunction {
  ...
  private Aggregator newAggregator(final PDataType type, SortOrder sortOrder,
    ...
    if (type == PDecimal.INSTANCE) {
      ...
      PDataType.equalsAny(type, PUnsignedDouble.INSTANCE, PUnsignedFloat.INSTANCE, PDouble.INSTANCE,
        PFloat.INSTANCE)
    ) {
  ...
  public boolean evaluate(Tuple tuple, ImmutableBytesWritable ptr) {
    ...
    if (isConstantExpression()) {
      ...
      if (type == PDecimal.INSTANCE) {
        BigDecimal value = ((BigDecimal) constantValue)
          .multiply((BigDecimal) PDecimal.INSTANCE.toObject(ptr, PLong.INSTANCE));
  ...
  public PDataType getDataType() {
    if (super.getDataType() == PDecimal.INSTANCE) {
      ...
      PDataType.equalsAny(super.getDataType(), PUnsignedFloat.INSTANCE, PUnsignedDouble.INSTANCE,
