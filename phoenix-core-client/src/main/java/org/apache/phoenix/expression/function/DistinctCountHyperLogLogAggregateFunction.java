...
/**
 * Built-in function for Distinct Count Aggregation function in approximation. This aggregator is
 * implemented using HyperLogLog. Please refer to PHOENIX-418
 * https://issues.apache.org/jira/browse/PHOENIX-418 1, Accuracy input is not a customizeable. In
 * HyperLogLog accuracy is propertional to 1/sqrt(m), m is the size of the hll hash. Also, this
 * process is irrelavent to runtime or space complexity. 2, The two parameters that requires during
 * HLL initialization. i.e., the precision value for the normal set and the precision value for the
 * sparse set, is hard coded as static final variable. Any change of them requires re-deployment of
 * the phoenix server coprocessors.
 */
...
/**
 * ClientSide HyperLogLogAggregator It will be called when server side aggregator has finished
 * Method aggregate is called for every new server aggregator returned Method evaluate is called
 * when the aggregate is done. the return of evaluate will be send back to user as counted result of
 * expression.evaluate
 */
class HyperLogLogClientAggregator extends DistinctCountClientAggregator {
  private HyperLogLogPlus hll =
    new HyperLogLogPlus(DistinctCountHyperLogLogAggregateFunction.NormalSetPrecision,
...
/**
 * ServerSide HyperLogLogAggregator It will be serialized and dispatched to region server Method
 * aggregate is called for every new row scanned Method evaluate is called when this remote scan is
 * over. the return of evaluate will be send back to ClientSideAggregator.aggregate
 */
