...
/**
 * BSON Update Expression Utility to perform the Document updates. All update expressions provided
 * by this utility supports operations on nested document fields. The field key can represent any
 * top level or nested fields within the document. The caller should use "." notation for accessing
 * nested document elements and "[n]" notation for accessing nested array elements. Top level fields
 * do not require any additional character.
 */
public class UpdateExpressionUtils {
   ...
   * <p/>
   * { "$SET": { &lt;field1&gt;: &lt;value1>, &lt;field2&gt;: &lt;value2&gt;, .... }, "$UNSET": {
   * &lt;field1&gt;: null, &lt;field2&gt;: null, ... }, "$ADD": { &lt;field1&gt;: &lt;value1&gt;,
   * &lt;field2&gt;: &lt;value2&gt;, .... }, "$DELETE_FROM_SET": { &lt;field1&gt;: &lt;value1&gt;,
   * &lt;field2&gt;: &lt;value2&gt;, .... } }
   * <p/>
   * "$SET": Use the SET action in an update expression to add one or more fields to a BSON
   * Document. If any of these fields already exists, they are overwritten by the new values. To
   * perform multiple SET actions, provide multiple fields key-value entries within the nested
   * document under $SET field key. "$UNSET": Use the UNSET action in an update expression to unset
   * or remove one or more fields from a BSON Document. To perform multiple UNSET actions, provide
   * multiple field key-value entries within the nested document under $UNSET field key. "$ADD": Use
   * the ADD action in an update expression to add a new field and its values to a BSON document. If
   * the field already exists, the behavior of ADD depends on the field's data type:
   * <p/>
   ...
   * <p/>
   * 2. If the field is a set, and the value you are adding is also a set, the value is appended to
   * the existing set.
   * <p/>
   ...
   * entries within the nested document under $DELETE_FROM_SET field key. Definition of path and
   * subset in the context of the expression:
   * <p/>
   * 1. The path element is the document path to a field. The field must be a set data type. 2. The
   * subset is one or more elements that you want to delete from the given path. Subset must be of
   * set type.
   * <p/>
  ...
  /**
   * Update the given document by performing DELETE operation. This operation is applicable only on
   * Set data structure. The document is updated by removing the given set of elements from the
   * given set of elements. Let's say if the document field is of string set data type, and the
   * elements are: {"yellow", "green", "red", "blue"}. The elements to be removed from the set are
   * provided as {"blue", "yellow"} with the delete expression. The operation is expected to update
   * the existing field by removing "blue" and "yellow" from the given set and the resultant set is
   * expected to contain: {"green", "red"}.
   * @param deleteExpr   Delete Expression Document with key-value pairs. Key represents field in
   *                     the given document, on which operation is to be performed. Value represents
   *                     set of elements to be removed from the existing set.
   * @param bsonDocument Document contents to be updated.
   */
   ...
   * {@code setValuesToDelete} must be of same set data type. For instance, both must be either set
   * of string, set of numbers or set of binary values.
  ...
  /**
   * Update the given document by performing ADD operation. This operation is applicable only on
   * either Set data structure or Numerical value represented by Int32, Int64, Double or Decimal. If
   * the field is of type set, the document is updated by adding the given set of elements to the
   * given set of elements. If the field is of type number, the document is updated by adding the
   * numerical value to the given number field value.
   * </p>
   * Let's say if the document field is of numeric data type with value "234.5" and the value to be
   * added is "10", the resultant value is expected to be "244.5". Adding negative value would
   * result in subtract operation. For example, adding "-10" would result in "224.5".
   * </p>
   * On the other hand, if the document field is of string set data type, and the elements are:
   * {"yellow", "green", "red"}. The elements to be added to the set are provided as {"blue",
   * "yellow"} with the add expression. The operation is expected to update the existing field by
   * removing adding unique value "blue" and the resultant set is expected to contain: {"yellow",
   * "green", "red", "blue"}.
   ...
   * {@code currentValue} is numeric, add numeric value represented by {@code newVal} to it. If the
   * data type of {@code currentValue} is set, add set values represented by {@code newVal} to it.
   * For this operation to be successful, both {@code currentValue} and {@code newVal} must be of
   * same set data type. For instance, both must be either set of string, set of numbers or set of
   * binary values, or both must be of number data type.
  ...
  private static BsonValue modifyFieldValueByAdd(final BsonValue currentValue,
    ...
    // Current value exists but is not a number or set, or is set of different type
    throw new BsonUpdateInvalidArgumentException(INVALID_UPDATE_PATH_MESSAGE);
  }
   ...
   * applicable to any field of the document. The SET operation represents either adding a new field
   * with the given value of updating the existing field with new value provided by the SET
   * Expression Document.
  ...
  /**
   * Update the nested field with the given update operation. The update operation is determined by
   * the enum value {@code updateOp}. The field key is expected to contain "." and/or "[]" notations
   * for nested documents and/or nested array elements. This function keeps recursively calling
   * itself until it reaches the leaf node in the given tree. For instance, for field key
   * "category.subcategories.brands[5]", first the function evaluates and retrieves the value for
   * top-level field "category". The value of "category" is expected to be nested document. First
   * function call has value as full document, it retries nested document under "category" field and
   * calls the function recursively with index value same as index value of first "." (dot) in the
   * field key. For field key "category.subcategories.brands[5]", the index value would be 8. The
   * second function call retrieves value of field key "subcategories", which is expected to be
   * nested document. The third function call gets this nested document as BsonValue and index value
   * as 22 as the field key has second "." (dot) notation at index 22. The third function call
   * searches for field key "brands" and expects its value as nested array. The forth function call
   * gets this nested array as BsonValue and index 29 as the field key has "[]" array notation
   * starting at index 29. The forth function call retrieves value of nested array element at index
   * 5. As the function is at leaf node in the tree, now it performs the update operation as per the
  ...
  private static void updateNestedField(final BsonValue value, final int idx, final String fieldKey,
    final BsonValue newVal, final UpdateOp updateOp) {
    int curIdx = idx;
  ...
  /**
   * Perform update operation at the leaf node. This method is called only when the leaf node or the
   * target node for the given field key is encountered while iterating through the tree structure.
   * This is specifically called when the closest ancestor of the given node is a nested array. The
   * leaf node is for the field key. For example, for "category.subcategories", the leaf node is
   * "subcategories" whereas for "category.subcategories.brands[2]", the leaf node is "brands".
   * @param fieldKey    The full field key.
   ...
   * @param nestedArray The parent or ancestor node, expected to be nested array only. For example,
   *                    for "category.subcategories.brands[5]" field key, the nestedArray is
   *                    expected to be "brands" array and arrayIdx is expected to be 5.
   */
  private static void updateArrayAtLeafNode(final String fieldKey, final BsonValue newVal,
    ...
    switch (updateOp) {
      ...
      case ADD: {
        if (arrayIdx < nestedArray.size()) {
          ...
          // For ADD beyond array size, just set the value directly (no initialization logic)
          nestedArray.add(newVal);
        }
        break;
      }
        ...
        if (arrayIdx < nestedArray.size()) {
          ...
          if (currentValue != null) {
            ...
            LOGGER.info("Nothing to be removed as nested list does not have value for field {}. "
              + "Update operator: {}", fieldKey, updateOp);
          }
        }
        break;
      }
   ...
   * Perform update operation at the leaf node. This method is called only when the leaf node or the
   * target node for the given field key is encountered while iterating through the tree structure.
   * This is specifically called when the closest ancestor of the given node is a nested document.
   * The leaf node is for the field key. For example, for "category.subcategories", the leaf node is
   * "subcategories" whereas for "category.subcategories.brands[2]", the leaf node is "brands".
   * @param newVal             New value to be used while performing update operation on the
   *                           existing node.
   * @param updateOp           Type of the update operation to be performed.
   ...
   *                           field key, the target node field key is expected to be
   *                           "subcategories" and the nestedDocument is expected to be "category"
   *                           document.
   */
  ...
  /**
   * Returns true if both values represent Set data structure and the contents of the Set are of
   * same type.
   * @param bsonValue1 First value.
   * @param bsonValue2 Second value.
   * @return True if both values represent Set data structure and the contents of the Set are of
   *         same type.
   */
  private static boolean areBsonSetOfSameType(final BsonValue bsonValue1,
    final BsonValue bsonValue2) {
    if (
      !CommonComparisonExpressionUtils.isBsonSet(bsonValue1)
