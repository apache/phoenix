 ...
 * result returned can be used for debugging(eg. using HBase shell), logging etc. Can also be used
 * in sql predicates.
 */
@BuiltInFunction(name = PhoenixRowTimestampFunction.NAME,
    nodeClass = PhoenixRowTimestampParseNode.class, args = {})
public class PhoenixRowTimestampFunction extends ScalarFunction {
   ...
   *                 will cause the empty column key value to be evaluated during scan filter
   *                 processing.
   */
  public PhoenixRowTimestampFunction(List<Expression> children) {
    ...
    if (
      ...
      throw new IllegalArgumentException(
        "PhoenixRowTimestampFunction should only have an " + "EMPTY_COLUMN key value expression.");
    }
    if (!(children.get(0).getDataType().equals(PDate.INSTANCE))) {
      throw new IllegalArgumentException("PhoenixRowTimestampFunction should have an "
  ...
  /**
   * The evaluate method is called under the following conditions - 1. When PHOENIX_ROW_TIMESTAMP()
   * is evaluated in the projection list. Since the EMPTY_COLUMN is not part of the table column
   * list, emptyColumnKV will be null. PHOENIX-4179 ensures that the maxTS (which will be
   * EMPTY_COLUMN ts) is returned for the tuple. 2. When PHOENIX_ROW_TIMESTAMP() is evaluated in the
   * backend as part of the where clause. Here the emptyColumnKV will not be null, since we ensured
   * that by adding it to scan column list in PhoenixRowTimestampParseNode. In this case the
   * emptyColumnKV.getTimestamp() is used.
   */
  @Override
