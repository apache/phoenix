 ...
 * {@code FIRST_VALUES(<expression>, <expression>) WITHIN GROUP (ORDER BY <expression> ASC/DESC) aggregate }
 * function
 */
@FunctionParseNode.BuiltInFunction(name = FirstValuesFunction.NAME,
    nodeClass = FirstValuesAggregateParseNode.class,
...
public class FirstValuesFunction extends FirstLastValueBaseFunction {
  ...
  public Aggregator newClientAggregator() {
    FirstLastValueBaseClientAggregator aggregator =
      new FirstLastValueBaseClientAggregator(getDataType());
  ...
  public PDataType getDataType() {
    ...
    return PDataType
      .fromTypeId(children.get(2).getDataType().getSqlType() + PArrayDataType.ARRAY_TYPE_BASE);
  }
}
