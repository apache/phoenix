...
/**
 * Implementation of the {@code TO_TIMESTAMP(<string>,[<format-string>,[<timezone-string>]])}
 * built-in function. The second argument is optional and defaults to the
 * phoenix.query.timestampFormat value from the HBase config. If present it must be a constant
 * string. The third argument is either a valid (constant) timezone id, or the string "local". The
 * third argument is also optional, and it defaults to GMT.
 */
...
public class ToTimestampFunction extends ToDateFunction {
  ...
  public ToTimestampFunction(List<Expression> children, StatementContext context)
    throws SQLException {
    super(children, context);
  }

  public ToTimestampFunction(List<Expression> children, String dateFormat, String timeZoneId)
    throws SQLException {
