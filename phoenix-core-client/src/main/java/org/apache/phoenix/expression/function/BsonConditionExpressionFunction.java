...
@FunctionParseNode.BuiltInFunction(name = BsonConditionExpressionFunction.NAME,
    ...
    args = { @FunctionParseNode.Argument(allowedTypes = { PBson.class, PVarbinary.class }),
    ...
    if (conditionExp != null && exprValues != null) {
      if (conditionExp.isString() && exprValues.isDocument()) {
        SQLComparisonExpressionUtils sqlComparisonExpressionUtils =
          new SQLComparisonExpressionUtils(rawBsonDocument, (BsonDocument) exprValues);
        boolean result = sqlComparisonExpressionUtils
          .evaluateConditionExpression(((BsonString) conditionExp).getValue());
      ...
      boolean result = DocumentComparisonExpressionUtils
        .evaluateConditionExpression(rawBsonDocument, conditionExpressionBsonDoc);
      ptr.set(PBoolean.INSTANCE.toBytes(result));
      return true;
    }
  }
