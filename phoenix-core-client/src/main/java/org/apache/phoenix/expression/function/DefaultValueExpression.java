...
/**
 * Internal function used to get the default value for a column not specified in UPSERT. If expr1 is
 * evaluated (can be null), then it is returned, otherwise expr2 is returned.
 */
  ...
  public boolean evaluate(Tuple tuple, ImmutableBytesWritable ptr) {
    ...
    if (tuple.isImmutable()) {// True for the last time an evaluation is happening on the row
      ...
      if (secondChild.evaluate(tuple, ptr)) {
        ...
        getDataType().coerceBytes(ptr, null, secondChild.getDataType(), secondChild.getMaxLength(),
          secondChild.getScale(), secondChild.getSortOrder(), getMaxLength(), getScale(),
          getSortOrder());
        return true;
      }
    }
