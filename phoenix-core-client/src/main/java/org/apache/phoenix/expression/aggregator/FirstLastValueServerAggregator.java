...
public class FirstLastValueServerAggregator extends BaseAggregator {
  ...
  private static final Logger LOGGER =
    LoggerFactory.getLogger(FirstLastValueServerAggregator.class);
  ...
  protected TreeMap<byte[], LinkedList<byte[]>> topValues =
    new TreeMap<byte[], LinkedList<byte[]>>(new Bytes.ByteArrayComparator());
    ...
    if (useOffset) {
      ...
      if (addFlag) {
        ...
        if (hasValueDescSortOrder) {
          topValues.get(currentOrder)
            .push(SortOrder.invert(ptr.get(), ptr.getOffset(), ptr.getLength()));
        } else {
  ...
  public String toString() {
    StringBuilder out = new StringBuilder(
      "FirstLastValueServerAggregator" + " is ascending: " + isAscending + " value=");
    if (useOffset) {
