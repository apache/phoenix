...
/**
 * Base class for non composite aggregation functions that optimize aggregation by delegating to
 * {@link CountAggregateFunction} when the child expression is a constant.
 * @since 0.1
 */
abstract public class DelegateConstantToCountAggregateFunction extends SingleAggregateFunction {
  private static final ImmutableBytesWritable ZERO =
    new ImmutableBytesWritable(PLong.INSTANCE.toBytes(0L));
  private CountAggregateFunction delegate;

  public DelegateConstantToCountAggregateFunction() {
  }

  public DelegateConstantToCountAggregateFunction(List<Expression> childExpressions,
