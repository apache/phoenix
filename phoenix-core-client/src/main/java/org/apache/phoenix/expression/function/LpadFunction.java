...
/**
 * Implementation of LPAD(input string, length int [, fill string]) Fills up the input to length
 * (number of characters) by prepending characters in fill (space by default). If the input is
 * already longer than length then it is truncated on the right.
 */
@BuiltInFunction(name = LpadFunction.NAME,
...
public class LpadFunction extends ScalarFunction {
  ...
  /**
   * Helper function to get the utf8 length of CHAR or VARCHAR points to the string sortOrder of the
   * string whether the string is of char type
  ...
  /**
   * Helper function to get the byte length of a utf8 encoded string points to the string sortOrder
   * of the string whether the string is of char type
   * @return byte length of the string
   */
  private int getSubstringByteLength(ImmutableBytesWritable ptr, int length, SortOrder sortOrder,
    boolean isCharType) {
    return isCharType
      ? length
      : StringUtil.getByteLengthForUtf8SubStr(ptr.get(), ptr.getOffset(), length, sortOrder);
  }
  ...
  public boolean evaluate(Tuple tuple, ImmutableBytesWritable ptr) {
    ...
    int outputStrLen =
      outputStrLenExpr.getDataType().getCodec().decodeInt(ptr, outputStrLenExpr.getSortOrder());
    if (outputStrLen < 0) {
      return false;
    }
    ...
    int fillByteLength =
      getSubstringByteLength(fillPtr, fillPtr.getLength(), fillSortOrder, isFillCharType);
    ...
    int fillCharsByteLength =
      getSubstringByteLength(fillPtr, numFillCharsPrepended, fillSortOrder, isFillCharType);
    ...
    byte[] paddedStr = StringUtil.lpad(ptr.get(), ptr.getOffset(), ptr.getLength(), fillPtr.get(),
      fillPtr.getOffset(), fillPtr.getLength(), invertFill, strWithPaddingByteLength);
    ptr.set(paddedStr);
    return true;
  }
