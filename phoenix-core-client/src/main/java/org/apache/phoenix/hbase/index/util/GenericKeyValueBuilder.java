...
public class GenericKeyValueBuilder extends KeyValueBuilder {
  ...
  private KeyValue build(ImmutableBytesWritable row, ImmutableBytesWritable family,
    ...
    return new KeyValue(copyBytesIfNecessary(row), copyBytesIfNecessary(family),
      copyBytesIfNecessary(qualifier), ts, type,
      value == null ? null : copyBytesIfNecessary(value));
  }

  @Override
  public int compareQualifier(Cell kv, byte[] key, int offset, int length) {
    return Bytes.compareTo(kv.getQualifierArray(), kv.getQualifierOffset(), kv.getQualifierLength(),
      key, offset, length);
  }

  @Override
  public int compareFamily(Cell kv, byte[] key, int offset, int length) {
    return Bytes.compareTo(kv.getFamilyArray(), kv.getFamilyOffset(), kv.getFamilyLength(), key,
      offset, length);
  }
  ...
  public int compareRow(Cell kv, byte[] rrow, int roffset, int rlength) {
    return Bytes.compareTo(kv.getRowArray(), kv.getRowOffset(), kv.getRowLength(), rrow, roffset,
      rlength);
  }
