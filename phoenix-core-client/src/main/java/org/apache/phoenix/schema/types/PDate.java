...
public class PDate extends PDataType<Date> {
  ...
  private PDate() {
    super("DATE", Types.DATE, Date.class, new DateCodec(), 11); // After TIMESTAMP and DATE to
                                                                // ensure toLiteral finds those
                                                                // first
  }
  ...
  Object dateToClass(java.sql.Date sqlDate, PDataType actualType, Class jdbcType)
    ...
    if (jdbcType == java.time.LocalDate.class) {
      ...
      return java.time.LocalDateTime
        .ofInstant(java.time.Instant.ofEpochMilli(sqlDate.getTime()), ZoneOffset.UTC).toLocalDate();
      ...
      return java.time.LocalDateTime.ofInstant(java.time.Instant.ofEpochMilli(sqlDate.getTime()),
        ZoneOffset.UTC);
    } else if (jdbcType == java.time.LocalTime.class) {
      // This is NOT JDBC compliant, but is useful because Dates are really Timestamps.
      return java.time.LocalDateTime
        .ofInstant(java.time.Instant.ofEpochMilli(sqlDate.getTime()), ZoneOffset.UTC).toLocalTime();
  ...
  public boolean isCoercibleTo(PDataType targetType) {
    return equalsAny(targetType, PDate.INSTANCE, PTime.INSTANCE, PTimestamp.INSTANCE,
  ...
  public boolean isCoercibleTo(PDataType targetType, Object value) {
    if (value != null) {
      if (
        equalsAny(targetType, PUnsignedTimestamp.INSTANCE, PUnsignedDate.INSTANCE,
