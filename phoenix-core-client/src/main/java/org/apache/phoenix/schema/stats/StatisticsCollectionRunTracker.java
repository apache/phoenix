...
public class StatisticsCollectionRunTracker {
  ...
  private final Set<ColumnFamilyRegionInfo> updateStatsRegions =
    Collections.newSetFromMap(new ConcurrentHashMap<ColumnFamilyRegionInfo, Boolean>());
  private final Set<RegionInfo> compactingRegions =
    Collections.newSetFromMap(new ConcurrentHashMap<RegionInfo, Boolean>());
  ...
  private StatisticsCollectionRunTracker(Configuration config) {
    ...
    ThreadFactoryBuilder builder =
      new ThreadFactoryBuilder().setDaemon(true).setNameFormat("phoenix-update-statistics-%s");
    executor = Executors.newFixedThreadPool(poolSize, builder.build());
  }

  /**
   * @param regionInfo for the region that should be marked as undergoing stats collection via major
   *                   compaction.
   ...
   * @return true if stats are being collected for the region via major compaction, false otherwise.
   */
