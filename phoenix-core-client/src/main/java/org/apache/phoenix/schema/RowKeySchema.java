...
/**
 * Schema for the bytes in a RowKey. For the RowKey, we use a null byte to terminate a variable
 * length type, while for KeyValue bytes we write the length of the var char preceding the value. We
 * can't do that for a RowKey because it would affect the sort order.
 * @since 0.1
 */
public class RowKeySchema extends ValueSchema {
  public static final RowKeySchema EMPTY_SCHEMA =
    new RowKeySchema(0, Collections.<Field> emptyList(), true);
  ...
  // "iterator" initialization methods that initialize a bytes ptr with a row key for further
  // navigation
  ...
  public Boolean iterator(byte[] src, int srcOffset, int srcLength, ImmutableBytesWritable ptr,
    int position, int extraColumnSpan) {
   ...
   * must have a complete row key. Use @link {@link #position(ImmutableBytesWritable, int, int)} if
   * you have a partial row key.
  ...
  private Boolean next(ImmutableBytesWritable ptr, int position, int maxOffset, boolean isFirst) {
    ...
    if (position > 0 && !isFirst && !getField(position - 1).getDataType().isFixedWidth()) {
      if (getField(position - 1).getDataType() != PVarbinaryEncoded.INSTANCE) {
        ptr.set(ptr.get(), ptr.getOffset() + ptr.getLength() + SEPARATOR_OFFSET_NON_ENCODED_TYPES,
          0);
      } else {
        ptr.set(ptr.get(), ptr.getOffset() + ptr.getLength() + SEPARATOR_OFFSET_ENCODED_TYPES, 0);
      }
    }
    ...
    if (field.getDataType().isFixedWidth()) {
      ...
      if (position + 1 == getFieldCount()) {
        ...
        if (field.getDataType() != PVarbinaryEncoded.INSTANCE) {
          ...
          boolean ascSort =
            !rowKeyOrderOptimizable || len == 0 || field.getSortOrder() == SortOrder.ASC;
          int lastFieldTerminatorLen = ascSort ? 0 : 2;
          ptr.set(ptr.get(), ptr.getOffset(), maxOffset - ptr.getOffset() - lastFieldTerminatorLen);
        }
      } else {
        byte[] buf = ptr.get();
        ...
        if (field.getDataType() != PVarbinaryEncoded.INSTANCE) {
          ...
          if (
            offset < maxOffset
              && !SchemaUtil.areSeparatorBytesForVarBinaryEncoded(buf, offset, SortOrder.ASC)
          ) {
            boolean ascSort = !rowKeyOrderOptimizable || field.getSortOrder() == SortOrder.ASC;
            do {
              offset++;
            } while (
              offset < maxOffset && !SchemaUtil.areSeparatorBytesForVarBinaryEncoded(buf, offset,
   ...
   * includes the next {@code extraSpan} additional fields in the bytes ptr. This allows multiple
   * fields to be treated as one concatenated whole.
   * @param ptr       bytes pointer pointing to the value at the positional index provided.
   * @param position  zero-based index of the next field in the value schema
   * @param maxOffset max possible offset value when iterating
  ...
  public Boolean previous(ImmutableBytesWritable ptr, int position, int minOffset) {
    ...
    if (!field.getDataType().isFixedWidth()) {
      ...
      if (field.getDataType() != PVarbinaryEncoded.INSTANCE) {
        ...
        if (
          ...
          do {
            ...
            offset > minOffset && !SchemaUtil.areSeparatorBytesForVarBinaryEncoded(buf, offset,
              ascSort ? SortOrder.ASC : SortOrder.DESC)
          );
        }
      }
      if (offset == minOffset) { // shouldn't happen
        ...
        ptr.set(buf, offset + 1, ptr.getOffset() - 1 - offsetAdjustment - offset); // Don't include
                                                                                   // null
                                                                                   // terminator in
                                                                                   // length
      }
    ...
    return iterator(ptr.get(), minOffset, ptr.getOffset() - minOffset - offsetAdjustment, ptr,
      position + 1);
  }
  ...
  /**
   * Like
   * {@link #reposition(org.apache.hadoop.hbase.io.ImmutableBytesWritable, int, int, int, int)}, but
   * also includes the next {@code extraSpan} additional fields in the bytes ptr. This allows
   * multiple fields to be treated as one concatenated whole.
   * @param extraSpan the number of extra fields to expand the ptr to contain.
   */
  public Boolean reposition(ImmutableBytesWritable ptr, int oldPosition, int newPosition,
  ...
  /**
   * Positions ptr at the part of the row key for the field at endPosition, starting from the field
   * at position.
   ...
   * @return true if the row key has a value at endPosition with ptr pointing to that value and
   *         false otherwise with ptr not necessarily set.
   */
  ...
  /**
   * Extends the boundaries of the {@code ptr} to contain the next {@code extraSpan} fields in the
   * row key.
   * @param ptr       bytes pointer pointing to the value at the positional index provided.
   ...
   * @param extraSpan the number of extra fields to expand the ptr to contain.
   */
  private int readExtraFields(ImmutableBytesWritable ptr, int position, int maxOffset,
    int extraSpan) {
  ...
  public KeyRange clipLeft(int pkPos, KeyRange keyRange, int spansToClip,
    ...
    if (spansToClip < 0) {
      throw new IllegalArgumentException(
        "Cannot specify a negative spansToClip (" + spansToClip + ")");
    }
    if (spansToClip == 0) {
      return keyRange;
    }
