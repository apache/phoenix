...
public class SequenceKey implements Comparable<SequenceKey> {
  ...
  public SequenceKey(String tenantId, String schemaName, String sequenceName, int nBuckets) {
    ...
    this.key = ByteUtil.concat(
      (nBuckets <= 0 ? ByteUtil.EMPTY_BYTE_ARRAY : QueryConstants.SEPARATOR_BYTE_ARRAY),
    ...
    if (nBuckets > 0) {
      key[0] = SaltingUtil.getSaltingByte(key, SaltingUtil.NUM_SALTING_BYTES,
        key.length - SaltingUtil.NUM_SALTING_BYTES, nBuckets);
    }
  }
