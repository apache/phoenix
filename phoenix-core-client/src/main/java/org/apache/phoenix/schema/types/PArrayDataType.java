...
/**
 * The datatype for PColummns that are Arrays. Any variable length array would follow the below
 * order. Every element would be seperated by a seperator byte '0'. Null elements are counted and
 * once a first non null element appears we write the count of the nulls prefixed with a seperator
 * byte. Trailing nulls are not taken into account. The last non null element is followed by two
 * seperator bytes. For eg {@code a, b, null, null, c, null -> 65 0 66 0 0 2 67 0 0 0 a null
 * null null b c null d -> 65 0 0 3 66 0 67 0 0 1 68 0 0 0 }. The reason we use this serialization
 * format is to allow the byte array of arrays of the same type to be directly comparable against
 * each other. This prevents a costly deserialization on compare and allows an array column to be
...
public abstract class PArrayDataType<T> extends PDataType<T> {
  ...
  public final void coerceBytes(ImmutableBytesWritable ptr, Object object, PDataType actualType,
    ...
    coerceBytes(ptr, object, actualType, maxLength, scale, desiredMaxLength, desiredScale, this,
      actualModifer, desiredModifier, expectedRowKeyOrderOptimizable);
  }
  ...
  public final void coerceBytes(ImmutableBytesWritable ptr, Object object, PDataType actualType,
    ...
    coerceBytes(ptr, object, actualType, maxLength, scale, desiredMaxLength, desiredScale, this,
      actualModifer, desiredModifier, true);
  }
  ...
  // array serialization format where bytes are immutable (does not support prepend/append or
  // sorting)
  @Deprecated
  ...
  protected PArrayDataType(String sqlTypeName, int sqlType, Class clazz, PDataCodec codec,
    int ordinal) {
  ...
  /**
   * Ensures that the provided {@code object} is a PhoenixArray, attempting a conversion in the case
   * when it is not.
   */
  ...
  public byte[] toBytes(Object object, PDataType baseType, SortOrder sortOrder,
    ...
    if (!baseType.isFixedWidth()) {
      ...
      size += ((2 * Bytes.SIZEOF_BYTE)
        + (noOfElements - nullsVsNullRepeationCounter.getFirst()) * Bytes.SIZEOF_BYTE)
        + (nullsVsNullRepeationCounter.getSecond() * 2 * Bytes.SIZEOF_BYTE);
      // Assume an offset array that fit into Short.MAX_VALUE. Also not considering nulls that could
      // be > 255
      // In both of these cases, finally an array copy would happen
      int capacity = noOfElements * Bytes.SIZEOF_SHORT;
      // Here the int for noofelements, byte for the version, int for the offsetarray position and 2
      // bytes for the
      // end seperator
      byteStream = new TrustedByteArrayOutputStream(
        size + capacity + Bytes.SIZEOF_INT + Bytes.SIZEOF_BYTE + Bytes.SIZEOF_INT);
    } else {
    ...
    return createArrayBytes(byteStream, oStream, arr, noOfElements, baseType, sortOrder,
  ...
  public static int serializeNulls(DataOutputStream oStream, int nulls) throws IOException {
    ...
    // 2) Arrays with repeating nulls in the middle which is less than 255 but greater than
    // bytes.MAX_VALUE
    ...
    // In both case the elements and the value array will be the same but the Array 1 is actually
    // smaller because it
    // has more nulls.
    // Now we should have mechanism to show that we treat arrays with more nulls as lesser. Hence in
    // the above case
    // as
    ...
    // Now for cases where the number of nulls is greater than 255, we would write an those many
    // (byte)1, it is
    // bigger than 255.
  ...
  public static void writeEndSeperatorForVarLengthArray(DataOutputStream oStream,
    SortOrder sortOrder) throws IOException {
    writeEndSeperatorForVarLengthArray(oStream, sortOrder, true);
  }

  public static void writeEndSeperatorForVarLengthArray(DataOutputStream oStream,
    SortOrder sortOrder, boolean rowKeyOrderOptimizable) throws IOException {
    byte sepByte = getSeparatorByte(rowKeyOrderOptimizable, sortOrder);
    oStream.write(sepByte);
    oStream.write(sepByte);
  }

  // this method is only for append/prepend/concat operations which are only supported for the
  ...
  public static boolean useShortForOffsetArray(int maxoffset, byte serializationVersion) {
    if (
      serializationVersion == IMMUTABLE_SERIALIZATION_VERSION
  ...
  // repetition factor
  public int estimateByteSize(Object o, Pair<Integer, Integer> nullsVsNullRepeationCounter,
    PDataType baseType) {
    if (baseType.isFixedWidth()) {
    ...
    if (baseType.isArrayType()) {
      ...
      for (int i = 0; i < noOfElements; i++) {
        ...
        if (
          !PDataType.fromTypeId((baseType.getSqlType() - PDataType.ARRAY_TYPE_BASE)).isFixedWidth()
        ) {
          if (array.isNull(i)) {
            nulls++;
          } else {
  ...
  public boolean isCoercibleTo(PDataType targetType) {
    if (targetType != null && targetType.isVectorType()) {
      ...
      return elemType.isCoercibleTo(PDouble.INSTANCE);
    }
  ...
  public boolean isCoercibleTo(PDataType targetType, PDataType expectedTargetType) {
    if (!targetType.isArrayType()) {
      ...
      PDataType targetElementType =
        PDataType.fromTypeId(targetType.getSqlType() - PDataType.ARRAY_TYPE_BASE);
  ...
  public boolean isSizeCompatible(ImmutableBytesWritable ptr, Object value, PDataType srcType,
    ...
    for (int i = 0; i < pArr.numElements; i++) {
      ...
      if (
        !baseType.isSizeCompatible(elementPtr, val, baseType, sortOrder, srcType.getMaxLength(val),
  ...
  private void coerceBytes(ImmutableBytesWritable ptr, Object value, PDataType actualType,
    Integer maxLength, Integer scale, Integer desiredMaxLength, Integer desiredScale,
    PDataType desiredType, SortOrder actualSortOrder, SortOrder desiredSortOrder,
    boolean expectedRowKeyOrderOptimizable) {
    if (ptr.getLength() == 0) { // a zero length ptr means null which will not be coerced to
                                // anything different
      return;
    }
    PDataType baseType = PDataType.fromTypeId(actualType.getSqlType() - PDataType.ARRAY_TYPE_BASE);
    PDataType desiredBaseType =
      PDataType.fromTypeId(desiredType.getSqlType() - PDataType.ARRAY_TYPE_BASE);
    if (
      (Objects.equal(maxLength, desiredMaxLength) || maxLength == null || desiredMaxLength == null)
        ...
        && (desiredSortOrder == SortOrder.ASC || desiredBaseType.isFixedWidth()
          || isRowKeyOrderOptimized(actualType, actualSortOrder, ptr)
    ...
    if (value == null || actualType != desiredType) {
      value = toObject(ptr.get(), ptr.getOffset(), ptr.getLength(), baseType, actualSortOrder,
        maxLength, desiredScale, desiredBaseType);
      ...
      if (
        actualType == desiredType && !pArr.isPrimitiveType() && maxLength != null
  ...
  public static boolean isRowKeyOrderOptimized(PDataType type, SortOrder sortOrder,
    ImmutableBytesWritable ptr) {
    return isRowKeyOrderOptimized(type, sortOrder, ptr.get(), ptr.getOffset(), ptr.getLength());
  }

  public static boolean isRowKeyOrderOptimized(PDataType type, SortOrder sortOrder, byte[] buf,
  ...
  private static boolean isRowKeyOrderOptimized(boolean isFixedWidth, SortOrder sortOrder,
  ...
  public Object toObject(byte[] bytes, int offset, int length, PDataType baseType,
    ...
    return createPhoenixArray(bytes, offset, length, sortOrder, baseType, maxLength,
      desiredDataType);
  }

  static int getOffset(byte[] bytes, int arrayIndex, boolean useShort, int indexOffset,
    byte serializationVersion) {
    return Math
      .abs(getSerializedOffset(bytes, arrayIndex, useShort, indexOffset, serializationVersion));
  }
  ...
  static int getSerializedOffset(byte[] bytes, int arrayIndex, boolean useShort, int indexOffset,
    byte serializationVersion) {
    int offset;
    if (useShort) {
  ...
  private static int getOffset(ByteBuffer indexBuffer, int arrayIndex, boolean useShort,
    int indexOffset) {
    int offset;
    if (useShort) {
  ...
  private byte[] createArrayBytes(TrustedByteArrayOutputStream byteStream, DataOutputStream oStream,
    ...
    boolean rowKeyOrderOptimizable) {
    PArrayDataTypeEncoder builder = new PArrayDataTypeEncoder(byteStream, oStream, noOfElements,
  ...
  /**
   * Appends an item to array. Uses the ptr bytes of item and the array bytes to create new array
   * bytes with appended item bytes, then sets the new array bytes to ptr.
   ...
   */
  public static boolean appendItemToArray(ImmutableBytesWritable ptr, int length, int offset,
    ...
    if (!baseType.isFixedWidth()) {
      ...
      int offsetArrayPosition = Bytes.toInt(arrayBytes,
        offset + length - Bytes.SIZEOF_INT - Bytes.SIZEOF_INT - Bytes.SIZEOF_BYTE,
        Bytes.SIZEOF_INT);
      int offsetArrayLength =
        length - offsetArrayPosition - Bytes.SIZEOF_INT - Bytes.SIZEOF_INT - Bytes.SIZEOF_BYTE;

      // checks whether offset array consists of shorts or integers
      boolean useInt =
        arrayLength == 0 ? false : offsetArrayLength / Math.abs(arrayLength) == Bytes.SIZEOF_INT;
      ...
      byte sepByte = getSeparatorByte(
        isRowKeyOrderOptimized(false, sortOrder, arrayBytes, offset, length), sortOrder);
      ...
      if (useInt) {
        ...
        writeEndBytes(newArray, newOffsetArrayPosition, offsetArrayLength, arrayLength,
          arrayBytes[offset + length - 1], true);
      } else {
        if (!convertToInt) {
          System.arraycopy(arrayBytes, offset + offsetArrayPosition, newArray,
            newOffsetArrayPosition, offsetArrayLength);
          Bytes.putShort(newArray, newOffsetArrayPosition + offsetArrayLength,
            (short) (newElementPosition - Short.MAX_VALUE));

          writeEndBytes(newArray, newOffsetArrayPosition, offsetArrayLength, arrayLength,
            arrayBytes[offset + length - 1], false);
        } else {
          int off = newOffsetArrayPosition;
          for (int arrayIndex = 0; arrayIndex < Math.abs(arrayLength) - 1; arrayIndex++) {
            Bytes.putInt(newArray, off, getOffset(arrayBytes, arrayIndex, true,
  ...
  private static void writeEndBytes(byte[] array, int newOffsetArrayPosition, int offsetArrayLength,
    int arrayLength, byte header, boolean useInt) {
    int byteSize = useInt ? Bytes.SIZEOF_INT : Bytes.SIZEOF_SHORT;

    Bytes.putInt(array, newOffsetArrayPosition + offsetArrayLength + byteSize,
      newOffsetArrayPosition);
    Bytes.putInt(array, newOffsetArrayPosition + offsetArrayLength + byteSize + Bytes.SIZEOF_INT,
      arrayLength);
    Bytes.putByte(array,
      newOffsetArrayPosition + offsetArrayLength + byteSize + 2 * Bytes.SIZEOF_INT, header);
  }

  public static boolean prependItemToArray(ImmutableBytesWritable ptr, int length, int offset,
    byte[] arrayBytes, PDataType baseType, int arrayLength, Integer maxLength,
    SortOrder sortOrder) {
    int elementLength = maxLength == null ? ptr.getLength() : maxLength;
    if (ptr.getLength() == 0) {
    ...
    if (!baseType.isFixedWidth()) {
      ...
      // checks whether offset array consists of shorts or integers
      boolean useInt =
        arrayLength == 0 ? false : offsetArrayLength / arrayLength == Bytes.SIZEOF_INT;
      boolean convertToInt = false;
      int endElementPosition = getOffset(arrayBytes, arrayLength - 1, !useInt,
        offsetArrayPosition + offset, serializationVersion) + elementLength + Bytes.SIZEOF_BYTE;
      ...
      if (elementLength == 0) {
        ...
        // nRemainingNulls == 1 and nMultiplesOver255 == 0 means there were no nulls at the
        // beginning
        // previously.
        // At that case we need to increase the length by two bytes, one for separator byte and one
        // for null
        // count.
        // ex: initial array - 65 0 66 0 0 0 after prepending null - 0 1(inverted) 65 0 66 0 0 0
        // nRemainingNulls == 1 and nMultiplesOver255 != 0 means there were null at the beginning
        // previously.
        ...
        // ex: initial array - 0 1 65 0 66 0 0 0 after prepending null - 0 1 1(inverted) 65 0 66 0 0
        // 0
        // nRemainingNulls == 0 case.
        // ex: initial array - 0 254(inverted) 65 0 66 0 0 0 after prepending null - 0 1 65 0 66 0 0
        // 0
        // nRemainingNulls > 1 case.
        // ex: initial array - 0 45(inverted) 65 0 66 0 0 0 after prepending null - 0 46(inverted)
        // 65 0 66 0 0 0
        lengthIncrease = nRemainingNulls == 1
          ? (nMultiplesOver255 == 0 ? 2 * Bytes.SIZEOF_BYTE : Bytes.SIZEOF_BYTE)
          : 0;
        endElementPosition = getOffset(arrayBytes, arrayLength - 1, !useInt,
        ...
        newArray[elementLength] = getSeparatorByte(
          isRowKeyOrderOptimized(false, sortOrder, arrayBytes, offset, length), sortOrder);
  ...
  private static void writeNewOffsets(byte[] arrayBytes, byte[] newArray, boolean useShortNew,
    boolean useShortPrevious, int newOffsetArrayPosition, int arrayLength, int offsetArrayPosition,
    int offset, int offsetShift, int length) {
    ...
    for (int arrayIndex = 0; arrayIndex < arrayLength - 1; arrayIndex++) {
      int oldOffset = getOffset(arrayBytes, arrayIndex, useShortPrevious,
        offsetArrayPosition + offset, serializationVersion);
      if (arrayBytes[offset + oldOffset] == QueryConstants.SEPARATOR_BYTE && nullsAtBeginning) {
        ...
        if (useShortNew) {
          Bytes.putShort(newArray, currentPosition,
            (short) (oldOffset + offsetShift - Short.MAX_VALUE));
        } else {
  ...
  public static boolean concatArrays(ImmutableBytesWritable ptr, int array1BytesLength,
    int array1BytesOffset, byte[] array1Bytes, PDataType baseType, int actualLengthOfArray1,
    ...
    if (!baseType.isFixedWidth()) {
      byte serializationVersion1 =
        array1Bytes[array1BytesOffset + array1BytesLength - Bytes.SIZEOF_BYTE];
      ...
      for (int index = actualLengthOfArray1 - 1; index > -1; index--) {
        ...
        if (
          array1Bytes[array1BytesOffset + offset] == QueryConstants.SEPARATOR_BYTE
            || array1Bytes[array1BytesOffset + offset] == QueryConstants.DESC_SEPARATOR_BYTE
        ) {
      ...
      byte serializationVersion2 =
        array2Bytes[array2BytesOffset + array2BytesLength - Bytes.SIZEOF_BYTE];
      ...
      int bytesForNullsBefore =
        nullsAtTheBeginningOfArray2 / 255 + (nullsAtTheBeginningOfArray2 % 255 == 0 ? 0 : 1);
      int bytesForNullsAfter =
        nullsInMiddleAfterConcat / 255 + (nullsInMiddleAfterConcat % 255 == 0 ? 0 : 1);
      // Increase of length required to store nulls
      int lengthIncreaseForNulls = bytesForNullsAfter - bytesForNullsBefore;
      // Length increase incremented by one when there were no nulls at the beginning of array and
      // when there are
      // nulls at the end of array 1 as we need to allocate a byte for separator byte in this case.
      lengthIncreaseForNulls +=
        nullsAtTheBeginningOfArray2 == 0 && nullsAtTheEndOfArray1 != 0 ? Bytes.SIZEOF_BYTE : 0;
      int newOffsetArrayPosition = offsetArrayPositionArray1 + offsetArrayPositionArray2
        + lengthIncreaseForNulls - 2 * Bytes.SIZEOF_BYTE;
      int endElementPositionOfArray2 = getOffset(array2Bytes, actualLengthOfArray2 - 1,
        !useIntArray2, array2BytesOffset + offsetArrayPositionArray2, serializationVersion2);
      int newEndElementPosition = lengthIncreaseForNulls + endElementPositionOfArray2
        + offsetArrayPositionArray1 - 2 * Bytes.SIZEOF_BYTE;
      // Creates a byte array to store the concatenated array
      if (PArrayDataType.useShortForOffsetArray(newEndElementPosition)) {
        newArray = new byte[newOffsetArrayPosition + newArrayLength * Bytes.SIZEOF_SHORT
          + Bytes.SIZEOF_INT + Bytes.SIZEOF_INT + Bytes.SIZEOF_BYTE];
      } else {
        useIntNewArray = true;
        newArray = new byte[newOffsetArrayPosition + newArrayLength * Bytes.SIZEOF_INT
          + Bytes.SIZEOF_INT + Bytes.SIZEOF_INT + Bytes.SIZEOF_BYTE];
      }

      int currentPosition = 0;
      // Copies all the elements from array 1 to new array
      System.arraycopy(array1Bytes, array1BytesOffset, newArray, currentPosition,
        offsetArrayPositionArray1 - 2 * Bytes.SIZEOF_BYTE);
      ...
      if (useIntNewArray) {
        ...
        for (int index = array2FirstNonNullIndex; index < actualLengthOfArray2; index++) {
          int offset = getOffset(array2Bytes, index, !useIntArray2,
            array2BytesOffset + offsetArrayPositionArray2, serializationVersion2);
          Bytes.putInt(newArray, currentPosition,
        ...
        for (int index = array2FirstNonNullIndex; index < actualLengthOfArray2; index++) {
          ...
          Bytes.putShort(newArray, currentPosition, (short) (offset
            - array2FirstNonNullElementOffset + part2NonNullStartingPosition - Short.MAX_VALUE));
          currentPosition += Bytes.SIZEOF_SHORT;
        }
      }
      Bytes.putInt(newArray, currentPosition, newOffsetArrayPosition);
      ...
      currentPosition += Bytes.SIZEOF_INT;
      Bytes.putByte(newArray, currentPosition,
        array1Bytes[array1BytesOffset + array1BytesLength - 1]);
    } else {
      ...
      System.arraycopy(array2Bytes, array2BytesOffset, newArray, array1BytesLength,
        array2BytesLength);
    }
    ptr.set(newArray);
    return true;
  ...
  public static boolean arrayToString(ImmutableBytesWritable ptr, PhoenixArray array,
    String delimiter, String nullString, SortOrder sortOrder) {
  ...
  public static boolean stringToArray(ImmutableBytesWritable ptr, String string, String delimiter,
    String nullString, SortOrder sortOrder) {
  ...
  public static int serializeOffsetArrayIntoStream(DataOutputStream oStream,
    TrustedByteArrayOutputStream byteStream, int noOfElements, int maxOffset, int[] offsetPos,
    byte serializationVersion) throws IOException {
    ...
    if (useInt) {
      ...
      for (int pos : offsetPos) {
        short val = serializationVersion == PArrayDataType.IMMUTABLE_SERIALIZATION_VERSION
          || serializationVersion == PArrayDataType.IMMUTABLE_SERIALIZATION_V2
            ? (short) pos
  ...
  public static void serializeHeaderInfoIntoStream(DataOutputStream oStream, int noOfElements,
    byte serializationVersion) throws IOException {
    // No of elements
  ...
  private Object createPhoenixArray(byte[] bytes, int offset, int length, SortOrder sortOrder,
    ...
    if (!baseDataType.isFixedWidth()) {
      ...
      if (baseDataType == desiredDataType) {
        elements =
          (Object[]) java.lang.reflect.Array.newInstance(baseDataType.getJavaClass(), noOfElements);
      } else {
        elements = (Object[]) java.lang.reflect.Array.newInstance(desiredDataType.getJavaClass(),
          noOfElements);
      }
      ...
      if (noOfElements != 0) {
        while (countOfElementsRead <= noOfElements) {
          ...
          countOfElementsRead++;
          if (
            (bytes[currOffset + initPos] != QueryConstants.SEPARATOR_BYTE
              && bytes[currOffset + initPos] != QueryConstants.DESC_SEPARATOR_BYTE) && foundNull
          ) {
  ...
  public static int getArrayLength(ImmutableBytesWritable ptr, PDataType baseType,
    Integer maxLength) {
    byte[] bytes = ptr.get();
    if (ptr.getLength() == 0) {
      return 0;
    ...
    // In case where the number of elements is greater than SHORT.MAX_VALUE we do negate the number
    // of
    // elements. So it is always better to return the absolute value
    return (Bytes.toInt(bytes,
      (ptr.getOffset() + ptr.getLength() - (Bytes.SIZEOF_BYTE + Bytes.SIZEOF_INT))));
  }
