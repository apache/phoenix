...
public class TransformClient {
  ...
  private static final String TRANSFORM_SELECT = "SELECT " + PhoenixDatabaseMetaData.TENANT_ID
    + ", " + PhoenixDatabaseMetaData.TABLE_SCHEM + ", " + PhoenixDatabaseMetaData.LOGICAL_TABLE_NAME
    + ", " + PhoenixDatabaseMetaData.NEW_PHYS_TABLE_NAME + ", "
    + PhoenixDatabaseMetaData.TRANSFORM_TYPE + ", " + PhoenixDatabaseMetaData.LOGICAL_PARENT_NAME
    + ", " + TRANSFORM_STATUS + ", " + PhoenixDatabaseMetaData.TRANSFORM_JOB_ID + ", "
  ...
  public static SystemTransformRecord getTransformRecord(String schema, String logicalTableName,
    String logicalParentName, String tenantId, PhoenixConnection connection) throws SQLException {
    return getTransformRecordFromDB(schema, logicalTableName, logicalParentName, tenantId,
      connection);
  }

  public static SystemTransformRecord getTransformRecordFromDB(String schema,
    String logicalTableName, String logicalParentName, String tenantId,
    ...
    String sql = TRANSFORM_SELECT + " WHERE  "
      ...
      + (Strings.isNullOrEmpty(logicalParentName)
        ? ""
        : (" AND " + PhoenixDatabaseMetaData.LOGICAL_PARENT_NAME + "='" + logicalParentName + "'"));
    try (ResultSet resultSet =
      ((PhoenixPreparedStatement) connection.prepareStatement(sql)).executeQuery()) {
      if (resultSet.next()) {
  ...
  private static boolean isTransformNeeded(MetaDataClient.MetaProperties metaProperties,
    PTable table) {
    if (
      metaProperties.getImmutableStorageSchemeProp() != null
    ...
    if (
      metaProperties.getColumnEncodedBytesProp() != null
  ...
  public static boolean checkIsTransformNeeded(MetaDataClient.MetaProperties metaProperties,
    String schemaName, PTable table, String logicalTableName, String parentTableName,
      ...
      if (existingTransform != null && existingTransform.isActive()) {
        throw new SQLExceptionInfo.Builder(
          SQLExceptionCode.CANNOT_TRANSFORM_ALREADY_TRANSFORMING_TABLE)
            .setMessage(" Only one transform at a time is allowed ").setSchemaName(schemaName)
            .setTableName(logicalTableName).build().buildException();
      }
    }
  ...
  protected static SystemTransformRecord getTransformRecord(PhoenixConnection connection,
    PTableType tableType, PName schemaName, PName tableName, PName dataTableName, PName tenantId,
    ...
    if (tableType == PTableType.TABLE) {
      ...
      return getTransformRecord(SchemaUtil.getSchemaNameFromFullName(parentLogicalName.getString()),
        SchemaUtil.getTableNameFromFullName(parentLogicalName.getString()), null,
        tenantId == null ? null : tenantId.getString(), connection);
    }
  ...
  private static String generateNewTableName(String schema, String logicalTableName, long seqNum) {
    ...
    String newName =
      String.format("%s_%d", SchemaUtil.getTableName(schema, logicalTableName), seqNum);
    return newName;
  }

  public static PTable addTransform(PhoenixConnection connection, String tenantId, PTable table,
    ...
    try {
      ...
      return addTransform(table, changingProperties, transformBuilder.build(), sequenceNum,
        connection);
      ...
      throw new SQLExceptionInfo.Builder(SQLExceptionCode.valueOf("CANNOT_MUTATE_TABLE"))
        ...
        .setRootCause(ex).setTableName(table.getName().getString()).build().buildException();
    }
  }

  protected static PTable addTransform(PTable table, MetaDataClient.MetaProperties changedProps,
    SystemTransformRecord systemTransformParams, long sequenceNum, PhoenixConnection connection)
    throws Exception {
    PName newTableName = PNameFactory.newName(systemTransformParams.getNewPhysicalTableName());
    PName newTableNameWithoutSchema = PNameFactory.newName(
      SchemaUtil.getTableNameFromFullName(systemTransformParams.getNewPhysicalTableName()));
    PIndexState defaultCreateState =
      PIndexState.valueOf(connection.getQueryServices().getConfiguration()
        .get(INDEX_CREATE_DEFAULT_STATE, QueryServicesOptions.DEFAULT_CREATE_INDEX_STATE));
    PTable newTable = new PTableImpl.Builder().setTableName(newTableNameWithoutSchema)
      .setParentTableName(table.getParentTableName())
      .setBaseTableLogicalName(table.getBaseTableLogicalName())
      .setPhysicalTableName(newTableNameWithoutSchema).setState(defaultCreateState)
      .setAllColumns(table.getColumns()).setAppendOnlySchema(table.isAppendOnlySchema())
      .setAutoPartitionSeqName(table.getAutoPartitionSeqName())
      .setBaseColumnCount(table.getBaseColumnCount()).setBucketNum(table.getBucketNum())
      .setDefaultFamilyName(table.getDefaultFamilyName()).setDisableWAL(table.isWALDisabled())
      ...
      .setIndexType(table.getIndexType()).setIndexes(Collections.<PTable> emptyList())
      ...
      .setPhoenixTTL(table.getPhoenixTTL()).setNamespaceMapped(table.isNamespaceMapped())
      .setSchemaName(table.getSchemaName()).setPkColumns(table.getPKColumns())
      .setPkName(table.getPKName()).setPhoenixTTLHighWaterMark(table.getPhoenixTTLHighWaterMark())
      .setRowKeySchema(table.getRowKeySchema()).setStoreNulls(table.getStoreNulls())
      .setTenantId(table.getTenantId()).setType(table.getType())
      // SchemaExtractor uses physical name to get the table descriptor from. So we use the existing
      // table here
      ...
      // Transformables
      .setImmutableStorageScheme((changedProps.getImmutableStorageSchemeProp() != null
        ? changedProps.getImmutableStorageSchemeProp()
        : table.getImmutableStorageScheme()))
      .setQualifierEncodingScheme((changedProps.getColumnEncodedBytesProp() != null
        ? changedProps.getColumnEncodedBytesProp()
        : table.getEncodingScheme()))
      .build();
    SchemaExtractionProcessor schemaExtractionProcessor =
      new SchemaExtractionProcessor(systemTransformParams.getTenantId(),
    ...
    addTransformTableLink(connection, systemTransformParams.getTenantId(),
    ...
    TableViewFinderResult childViewsResult =
      ViewUtil.findChildViews(connection, systemTransformParams.getTenantId(),
        systemTransformParams.getSchemaName(), systemTransformParams.getLogicalTableName());
    for (TableInfo view : childViewsResult.getLinks()) {
      addTransformTableLink(connection,
        view.getTenantId() == null ? null : Bytes.toString(view.getTenantId()),
        (view.getSchemaName() == null ? null : Bytes.toString(view.getSchemaName())),
        Bytes.toString(view.getTableName()), newTableName, sequenceNum);
    }

    if (defaultCreateState != PIndexState.CREATE_DISABLE) {
      // add a monitoring task
      addTransformMonitorTask(connection, connection.getQueryServices().getConfiguration(),
        systemTransformParams, PTable.TaskStatus.CREATED,
  ...
  public static void upsertTransform(SystemTransformRecord systemTransformParams,
    PhoenixConnection connection) throws SQLException {
    try (PreparedStatement stmt = connection.prepareStatement("UPSERT INTO "
      + PhoenixDatabaseMetaData.SYSTEM_TRANSFORM_NAME + " ( " + PhoenixDatabaseMetaData.TABLE_SCHEM
      + ", " + PhoenixDatabaseMetaData.LOGICAL_TABLE_NAME + ", " + PhoenixDatabaseMetaData.TENANT_ID
      + "," + PhoenixDatabaseMetaData.NEW_PHYS_TABLE_NAME + ", "
      + PhoenixDatabaseMetaData.TRANSFORM_TYPE + ", " + PhoenixDatabaseMetaData.LOGICAL_PARENT_NAME
      + ", " + TRANSFORM_STATUS + ", " + PhoenixDatabaseMetaData.TRANSFORM_JOB_ID + ", "
      + PhoenixDatabaseMetaData.TRANSFORM_RETRY_COUNT + ", "
  ...
  private static void addTransformTableLink(Connection connection, String tenantId,
    String schemaName, String tableName, PName newTableName, long sequenceNum) throws SQLException {
  ...
  public static void addTransformMonitorTask(PhoenixConnection connection,
    ...
    PTable.TaskStatus taskStatus, Timestamp startTimestamp, Timestamp endTimestamp)
    ...
    if (!transformMonitorEnabled) {
      LOGGER.warn(
        "TransformMonitor is not enabled. Monitoring/retrying TransformTool and doing cutover will not be done automatically");
      return;
    }

    List<Mutation> sysTaskUpsertMutations =
      Task.getMutationsForAddTask(new SystemTaskParams.SystemTaskParamsBuilder().setConn(connection)
        ...
        .setTaskStatus(taskStatus.toString()).setStartTs(startTimestamp).setEndTs(endTimestamp)
    ...
    if (
      ...
      throw new SQLExceptionInfo.Builder(SQLExceptionCode.UNABLE_TO_UPSERT_TASK)
        .setSchemaName(SYSTEM_SCHEMA_NAME).setTableName(SYSTEM_TASK_TABLE).build().buildException();
    }
  }
}
