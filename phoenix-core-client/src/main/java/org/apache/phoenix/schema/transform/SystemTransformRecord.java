...
/**
 * Task params to be used while upserting records in SYSTEM.TRANSFORM table. This POJO is mainly
 * used while upserting(and committing) or generating upsert mutations plan in {@link Transform}
 * class
 */
...
public class SystemTransformRecord {
  ...
  public static class SystemTransformBuilder {
    ...
    public SystemTransformRecord build() {
      ...
      if (
        lastTs == null && transformStatus != null
          && transformStatus.equals(PTable.TaskStatus.COMPLETED.toString())
      ) {
