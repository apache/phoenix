...
public class RowKeyValueAccessor implements Writable {
  ...
  /**
   * Constructor solely for use during deserialization. Should not otherwise be used.
   */
  public RowKeyValueAccessor() {
  }

  /**
   * The class used to keep list of booleans representing whether the data type of the given field
   * is of type VARBINARY_ENCODED. For example, let's say we have variable length data types in the
   * order (VARCHAR, VARBINARY_ENCODED, VARCHAR). The corresponding object of this class will have
   * binaryEncodedDataTypes list with values (false, true, false). The array of this class is
   * serialized and deserialized together with other fields of RowKeyValueAccessor. The value of the
   * list determines which separator bytes need to be used while iterating through the rowkey bytes.
   */
  ...
  /**
   * The class used to keep list of booleans representing whether the order of the given field is
   * Ascending. For example, let's say we have rowkey with pk columns (A ASC, B DESC, C ASC). The
   * corresponding object of this class will have sortOrderAsc list with values (true, false, true).
   * The array of this class is serialized and deserialized together with other fields of
   * RowKeyValueAccessor. The value of the list determines which separator bytes need to be used
   * while iterating through the rowkey bytes.
   */
  ...
  public RowKeyValueAccessor(List<? extends PDatum> data, int index) {
    ...
    while (pos < index) {
      ...
      if (datum.getDataType().isFixedWidth()) {
        ...
        do {
          encodedTypesLists
            .addBinaryEncodedDataTypes(datum.getDataType() == PVarbinaryEncoded.INSTANCE);
          sortOrders.addSortOrderAsc(datum.getSortOrder() == SortOrder.ASC);
  ...
  private int[] offsets;
  /**
   * An array of BinaryEncodedTypesLists. Each element of BinaryEncodedTypesLists consists of list
   * of booleans representing whether the given column is of type VARBINARY_ENCODED or other
   * variable length data type. All the continuous fixed length as well as variable length data type
   * columns are clubbed together. For example, let's say we have columns (A INTEGER, B FLOAT, C
   * DOUBLE, D VARCHAR, E VARBINARY_ENCODED, F VARCHAR). Here, we have three continuous fixed-length
   * columns and three continuous variable length columns. Hence, binaryEncodedTypesLists array
   * consists of only two elements. binaryEncodedTypesLists[0] consists of List<Boolean> as [false,
   * false, false] because INTEGER, FLOAT and DOUBLE are of fixed length data type. Whereas
   * binaryEncodedTypesLists[1] consists of List<Boolean> as [false, true, false] because D and F
   * are not VARBINARY_ENCODED whereas E is VARBINARY_ENCODED.
   */
   ...
   * representing whether the given column is ASC of DESC order. All the continuous fixed length as
   * well as variable length data type columns are clubbed together. For example, let's say we have
   * columns (A INTEGER, B FLOAT DESC, C DOUBLE, D VARCHAR DESC, E VARBINARY_ENCODED, F VARCHAR).
   * Here, we have three continuous fixed-length columns and three continuous variable length
   * columns. Hence, sortOrderLists array consists of only two elements. sortOrderLists[0] consists
   * of List<Boolean> as [true, false, true] because A and C are ASC whereas B is DESC ordered
   * column. sortOrderLists[1] consists of List<Boolean> as [false, true, true] because D is DESC
   * whereas E and F are ASC ordered columns.
   */
  ...
  public String toString() {
    return "RowKeyValueAccessor [offsets=" + Arrays.toString(offsets) + ", isFixedLength="
      + isFixedLength + ", hasSeparator=" + hasSeparator + "]";
  }
  ...
  public void readFields(DataInput input) throws IOException {
    ...
    if (input instanceof DataInputBuffer) {
      ...
      if (
        !Bytes.equals(data, offset, QueryConstants.ROW_KEY_VAL_ACCESSOR_NEW_FIELDS_SEPARATOR.length,
      ...
      if (dataInputStream.markSupported()) {
        dataInputStream
          .mark(QueryConstants.ROW_KEY_VAL_ACCESSOR_NEW_FIELDS_SEPARATOR.length * 1000);
        ...
        if (
          !Bytes.equals(data, 0, QueryConstants.ROW_KEY_VAL_ACCESSOR_NEW_FIELDS_SEPARATOR.length,
            QueryConstants.ROW_KEY_VAL_ACCESSOR_NEW_FIELDS_SEPARATOR, 0,
            QueryConstants.ROW_KEY_VAL_ACCESSOR_NEW_FIELDS_SEPARATOR.length)
        ) {
          dataInputStream.reset();
      ...
      LOGGER.error(
        "Type of DataInput is neither DataInputBuffer nor DataInputStream. "
          + "This is not expected. Do not attempt deserialization of "
          + "binaryEncodedTypesLists and sortOrderLists for compatibility purpose." + " input: {}",
        input);
      return;
    }
    ...
    if (sortOrderListsLen == 0) {
      ...
      this.sortOrderLists =
        JacksonUtil.getObjectReader().readValue(sortOrdersListsBytes, SortOrderList[].class);
    }
  }
      ...
      if (offset >= 0) { // If offset is non negative, it's a byte offset
        ...
        while (offset++ < 0) {
          ...
          if (!isVarBinaryEncoded) {
            ...
            while (
              keyOffset < keyBuffer.length
                && !SchemaUtil.areSeparatorBytesForVarBinaryEncoded(keyBuffer, keyOffset++,
                  sortOrderAsc ? SortOrder.ASC : SortOrder.DESC)
            ) {
              // empty
            }
  ...
  public int getOffsetWithNullTypeAndOrderInfo(byte[] keyBuffer, int keyOffset) {
    ...
    for (int offset : offsets) {
      if (offset >= 0) { // If offset is non negative, it's a byte offset
        ...
        while (offset++ < 0) {
          // FIXME: keyOffset < keyBuffer.length required because HBase passes bogus keys to filter
          // to position scan (HBASE-6562)
          while (keyOffset < keyBuffer.length && !isSeparatorByte(keyBuffer[keyOffset++])) {
          }
        }
      }
  ...
  public int getLength(byte[] keyBuffer, int keyOffset, int maxOffset, PDataType type,
    ...
    if (type == PVarbinaryEncoded.INSTANCE) {
      while (
        offset < maxOffset
          && !SchemaUtil.areSeparatorBytesForVarBinaryEncoded(keyBuffer, offset, sortOrder)
      ) {
        offset++;
      }
    } else {
