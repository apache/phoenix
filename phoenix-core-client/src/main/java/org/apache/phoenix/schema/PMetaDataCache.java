...
class PMetaDataCache {
  ...
  public PMetaDataCache(int initialCapacity, long maxByteSize, TimeKeeper timeKeeper) {
    this.tables =
      CacheBuilder.newBuilder().removalListener(new RemovalListener<PTableKey, PTableRef>() {
        @Override
      ...
      }).maximumWeight(maxByteSize).weigher(new Weigher<PTableKey, PTableRef>() {
        @Override
        public int weigh(PTableKey tableKey, PTableRef tableRef) {
          if (
            PhoenixDatabaseMetaData.SYSTEM_CATALOG_SCHEMA
              .equals(SchemaUtil.getSchemaNameFromFullName(tableKey.getName()))
          ) {
