...
public interface PTable extends PMetaDataEntity {
  ...
  public enum LinkType {
    ...
    PARENT_TABLE((byte) 3),
    /**
     * Link from a parent table to its child view (these are stored in SYSTEM.CHILD_LINK for
     * scalability)
     */
    CHILD_TABLE((byte) 4),
    /**
  ...
  public enum ImmutableStorageScheme implements ColumnValueEncoderDecoderSupplier {
    ...
    SINGLE_CELL_ARRAY_WITH_OFFSETS((byte) 2, PArrayDataType.IMMUTABLE_SERIALIZATION_V2) {
      ...
      public ColumnValueEncoder getEncoder(int numElements) {
        ...
        return new PArrayDataTypeEncoder(byteStream, oStream, numElements, type, SortOrder.ASC,
          false, getSerializationVersion());
      }
  ...
  public enum QualifierEncodingScheme implements QualifierEncoderDecoder {
    ...
    public static class InvalidQualifierBytesException extends RuntimeException {
      public InvalidQualifierBytesException(int expectedLength, int actualLength) {
        super("Invalid number of qualifier bytes. Expected length: " + expectedLength + ". Actual: "
          + actualLength);
      }
    }
  ...
  /** Returns A list of the column families of this table ordered by position. */
   ...
   * @throws ColumnNotFoundException  if no column with the given name can be found
   * @throws AmbiguousColumnException if multiple columns are found with the given name
   */
  PColumn getColumnForColumnName(String name)
   ...
   */
  PColumn getColumnForColumnQualifier(byte[] cf, byte[] cq)
    throws ColumnNotFoundException, AmbiguousColumnException;
   ...
   * @param name the column name
   * @return the PColumn with the given name
   * @throws ColumnNotFoundException if no PK column with the given name can be found
   */
  PColumn getPKColumn(String name) throws ColumnNotFoundException;

  /**
   * Creates a new row at the specified timestamp using the key for the PK values (from
   * {@link #newKey(ImmutableBytesWritable, byte[][])} and the optional key values specified using
   * values.
   ...
   * @return the new row. Use {@link org.apache.phoenix.schema.PRow#toRowMutations()} to generate
   *         the Row to send to the HBase server.
   * @throws ConstraintViolationException if row data violates schema constraint
   */
  PRow newRow(KeyValueBuilder builder, long ts, ImmutableBytesWritable key, boolean hasOnDupKey,
    byte[]... values);

  /**
   * Creates a new row for the PK values (from {@link #newKey(ImmutableBytesWritable, byte[][])} and
   * the optional key values specified using values. The timestamp of the key value will be set by
   * the HBase server.
   * @param key         the row key of the key value
   * @param hasOnDupKey true if row has an ON DUPLICATE KEY clause and false otherwise.
   * @param values      the optional key values
   ...
   * @throws ConstraintViolationException if row data violates schema constraint
   */
  PRow newRow(KeyValueBuilder builder, ImmutableBytesWritable key, boolean hasOnDupKey,
    byte[]... values);

  /**
   * Formulates a row key using the values provided. The values must be in the same order as
   * {@link #getPKColumns()}.
  ...
  /**
   * Return the number of buckets used by this table for salting. If the table does not use salting,
   * returns null.
  ...
  /**
   * @return the full name of the parent view for a view or data table for an index table or null if
   *         this is not a view or index table. Also returns null for a view of a data table
   *         (use @getPhysicalName for this case)
   */
  PName getParentName();

  /**
   * @return the table name of the parent view for a view or data table for an index table or null
   *         if this is not a view or index table. Also returns null for a view of a data table
   *         (use @getPhysicalTableName for this case)
   */
  PName getParentTableName();

  /**
   * @return the logical full name of the base table. In case of the view index, it is the
   *         _IDX_+logical name of base table Ex: For hierarchical views like tableLogicalName -->
   *         view1 --> view2, for view2, returns sc.tableLogicalName For view2, getParentTableName
   *         returns view1 and getBaseTableLogicalName returns sc.tableLogicalName
   */
  PName getBaseTableLogicalName();

  /**
   * @return the schema name of the parent view for a view or data table for an index table or null
   *         if this is not a view or index table. Also returns null for view of a data table
   *         (use @getPhysicalSchemaName for this case)
   */
  PName getParentSchemaName();

  /**
   * For a view, return the name of table in Phoenix that physically stores data. Currently a single
   * name, but when views are allowed over multiple tables, will become multi-valued.
  ...
  /**
   * If returnColValueFromSyscat is true, returns the column value set in the syscat. Otherwise,
   * behaves like getPhysicalName()
   * @return the name of the physical HBase table storing the data.
   */
  ...
  /**
   * Determines whether or not we may optimize out an ORDER BY or do a GROUP BY in-place when the
   * optimizer tells us it's possible. This is due to PHOENIX-2067 and only applicable for tables
   * using DESC primary key column(s) which have not been upgraded.
   * @return true if optimizations row key order optimizations are possible
   */
  boolean rowKeyOrderOptimizable();

  /**
   * @return Position of the column with {@link PColumn#isRowTimestamp()} as true. -1 if there is no
   *         such column.
   */
  ...
  /**
   * @return true if the you can only add (and never delete) columns to the table, you are also not
   *         allowed to delete the table
   */
  ...
  /**
   * @return The TTL expression associated with the entity when Phoenix level TTL is enabled. The
   *         expression can be a Literal value or a boolean Condition.
   */
  TTLExpression getTTLExpression();

  /**
   * @return Returns the compiled TTL expression associated with the entity when Phoenix level TTL
   *         is enabled. For Literal expressions there is no difference between getTTL() and
   *         getCompiledTTLExpression(). For Conditional TTL, getTTL() returns the expression as
   ...
   */
  CompiledTTLExpression getCompiledTTLExpression(PhoenixConnection connection) throws SQLException;

  /** Returns Returns true if table has conditional TTL set */
  boolean hasConditionalTTL();

  /**
   * @return the last timestamp at which this entity had its data shape created or modified (e .g,
   *         create entity, adding or dropping a column. Not affected by changing table properties
   */
  ...
  /**
   * @return Whether strict TTL mode is enabled on a given table. True if the TTL is strict, false
   *         if relaxed.
   */
  boolean isStrictTTL();

  /**
   * @return User-provided string identifying the application version that last created or modified
   *         this schema object. Used only on tables, views, and indexes.
   */
  String getSchemaVersion();

  /**
   * @return String provided by an external schema registry to be used to lookup the schema for a
   *         Phoenix table or view in the registry.
   */
  String getExternalSchemaId();

  /**
   * @return Optional string to be used for a logical topic name that change detection capture will
   *         use to persist changes for this table or view
   */
  String getStreamingTopicName();

  /**
   * Returns Optional string that represents the default include scopes to be used for CDC queries.
   */
  Set<CDCChangeScope> getCDCIncludeScopes();

  /** Returns the optional where clause in string used for partial indexes */
  String getIndexWhere();

  /** Returns the map of all ancestors to their LAST_DDL_TIMESTAMP */
  ...
  /** Returns the set of column references for the columns included in the index where clause */
  Set<ColumnReference> getIndexWhereColumns(PhoenixConnection connection) throws SQLException;

  /**
   * @return Prefixed KeyRange generated by the expression representing the view statement. In other
  ...
  public class EncodedCQCounter {
    ...
    /**
     * Get the next qualifier to be used for the column family. This method also ends up
     * initializing the counter if the column family already doesn't have one.
     */
    @Nullable
    public Integer getNextQualifier(String columnFamily) {
