...
public class ArgumentTypeMismatchException extends SQLException {
  ...
  public ArgumentTypeMismatchException(PDataType expected, PDataType actual, String location) {
    super(new SQLExceptionInfo.Builder(code)
      .setMessage("expected: " + expected + " but was: " + actual + " at " + location).build()
      .toString(), code.getSQLState(), code.getErrorCode());
  }

  public ArgumentTypeMismatchException(Class<? extends PDataType>[] expecteds, PDataType actual,
    String location) {
    this(Arrays.toString(Collections2
      .transform(Arrays.asList(expecteds), new Function<Class<? extends PDataType>, PDataType>() {
        @Nullable
        @Override
