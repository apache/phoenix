...
/**
 * Exception thrown when a table name is used without being qualified with an alias and more than
 * one schema contains that table.
 * @since 0.1
 */
public class AmbiguousTableException extends SQLException {
  ...
  public AmbiguousTableException(String tableName) {
    super(new SQLExceptionInfo.Builder(code).setTableName(tableName).toString(), code.getSQLState(),
      code.getErrorCode(), null);
  }
