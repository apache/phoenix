...
public class ExecuteUpdateNotApplicableException extends SQLException {
  ...
  public ExecuteUpdateNotApplicableException(Operation op) {
    super(new SQLExceptionInfo.Builder(code).setMessage("Disallowed operation: " + op.name())
      .build().toString(), code.getSQLState(), code.getErrorCode());
  }

  public ExecuteUpdateNotApplicableException(String query) {
    super(new SQLExceptionInfo.Builder(code).setMessage("Query: " + query).build().toString(),
  ...
  public ExecuteUpdateNotApplicableException(String command, String statement) {
    super(
      new SQLExceptionInfo.Builder(code)
        .setMessage("Command: " + command + ". Statement: " + statement).build().toString(),
      code.getSQLState(), code.getErrorCode());
  }
}
