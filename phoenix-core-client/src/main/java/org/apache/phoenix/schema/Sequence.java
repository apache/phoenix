...
public class Sequence {
  ...
  private static final Cell CURRENT_VALUE_KV = org.apache.hadoop.hbase.KeyValueUtil
    .createFirstOnRow(ByteUtil.EMPTY_BYTE_ARRAY, SYSTEM_SEQUENCE_FAMILY_BYTES, CURRENT_VALUE_BYTES);
  private static final Cell INCREMENT_BY_KV = org.apache.hadoop.hbase.KeyValueUtil
    .createFirstOnRow(ByteUtil.EMPTY_BYTE_ARRAY, SYSTEM_SEQUENCE_FAMILY_BYTES, INCREMENT_BY_BYTES);
  private static final Cell CACHE_SIZE_KV = org.apache.hadoop.hbase.KeyValueUtil
    .createFirstOnRow(ByteUtil.EMPTY_BYTE_ARRAY, SYSTEM_SEQUENCE_FAMILY_BYTES, CACHE_SIZE_BYTES);
  private static final Cell MIN_VALUE_KV = org.apache.hadoop.hbase.KeyValueUtil
    .createFirstOnRow(ByteUtil.EMPTY_BYTE_ARRAY, SYSTEM_SEQUENCE_FAMILY_BYTES, MIN_VALUE_BYTES);
  private static final Cell MAX_VALUE_KV = org.apache.hadoop.hbase.KeyValueUtil
    .createFirstOnRow(ByteUtil.EMPTY_BYTE_ARRAY, SYSTEM_SEQUENCE_FAMILY_BYTES, MAX_VALUE_BYTES);
  private static final Cell CYCLE_KV = org.apache.hadoop.hbase.KeyValueUtil
    .createFirstOnRow(ByteUtil.EMPTY_BYTE_ARRAY, SYSTEM_SEQUENCE_FAMILY_BYTES, CYCLE_FLAG_BYTES);
  private static final Cell LIMIT_REACHED_KV =
    org.apache.hadoop.hbase.KeyValueUtil.createFirstOnRow(ByteUtil.EMPTY_BYTE_ARRAY,
      SYSTEM_SEQUENCE_FAMILY_BYTES, LIMIT_REACHED_FLAG_BYTES);
  private static final List<Cell> SEQUENCE_KV_COLUMNS =
    Arrays.<Cell> asList(CURRENT_VALUE_KV, INCREMENT_BY_KV, CACHE_SIZE_KV,
      // The following three columns were added in 3.1/4.1
      MIN_VALUE_KV, MAX_VALUE_KV, CYCLE_KV, LIMIT_REACHED_KV);
  static {
  ...
  private static final EmptySequenceCacheException EMPTY_SEQUENCE_CACHE_EXCEPTION =
    new EmptySequenceCacheException();
      ...
      if (i >= 0 && values.get(i).timestamp == value.timestamp) {
        if (values.get(i).isDeleted) {
          throw new IllegalStateException(
            "Unexpected delete marker at timestamp " + value.timestamp + " for " + key);
        }
        values.set(i, value);
      } else {
  ...
  private long increment(SequenceValue value, ValueOp op, long numToAllocate) throws SQLException {
    ...
    if (value.limitReached && op != ValueOp.VALIDATE_SEQUENCE) {
      if (value.cycle) {
        ...
        SQLExceptionCode code = increasingSeq
    ...
    if (op == ValueOp.INCREMENT_SEQUENCE) {
      ...
      try {
        ...
        value.currentValue =
          LongMath.checkedAdd(value.currentValue, numToAllocate * value.incrementBy);
      ...
      if (
        overflowOrUnderflow || (increasingSeq && value.currentValue > value.maxValue)
   ...
   * determines whether we need to refresh the cache when evaluating NEXT VALUE FOR. Once current
   * value reaches the next value we know the cache is exhausted as we give sequence values out one
   * at time. However for bulk allocations, evaluated by NEXT <n> VALUE FOR, we need a different
   * check
   * @see isSequenceCacheExhaustedForBulkAllocation Using the bulk allocation method for determining
   *      if the cache is exhausted for both cases works in most of the cases, however when dealing
   *      with CYCLEs and overflow and underflow, things break down due to things like sign changes
   *      that can happen if we overflow from a positive to a negative number and vice versa.
   *      Therefore, leaving both checks in place.
   */
  private boolean isSequenceCacheExhausted(final long numToAllocate, final SequenceValue value)
    throws SQLException {
  ...
  /**
   * This method checks whether there are sufficient values in the SequenceValue cached on the
   * client to allocate the requested number of slots. It handles decreasing and increasing
   * sequences as well as any overflows or underflows encountered.
   */
  private boolean isSequenceCacheExhaustedForBulkAllocation(final long numToAllocate,
    final SequenceValue value) throws SQLException {
    ...
    try {
      ...
      // We don't treat this as the cache being exhausted as the current value may be valid in the
      // case
      // of no cycle, logic in increment() will take care of detecting we've hit the limit of the
      // sequence
      return false;
    }
  ...
  /**
   * @throws SQLException with the correct error code if sequence limit is reached with this request
   *                      for allocation or we attempt to perform a bulk allocation on a sequence
   *                      with cycles.
   */
  private void performValidationForBulkAllocation(final long numToAllocate,
    final SequenceValue value) throws SQLException {
    ...
    if (value.cycle && !SequenceUtil.isCycleAllowed(numToAllocate)) {
      throw new SQLExceptionInfo.Builder(SQLExceptionCode.NUM_SEQ_TO_ALLOCATE_NOT_SUPPORTED)
        .setSchemaName(key.getSchemaName()).setTableName(key.getSequenceName()).build()
        .buildException();
    }

    if (
      SequenceUtil.checkIfLimitReached(value.currentValue, value.minValue, value.maxValue,
  ...
  private Append newReturn(SequenceValue value) {
    ...
    append.setAttribute(SequenceRegionObserverConstants.CURRENT_VALUE_ATTRIB,
      PLong.INSTANCE.toBytes(value.nextValue));
    Map<byte[], List<Cell>> familyMap = append.getFamilyCellMap();
    familyMap.put(PhoenixDatabaseMetaData.SYSTEM_SEQUENCE_FAMILY_BYTES,
      Arrays.<Cell> asList(
        PhoenixKeyValueUtil.newKeyValue(key, PhoenixDatabaseMetaData.SYSTEM_SEQUENCE_FAMILY_BYTES,
  ...
  public long incrementValue(Result result, ValueOp op, long numToAllocate) throws SQLException {
    ...
    if (result.rawCells().length == 1) {
      ...
      int errorCode = PInteger.INSTANCE.getCodec().decodeInt(errorKV.getValueArray(),
        errorKV.getValueOffset(), SortOrder.getDefault());
      ...
      // }
      throw new SQLExceptionInfo.Builder(code).setSchemaName(key.getSchemaName())
        .setTableName(key.getSequenceName()).build().buildException();
    }
  ...
  public Increment newIncrement(long timestamp, Sequence.ValueOp action, long numToAllocate) {
    ...
    try {
      ...
      inc.setAttribute(SequenceRegionObserverConstants.NUM_TO_ALLOCATE,
        Bytes.toBytes(numToAllocate));
    } catch (IOException e) {
    ...
    for (Cell kv : SEQUENCE_KV_COLUMNS) {
      try {
        ...
        Cell cell =
          new KeyValue(incKey, 0, incKey.length, kv.getFamilyArray(), kv.getFamilyOffset(),
            kv.getFamilyLength(), kv.getQualifierArray(), kv.getQualifierOffset(),
            kv.getQualifierLength(), timestamp, KeyValue.Type.Put, incValue, 0, incValue.length);
        inc.add(cell);
      } catch (IOException e) {
        throw new RuntimeException(e); // Impossible
      }
   ...
   * @param kv        an empty KeyValue used only to get the column family and column qualifier of
   *                  the key value to be returned (if the sequence row is from a previous version)
   * @param cellIndex index of the KeyValue to be returned (if the sequence row is from a previous
   *                  version
   */
  private static Cell getKeyValue(Result r, Cell kv, int cellIndex) {
    Cell[] cells = r.rawCells();
    // if the sequence row is from a previous version then MIN_VALUE, MAX_VALUE, CYCLE and
    // LIMIT_REACHED key values are not present,
    // the sequence row has only three columns (INCREMENT_BY, CACHE_SIZE and CURRENT_VALUE) and the
    // order of the cells
    // in the array returned by rawCells() is not what what we expect so use getColumnLatestCell()
    // to get the cell we want
    return cells.length == NUM_SEQUENCE_KEY_VALUES
      ? cells[cellIndex]
      : r.getColumnLatestCell(kv.getFamilyArray(), kv.getFamilyOffset(), kv.getFamilyLength(),
  ...
  /**
   * Returns the KeyValues of r if it contains the expected number of KeyValues, else returns a list
   * of KeyValues corresponding to SEQUENCE_KV_COLUMNS
   */
  public static List<Cell> getCells(Result r, int numKVs) {
    // if the sequence row is from a previous version
    if (r.rawCells().length == numKVs) return Lists.newArrayList(r.rawCells());
  ...
  private static final class SequenceValue {
    ...
    public SequenceValue(Result r, ValueOp op, long numToAllocate) {
      ...
      this.nextValue = PLong.INSTANCE.getCodec().decodeLong(currentValueKV.getValueArray(),
        currentValueKV.getValueOffset(), SortOrder.getDefault());
      this.incrementBy = PLong.INSTANCE.getCodec().decodeLong(incrementByKV.getValueArray(),
        incrementByKV.getValueOffset(), SortOrder.getDefault());
      this.cacheSize = PLong.INSTANCE.getCodec().decodeLong(cacheSizeKV.getValueArray(),
        cacheSizeKV.getValueOffset(), SortOrder.getDefault());
      this.minValue = PLong.INSTANCE.getCodec().decodeLong(minValueKV.getValueArray(),
        minValueKV.getValueOffset(), SortOrder.getDefault());
      this.maxValue = PLong.INSTANCE.getCodec().decodeLong(maxValueKV.getValueArray(),
        maxValueKV.getValueOffset(), SortOrder.getDefault());
      this.cycle = (Boolean) PBoolean.INSTANCE.toObject(cycleKV.getValueArray(),
      ...
      if (op != ValueOp.VALIDATE_SEQUENCE) {
        ...
        currentValue -=
          incrementBy * (SequenceUtil.isBulkAllocation(numToAllocate) ? numToAllocate : cacheSize);
      }
    }
  }
  ...
  public boolean returnValue(Result result) throws SQLException {
    ...
    int statusCode = PInteger.INSTANCE.getCodec().decodeInt(statusKV.getValueArray(),
      statusKV.getValueOffset(), SortOrder.getDefault());
  ...
  public Append createSequence(long startWith, long incrementBy, long cacheSize, long timestamp,
    ...
    byte[] key = this.key.getKey();
    Append append = new Append(key);
    append.setAttribute(SequenceRegionObserverConstants.OPERATION_ATTRIB,
      new byte[] { (byte) MetaOp.CREATE_SEQUENCE.ordinal() });
    ...
    familyMap.put(PhoenixDatabaseMetaData.SYSTEM_SEQUENCE_FAMILY_BYTES, Arrays.<Cell> asList(
      ...
      PhoenixKeyValueUtil.newKeyValue(key, PhoenixDatabaseMetaData.SYSTEM_SEQUENCE_FAMILY_BYTES,
        PhoenixDatabaseMetaData.INCREMENT_BY_BYTES, timestamp, PLong.INSTANCE.toBytes(incrementBy)),
      PhoenixKeyValueUtil.newKeyValue(key, PhoenixDatabaseMetaData.SYSTEM_SEQUENCE_FAMILY_BYTES,
        PhoenixDatabaseMetaData.CACHE_SIZE_BYTES, timestamp, PLong.INSTANCE.toBytes(cacheSize)),
      PhoenixKeyValueUtil.newKeyValue(key, PhoenixDatabaseMetaData.SYSTEM_SEQUENCE_FAMILY_BYTES,
        PhoenixDatabaseMetaData.MIN_VALUE_BYTES, timestamp, PLong.INSTANCE.toBytes(minValue)),
      PhoenixKeyValueUtil.newKeyValue(key, PhoenixDatabaseMetaData.SYSTEM_SEQUENCE_FAMILY_BYTES,
        PhoenixDatabaseMetaData.MAX_VALUE_BYTES, timestamp, PLong.INSTANCE.toBytes(maxValue)),
      PhoenixKeyValueUtil.newKeyValue(key, PhoenixDatabaseMetaData.SYSTEM_SEQUENCE_FAMILY_BYTES,
        PhoenixDatabaseMetaData.CYCLE_FLAG_BYTES, timestamp, PBoolean.INSTANCE.toBytes(cycle)),
      PhoenixKeyValueUtil.newKeyValue(key, PhoenixDatabaseMetaData.SYSTEM_SEQUENCE_FAMILY_BYTES,
        PhoenixDatabaseMetaData.LIMIT_REACHED_FLAG_BYTES, timestamp, PDataType.FALSE_BYTES)));
    return append;
  }

  public long createSequence(Result result, long minValue, long maxValue, boolean cycle)
    throws SQLException {
    Cell statusKV = result.rawCells()[0];
    long timestamp = statusKV.getTimestamp();
    int statusCode = PInteger.INSTANCE.getCodec().decodeInt(statusKV.getValueArray(),
  ...
  public Append dropSequence(long timestamp) {
    ...
    Append append = new Append(key);
    append.setAttribute(SequenceRegionObserverConstants.OPERATION_ATTRIB,
      new byte[] { (byte) MetaOp.DROP_SEQUENCE.ordinal() });
    if (timestamp != HConstants.LATEST_TIMESTAMP) {
      append.setAttribute(SequenceRegionObserverConstants.MAX_TIMERANGE_ATTRIB,
        Bytes.toBytes(timestamp));
    }
