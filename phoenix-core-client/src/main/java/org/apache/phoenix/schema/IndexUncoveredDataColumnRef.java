...
/**
 * Even when a column is not covered by an index table for a given query, we may still want to use
 * index in the query plan and fetch the missing columns from the data table rows on the server
 * side. This class is used to keep track of such data columns.
 */
public class IndexUncoveredDataColumnRef extends ColumnRef {
  ...
  public IndexUncoveredDataColumnRef(StatementContext context, TableRef tRef,
    String indexColumnName) throws MetaDataEntityNotFoundException, SQLException {
    super(
      FromCompiler
        .getResolver(FACTORY.namedTable(null,
  ...
  protected IndexUncoveredDataColumnRef(IndexUncoveredDataColumnRef indexDataColumnRef,
    long timestamp) {
  ...
  @Override
  public ColumnExpression newColumnExpression(boolean schemaNameCaseSensitive,
    boolean colNameCaseSensitive) {
    String displayName =
      this.getTableRef().getColumnDisplayName(this, schemaNameCaseSensitive, colNameCaseSensitive);
