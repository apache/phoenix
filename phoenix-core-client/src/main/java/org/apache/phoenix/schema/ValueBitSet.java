...
/**
 * Class to track whether or not a value is null. The value is a zero-based position in the schema
 * provided.
 * @since 0.1
 */
public class ValueBitSet {
  ...
  private ValueBitSet(ValueSchema schema) {
    this.schema = schema;
    bits = new long[Math.max(1,
      (schema.getFieldCount() - schema.getMinNullable() + BITS_PER_LONG - 1) / BITS_PER_LONG)];
  }
  ...
  private boolean isVarLength() {
    return schema == null
      ? false
      : schema.getFieldCount() - schema.getMinNullable() > BITS_PER_SHORT;
  }
  ...
  public int getNullCount(int nBit, int nFields) {
    ...
    count +=
      Math.min(nFields, bitsToLeft) - (Long.bitCount((bits[index] >>> shiftRight) << shiftLeft));
  ...
  /**
   * Serialize the value bit set into a byte array. The byte array is expected to have enough room
   * (use {@link #getEstimatedLength()} to ensure enough room exists.
  ...
  /** Returns Max serialization size */
  public int getEstimatedLength() {
    if (schema == null) {
      return 0;
    }
    return Bytes.SIZEOF_SHORT
      + (isVarLength() ? (maxSetBit + BITS_PER_LONG) / BITS_PER_LONG * Bytes.SIZEOF_LONG : 0);
  }

  public static int getSize(int nBits) {
    return SizedUtil.OBJECT_SIZE + SizedUtil.POINTER_SIZE + SizedUtil.ARRAY_SIZE
      + SizedUtil.INT_SIZE + (nBits + BITS_PER_LONG - 1) / BITS_PER_LONG * Bytes.SIZEOF_LONG;
  }
  ...
  public int getSize() {
    ...
    return SizedUtil.OBJECT_SIZE + SizedUtil.POINTER_SIZE + SizedUtil.ARRAY_SIZE
      + SizedUtil.LONG_SIZE * bits.length + SizedUtil.INT_SIZE;
  }
