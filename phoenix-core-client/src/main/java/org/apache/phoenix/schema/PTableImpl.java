  ...
  public static PTable createFromProto(PTableProtos.PTable table) {
    ...
    try {
      return new PTableImpl.Builder().setType(tableType).setState(indexState)
        ...
        .setTenantId(tenantId).setSchemaName(schemaName).setTableName(tableName)
        .setPhysicalTableName(physicalTableName).setPkName(pkName)
        ...
        .setIsChangeDetectionEnabled(isChangeDetectionEnabled).setSchemaVersion(schemaVersion)
        .setExternalSchemaId(externalSchemaId).setStreamingTopicName(streamingTopicName)
        .setIndexWhere(indexWhere).build();
