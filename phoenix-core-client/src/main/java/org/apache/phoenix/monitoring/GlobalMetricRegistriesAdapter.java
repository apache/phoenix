 ...
 * adapter attaches HBase's MetricRegistry to Hadoop's DefaultMetricsSystem Note: This DOES NOT
 * handle dynamic attach/detach of registries
 */
public class GlobalMetricRegistriesAdapter {
  ...
  private GlobalMetricRegistriesAdapter() {
    if (MetricUtil.isDefaultMetricsInitialized()) {
      ...
      LOGGER.info(
        "HBase metrics is already initialized. " + "Skipping Phoenix metrics initialization.");
      return;
    }
  ...
  private static class HBaseMetrics2HadoopMetricsAdapter implements MetricsSource {
    ...
    private void registerToDefaultMetricsSystem() {
      ...
      LOGGER.info("Registering " + info.getMetricsJmxContext() + " " + info.getMetricsDescription()
