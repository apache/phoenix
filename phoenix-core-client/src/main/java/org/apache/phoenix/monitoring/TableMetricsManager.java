 ...
 * store the instance of it associated with TableName in a map This class exposes following
 * functions as static methods to help catch all execptions 1.clearTableLevelMetricsMethod
 * 2.getTableMetricsMethod 3.pushMetricsFromConnInstanceMethod 4.updateMetricsMethod
 */
...
public class TableMetricsManager {
  ...
  public TableMetricsManager(QueryServicesOptions ops) {
    ...
    LOGGER
      .info(String.format("Phoenix Table metrics enabled status: %s", isTableLevelMetricsEnabled));
      ...
      synchronized (TableMetricsManager.class) {
        if (localRef == null) {
          ...
          if (!options.isTableLevelMetricsEnabled()) {
            localRef = tableMetricsManager = NoOpTableMetricsManager.noOpsTableMetricManager;
            return localRef;
          }
          ...
          if (isMetricPublisherEnabled) {
            ...
            if (className != null) {
              ...
              LOGGER.info(
                String.format("Phoenix table level metrics publisher className %s", className));
              try {
  ...
  public void updateMetrics(String tableName, MetricType type, long value) {
    ...
    LOGGER.debug(String.format(
      "Phoenix table level metrics completed updating metric" + " %s to value %s, timetaken = %s",
      type, value, EnvironmentEdgeManager.currentTime() - startTime));
  }
  ...
  public TableClientMetrics getTableClientMetrics(String tableName) {
    ...
    if (tInstance == null) {
      synchronized (TableMetricsManager.class) {
        ...
        if (tInstance == null) {
          LOGGER
            .info(String.format("Phoenix Table metrics creating object for table: %s", tableName));
  ...
  /**
   * Update the Metrics for systemCatalog Table. For every userTable which is non empty we update
   * success/Failure RPC call metric and for systemCatalog table we update the total no. of
   * success/failure rpc calls made.
   */
  public static void updateMetricsForSystemCatalogTableMethod(String userTable, MetricType mType,
    long value) {
    try {
  ...
  public static void updateSizeHistogramMetricsForMutations(String tableName, long mutationBytes,
    ...
    TableClientMetrics tableMetrics;
    if ((tableMetrics = getTableClientMetricsInstance(tableName)) != null) {
      LOGGER.trace("Updating size histograms for mutations: tableName: " + tableName + " isUpsert: "
        + isUpsert + "  mutation bytes: " + mutationBytes);
