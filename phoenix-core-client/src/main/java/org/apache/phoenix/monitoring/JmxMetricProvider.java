...
/**
 * This class implements the JMX based default Metric publishing of Metrics to JMX end point. This
 * class is defined in
 * phoenix/phoenix-core/src/main/resources/META-INF/services/org.apache.phoenix.monitoring.MetricPublisherSupplierFactory
 */
public class JmxMetricProvider implements MetricPublisherSupplierFactory {
  ...
  private MetricRegistry createMetricRegistry() {
    ...
    MetricRegistryInfo registryInfo =
      new MetricRegistryInfo(metricsName, metricsDesc, metricsjmxContext, metricsContext, true);
  ...
  public void registerMetrics(TableClientMetrics tInstance) {
    for (Map.Entry<MetricType, PhoenixTableMetric> entry : tInstance.getMetricRegistry()
      .entrySet()) {
      metricRegistry.register(getMetricNameFromMetricType(entry.getKey(), tInstance.getTableName()),
  ...
  public void unRegisterMetrics(TableClientMetrics tInstance) {
    for (Map.Entry<MetricType, PhoenixTableMetric> entry : tInstance.getMetricRegistry()
      .entrySet()) {
      metricRegistry.remove(getMetricNameFromMetricType(entry.getKey(), tInstance.getTableName()));
    }
  }
