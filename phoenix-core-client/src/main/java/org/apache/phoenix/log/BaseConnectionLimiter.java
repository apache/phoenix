...
/**
 * A base class for concrete implementation of ConnectionLimiter. 1. Should be called only when
 * holding the ConnectionQueryServicesImpl.connectionCountLock 2. Does the basic accounting on open
 * and close connection calls. 3. If connection throttling is enabled checks and calls onLimit if
...
public abstract class BaseConnectionLimiter implements ConnectionLimiter {
  ...
  protected BaseConnectionLimiter(String profileName, boolean shouldThrottleNumConnections,
    int maxConnectionsAllowed, int maxInternalConnectionsAllowed) {
  ...
  public void acquireConnection(PhoenixConnection connection) throws SQLException {
    ...
    if (shouldThrottleNumConnections) {
      ...
      int allowedConnections =
        connection.isInternalConnection() ? maxInternalConnectionsAllowed : maxConnectionsAllowed;
      // if throttling threshold is reached, try reclaiming garbage collected phoenix connections.
      if (
        (allowedConnections != 0) && (futureConnections > allowedConnections)
          && (onSweep(connection.isInternalConnection()) == 0)
      ) {
        GLOBAL_PHOENIX_CONNECTIONS_THROTTLED_COUNTER.increment();
        connectionThrottledCounter++;
        String connectionQueryServiceName =
          connection.getQueryServices().getConfiguration().get(QUERY_SERVICES_NAME);
        ...
        SQLException connectionThrottledException = connection.isInternalConnection()
          ? new SQLExceptionInfo.Builder(SQLExceptionCode.NEW_INTERNAL_CONNECTION_THROTTLED).build()
            .buildException()
          : new SQLExceptionInfo.Builder(SQLExceptionCode.NEW_CONNECTION_THROTTLED).build()
            .buildException();
