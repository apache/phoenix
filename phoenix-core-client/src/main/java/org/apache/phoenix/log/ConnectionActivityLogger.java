...
/**
 * Logger for connection related activities. See also {@link ActivityLogInfo}
 */
public class ConnectionActivityLogger {
  ...
  List<String> activityList =
    Stream.of(ActivityLogInfo.values()).map(f -> "").collect(Collectors.toList());
   ...
   */
  public String getActivityLog() {
    return IntStream.range(0, ActivityLogInfo.values().length).filter((i) -> {
      return !Strings.isNullOrEmpty(activityList.get(i))
        && isLevelEnabled(ActivityLogInfo.values()[i].getLogLevel());
    }).mapToObj(i -> new StringBuilder().append(ActivityLogInfo.values()[i].shortName).append("=")
      .append(activityList.get(i)).toString()).collect(Collectors.joining(", "));
  }

  /**
   * Is Info logging currently enabled? Call this method to prevent having to perform expensive
   * operations (for example, String concatenation) when the log level is more than info.
   */
  public boolean isInfoEnabled() {
  ...
  /**
   * Is debug logging currently enabled? Call this method to prevent having to perform expensive
   * operations (for example, String concatenation) when the log level is more than debug.
   */
  public boolean isDebugEnabled() {
