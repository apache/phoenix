...
public class CDCTableInfo {
  ...
  private CDCTableInfo(List<CDCColumnInfo> columnInfoList, Set<PTable.CDCChangeScope> includeScopes,
  ...
  public CDCTableInfo(byte[] defaultColumnFamily, List<CDCColumnInfo> columnInfoList,
    ...
    PTable.QualifierEncodingScheme qualifierEncodingScheme, byte[] cdcJsonColQualBytes,
  ...
  public static CDCTableInfo createFromProto(CDCInfoProtos.CDCTableDef table) {
    ...
    if (table.hasQualifierEncodingScheme()) {
      qualifierEncodingScheme = PTable.QualifierEncodingScheme
        .fromSerializedValue(table.getQualifierEncodingScheme().toByteArray()[0]);
    }
    ...
    if (table.hasDataTableProjectorBytes()) {
      dataTableProjector = TupleProjector
        .deserializeProjectorFromBytes(table.getDataTableProjectorBytes().toByteArray());
    }
    return new CDCTableInfo(defaultColumnFamily, columns, changeScopeSet, qualifierEncodingScheme,
      table.getCdcJsonColQualBytes().toByteArray(), dataTableProjector);
  }
  ...
  public static CDCInfoProtos.CDCTableDef toProto(StatementContext context) throws SQLException {
    ...
    if (dataTable.getDefaultFamilyName() != null) {
      builder.setDefaultFamilyName(ByteStringer.wrap(dataTable.getDefaultFamilyName().getBytes()));
    }
    ...
    if (dataTable.getEncodingScheme() != null) {
      builder.setQualifierEncodingScheme(ByteStringer
        .wrap(new byte[] { dataTable.getEncodingScheme().getSerializedMetadataValue() }));
    }
    ...
    if (
      cdcDataTableRef.getTable().isImmutableRows()
      ...
      PTable projectedDataTable =
        TupleProjectionCompiler.createProjectedTable(cdcDataTableRef, dataColumns, false);
      ;
      TupleProjector dataTableProjector = new TupleProjector(projectedDataTable);
      builder.setDataTableProjectorBytes(
        ByteStringer.wrap(TupleProjector.serializeProjectorIntoBytes(dataTableProjector)));
    }
  ...
  public static class CDCColumnInfo implements Comparable<CDCColumnInfo> {
    ...
    public CDCColumnInfo(byte[] columnFamily, byte[] columnQualifier, String columnName,
      PDataType columnType, String columnFamilyName) {
    ...
    public int compareTo(CDCColumnInfo columnInfo) {
      return CDCUtil.compareCellFamilyAndQualifier(this.getColumnFamily(),
        this.getColumnQualifier(), columnInfo.getColumnFamily(), columnInfo.getColumnQualifier());
    }
    ...
    public static CDCColumnInfo createFromProto(CDCInfoProtos.CDCColumnDef column) {
      ...
      String columnFamilyName =
        StandardCharsets.UTF_8.decode(ByteBuffer.wrap(familyNameBytes)).toString();
    ...
    public String getColumnDisplayName(CDCTableInfo tableInfo) {
      if (columnDisplayName == null) {
        ...
        if (Arrays.equals(getColumnFamily(), tableInfo.getDefaultColumnFamily())) {
          ...
          columnDisplayName = getColumnFamilyName() + NAME_SEPARATOR + getColumnName();
        }
      }
