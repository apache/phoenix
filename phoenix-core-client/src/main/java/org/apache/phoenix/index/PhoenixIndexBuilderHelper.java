...
public final class PhoenixIndexBuilderHelper {
  ...
  /**
   * Serialize ON DUPLICATE KEY UPDATE info with the following format: 1) Boolean value tracking
   * whether or not to execute the first ON DUPLICATE KEY clause. We know the clause should be
   * executed when there are other UPSERT VALUES clauses earlier in the same batch for this row key.
   * We need this for two main cases: UPSERT VALUES followed by UPSERT VALUES ON DUPLICATE KEY
   * UPDATE UPSERT VALUES ON DUPLICATE KEY IGNORE followed by UPSERT VALUES ON DUPLICATE KEY UPDATE
   * 2) Short value tracking how many times the next first clause should be executed. This optimizes
   * the same clause be executed many times by only serializing it once. 3) Repeating
   * {@code List<Expression>, PTable } pairs that encapsulate the ON DUPLICATE KEY clause.
  ...
  public static byte[] combineOnDupKey(byte[] oldOnDupKeyBytes, byte[] newOnDupKeyBytes) {
    ...
    try (TrustedByteArrayOutputStream stream = new TrustedByteArrayOutputStream(
      ...
      if (isOldDupKeyIgnore) {
        output.writeBoolean(false); // Will force subsequent ON DUPLICATE KEY UPDATE statement to
                                    // execute
        ...
        if (
          Bytes.compareTo(oldOnDupKeyBytes, ON_DUP_KEY_HEADER_BYTE_SIZE,
          ...
          // If the new ON DUPLICATE KEY UPDATE was repeating, we need to write it multiple times as
          // only the first
