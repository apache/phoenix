...
public class TenantCacheImpl implements TenantCache {
  ...
  private Cache<ImmutableBytesPtr, CacheEntry> buildCache(final int ttl,
    ...
    return builder.ticker(getTicker())
  ...
  synchronized private void evictInactiveEntries(long bytesNeeded) {
    ...
    for (int i = 0; i < entries.length && available < bytesNeeded; i++) {
      ...
      LOGGER.debug("Evicted cache ID " + Bytes.toLong(cacheId.get()) + ", we now have " + available
        + " bytes available");
    }
  }
  ...
  @Override
  public Closeable addServerCache(ImmutableBytesPtr cacheId, ImmutableBytesWritable cachePtr,
    ...
    try {
      ...
      synchronized (this) {
        ...
        if (entry == null) {
          entry = new CacheEntry(cacheId, cachePtr, cacheFactory, txState, chunk,
            usePersistentCache, useProtoForIndexMaintainer, clientVersion);
