...
/**
 * Child QueryServices that delegates through to global QueryService. Used to track memory used by
 * each org to allow a max percentage threshold.
 * @since 0.1
 */
public class ChildQueryServices extends DelegateConnectionQueryServices {
  ...
  public ChildQueryServices(ConnectionQueryServices services) {
    super(services);
    int maxOrgMemPerc =
      getProps().getInt(MAX_TENANT_MEMORY_PERC_ATTRIB, DEFAULT_MAX_ORG_MEMORY_PERC);
