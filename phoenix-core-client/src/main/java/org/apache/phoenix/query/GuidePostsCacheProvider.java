...
public class GuidePostsCacheProvider {
  ...
  GuidePostsCacheFactory loadAndGetGuidePostsCacheFactory(String classString) {
    ...
    if (guidePostsCacheFactory == null) {
      try {
        ...
        List<GuidePostsCacheFactory> factoryList =
          InstanceResolver.get(GuidePostsCacheFactory.class, null);
        for (GuidePostsCacheFactory factory : factoryList) {
          if (clazz.isInstance(factory)) {
            guidePostsCacheFactory = factory;
            LOGGER.info(String.format(
              "Sucessfully loaded class for GuidePostsCacheFactor of type: %s", classString));
            break;
          }
        }
        if (guidePostsCacheFactory == null) {
  ...
  public GuidePostsCacheWrapper getGuidePostsCache(String classStr,
    ...
    PhoenixStatsLoader phoenixStatsLoader =
      guidePostsCacheFactory.getPhoenixStatsLoader(queryServices, props, config);
    GuidePostsCache guidePostsCache =
