 ...
 * upgrade from HBase 1.x to 2.x versions. Some Admin APIs fail to retrieve table state details from
 * meta table if master is running on 1.x version and coprocs are running on 2.x version. Hence, as
 * a fallback, server side coproc can directly perform zookeeper look-up to retrieve table state
 * data. The fallback use-cases would not be encountered for fully upgraded 2.x cluster.
 */
public class AdminUtilWithFallback {
  ...
  public static boolean tableExists(Admin admin, TableName tableName)
    ...
    try {
      ...
      if (
        e instanceof NoSuchColumnFamilyException
  ...
  private static TableState.State getTableStateFromZk(TableName tableName, Admin admin)
    throws IOException, InterruptedException {
    try (ZKWatcher zkWatcher =
      new ZKWatcher(admin.getConfiguration(), "phoenix-admin-fallback", null)) {
      ZooKeeperProtos.DeprecatedTableState.State state = getTableState(zkWatcher, tableName);
      if (state == null) {
        return null;
      }
