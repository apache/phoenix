...
public abstract class AbstractRoundRobinQueue<E> extends AbstractQueue<E>
  ...
  /**
   * Construct an AbstractBlockingRoundRobinQueue that limits the size of the queued elements to at
   * most maxSize. Attempts to insert new elements after that point will cause the caller to block.
   */
  ...
  /**
   * @param newProducerToFront If true, new producers go to the front of the round-robin list, if
  ...
  /**
   * Implementations must extracts the producer object which is used as the key to identify a unique
   * producer.
   */
    ...
    synchronized (lock) {
      ...
      if (tList != null && !tList.list.isEmpty()) {
        ...
        if (tList.list.isEmpty()) {
          ...
          this.producerMap.remove(tList.producer);
          // we need to adjust the current thread pointer in case it pointed to this thread list,
