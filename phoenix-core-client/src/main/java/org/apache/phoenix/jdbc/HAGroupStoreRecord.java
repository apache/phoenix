...
/**
 * Immutable class representing an HA group store record with simplified fields. This is a
 * simplified version of ClusterRoleRecord that contains essential information about an HA group
 * only for a single cluster.
 */
public class HAGroupStoreRecord {
  ...
  public enum HAGroupState {
    ...
    static {
      ...
      ACTIVE_NOT_IN_SYNC.allowedTransitions = ImmutableSet.of(ACTIVE_NOT_IN_SYNC, ACTIVE_IN_SYNC,
        ACTIVE_NOT_IN_SYNC_TO_STANDBY, ACTIVE_NOT_IN_SYNC_WITH_OFFLINE_PEER);

      ACTIVE_IN_SYNC.allowedTransitions =
        ImmutableSet.of(ACTIVE_NOT_IN_SYNC, ACTIVE_WITH_OFFLINE_PEER, ACTIVE_IN_SYNC_TO_STANDBY);

      STANDBY.allowedTransitions = ImmutableSet.of(STANDBY_TO_ACTIVE, DEGRADED_STANDBY_FOR_READER,
        DEGRADED_STANDBY_FOR_WRITER);
      ...
      ACTIVE_NOT_IN_SYNC_TO_STANDBY.allowedTransitions =
        ImmutableSet.of(ABORT_TO_ACTIVE_NOT_IN_SYNC, ACTIVE_IN_SYNC_TO_STANDBY);
    ...
    public static HAGroupState from(byte[] bytes) {
      ...
      return Arrays.stream(HAGroupState.values()).filter(r -> r.name().equalsIgnoreCase(value))
  ...
  /**
   * Convenience constructor for backward compatibility without recordVersion and
  ...
  public static byte[] toJson(HAGroupStoreRecord record) throws IOException {
    return JacksonUtil.getObjectWriter().withoutAttribute("clusterRole").writeValueAsBytes(record);
  }
  ...
  public int hashCode() {
    return new HashCodeBuilder().append(protocolVersion).append(haGroupName).append(haGroupState)
      .append(recordVersion).append(lastSyncStateTimeInMs).hashCode();
  }
  ...
  public String toString() {
    return "HAGroupStoreRecord{" + "protocolVersion='" + protocolVersion + '\'' + ", haGroupName='"
      + haGroupName + '\'' + ", haGroupState=" + haGroupState + ", recordVersion=" + recordVersion
