...
/**
 * JDBC Connection implementation of Phoenix. Currently the following are supported: - Statement -
 * PreparedStatement The connection may only be used with the following options: -
 * ResultSet.TYPE_FORWARD_ONLY - Connection.TRANSACTION_READ_COMMITTED
 * @since 0.1
 */
public class PhoenixConnection
  implements MetaDataMutated, SQLCloseable, PhoenixMonitoredConnection {
  ...
  public PhoenixConnection(PhoenixConnection connection, MutationState mutationState)
    throws SQLException {
    this(connection.getQueryServices(), connection.getURL(), connection.getClientInfo(),
  ...
  public PhoenixConnection(ConnectionQueryServices services, String url, Properties info)
    throws SQLException {
    this(services, url, info, null, false, false, false, false, null);
  }

  public PhoenixConnection(ConnectionQueryServices services, String url, Properties info,
    HighAvailabilityGroup haGroup) throws SQLException {
  ...
  private PhoenixConnection(ConnectionQueryServices services, String url, Properties info,
    MutationState mutationState, boolean isDescVarLengthRowKeyUpgrade, boolean isRunningUpgrade,
    boolean buildingIndex, boolean isInternalConnection, HighAvailabilityGroup haGroup)
    throws SQLException {
    this.url = url;
    ...
    if (this.info.isEmpty() && tenantId == null) {
      ...
      this.services =
        augmentedProps == currentProps ? services : new DelegateConnectionQueryServices(services) {
          @Override
    ...
    this.isAutoFlush = this.services.getProps().getBoolean(QueryServices.TRANSACTIONS_ENABLED,
      QueryServicesOptions.DEFAULT_TRANSACTIONS_ENABLED)
      && this.services.getProps().getBoolean(QueryServices.AUTO_FLUSH_ATTRIB,
        QueryServicesOptions.DEFAULT_AUTO_FLUSH);
    this.isAutoCommit = JDBCUtil.getAutoCommit(url, this.info, this.services.getProps()
      .getBoolean(QueryServices.AUTO_COMMIT_ATTRIB, QueryServicesOptions.DEFAULT_AUTO_COMMIT));
    this.consistency = JDBCUtil.getConsistencyLevel(url, this.info, this.services.getProps()
      .get(QueryServices.CONSISTENCY_ATTRIB, QueryServicesOptions.DEFAULT_CONSISTENCY_LEVEL));
    ...
    this.schema = JDBCUtil.getSchema(url, this.info, this.services.getProps()
    ...
    timePattern =
      this.services.getProps().get(QueryServices.TIME_FORMAT_ATTRIB, DateUtil.DEFAULT_TIME_FORMAT);
    timestampPattern = this.services.getProps().get(QueryServices.TIMESTAMP_FORMAT_ATTRIB,
      DateUtil.DEFAULT_TIMESTAMP_FORMAT);
    String numberPattern = this.services.getProps().get(QueryServices.NUMBER_FORMAT_ATTRIB,
      NumberUtil.DEFAULT_NUMBER_FORMAT);
    int maxSize = this.services.getProps().getInt(QueryServices.MAX_MUTATION_SIZE_ATTRIB,
      QueryServicesOptions.DEFAULT_MAX_MUTATION_SIZE);
    long maxSizeBytes =
      this.services.getProps().getLongBytes(QueryServices.MAX_MUTATION_SIZE_BYTES_ATTRIB,
        QueryServicesOptions.DEFAULT_MAX_MUTATION_SIZE_BYTES);
    this.isApplyTimeZoneDisplacement =
      this.services.getProps().getBoolean(QueryServices.APPLY_TIME_ZONE_DISPLACMENT_ATTRIB,
        QueryServicesOptions.DEFAULT_APPLY_TIME_ZONE_DISPLACMENT);
    this.dateFormatTimeZoneId = this.services.getProps()
      .get(QueryServices.DATE_FORMAT_TIMEZONE_ATTRIB, DateUtil.DEFAULT_TIME_ZONE_ID);
    ...
    this.auditLogLevel = LogLevel.valueOf(this.services.getProps()
    ...
    this.logSamplingRate = Double.parseDouble(this.services.getProps()
      .get(QueryServices.LOG_SAMPLE_RATE, QueryServicesOptions.DEFAULT_LOG_SAMPLE_RATE));
    String connectionQueryServiceName = this.services.getConfiguration().get(QUERY_SERVICES_NAME);
    if (isInternalConnection) {
      ...
      ConnectionQueryServicesMetricsManager
        .updateConnectionQueryServiceOpenInternalConnectionHistogram(currentInternalConnectionCount,
  ...
  private static void checkScnAndBuildIndexAtEquality(Long scnParam, Long replayAt)
    ...
    if (scnParam != null && replayAt != null && !scnParam.equals(replayAt)) {
      throw new SQLExceptionInfo.Builder(SQLExceptionCode.UNEQUAL_SCN_AND_BUILD_INDEX_AT).build()
        .buildException();
    }
  }
  ...
  /**
   * Add connection to the internal childConnections queue This method is thread safe
   */
          ...
          if (isQuery) {
            ...
            if (!rs.next()) {
              ...
              if (out != null) {
                ...
                for (int i = 1; i <= columnCount; i++) {
                  ...
                  if (md.isSigned(i)) {
                    out.print(displayWidth < label.length()
                      ? label.substring(0, displayWidth)
            ...
            if (updateCount >= 0) {
              out.println((updateCount == 0 ? "no" : updateCount)
                + (updateCount == 1 ? " row " : " rows ") + stmt.getUpdateOperation().toString());
            }
          }
  ...
  private boolean prune(PTable table) {
    ...
    return (table.getType() != PTableType.SYSTEM && (table.getTimeStamp() >= maxTimestamp
      || (table.getTenantId() != null && !Objects.equal(tenantId, table.getTenantId()))));
  }

  /**
   * Similar to {@link #getTable(String, String, Long)} but returns the most recent PTable
   */
   ...
   * get the latest metadata use {@link #getTable(String, String)}
   * @throws NullPointerException     if conn or fullTableName is null
   * @throws IllegalArgumentException if timestamp is negative
   */
  public PTable getTable(@Nullable String tenantId, String fullTableName, @Nullable Long timestamp)
    throws SQLException {
    ...
    try {
      ...
      if (timestamp == null || (tableref != null && tableref.getResolvedTimeStamp() == timestamp)) {
  ...
  public PTable getTableNoCache(PName tenantId, String name, long timestamp) throws SQLException {
    ...
    MetaDataProtocol.MetaDataMutationResult result =
      new MetaDataClient(this).updateCache(tenantId, schemaName, tableName, false, timestamp);
  ...
  public PTable getTableNoCache(PName tenantId, String name) throws SQLException {
    ...
    String tableName = SchemaUtil.getTableNameFromFullName(name);
    MetaDataProtocol.MetaDataMutationResult result =
      new MetaDataClient(this).updateCache(tenantId, schemaName, tableName, true);
    if (result.getMutationCode() != MetaDataProtocol.MutationCode.TABLE_ALREADY_EXISTS) {
      throw new TableNotFoundException(schemaName, tableName);
    }
    return result.getTable();
  ...
  public PTable getTableFromServerNoCache(byte[] schemaName, byte[] tableName) throws SQLException {
    ...
    MetaDataProtocol.MetaDataMutationResult result = getQueryServices().getTable(getTenantId(),
      schemaName, tableName, HConstants.LATEST_TIMESTAMP, HConstants.LATEST_TIMESTAMP);
    if (result.getMutationCode() != MetaDataProtocol.MutationCode.TABLE_ALREADY_EXISTS) {
  ...
  /**
   * Returns the table if it is found in the client metadata cache. If the metadata of this table
   * has changed since it was put in the cache these changes will not necessarily be reflected in
   * the returned table. If the table is not found, makes a call to the server to fetch the latest
   * metadata of the table. This is different than how a table is resolved when it is referenced
   * from a query (a call is made to the server to fetch the latest metadata of the table depending
   * on the UPDATE_CACHE_FREQUENCY property) See https://issues.apache.org/jira/browse/PHOENIX-4475
  ...
  void checkOpen() throws SQLException {
    if (isClosed || isClosing) {
      throw reasonForClose != null
        ? reasonForClose
        : new SQLExceptionInfo.Builder(SQLExceptionCode.CONNECTION_CLOSED).build()
   ...
   * @param reasonForClose The reason for closing the phoenix connection to be set as state in
   *                       phoenix connection.
   ...
   */
  public PhoenixStatement createStatement(PhoenixStatementFactory statementFactory)
    throws SQLException {
  ...
  public void setAutoFlush(boolean autoFlush) throws SQLException {
    if (
      autoFlush && !this.services.getProps().getBoolean(QueryServices.TRANSACTIONS_ENABLED,
        QueryServicesOptions.DEFAULT_TRANSACTIONS_ENABLED)
    ) {
  ...
  /**
   * Get the HAGroup for which this connection is part of, This will return null for Single Cluster
   * Connections
  ...
  @Override
  public void removeFunction(PName tenantId, String functionName, long tableTimeStamp)
    throws SQLException {
  ...
  /**
   * Used to track executions of {@link Statement}s and {@link PreparedStatement}s that were created
   * from this connection before commit or rollback. 0-based. Used to associate partial save errors
   * with SQL statements invoked by users.
   * @see CommitException
  ...
  /**
   * Returns true if this connection is being used to upgrade the data due to PHOENIX-2067 and false
   * otherwise.
   */
