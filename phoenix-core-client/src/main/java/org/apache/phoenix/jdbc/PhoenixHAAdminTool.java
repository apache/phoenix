...
public class PhoenixHAAdminTool extends Configured implements Tool {
  ...
  public int run(String[] args) throws Exception {
    ...
    try {
      ...
      String zkUrl = PhoenixHAAdmin.getLocalZkUrl(getConf()); // Admin is created against local ZK
                                                              // cluster
      if (commandLine.hasOption(LIST_OPT.getOpt())) { // list
        try (PhoenixHAAdmin admin =
          new PhoenixHAAdmin(zkUrl, getConf(), HighAvailibilityCuratorProvider.INSTANCE)) {
