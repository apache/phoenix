...
enum HighAvailabilityPolicy {
  FAILOVER {
    ...
    private void transitStandby(HighAvailabilityGroup haGroup, String zkUrl) throws SQLException {
      // Close connections when a previously ACTIVE HBase cluster becomes STANDBY.
      LOG.info("Cluster {} becomes STANDBY in HA group {}, now close all its connections", zkUrl,
      ...
      for (HAURLInfo haurlInfo : HighAvailabilityGroup.URLS.get(haGroup.getGroupInfo())) {
        try {
          ...
          cqs =
            PhoenixDriver.INSTANCE.getConnectionQueryServices(jdbcZKUrl, haGroup.getProperties());
          ...
          LOG.info("Closed all connections to cluster {} for HA group {}", jdbcZKUrl,
            haGroup.getGroupInfo());
        } finally {
          if (cqs != null) {
    ...
    private void transitActive(HighAvailabilityGroup haGroup, String zkUrl) throws SQLException {
      ...
      for (HAURLInfo haurlInfo : HighAvailabilityGroup.URLS.get(haGroup.getGroupInfo())) {
        ...
        PhoenixDriver.INSTANCE.invalidateCache(jdbcZKUrl, haGroup.getProperties());
      }
    }
  },
