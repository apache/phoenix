...
/**
 * ParallelPhoenixContext holds the state of the execution of a parallel phoenix operation as well
 * as metrics.
 */
public class ParallelPhoenixContext {
  ...
  /**
   * @param executors          Executors to use for operations on connections. We use first executor
   *                           in the list for connection1 and second for connection2
   * @param executorCapacities Ordered list of executorCapacities corresponding to executors. Null
   *                           is interpreted as executors having capacity
   */
  ParallelPhoenixContext(Properties properties, HighAvailabilityGroup haGroup,
    ...
    cluster1Context = new ParallelPhoenixClusterContext(1, executors.get(0).getExecutorService(),
      executors.get(0).getCloseExecutorService());
    cluster2Context = new ParallelPhoenixClusterContext(2, executors.get(1).getExecutorService(),
      executors.get(1).getCloseExecutorService());

    /**
     * Initializes ClusterContext's chainOnConnections according to capacity available in the
     * threadpools. If there is no capacity available we initialize the chain for that connection
     * exceptionally so any further operations on the chain also complete exceptionally
     */
  ...
  private void disableChainOnConn(ParallelPhoenixClusterContext context, String url) {
    ...
    chainOnConn.completeExceptionally(new SQLException(
      "No capacity available for connection " + context.clusterIndex + " for cluster " + url));
    LOG.debug("No capacity available for connection " + context.clusterIndex + " for cluster {}",
      url);
   ...
   * we operate on the underlying phoenix connection (and related objects) using a single thread at
   * any given time. Operations are supposed to be expressed in the form of Supplier. All async
  ...
  private <T> CompletableFuture<T> chainOnConnClusterContext(Supplier<T> s,
    ParallelPhoenixClusterContext context) {
  ...
  public void resetMetrics() {
    // We don't use ParallelPhoenixMetrics::reset() here as that will race with any remaining
    // operations
  ...
  private long getOperationTimeoutMs(Properties properties) {
    ...
    if (properties.getProperty(PHOENIX_HA_PARALLEL_OPERATION_TIMEOUT_ATTRIB) != null) {
      operationTimeoutMs =
        Long.parseLong(properties.getProperty(PHOENIX_HA_PARALLEL_OPERATION_TIMEOUT_ATTRIB));
    } else {
      operationTimeoutMs =
  ...
  private static class ParallelPhoenixClusterContext {
    ...
    public ParallelPhoenixClusterContext(int clusterIndex, ExecutorService executorForCluster,
      ExecutorService connectionCloseExecutor) {
    ...
    public ExecutorService getConnectionCloseExecutor() {
      return connectionCloseExecutor;
    }
