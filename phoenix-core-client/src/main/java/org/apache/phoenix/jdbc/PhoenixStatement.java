...
public class PhoenixStatement implements PhoenixMonitoredStatement, SQLCloseable {
  ...
  private Pair<Integer, Tuple> executeMutation(final CompilableStatement stmt,
    ...
    try {
      return CallRunner.run(new CallRunner.CallableThrowable<Pair<Integer, Tuple>, SQLException>() {
        @Override
          ...
          // Force update cache and retry if meta not found error occurs
            ...
            if (tableName != null) {
              ...
              if (isUpsert || isDelete) {
                ...
                if (isAtomicUpsert) {
                  TableMetricsManager.updateMetricsMethod(tableName, ATOMIC_UPSERT_SQL_COUNTER, 1);
   ...
   * @return The pair of int and Tuple, where int represents value 1 for successful row update and 0
   *         for non-successful row update, and Tuple represents the state of the row.
