        ...
        public PhoenixResultSet call() throws SQLException {
          ...
          try {
            ...
            if (plan.getContext().getScanRanges().isPointLookup()) {
              ...
              connection.getHAGroup() != null && connection.getHAGroup().getRoleRecord() != null
              ...
              // interval is expired
              LOGGER.debug("Refreshing cluster role record for before " + "executing query");
              Optional<String> activeUrl = connection.getHAGroup().getRoleRecord().getActiveUrl();
              if (!connection.getHAGroup().refreshClusterRoleRecord(false)) {
                LOGGER.error("Error while refreshing HAGroup");
              }
              if (!activeUrl.equals(connection.getHAGroup().getRoleRecord().getActiveUrl())) {
                throw new SQLExceptionInfo.Builder(SQLExceptionCode.FAILOVER_IN_PROGRESS)
                  .setMessage(String.format(
                    "Operation failed because failover is in " + "progress for HAGroup %s",
