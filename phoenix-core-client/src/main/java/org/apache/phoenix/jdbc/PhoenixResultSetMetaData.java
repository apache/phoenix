...
/**
 * JDBC ResultSetMetaData implementation of Phoenix. Currently only the following methods are
 * supported: - {@link #getColumnCount()} - {@link #getColumnDisplaySize(int)} -
 * {@link #getColumnLabel(int)} displays alias name if present and column name otherwise -
 * {@link #getColumnName(int)} same as {@link #getColumnLabel(int)} - {@link #isCaseSensitive(int)}
 * - {@link #getColumnType(int)} - {@link #getColumnTypeName(int)} - {@link #getTableName(int)} -
 * {@link #getSchemaName(int)} always returns empty string - {@link #getCatalogName(int)} always
 * returns empty string - {@link #isNullable(int)} - {@link #isSigned(int)} -
 * {@link #isAutoIncrement(int)} always false - {@link #isCurrency(int)} always false -
 * {@link #isDefinitelyWritable(int)} always false - {@link #isReadOnly(int)} always true -
...
public class PhoenixResultSetMetaData implements ResultSetMetaData {
  ...
  public int isNullable(int column) throws SQLException {
    return rowProjector.getColumnProjector(column - 1).getExpression().isNullable()
