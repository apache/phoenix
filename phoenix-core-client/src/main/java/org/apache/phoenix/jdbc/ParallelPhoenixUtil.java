...
public class ParallelPhoenixUtil {
  ...
  public static final String PHOENIX_HA_PARALLEL_OPERATION_TIMEOUT_ATTRIB =
    "phoenix.ha.parallel.operation.timeout.ms";
   ...
   */
  public Object getAnyOfNonExceptionally(List<CompletableFuture<? extends Object>> futures,
    ParallelPhoenixContext context) throws SQLException {
    long timeoutMs = context.getOperationTimeout();
    long endTime =
      (timeoutMs == 0) ? Long.MAX_VALUE : EnvironmentEdgeManager.currentTime() + timeoutMs;
    long internalTimeoutMs = (timeoutMs == 0)
      ? DEFAULT_INTERNAL_OPERATION_TIMEOUT_MS
    ...
    while (!futures.isEmpty() && result == null) {
      ...
      /**
       * CompletableFuture.anyOf adds a completion handler to every future its passed so if we call
       * it too often we can quickly wind up with thousands of completion handlers which take a long
       * time to iterate through and notify. So only create it when the futures it covers have
       * actually changed.
       */
      if (changed) {
      ...
      try {
        ...
        List<CompletableFuture<? extends Object>> filteredResults =
          futures.stream().filter(f -> !f.isCompletedExceptionally()).collect(Collectors.toList());
      ...
      for (CompletableFuture<?> failedFuture : originalFutures) {
        try {
          ...
          if (futuresException == null) {
            futuresException =
              new SQLException("All futures failed. HAGroup:" + context.getHaGroup(), e);
          } else {
        ...
        for (CompletableFuture future : originalFutures) {
          if (future.isCompletedExceptionally()) {
            try {
              ...
              LOGGER.info("For timeout cluster " + i + " completed exceptionally. HAGroup:"
                + context.getHaGroup(), e);
            }
          } else {
            if (future.isDone()) {
              LOGGER.info("For timeout cluster " + i
                + " finished post timeout prior to recording. HAGroup:" + context.getHaGroup());
            } else {
              LOGGER.info(
                "For timeout cluster " + i + " still running. HAGroup:" + context.getHaGroup());
            }
          }
          i++;
        }
  ...
  <T, R> CompletableFuture<T> getFutureAndChainOnContextNoMetrics(Function<R, T> functionToApply,
    CompletableFuture<R> future1, Function<Supplier<T>, CompletableFuture<T>> chainOnConn) {
  ...
  <T, R> CompletableFuture<T> getFutureAndChainOnContext(Function<R, T> functionToApply,
    CompletableFuture<R> future1, Function<Supplier<T>, CompletableFuture<T>> chainOnConn,
    Metric operationCount, Metric failureCount) {
    return getFutureAndChainOnContext(functionToApply, future1, chainOnConn, operationCount,
      failureCount, true);
  }

  private <T, R> CompletableFuture<T> getFutureAndChainOnContext(Function<R, T> functionToApply,
    CompletableFuture<R> future1, Function<Supplier<T>, CompletableFuture<T>> chainOnConn,
    Metric operationCount, Metric failureCount, boolean useMetrics) {
  ...
  <T, R> List<CompletableFuture<T>> applyFunctionToFutures(Function<R, T> function,
    CompletableFuture<R> future1, CompletableFuture<R> future2, ParallelPhoenixContext context,
    boolean useMetrics) {

    CompletableFuture<T> result1 = getFutureAndChainOnContext(function, future1,
      context::chainOnConn1, context.getParallelPhoenixMetrics().getActiveClusterOperationCount(),
      context.getParallelPhoenixMetrics().getActiveClusterFailedOperationCount(), useMetrics);
    CompletableFuture<T> result2 = getFutureAndChainOnContext(function, future2,
   ...
   * @param futures list of futures to run, for 2 clusters 0 will be the active 1 will be the
   *                standby
   ...
   */
  <T> Object runFutures(List<CompletableFuture<T>> futures, ParallelPhoenixContext context,
    boolean useMetrics) throws SQLException {

    List<CompletableFuture<? extends Object>> ranFutures = new ArrayList<>();
    for (int i = 0; i < futures.size(); i++) {
      CompletableFuture<T> future = futures.get(i);
      int finalI = i;
      CompletableFuture<FutureResult<T>> decoratedFuture =
        future.thenApply(t -> new FutureResult<>(t, finalI));
      ranFutures.add(decoratedFuture);
    }
    FutureResult<T> result = (FutureResult<T>) getAnyOfNonExceptionally(ranFutures, context);
    if (useMetrics) {
      context.getParallelPhoenixMetrics()
        .get(PhoenixHAGroupMetrics.HAMetricType.HA_PARALLEL_USED_OPERATIONS, result.index)
        .increment();
    }
    return result.t;
  }
  ...
  <T, R> Object runFutures(Function<R, T> function, CompletableFuture<R> future1,
    ...
    throws SQLException {
    List<CompletableFuture<T>> list =
      applyFunctionToFutures(function, future1, future2, context, useMetrics);
   ...
   */
  <T, R> PairOfSameType<Object> runOnFuturesGetAll(Function<R, T> function,
    CompletableFuture<R> future1, CompletableFuture<R> future2, ParallelPhoenixContext context,
    boolean useMetrics) throws SQLException {
