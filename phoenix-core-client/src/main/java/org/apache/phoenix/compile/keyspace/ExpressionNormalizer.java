 ...
 * <ul>
 * <li><b>RVC inequality</b>: {@code (c1,...,cK) OP (v1,...,vK)} for OP in {@code <, ≤, >, ≥}
 * expands to the lexicographic OR of ANDs. Example: {@code (c1,c2,c3) > (v1,v2,v3)} becomes
 * {@code (c1>v1) OR (c1=v1 AND c2>v2) OR (c1=v1 AND c2=v2 AND c3>v3)}. Equality is already expanded
 * upstream by {@link ComparisonExpression#create}.</li>
 * </ul>
 * This rewrite is load-bearing: by replacing RVC inequality with an equivalent AND/OR tree we
 * guarantee that per-dim intersection composes correctly with any other scalar predicate, matching
 * the design's key-space model.
 * <p>
 * <b>IN lists are not rewritten here.</b> Scalar {@code a IN (v1,...,vK)} and RVC
 * {@code (c1,...,cK) IN ((r11,...,r1K), ...)} are both left intact; the visitor consumes them
 * directly via {@link KeySpaceExpressionVisitor#visitLeave(InListExpression, java.util.List)}. An
 * earlier draft rewrote scalar IN into an OR chain of equalities here, but that reshaped the
 * Expression tree in ways downstream consumers (HavingCompiler, WhereCompiler tree-equality
 * assertions) didn't expect and wrapped literals in spurious TO_VARCHAR coercions. See
 * {@code rewriteScalarInList}'s comment for the history.
 * <p>
 * BETWEEN is lowered at parse time by {@link org.apache.phoenix.compile.StatementNormalizer} and
 * does not reach this pass.
 */
public final class ExpressionNormalizer {
   ...
   * {@code (a,b) OP (v,w)} or a scalar {@link InListExpression}. These are the only two node shapes
   * rewriteNode transforms; anything else is a pass-through.
   */
  private static boolean needsRewrite(Expression e) {
    if (e instanceof ComparisonExpression) {
      ...
      if (isInequality(cmp.getFilterOp())) {
        ...
        if (
          kids.size() >= 2 && kids.get(0) instanceof RowValueConstructorExpression
   ...
   * lexicographic OR-of-ANDs form. Returns {@code null} if the node is not an RVC inequality, so
  ...
  private static Expression rewriteRvcInequality(ComparisonExpression cmp) throws SQLException {
    ...
    CompareOperator strictOp =
      (op == CompareOperator.GREATER || op == CompareOperator.GREATER_OR_EQUAL)
        ? CompareOperator.GREATER
        : CompareOperator.LESS;
    CompareOperator finalOp = strict
      ? strictOp
      : (strictOp == CompareOperator.GREATER
  ...
  /**
   * Previously this rewrote scalar IN lists to OR chains of equalities so that the existing
   * equality/OR visitor paths could handle them. That rewrite changed the Expression tree shape —
   * callers that inspect the WHERE clause (e.g. HavingCompiler "HAVING IN → WHERE IN" lowering,
   * WhereCompiler assertions) saw an OrExpression where they expected an InListExpression, breaking
   * many tests that assert on tree equality. Plus, the rewrite wrapped literals in TO_VARCHAR
   * coercions via ComparisonExpression.create, distorting the tree further.
   * <p>
   * Now the visitor handles scalar {@code InListExpression} directly (see
   * {@link KeySpaceExpressionVisitor#visitLeave(InListExpression, List)}), so there's no need to
   * rewrite here. This method is kept as a documented no-op to make the normalization boundary
   * explicit.
   */
