...
public class OrderPreservingTracker {
   ...
   * of {@link PTable}.
   */
  private static Pair<List<OrderBy>, Integer> getInputOrderBys(QueryPlan innerQueryPlan,
    GroupBy groupBy, StatementContext statementContext) throws SQLException {
    if (!groupBy.isEmpty()) {
      return Pair.newPair(
        Collections.singletonList(ExpressionUtil.convertGroupByToOrderBy(groupBy, false)), 0);
    }
    ...
    return Pair.newPair(orderBy != OrderBy.EMPTY_ORDER_BY
      ? Collections.singletonList(orderBy)
      : Collections.<OrderBy> emptyList(), rowKeyColumnOffset);
  }
  ...
  private class TrackOrderByContext {
    ...
    private void checkAscendingAndNullsLast(Info trackedInfo) {
      ...
      if (isNullsLast != null && expression.isNullable()) {
        if (
          trackedInfo.nullsLast == isNullsLast.booleanValue() && isReverse.booleanValue()
            || trackedInfo.nullsLast != isNullsLast.booleanValue() && !isReverse.booleanValue()
        ) {
          isOrderPreserving = false;
          isReverse = false;
    ...
    /**
     * Only valid AFTER call to isOrderPreserving. This value represents the input column count of
     * {@link TrackOrderByContext#inputOrderBy} corresponding to longest continuous ordering columns
     * returned by {@link TrackOrderByContext#getOrderPreservingTrackInfos}, it may not equal to the
     * size of {@link TrackOrderByContext#getOrderPreservingTrackInfos}.
     */
    ...
    public List<Info> getOrderPreservingTrackInfos() {
      ...
      return ImmutableList
        .copyOf(this.orderPreservingTrackedInfos.subList(0, this.orderedTrackedInfosCount));
    }
  ...
  /**
   * Only valid AFTER call to isOrderPreserving. This value represents the input column count
   * corresponding to longest continuous ordering columns returned by
   * {@link OrderPreservingTracker#getOrderPreservingTrackInfos}, it may not equal to the size of
