 ...
 * if literal found on the LHS, then the operator is reversed and the literal is put on the RHS)
 * @since 0.1
 */
public class StatementNormalizer extends ParseNodeRewriter {
  ...
  /**
   * Rewrite the select statement by switching any constants to the right hand side of the
   * expression.
  ...
  public static SelectStatement normalize(SelectStatement statement, ColumnResolver resolver)
    ...
    if (multiTable) {
      ...
      if (selectNodes != normSelectNodes) {
        statement = NODE_FACTORY.select(statement.getFrom(), statement.getHint(),
          statement.isDistinct(), normSelectNodes, statement.getWhere(), statement.getGroupBy(),
    ...
    return rewrite(statement,
      new StatementNormalizer(resolver, statement.getSelect().size(), multiTable));
  }
  ...
  private static class TableNameVisitor implements TableNodeVisitor<List<TableName>> {
    ...
    public List<TableName> visit(BindTableNode boundTableNode) throws SQLException {
      TableName name = boundTableNode.getAlias() == null
    ...
    public List<TableName> visit(JoinTableNode joinNode) throws SQLException {
      ...
      List<TableName> rhs =
        joinNode.getType() == JoinType.Semi || joinNode.getType() == JoinType.Anti
    ...
    public List<TableName> visit(NamedTableNode namedTableNode) throws SQLException {
      TableName name = namedTableNode.getAlias() == null
  ...
  public ParseNode visitLeave(ComparisonParseNode node, List<ParseNode> nodes) throws SQLException {
    if (
      nodes.get(0).isStateless() && !nodes.get(1).isStateless()
  ...
  @Override
  public ParseNode visitLeave(final BetweenParseNode node, List<ParseNode> nodes)
    throws SQLException {

    LessThanOrEqualParseNode lhsNode =
      NODE_FACTORY.lte(node.getChildren().get(1), node.getChildren().get(0));
    LessThanOrEqualParseNode rhsNode =
      NODE_FACTORY.lte(node.getChildren().get(0), node.getChildren().get(2));
  ...
  public ParseNode visit(ColumnParseNode node) throws SQLException {
    if (
      multiTable && node.getAlias() != null && node.getTableName() != null
  ...
  public ParseNode visit(FamilyWildcardParseNode node) throws SQLException {
    if (!multiTable) return super.visit(node);

    return super.visit(NODE_FACTORY.tableWildcard(NODE_FACTORY.table(null,
      node.isCaseSensitive() ? '"' + node.getName() + '"' : node.getName())));
  }
}
