...
public class RVCOffsetCompiler {
  ...
  public CompiledOffset getRVCOffset(StatementContext context, FilterableStatement statement,
    ...
    if (statement.isAggregate()) {
      throw new RowValueConstructorOffsetNotAllowedInQueryException(
        "RVC Offset not allowed in Aggregates");
    }

    // Get the Select Type should not be join/union
    // Note cannot use the SelectStatement as for Left/Right joins we won't get passed in the join
    // context
    if (inJoin || inUnion) {
      throw new RowValueConstructorOffsetNotAllowedInQueryException(
        "RVC Offset not allowed in Joins or Unions");
    }

    // Get the tables primary keys
    if (context.getResolver().getTables().size() != 1) {
      throw new RowValueConstructorOffsetNotAllowedInQueryException(
        "RVC Offset not allowed with zero or multiple tables");
    }
    ...
    // Make sure the order is the same and all the user defined columns are mentioned in the column
    // RVC
    ...
    List<ColumnParseNode> rvcColumnParseNodeList =
      buildListOfColumnParseNodes(rvcColumnsParseNode, isIndex);
    ...
    // We resolve the mini-where now so we can compare to tables pks PColumns and to produce a row
    // offset
    ...
    WhereCompiler.WhereExpressionCompiler whereCompiler =
      new WhereCompiler.WhereExpressionCompiler(context);
    ...
    if (rowKeyColumnExpressionOutput.isTrailingNull()) {
      ...
      KeyRange keyRange =
        KeyRange.getKeyRange(pointKeyRange.getLowerRange(), false, KeyRange.UNBOUND, true);
  ...
  static class RowKeyColumnExpressionOutput {
    public RowKeyColumnExpressionOutput(List<RowKeyColumnExpression> rowKeyColumnExpressions,
      boolean trailingNull) {
  ...
  @VisibleForTesting
  RowKeyColumnExpressionOutput buildListOfRowKeyColumnExpressions(Expression whereExpression,
    boolean isIndex) throws RowValueConstructorOffsetNotCoercibleException,
    ...
    if ((whereExpression instanceof AndExpression)) {
      ...
      whereExpression instanceof ComparisonExpression || whereExpression instanceof IsNullExpression
    ) {
      expressions = Lists.newArrayList(whereExpression);
    } else {
      LOGGER.warn(
        "Unexpected error while compiling RVC Offset, expected either a Comparison/IsNull Expression of a AndExpression got "
    ...
    for (int i = 0; i < expressions.size(); i++) {
      ...
      // Note that since we store indexes in variable length form there may be casts from fixed
      // types to
      // variable length
      if (isIndex) {
  ...
  List<ColumnParseNode>
    buildListOfColumnParseNodes(RowValueConstructorParseNode rvcColumnsParseNode, boolean isIndex)
