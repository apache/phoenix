...
public abstract class MutatingParallelIteratorFactory implements ParallelIteratorFactory {
  ...
  private static final Logger LOGGER =
    LoggerFactory.getLogger(MutatingParallelIteratorFactory.class);
  ...
  public PeekingResultIterator newIterator(final StatementContext parentContext,
    ResultIterator iterator, Scan scan, String tableName, QueryPlan plan) throws SQLException {
    ...
    try {
      ...
      final boolean autoFlush =
        connection.getAutoCommit() || plan.getTableRef().getTable().isTransactional();
      if (autoFlush) {
      ...
      Cell keyValue = PhoenixKeyValueUtil.newKeyValue(UNGROUPED_AGG_ROW_KEY, SINGLE_COLUMN_FAMILY,
      ...
      return new PeekingResultIterator() {
        ...
        public void close() throws SQLException {
          try {
            /*
             * Join the child mutation states in close, since this is called in a single threaded
             * manner after the parallel results have been processed. If auto-commit is on for the
             * cloned child connection, then the finalState here is an empty mutation state (with no
             * mutations). However, it still has the metrics for mutation work done by the
             * mutating-iterator. Joining the mutation state makes sure those metrics are passed
             * over to the parent connection.
             */
      ...
      try {
        ...
        LOGGER.error("Closing cloned Phoenix connection inside iterator, failed with: ", sqlEx);
      }
      throw ex;
    }
  }
