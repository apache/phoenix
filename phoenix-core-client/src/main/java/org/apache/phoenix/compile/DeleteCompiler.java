...
public class DeleteCompiler {
  ...
  /**
   * Handles client side deletion of rows for a DELETE statement. We determine the "best" plan to
   * drive the query using our standard optimizer. The plan may be based on using an index, in which
   * case we need to translate the index row key to get the data row key used to form the delete
   * mutation. We always collect up the data table mutations, but we only collect and send the index
   * mutations for global, immutable indexes. Local indexes and mutable indexes are always
   ...
   * @param projectedTableRef TableRef containing all indexed and covered columns across all indexes
   *                          on the data table
   * @param otherTableRefs    other TableRefs needed to be maintained apart from the one over which
   *                          the scan is executing. Might be other index tables (if we're driving
   *                          off of the data table table), the data table (if we're driving off of
   *                          an index table), or a mix of the data table and additional index
   *                          tables.
   * @return MutationState representing the uncommitted data across the data table and indexes. Will
   *         be joined with the MutationState on the connection over which the delete is occurring.
   */
  private static MutationState deleteRows(StatementContext context, ResultIterator iterator,
    QueryPlan bestPlan, TableRef projectedTableRef, List<TableRef> otherTableRefs)
    throws SQLException {
    ...
    try (final PhoenixResultSet rs = new PhoenixResultSet(iterator, projector, context)) {
      ...
      if (table.getType() == PTableType.INDEX) {
        if (!otherTableRefs.isEmpty()) {
          ...
          if (!isMaintainedOnClient(table, connection)) {
            ...
            dataTable =
              connection.getTable(SchemaUtil.getTableName(dataTable.getSchemaName().getString(),
        ...
        for (int i = 0; i < otherTableRefs.size(); i++) {
          // Create IndexMaintainer based on projected table (i.e. SELECT expressions) so that
          // client-side
        ...
        for (int i = 0; i < otherTableRefs.size(); i++) {
          ...
          // client-side
          // expressions are used instead of server-side ones.
          maintainers[i] = IndexMaintainer.create(projectedTableRef.getTable(),
            otherTableRefs.get(i).getTable(), connection);
        }
      ...
      int rowCount = 0;
      while (rs.next()) {
        ImmutableBytesPtr rowKeyPtr = new ImmutableBytesPtr(); // allocate new as this is a key in a
                                                               // Map
        rs.getCurrentRow().getKey(rowKeyPtr);
        // When issuing deletes, we do not care about the row time ranges. Also, if the table had a
      ...
      // tableRef can be index if the index table is selected by the query plan or if we do the
      // DELETE
      ...
      if (otherTableRefs.isEmpty()) {
        ...
        // then join the other states. We only need to count the data table rows that will be
        // deleted.
        // MutationState.join() correctly maintains that accounting and ignores the index table
        // rows.
        // This way we always return the correct number of rows that are deleted.
        state.join(tableState);
      }
      for (int i = 0; i < otherTableRefs.size(); i++) {
        MutationState indexState = new MutationState(otherTableRefs.get(i), otherMutations.get(i),
  ...
  private static class DeletingParallelIteratorFactory extends MutatingParallelIteratorFactory {
    ...
    protected MutationState mutate(StatementContext parentContext, ResultIterator iterator,
      ...
      MutationState state =
        deleteRows(context, iterator, queryPlan, projectedTableRef, otherTableRefs);
      return state;
    }
  ...
  private List<PTable> getClientSideMaintainedIndexes(TableRef tableRef) {
    ...
    if (!table.getIndexes().isEmpty()) {
      ...
      for (PTable index : table.getIndexes()) {
        if (!index.getIndexState().isDisabled() && isMaintainedOnClient(index, statement.getConnection())) {
  ...
  /**
   * Implementation of MutationPlan that is selected if 1) the query is strictly point lookup, and
  ...
  public class MultiRowDeleteMutationPlan implements MutationPlan {
    ...
    public Long getEstimatedRowsToScan() throws SQLException {
      ...
      for (MutationPlan plan : plans) {
        /*
         * If any of the plan doesn't have estimate information available, then we cannot provide
         * estimate for the overall plan.
         */
  ...
  public MutationPlan compile(DeleteStatement delete, MutationState.ReturnResult returnResult)
    ...
    // Project all non PK indexed columns so that we can do the proper index maintenance on the
    // indexes for which
    ...
    select = FACTORY.select(delete.getTable(), delete.getHint(), false, aliasedNodes,
      delete.getWhere(), Collections.<ParseNode> emptyList(), null, delete.getOrderBy(),
    ...
    boolean runOnServer = isAutoCommit && !hasPreOrPostProcessing && !table.isTransactional()
      && !hasClientSideIndexes && allowServerMutations;
    HintNode hint = delete.getHint();
    if (runOnServer && !delete.getHint().hasHint(Hint.USE_INDEX_OVER_DATA_TABLE)) {
      select =
        SelectStatement.create(select, HintNode.create(hint, Hint.USE_DATA_OVER_INDEX_TABLE));
    }

    parallelIteratorFactoryToBe =
      hasPreOrPostProcessing ? null : new DeletingParallelIteratorFactory(connection);
    ...
    queryPlans = Lists.newArrayList(!clientSideIndexes.isEmpty()
      ? optimizer.getApplicablePlans(dataPlan, statement, select, resolverToBe,
        Collections.<PColumn> emptyList(), parallelIteratorFactoryToBe)
      : optimizer.getBestPlan(dataPlan, statement, select, resolverToBe,
        Collections.<PColumn> emptyList(), parallelIteratorFactoryToBe));
    ...
    // A simple check of the non existence of a where clause in the parse node is not sufficient, as
    // the where clause
    // may have been optimized out. Instead, we check that there's a single SkipScanFilter
    // If we can generate a plan for every index, that means all the required columns are available
    // in every index,
    ...
    while (noQueryReqd && queryPlanIndex < queryPlans.size()) {
      ...
      noQueryReqd &=
        (!context.getScan().hasFilter() || context.getScan().getFilter() instanceof SkipScanFilter)
    ...
    // If we're doing a query for a set of rows with no where clause, then we don't need to contact
    // the server at all.
    if (noQueryReqd) {
      ...
      if (
        context.getScanRanges().getPointLookupCount() == 1
      ...
      PTable projectedTable = PTableImpl.builderWithColumns(table, adjustedProjectedColumns)
        .setType(PTableType.PROJECTED).build();
      ...
      final List<TableRef> otherTableRefs =
        Lists.newArrayListWithExpectedSize(clientSideIndexes.size());
      for (PTable index : clientSideIndexes) {
        if (!bestPlan.getTableRef().getTable().equals(index)) {
          otherTableRefs.add(new TableRef(index, targetTableRef.getLowerBoundTimeStamp(),
            targetTableRef.getTimeStamp()));
        }
      }
  ...
  /**
   * Implementation of MutationPlan that is selected if 1) there is no immutable index presented for
   * the table, 2) auto commit is enabled as well as server side delete mutations are enabled, 3)
  ...
  public class ServerSelectDeleteMutationPlan implements MutationPlan {
      ...
      try {
        ...
        try {
          byte[] singleRowDelete =
            context.getScan().getAttribute(BaseScannerRegionObserverConstants.SINGLE_ROW_DELETE);
          boolean isSingleRowDelete =
    ...
    /**
     * Initiate server side single row delete operation atomically and return the Result only if the
     * row is deleted.
    ...
    private MutationState deleteRowAndGetMutationState(PTable table) throws SQLException {
      ...
      try (ResultScanner scanner = hTable.getScanner(new Scan(context.getScan()))) {
  ...
  public class ClientSelectDeleteMutationPlan implements MutationPlan {
    ...
    public MutationState execute() throws SQLException {
      ...
      try {
        // If we're not doing any pre or post processing, we can produce the delete mutations
        // directly
        ...
        if (!hasPreOrPostProcessing) {
          ...
          // Return total number of rows that have been deleted from the table. In the case of auto
          // commit being off
          // the mutations will all be in the mutation state of the current connection.
          MutationState state = new MutationState(maxSize, maxSizeBytes, connection, totalRowCount);

          // set the read metrics accumulated in the parent context so that it can be published when
          // the mutations are committed.
          state.setReadMetricQueue(context.getReadMetricsQueue());

          return state;
        } else {
          // Otherwise, we have to execute the query and produce the delete mutations in the single
          // thread
  ...
  private static boolean isMaintainedOnClient(PTable table, PhoenixConnection connection) {
    ...
    if (!table.isTransactional() &&
            ...
            && connection.getQueryServices().getConfiguration().getBoolean(SERVER_SIDE_IMMUTABLE_INDEXES_ENABLED_ATTRIB,
