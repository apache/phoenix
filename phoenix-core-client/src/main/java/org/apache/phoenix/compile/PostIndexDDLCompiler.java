    ...
    for (PColumnFamily family : indexTable.getColumnFamilies()) {
      for (PColumn col : family.getColumns()) {
        if (col.getViewConstant() == null) {
          String indexColName = col.getName().getString();
          // Transforming tables also behave like indexes but they don't have index_col_name_sep. So
          // we use family name directly.
          String dataFamilyName = indexColName.indexOf(INDEX_COLUMN_NAME_SEP) != -1
