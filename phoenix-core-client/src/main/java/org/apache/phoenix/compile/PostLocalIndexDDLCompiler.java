...
/**
 * For local indexes, we optimize the initial index population by *not* sending Puts over the wire
 * for the index rows, as we don't need to do that. Instead, we tap into our region observer to
 * generate the index rows based on the data rows as we scan
 */
  ...
  public MutationPlan compile(PTable index) throws SQLException {
    try (final PhoenixStatement statement = new PhoenixStatement(connection)) {
      ...
      for (ColumnReference columnRef : indexMaintainer.getAllColumns()) {
        if (
          index.getImmutableStorageScheme() == ImmutableStorageScheme.SINGLE_CELL_ARRAY_WITH_OFFSETS
        ) {
