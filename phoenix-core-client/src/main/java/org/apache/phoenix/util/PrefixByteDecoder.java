...
public class PrefixByteDecoder {
  ...
  /**
   * Used when the maximum length of encoded byte array is not known. Will cause a new byte array to
   * be allocated for each call to {@link #decode(DataInput)}.
   */
  ...
  /**
   * Used when the maximum length of encoded byte array is known in advance. Will not allocate new
   * byte array with each call to {@link #decode(DataInput)}.
   ...
   * @return Pointer containing bytes that were decoded. Note that the same pointer will be returned
   *         with each call, so it must be consumed prior to calling decode again.
   */
