 ...
 * {@link org.apache.hadoop.hbase.client.Result}. These methods work off of the raw bytes preventing
 * the explosion of Result into object form.
 * @since 0.1
 */
public class ResultUtil {
  ...
  /**
   * Return a pointer into a potentially much bigger byte buffer that points to the key of a Result.
   * Use offset to return a subset of the key bytes, for example to skip the organization ID
   * embedded in all of our keys.
   * @param offset offset added to start of key and subtracted from key length (to select subset of
   *               key bytes)
   */
  public static ImmutableBytesWritable getKey(Result r, int offset) {
    return new ImmutableBytesWritable(getRawBytes(r), getKeyOffset(r) + offset,
      getKeyLength(r) - offset);
  }
  ...
  public static final Comparator<Result> KEY_COMPARATOR = new Comparator<Result>() {
    ...
    public int compare(Result r1, Result r2) {
      ...
      return Bytes.compareTo(r1Bytes, getKeyOffset(r1), getKeyLength(r1), r2Bytes, getKeyOffset(r2),
        getKeyLength(r2));
    }
  ...
  public static int compareKeys(Result r1, Result r2) {
    return Bytes.compareTo(getRawBytes(r1), getKeyOffset(r1), getKeyLength(r1), getRawBytes(r2),
      getKeyOffset(r2), getKeyLength(r2));
  }
