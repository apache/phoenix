...
public class PrefixByteEncoder {
  ...
  /**
   * Resets the state of the encoder to its initial state (i.e. forgetting the previous byte array
   * that may have been encoded).
   */
  public void reset() {
    previous.set(ByteUtil.EMPTY_BYTE_ARRAY);
  }

  /** Returns the maximum length byte array encountered while encoding */
  ...
  /**
   * Prefix encodes the byte array from offset to length into output stream. Instead of writing the
   * entire byte array, only the portion of the byte array that differs from the beginning of the
   * previous byte array written is written.
   * @param out    output stream to encode into
