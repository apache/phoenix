...
public class PrefixByteCodec {
  ...
  public static List<byte[]> decodeBytes(ImmutableBytesWritable encodedBytes, int maxLength)
    throws IOException {
    ByteArrayInputStream stream = new ByteArrayInputStream(encodedBytes.get(),
      encodedBytes.getOffset(), encodedBytes.getLength());
  ...
  public static int encodeBytes(List<byte[]> listOfBytes, ImmutableBytesWritable ptr)
    throws IOException {
    try (TrustedByteArrayOutputStream stream =
      new TrustedByteArrayOutputStream(calculateSize(listOfBytes))) {
  ...
  public static ImmutableBytesWritable decode(PrefixByteDecoder decoder, DataInput input)
    throws EOFException {
    try {
