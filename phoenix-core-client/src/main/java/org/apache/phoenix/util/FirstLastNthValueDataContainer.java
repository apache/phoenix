...
public class FirstLastNthValueDataContainer {
  ...
  public byte[] getPayload() throws IOException {
    /*
     * PAYLOAD STUCTURE what | size (bytes) | info is ascending | 1 | 1 = asc, 0 = desc length of
     * order by vals | 4 | 0 if dynamic length, size otherwise length of values | 4 | 0 if dynamic
     * length, size otherwise [ length of first order | 4 | set if order is var length (optional) ]
     * first order value | n | order by val [ length of first value | 4 | set if value is var length
     * (optional) ] first value | n | data val ... and so on, repeat order by values and data values
     * example with fixed length for data and order by values 0 | 0000 0004 | 0000 0004 | 0000 0001
     * | 0000 000FF | ... is ascendig | length order vals | length data vals | first order val |
     * first value | ... more values example with dynamic length for data (length will be zeros) 0 |
     * 0000 0000 | 0000 0000 | 0000 0004 | 0000 000FF | ... is ascendig | length order vals | length
     * data vals | first order length | first order value | ... more values
     */
