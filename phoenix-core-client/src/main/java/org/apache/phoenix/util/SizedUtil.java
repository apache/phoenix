...
/**
 * Utilities for computing an object's size. All size measurements are in bytes. Note, all of the
 * sizes here, but especially OBJECT_SIZE and ARRAY_SIZE are estimates and will depend on the JVM
 * itself (and which JVM, 64bit vs. 32bit, etc). The current values are based on: Java HotSpot(TM)
 * 64-Bit Server VM/14.2-b01 (Uncomment out and run the main w/ appropriate object to test) Also,
 * see this link:
...
public class SizedUtil {
  ...
  public static final int IMMUTABLE_BYTES_PTR_SIZE = IMMUTABLE_BYTES_WRITABLE_SIZE + INT_SIZE;// Extra
                                                                                              // is
                                                                                              // an
                                                                                              // int
                                                                                              // field
  ...
  public static final int RESULT_SIZE =
    OBJECT_SIZE + 3 * POINTER_SIZE + IMMUTABLE_BYTES_WRITABLE_SIZE;
  ...
  public static final int BIG_DECIMAL_SIZE =
    OBJECT_SIZE + 2 * INT_SIZE + LONG_SIZE + 2 * POINTER_SIZE + OBJECT_SIZE /* BigInteger */
      + 5 * INT_SIZE + ARRAY_SIZE /* mag[] */ + 2 * INT_SIZE /* est mag[2] */;
  ...
  public static int sizeOfArrayList(int capacity) {
    return SizedUtil.OBJECT_SIZE + SizedUtil.POINTER_SIZE + SizedUtil.INT_SIZE
      + SizedUtil.ARRAY_SIZE + SizedUtil.POINTER_SIZE * capacity;
  }
  ...
  public static long sizeOfMap(int nRows, int keySize, int valueSize) {
    return SizedUtil.OBJECT_SIZE * 4 + sizeOfArrayList(nRows) /* key set */
      + nRows * (SizedUtil.MAP_ENTRY_SIZE * 1L + /* entry set */
        keySize + // key size
