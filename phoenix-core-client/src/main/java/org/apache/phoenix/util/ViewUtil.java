    ...
    for (TableInfo viewInfo : currentResult.getLinks()) {
      ...
      try (PhoenixConnection connection =
        ...
        if (isLegitimateChildView(view, parentSchemaName, parentTableOrViewName)) {
          ...
          findAllDescendantViews(sysCatOrsysChildLink, serverSideConfig, viewInfo.getTenantId(),
            viewInfo.getSchemaName(), viewInfo.getTableName(), clientTimeStamp,
      ...
      if (containsAllReqdCols) {
        ...
        if (Objects.equal(viewStatement, index.getViewStatement())) {
          ...
          inheritedIndexes.add(PTableImpl.builderWithColumns(index, getColumnsToClone(index))
            ...
            .setUpdateCacheFrequency(view.getUpdateCacheFrequency()).setTenantId(view.getTenantId())
  ...
  public static void addTagsToPutsForViewAlteredProperties(List<Mutation> tableMetaData,
    ...
    for (Mutation m : tableMetaData) {
      if (m instanceof Put) {
        ...
        MetaDataUtil.conditionallyAddTagsToPutCells((Put) m,
          PhoenixDatabaseMetaData.TABLE_FAMILY_BYTES, PhoenixDatabaseMetaData.PHOENIX_TTL_BYTES,
          extendedCellBuilder, parentPhoenixTTLBytes,
          MetaDataEndpointImplConstants.VIEW_MODIFIED_PROPERTY_BYTES);
      }
