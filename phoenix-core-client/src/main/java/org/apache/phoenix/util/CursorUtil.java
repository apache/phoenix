...
public final class CursorUtil {
  ...
  private static class CursorWrapper {
    ...
    private void closeCursor() throws SQLException {
      ...
      // Semantically it makes sense that something which is 'Closed' one should be able to 'Open'
      // again.
   ...
   * @return Returns true if the new cursor was successfully declared. False if a cursor with the
   *         same identifier already exists.
   */
  public static boolean declareCursor(DeclareCursorStatement stmt, QueryPlan queryPlan)
    throws SQLException {
    if (mapCursorIDQuery.containsKey(stmt.getCursorName())) {
      throw new SQLException(
        "Can't declare cursor " + stmt.getCursorName() + ", cursor identifier already in use.");
    } else {
      mapCursorIDQuery.put(stmt.getCursorName(),
        new CursorWrapper(stmt.getCursorName(), stmt.getQuerySQL(), queryPlan));
      return true;
    }
  }
  ...
  public static QueryPlan getFetchPlan(String cursorName, boolean isNext, int fetchSize)
    throws SQLException {
  ...
  public static void updateCursor(String cursorName, Tuple rowValues, Tuple nextRowValues)
    throws SQLException {
