...
/**
 * Utility functions shared between IndexRegionObserver and MutationState for annotating the HBase
 * WAL with Phoenix-level metadata about mutations.
 */
public class WALAnnotationUtil {

  /**
   * Add metadata about a mutation onto the attributes of the mutation. This will be written as an
   * annotation into the HBase write ahead log (WAL) when the mutation is processed server-side,
   * usually in IndexRegionObserver
   * @param m                Mutation
  ...
  public static void annotateMutation(Mutation m, byte[] tenantId, byte[] schemaName,
    byte[] logicalTableName, byte[] tableType, byte[] ddlTimestamp, byte[] haGroupName) {
    if (!m.getDurability().equals(Durability.SKIP_WAL)) {
      if (tenantId != null) {
      ...
      if (haGroupName != null) {
        m.setAttribute(BaseScannerRegionObserverConstants.HA_GROUP_NAME_ATTRIB, haGroupName);
      }
    }
  }
