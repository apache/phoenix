...
public class Closeables {
  ...
  /**
   * Close a {@code Closeable}, returning an {@code IOException} if it occurs while closing instead
   * of throwing it. This is nearly a clone of the Guava Closeables.closeQuietly method which has
   * long since been removed from Guava. Use of this method should be avoided -- quietly swallowing
   * IOExceptions (particularly on Closeables that are being written to) is a code smell. Use of the
   * equivalent method in Guava was done for this reason.
  ...
  /**
   * Allows you to close as many of the {@link Closeable}s as possible. If any of the close's fail
   * with an IOException, those exception(s) will be thrown after attempting to close all of the
   * inputs.
   */
  ...
  static private class MultipleCausesIOException extends IOException {
    ...
    public StackTraceElement[] getStackTrace() {
      if (!this.hasSetStackTrace) {
        ArrayList<StackTraceElement> frames =
          new ArrayList<StackTraceElement>(this.exceptions.size() * 20);

        int exceptionNum = 0;
        for (IOException exception : this.exceptions) {
          StackTraceElement header =
            new StackTraceElement(MultipleCausesIOException.class.getName(),
              "Exception Number " + exceptionNum, "<no file>", 0);
