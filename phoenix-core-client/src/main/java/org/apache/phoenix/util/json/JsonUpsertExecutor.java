...
public class JsonUpsertExecutor extends UpsertExecutor<Map<?, ?>, Object> {
  ...
  protected void execute(Map<?, ?> record) {
    ...
    try {
      if (record.size() < conversionFunctions.size()) {
        String message =
          String.format("JSON record does not have enough values (has %d, but needs %d)",
  ...
  protected Function<Object, Object> createConversionFunction(PDataType dataType) {
    if (dataType.isArrayType()) {
      return new ArrayDatatypeConversionFunction(new ObjectToArrayConverter(conn,
    ...
    SimpleDatatypeConversionFunction(PDataType dataType, Connection conn) {
      ...
      if (dataType.isCoercibleTo(PTimestamp.INSTANCE)) {
        ...
        if (dateSqlType == Types.DATE) {
          dateFormat =
            props.getProperty(QueryServices.DATE_FORMAT_ATTRIB, DateUtil.DEFAULT_DATE_FORMAT);
        } else if (dateSqlType == Types.TIME) {
          dateFormat =
            props.getProperty(QueryServices.TIME_FORMAT_ATTRIB, DateUtil.DEFAULT_TIME_FORMAT);
        } else {
