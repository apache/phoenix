...
public class CompatBaseScannerRegionObserver implements RegionObserver {
  ...
  public void preMemStoreCompactionCompactScannerOpen(
    ...
    if (isMaxLookbackTimeEnabled(conf)) {
      ...
      if (
        inMemPolicy.equals(MemoryCompactionPolicy.EAGER)
  ...
  private boolean storeFileScanDoesntNeedAlteration(ScanOptions options) {
    ...
    boolean timestampIsTransactional = isTransactionalTimestamp(scan.getTimeRange().getMax());
  ...
  /*
   * If KeepDeletedCells.FALSE, KeepDeletedCells.TTL , let delete markers age once lookback age is
   * done.
   */
  ...
  public int getMinVersions(ScanOptions options, ColumnFamilyDescriptor cfDescriptor) {
    return cfDescriptor.getTimeToLive() != HConstants.FOREVER
      ...
      : Math.max(Math.max(options.getMinVersions(), cfDescriptor.getMaxVersions()), 1);
  }
  ...
  public long getTimeToLiveForCompactions(Configuration conf,
    ColumnFamilyDescriptor columnDescriptor, ScanOptions options) {
    ...
    if (isMaxLookbackTimeEnabled(maxLookbackTtl)) {
      if (
        ttlConfigured == HConstants.FOREVER
  ...
  public void setScanOptionsForFlushesAndCompactions(Configuration conf, ScanOptions options,
    final Store store, ScanType type) {
    ColumnFamilyDescriptor cfDescriptor = store.getColumnFamilyDescriptor();
    options.setTTL(getTimeToLiveForCompactions(conf, cfDescriptor, options));
  ...
  public static long getMaxLookbackInMillis(Configuration conf) {
    ...
    return conf.getLong(PHOENIX_MAX_LOOKBACK_AGE_CONF_KEY, DEFAULT_PHOENIX_MAX_LOOKBACK_AGE) * 1000;
  }

  public static boolean isMaxLookbackTimeEnabled(Configuration conf) {
    return isMaxLookbackTimeEnabled(
      conf.getLong(PHOENIX_MAX_LOOKBACK_AGE_CONF_KEY, DEFAULT_PHOENIX_MAX_LOOKBACK_AGE));
  }
