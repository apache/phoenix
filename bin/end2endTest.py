...
if os.getenv('HBASE_HOME'):
    ...
    hbase_classpath = subprocess.check_output([os.path.join(hbase_bin_dir, "hbase"), "--internal-classpath", "classpath"]).decode().strip()
else:
...
# same files. If HBASE_HOME is set, then the jars from it come before the cached jars, so those should
# be used. Otherwise we use the HBase jars from the cached classpath.
# If HBASE_HOME is specified, then the cached classpath should only be used for Junit.

print("Current ClassPath=%s:%s:%s" % (hbase_classpath, phoenix_utils.phoenix_test_jar_path, cached_classpath))

java_cmd = "java -cp " + hbase_classpath + os.pathsep + phoenix_utils.phoenix_test_jar_path + os.pathsep + cached_classpath \
