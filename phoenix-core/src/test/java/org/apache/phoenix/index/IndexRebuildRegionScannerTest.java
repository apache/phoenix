...
public class IndexRebuildRegionScannerTest {
  ...
  public void testGetPerTaskIndexKeyToMutationMaps() {
    ...
    for (int i = 1; i <= REPEAT; i++) {
      ...
      for (int j = 0; j < i - 1; j++) {
        endKeys[j] = Bytes.toBytes(RAND.nextLong() % (MAP_SIZE * 5)); // 5 vs 11 is to create some
                                                                      // imbalance
      }
      // The last end key is always null
      endKeys[i - 1] = null;
      List<Map<byte[], List<Mutation>>> mapList =
        getPerTaskIndexMutationMaps(indexKeyToMutationMap, endKeys, MAX_SPLIT_SIZE);
      int regionIndex = 0;
      int regionCount = i;
      for (Map<byte[], List<Mutation>> map : mapList) {
        ...
        while (
          regionIndex < regionCount - 1
            && Bytes.BYTES_COMPARATOR.compare(firstKey, endKeys[regionIndex]) > 0
        ) {
          regionIndex++;
        }
        ...
        if (regionIndex != regionCount - 1) {
          Assert.assertTrue(
            Bytes.BYTES_COMPARATOR.compare(treeMap.lastKey(), endKeys[regionIndex]) <= 0);
        }
      }
      // The number of splits must be more than or equal to (map size / MAX_SPLIT_SIZE) and must be
      // less than or
