...
public class PhoenixConfigurationUtilTest extends BaseConnectionlessQueryTest {
  ...
  public void testSyncTableConfiguration() {
    ...
    PhoenixConfigurationUtil.setPhoenixSyncTableName(conf, testTableName);
    assertEquals("Should retrieve set table name", testTableName,
      PhoenixConfigurationUtil.getPhoenixSyncTableName(conf));

    // Test target ZK quorum
    String testZkQuorum = "testhost1:2181,testhost2:2181:/hbase";
    PhoenixConfigurationUtil.setPhoenixSyncTableTargetZkQuorum(conf, testZkQuorum);
    ...
    PhoenixConfigurationUtil.setPhoenixSyncTableFromTime(conf, testFromTime);
    assertEquals("Should retrieve set from time", testFromTime,
      PhoenixConfigurationUtil.getPhoenixSyncTableFromTime(conf));

    // Test to time
    Long testToTime = 2000000L;
    PhoenixConfigurationUtil.setPhoenixSyncTableToTime(conf, testToTime);
    assertEquals("Should retrieve set to time", testToTime,
      PhoenixConfigurationUtil.getPhoenixSyncTableToTime(conf));

    // Test dry run flag
    PhoenixConfigurationUtil.setPhoenixSyncTableDryRun(conf, true);
    assertEquals("Should retrieve dry run as true", true,
      PhoenixConfigurationUtil.getPhoenixSyncTableDryRun(conf));

    PhoenixConfigurationUtil.setPhoenixSyncTableDryRun(conf, false);
    assertEquals("Should retrieve dry run as false", false,
      PhoenixConfigurationUtil.getPhoenixSyncTableDryRun(conf));

    // Test chunk size bytes
    Long testChunkSize = 5000000L; // 5MB
    PhoenixConfigurationUtil.setPhoenixSyncTableChunkSizeBytes(conf, testChunkSize);
    ...
    assertEquals("Should return default chunk size when not set", defaultChunkSize,
      PhoenixConfigurationUtil.getPhoenixSyncTableChunkSizeBytes(freshConf));
  }
