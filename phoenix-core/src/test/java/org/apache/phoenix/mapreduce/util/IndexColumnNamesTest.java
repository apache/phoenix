...
public class IndexColumnNamesTest extends BaseIndexTest {
  ...
  public void testGetColumnNames() {
    ...
    assertEquals("[:ID, :PK_PART2, 0:NAME, 0:ZIP]", indexColumnNames.getIndexColNames().toString()); // index
                                                                                                     // column
                                                                                                     // names,
                                                                                                     // leading
                                                                                                     // with
    ...
    assertEquals("[\"ID\" INTEGER, \"PK_PART2\" TINYINT, \"NAME\" VARCHAR, \"ZIP\" BIGINT]",
      indexColumnNames.getDynamicDataCols().toString());
    assertEquals("[\":ID\" INTEGER, \":PK_PART2\" TINYINT, \"0:NAME\" VARCHAR, \"0:ZIP\" BIGINT]",
      indexColumnNames.getDynamicIndexCols().toString());
    assertEquals(
      "UPSERT /*+ NO_INDEX */  INTO TEST_SCHEMA.TEST_INDEX_COLUMN_NAMES_UTIL (\"ID\" INTEGER, \"PK_PART2\" TINYINT, \"NAME\" VARCHAR, \"ZIP\" BIGINT) VALUES (?, ?, ?, ?)",
      QueryUtil.constructUpsertStatement(DATA_TABLE_FULL_NAME,
        indexColumnNames.getDynamicDataCols(), Hint.NO_INDEX));
  }
  ...
  public void testGetDynamicColPrecision() throws SQLException {
    ...
    IndexColumnNames indexColumnNames = new IndexColumnNames(pDataTable, pIndexTable);
    assertEquals(
      "[\"CHAR_TEST\" CHAR(15), \"VARCHAR_TEST\" VARCHAR(1), \"DECIMAL_TEST\" DECIMAL(10,2), \"BINARY_TEST\" BINARY(11), \"VARCHAR_UNSPEC\" VARCHAR, \"DEC_UNSPEC\" DECIMAL]",
      indexColumnNames.getDynamicDataCols().toString());
    assertEquals(
      "[\":CHAR_TEST\" CHAR(15), \"0:VARCHAR_TEST\" VARCHAR(1), \"0:DECIMAL_TEST\" DECIMAL(10,2), \"0:BINARY_TEST\" BINARY(11), \"0:VARCHAR_UNSPEC\" VARCHAR, \"0:DEC_UNSPEC\" DECIMAL]",
      indexColumnNames.getDynamicIndexCols().toString());
  }
}
