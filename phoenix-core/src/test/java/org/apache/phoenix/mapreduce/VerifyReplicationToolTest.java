...
public class VerifyReplicationToolTest {
    ...
    @Test
    public void testParseScnOptions() throws Exception {
        String[] opts =
                new String[] { "-t", SOURCE_TABLE, "-tt", TARGET_TABLE, "-ts", Long.toString(TIMESTAMP)};
        assertTrue(tool.parseCommandLine(opts));
    ...
    public void testBuilderWithTargetCluster() {
        builder.tableName(SOURCE_TABLE).zkQuorum(SOURCE_ZK).targetZkQuorum(TARGET_ZK)
                .sqlConditions(CONDITIONS).timestamp(TIMESTAMP);
        tool = builder.build();
