  ...
  public void testDistinctAggregatingResultIterator() throws Throwable {
    ...
    Tuple[] input1 = new Tuple[] {
      new MultiKeyValueTuple(
        Arrays.<Cell> asList(new KeyValue(rowKey1, cf, cq1, PInteger.INSTANCE.toBytes(1)),
          new KeyValue(rowKey1, cf, cq2, PInteger.INSTANCE.toBytes(2)))),

      new MultiKeyValueTuple(
        Arrays.<Cell> asList(new KeyValue(rowKey2, cf, cq1, PInteger.INSTANCE.toBytes(11)),
          new KeyValue(rowKey2, cf, cq2, PInteger.INSTANCE.toBytes(12)))),

      new MultiKeyValueTuple(
        Arrays.<Cell> asList(new KeyValue(rowKey3, cf, cq1, PInteger.INSTANCE.toBytes(4)),
      ...
      new MultiKeyValueTuple(
        Arrays.<Cell> asList(new KeyValue(rowKey4, cf, cq1, PInteger.INSTANCE.toBytes(7)),
      ...
      new MultiKeyValueTuple(
        Arrays.<Cell> asList(new KeyValue(rowKey5, cf, cq1, PInteger.INSTANCE.toBytes(90)),
    ...
    Mockito.when(mockRowProjector.getColumnProjectors())
      .thenAnswer(new Answer<List<ColumnProjector>>() {
        @Override
    ...
    assertResults(input1, result1, mockRowProjector);
    ...
    Tuple[] input2 = new Tuple[] {
      ...
      new MultiKeyValueTuple(
        Arrays.<Cell> asList(new KeyValue(rowKey1, cf, cq2, PInteger.INSTANCE.toBytes(1)),
    ...
    assertResults(input2, result2, mockRowProjector);
    ...
    for (int i = 0; i <= n; i++) {
      ...
      input3[i] = new MultiKeyValueTuple(
        Arrays.<Cell> asList(new KeyValue(rowKey, cf, cq1, PInteger.INSTANCE.toBytes(i + 1)),
          new KeyValue(rowKey, cf, cq2, PInteger.INSTANCE.toBytes(i + 2))));
    }
    input3[n] = null;
    Tuple[] result3 = Arrays.copyOfRange(input3, 0, n);
    assertResults(input3, result3, mockRowProjector);
    ...
    for (int i = 0; i <= n; i++) {
      ...
      input4[i] = new MultiKeyValueTuple(
        Arrays.<Cell> asList(new KeyValue(rowKey, cf, cq1, PInteger.INSTANCE.toBytes(2)),
          new KeyValue(rowKey, cf, cq2, PInteger.INSTANCE.toBytes(3))));
    }
    input4[n] = null;
    Tuple[] result4 = new Tuple[] { input4[0] };
    assertResults(input4, result4, mockRowProjector);

  }

  private void assertResults(Tuple[] input, Tuple[] result, RowProjector rowProjector)
    throws Exception {
    AggregatingResultIterator mockAggregatingResultIterator =
      Mockito.mock(AggregatingResultIterator.class);
    Mockito.when(mockAggregatingResultIterator.next()).thenReturn(input[0],
    ...
    AssertResults.assertResults(distinctAggregatingResultIterator, result);
  }
