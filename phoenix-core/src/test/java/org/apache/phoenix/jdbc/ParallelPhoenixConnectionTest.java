...
public class ParallelPhoenixConnectionTest {
  ...
  public void init() throws SQLException {
    ...
    parallelPhoenixConnection =
      new ParallelPhoenixConnection(context, CompletableFuture.completedFuture(connection1),
  ...
  public void unwrapPhoenixMonitoredConnectionTest() throws SQLException {
    PhoenixMonitoredConnection result =
      parallelPhoenixConnection.unwrap(PhoenixMonitoredConnection.class);
  ...
  public void testOpenConnection1Error() throws SQLException {
    ...
    // was initialized successfully - the ParallelPhoenixConnection object should be returned. Also,
    // the
    // close should be successful, as one of the connection closed successfully.
    parallelPhoenixConnection =
      new ParallelPhoenixConnection(context, futureConnection1, futureConnection2);
  ...
  public void testOpenConnection2Delay() throws Exception {
    ...
    CompletableFuture<PhoenixConnection> futureConnection1 =
      CompletableFuture.completedFuture(connection1);
    CompletableFuture<PhoenixConnection> futureConnection2 =
      CompletableFuture.supplyAsync(getDelayConnectionSupplier(cdl, connection2));
  ...
  public void testOpenBothConnectionDelay() throws SQLException {
    ...
    CompletableFuture<PhoenixConnection> futureConnection2 =
      CompletableFuture.supplyAsync(getDelayConnectionSupplier(cdl2, connection2));

    long prevTimeoutCounter =
      GlobalClientMetrics.GLOBAL_HA_PARALLEL_TASK_TIMEOUT_COUNTER.getMetric().getValue();
    ...
    try {
      ...
      assertTrue(GlobalClientMetrics.GLOBAL_HA_PARALLEL_TASK_TIMEOUT_COUNTER.getMetric().getValue()
          > prevTimeoutCounter);
    }
  }
  ...
  public void testConnection1CloseDelay() throws Exception {
    ...
    assertTrue("Close should be called on at least one of the connections",
      countConnection1 > 0 || countConnection2 > 0);
  }
  ...
  private Supplier<PhoenixConnection> getDelayConnectionSupplier(CountDownLatch cdl,
    PhoenixConnection returnConnection) {
    return (() -> {
      try {
        cdl.await();
      } catch (InterruptedException e) {
