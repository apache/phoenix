...
/**
 * Test to make sure once an error is encountered on an underlying phoenix connection we don't use
 * that connection during the entire lifecycle of client conenction
 */
public class ParallelPhoenixConnectionFailureTest extends BaseTest {
  ...
  public void testExecuteQueryChainFailure() throws SQLException {
    ...
    ParallelPhoenixContext context =
      new ParallelPhoenixContext(new Properties(), Mockito.mock(HighAvailabilityGroup.class),
        HighAvailabilityTestingUtility.getListOfSingleThreadExecutorServices(), null);
    ParallelPhoenixConnection parallelConn = new ParallelPhoenixConnection(context,
      CompletableFuture.completedFuture(connSpy1), CompletableFuture.completedFuture(connSpy2));
    ...
    hbaseTestingUtility.waitFor(WAIT_MS,
      () -> (numStatementsCreatedOnConn1.get() == 2) && (numStatementsCreatedOnConn2.get() == 2));
    ...
    parallelConn.createStatement().execute("SELECT * FROM SYSTEM.CATALOG");
    hbaseTestingUtility.waitFor(WAIT_MS,
      () -> (numStatementsCreatedOnConn1.get() == 2) && (numStatementsCreatedOnConn2.get() == 4));
