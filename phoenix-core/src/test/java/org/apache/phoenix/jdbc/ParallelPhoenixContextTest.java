...
public class ParallelPhoenixContextTest {
  ...
  public void init() {
    executorList = Lists.newArrayList(
      new PhoenixHAExecutorServiceProvider.PhoenixHAClusterExecutorServices(
        new TrackingThreadPoolExecutor(), new TrackingThreadPoolExecutor()),
      new PhoenixHAExecutorServiceProvider.PhoenixHAClusterExecutorServices(
        new TrackingThreadPoolExecutor(), new TrackingThreadPoolExecutor()));
  }
    ...
    try {
      ParallelPhoenixContext context =
        new ParallelPhoenixContext(new Properties(), Mockito.mock(HighAvailabilityGroup.class),
          Lists.newArrayList(
            Mockito.mock(PhoenixHAExecutorServiceProvider.PhoenixHAClusterExecutorServices.class)),
  ...
  public void testPool1OutOfCapacity() throws Exception {
    ...
    assertEquals(0,
      ((TrackingThreadPoolExecutor) executorList.get(0).getExecutorService()).tasksExecuted.get());
    assertEquals(0,
      ((TrackingThreadPoolExecutor) executorList.get(1).getExecutorService()).tasksExecuted.get());
  ...
  public void testPoolsHaveCapacity() throws Exception {
    ParallelPhoenixContext context = new ParallelPhoenixContext(new Properties(),
      ...
      Lists.newArrayList(Boolean.TRUE, Boolean.TRUE), Mockito.mock(HAURLInfo.class));
    CompletableFuture<Boolean> future1 = context.chainOnConn1(() -> true);
    assertTrue(future1.get());
    assertEquals(1,
      ((TrackingThreadPoolExecutor) executorList.get(0).getExecutorService()).tasksExecuted.get());
