...
public class PhoenixStatementTest extends BaseConnectionlessQueryTest {
  ...
  @Test
  /**
   * Validates that if a user sets the query timeout via the stmt.setQueryTimeout() JDBC method, we
   * correctly store the timeout in both milliseconds and seconds.
   */
  ...
  @Test
  /**
   * Validates if a user sets the timeout to zero that we store the timeout in millis as the
   * Integer.MAX_VALUE.
   */
  ...
  public void testSettingNegativeQueryTimeoutViaJdbc() throws Exception {
    ...
    int defaultQueryTimeout = phoenixConnection.getQueryServices().getProps().getInt(
  ...
  @Test
  /**
   * Validates that setting custom phoenix query timeout using the phoenix.query.timeoutMs config
   * property is honored.
   */
