 ...
 * Failures:
 * <ul>
 * <li>A V2 soundness violation (row matching the expression NOT in V2's emission) fails the test
 * immediately — that's a correctness bug.</li>
 * <li>Widening (V2 contains rows outside the oracle's view) is logged for visibility but does not
 * fail the test — it's a performance concern, not correctness.</li>
 * <li>Skipped queries (unsupported shape in adapter or decoder) are logged; many skips indicate
 * harness coverage is insufficient.</li>
 * </ul>
 */
public class HarnessCorpusTest extends BaseConnectionlessQueryTest {

  /**
   * The corpus harness decodes V2's compound-byte ScanRanges shape to compare against the abstract
   * algebra reference. V1's per-slot byte form isn't compatible with the decoder; skip the suite
   * when V2 is disabled.
   */
  @Before
  ...
  private static final class Case {
    ...
    final String query;
    /**
     * When non-null, this query is <em>expected</em> to show scan-range widening vs the oracle and
     * the reason is documented here. Used to distinguish unfixable scan-primitive limitations (e.g.
     * SkipScanFilter can't express cross-dim OR) from real regressions. Any widening not annotated
     * here fails the test.
     */
  ...
  public void runCorpus() {
    ...
    for (Case c : cases) {
      ...
      if (actuallyWider && expectedWider) {
        ...
        unexpectedTightening.add(
          c.query + "\n      documented reason no longer applies: " + c.expectedWideningReason);
      }
    }
    System.err.println("============================================");
    System.err.println("Corpus: " + cases.size() + " total, " + sound + " sound, " + violations
      + " violations, " + expectedWidening + " expected-widening, " + unexpectedWidening.size()
      + " unexpected-widening, " + unexpectedTightening.size() + " unexpected-tightening, "
      + skipped + " skipped");
    ...
    if (violations > 0) {
      ...
      for (HarnessRunner.Report r : violationReports) {
        sb.append("  ").append(r.query).append("\n").append("    rows: ")
          .append(r.assertions.soundnessViolations).append("\n");
      }
      throw new AssertionError(sb.toString());
    }
    if (!unexpectedWidening.isEmpty()) {
      StringBuilder sb = new StringBuilder(
        unexpectedWidening.size() + " unexpected V2 widening(s) — either fix V2 or annotate with "
          + "expectedWideningReason:\n");
      for (String s : unexpectedWidening)
        sb.append("  ").append(s).append("\n");
      throw new AssertionError(sb.toString());
    }
    if (!unexpectedTightening.isEmpty()) {
      StringBuilder sb = new StringBuilder(
        unexpectedTightening.size() + " V2 now tighter than the documented expected widening — "
          + "remove the expectedWideningReason annotation:\n");
      for (String s : unexpectedTightening)
        sb.append("  ").append(s).append("\n");
      throw new AssertionError(sb.toString());
    }
  }
  ...
  private static List<Case> corpus() {
    ...
    out.add(new Case("C_LEAD_EQ", "CREATE TABLE C_LEAD_EQ (a BIGINT NOT NULL, b BIGINT NOT NULL, "
      + "CONSTRAINT pk PRIMARY KEY (a, b))", "SELECT * FROM C_LEAD_EQ WHERE a = 5"));

    // OR on leading PK.
    out.add(new Case("C_OR_LEAD", "CREATE TABLE C_OR_LEAD (a BIGINT NOT NULL, b BIGINT NOT NULL, "
      + "CONSTRAINT pk PRIMARY KEY (a, b))", "SELECT * FROM C_OR_LEAD WHERE a = 3 OR a = 7"));

    // IN list on leading PK.
    out.add(new Case("C_IN_LIST", "CREATE TABLE C_IN_LIST (a BIGINT NOT NULL, b BIGINT NOT NULL, "
      + "CONSTRAINT pk PRIMARY KEY (a, b))", "SELECT * FROM C_IN_LIST WHERE a IN (3, 5, 7)"));
    ...
    out.add(new Case("C_POINT_OR",
      "CREATE TABLE C_POINT_OR (a BIGINT NOT NULL, " + "CONSTRAINT pk PRIMARY KEY (a))",
    ...
    out.add(new Case("C_TAUTO",
      "CREATE TABLE C_TAUTO (a BIGINT NOT NULL, " + "CONSTRAINT pk PRIMARY KEY (a))",
    ...
    out.add(new Case("C_NONPK",
      "CREATE TABLE C_NONPK (a BIGINT NOT NULL, b BIGINT, " + "CONSTRAINT pk PRIMARY KEY (a))",
      "SELECT * FROM C_NONPK WHERE a = 5 AND b = 7"));

    // LIKE with prefix on a varchar PK.
    out
      .add(new Case("C_LIKE_PREFIX",
    ...
    out.add(new Case("C_NEGATIVE",
      "CREATE TABLE C_NEGATIVE (a BIGINT NOT NULL, " + "CONSTRAINT pk PRIMARY KEY (a))",
    ...
    out.add(new Case("C_DISJOINT_ADJ",
      "CREATE TABLE C_DISJOINT_ADJ (a BIGINT NOT NULL, " + "CONSTRAINT pk PRIMARY KEY (a))",
      "SELECT * FROM C_DISJOINT_ADJ WHERE a < 5 OR a >= 5"));

    // VARCHAR equality.
    out.add(
      new Case("C_VARCHAR_EQ", "CREATE TABLE C_VARCHAR_EQ (a VARCHAR NOT NULL, b BIGINT NOT NULL, "
        + "CONSTRAINT pk PRIMARY KEY (a, b))", "SELECT * FROM C_VARCHAR_EQ WHERE a = 'hello'"));

    // Multiple ANDs on the same column (redundant but common from user code).
    out.add(new Case("C_REDUNDANT_AND",
      "CREATE TABLE C_REDUNDANT_AND (a BIGINT NOT NULL, " + "CONSTRAINT pk PRIMARY KEY (a))",
    ...
    out.add(new Case("C_CONTRADICTION",
      "CREATE TABLE C_CONTRADICTION (a BIGINT NOT NULL, " + "CONSTRAINT pk PRIMARY KEY (a))",
    ...
    out.add(new Case("C_RVC_EQ", "CREATE TABLE C_RVC_EQ (a BIGINT NOT NULL, b BIGINT NOT NULL, "
      + "CONSTRAINT pk PRIMARY KEY (a, b))", "SELECT * FROM C_RVC_EQ WHERE (a, b) = (5, 10)"));
