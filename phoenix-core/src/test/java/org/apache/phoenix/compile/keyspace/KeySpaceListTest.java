...
/**
 * Algebra-level tests for {@link KeySpaceList}. Verifies AND/OR closure over lists of key spaces
 * and the merge-to-fixpoint invariant.
 */
public class KeySpaceListTest {
  ...
  public void andDistributesOverOr() {
    ...
    KeySpaceList left =
      KeySpaceList.of(KeySpace.single(0, pt("1"), 2), KeySpace.single(0, pt("2"), 2));
    KeySpaceList right =
      KeySpaceList.of(KeySpace.single(1, pt("x"), 2), KeySpace.single(1, pt("y"), 2));
  ...
  public void orConcatenatesThenMergesContainment() {
    KeySpaceList big = KeySpaceList.of(KeySpace.single(0,
      KeyRange.getKeyRange(Bytes.toBytes("a"), true, Bytes.toBytes("z"), false), 2));
  ...
  public void orMergesAdjacentRangesInSameDim() {
    KeySpaceList a = KeySpaceList
      .of(ks2(KeyRange.getKeyRange(Bytes.toBytes("1"), true, Bytes.toBytes("5"), false), pt("k")));
    KeySpaceList b = KeySpaceList
      .of(ks2(KeyRange.getKeyRange(Bytes.toBytes("5"), true, Bytes.toBytes("9"), false), pt("k")));
    KeySpaceList unioned = a.or(b);
    assertEquals(1, unioned.size());
    KeyRange mergedDim0 = unioned.spaces().get(0).get(0);
    assertEquals(KeyRange.getKeyRange(Bytes.toBytes("1"), true, Bytes.toBytes("9"), false),
      mergedDim0);
  }
  ...
  public void emptySpacesAreFilteredOut() {
    KeySpaceList list = KeySpaceList.of(KeySpace.single(0, pt("a"), 2), KeySpace.empty(2),
  ...
  public void orOfOverlappingRangesCoveringEverythingCollapses() {
    ...
    KeyRange geSeven = KeyRange.getKeyRange(Bytes.toBytes("7"), true, KeyRange.UNBOUND, false);
  ...
  /**
   * Regression for SkipScanQueryIT.testOrWithMixedOrderPKs: an OR chain of distinct inverted
   * (DESC-encoded) single-key values must NOT collapse adjacent byte prefixes into a range. E.g.,
   * '2' DESC = {@code \xCD} and '23' DESC = {@code \xCD\xCC} are distinct points; their OR must
   * yield 2 spaces, not 1 merged range.
   * <p>
   * {@link KeySpaceList#mergeSingleDim} previously used {@link KeyRange#coalesce} which inherits a
   * bug in {@code KeyRange.intersect} for inverted singletons — it now uses
  ...
  public void orOfDistinctInvertedSingletonsDoesNotOverMerge() {
    ...
    KeySpaceList unioned = a.or(b);
    assertEquals(
      "Distinct inverted singletons should OR into 2 separate spaces, not 1 merged range", 2,
      unioned.size());
  }

  /**
   * A 10-way OR chain on inverted (DESC) singletons should preserve all 10 points after the
   * merge-fixpoint pass. Mirrors the shape of SkipScanQueryIT's {@code testOrWithMixedOrderPKs}
   * where COL1 VARCHAR DESC has distinct values {@code '1','2','3','4','5','6','8','12','17','23'}
   * encoded as inverted singletons.
   */
  @Test
  public void orOfTenInvertedSingletonsPreservesAllPoints() {
    byte[][] values = new byte[][] { { (byte) 0xCE }, // '1'
      { (byte) 0xCD }, // '2'
      { (byte) 0xCC }, // '3'
      { (byte) 0xCB }, // '4'
      { (byte) 0xCA }, // '5'
