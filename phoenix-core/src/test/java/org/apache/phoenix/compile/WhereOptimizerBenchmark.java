...
/**
 * JMH microbenchmark comparing v1 vs v2 WHERE optimizer compile-time latency. Measures the time to
 * compile a SQL statement (parse + resolve + WHERE push + plan) under each flag setting, which
 * isolates optimizer cost without HBase network noise.
 * <p>
 ...
 * </ul>
 * <p>
 * Run with {@code mvn -pl phoenix-core test -Dtest=WhereOptimizerBenchmark} to invoke the main
 * method; JMH prints a table like
 *
 * <pre>
...
public class WhereOptimizerBenchmark extends BaseConnectionlessQueryTest {
  ...
  public void setUp() throws Exception {
    ...
    for (java.util.Enumeration<java.sql.Driver> e = DriverManager.getDrivers(); e
      .hasMoreElements();) {
  ...
  private static String buildRvcInList(int n) {
    StringBuilder sb = new StringBuilder(
      "select * from " + ATABLE_NAME + " where (organization_id, entity_id) in (");
    for (int i = 0; i < n; i++) {
      if (i > 0) {
        sb.append(',');
      }
  ...
  /**
   * Creates a table with 3 contiguous CHAR PK columns so we can expand independent IN lists on the
   * middle and trailing PK dims and observe the per-dim cartesian blow-up. Using CHAR everywhere
   * keeps param binding identical to the other benchmarks (all strings, all
   * {@code pstmt.setString}). The real {@code entity_history} table has {@code created_date DATE}
   * which would require a different binding path.
   */
  private void ensureCartesianExplosionTable() throws Exception {
    try (java.sql.Connection conn = DriverManager.getConnection(PHOENIX_CONNECTIONLESS_JDBC_URL)) {
      try {
        conn.createStatement()
          .execute("CREATE TABLE " + CART_TABLE + " (a CHAR(15) NOT NULL, b CHAR(15) NOT NULL, "
  ...
  /**
   * Builds a query whose per-column cartesian product exceeds the skip-scan bound (50,000 by
   * default in both v1 and v2) so that v2's "drop trailing dim" rule is exercised. The table's PK
   * is {@code (a, b, c, d)} with all four columns CHAR(15). Query is
   * {@code a = ? AND b IN (...) AND c IN (...) AND d IN (...)}; pinning dim 0 and expanding IN
   * lists of size n on dims 1, 2, 3, so the naive cross-product is {@code 1 · n · n · n = n³}.
   * <p>
   * At {@code n=5} the product is 125 (fits under 50,000); at {@code n=50} it's 125,000 (exceeds —
   * trip at dim 2 since {@code 1·50·50·50 = 125,000} but the running product already hits
   * {@code 2,500} at dim 1, {@code 125,000} at dim 2 → v2 drops dim 3); at {@code n=500} the
   * product is 1.25×10⁸ — v2 drops dim 2 and dim 3.
   * <p>
   * When the bound trips, v2's extractor stops at the slot that tripped it — so the trailing dim(s)
   * are dropped from the emitted scan ranges and their IN predicates move into the residual filter.
   * V1 accumulates the full cartesian cardinality in {@code inListSkipScanCardinality} and flips
   * {@code forcedRangeScan=true}, widening to a plain range scan over the whole dim-1 bounding box.
   */
  private static String buildCartesianExplosion(int n) {
    StringBuilder sb = new StringBuilder("select * from " + CART_TABLE + " where a = ? and b in (");
    for (int i = 0; i < n; i++) {
      if (i > 0) sb.append(',');
      sb.append('?');
    }
  ...
  /**
   * {@code org_id = ? AND parent_id IN (... n ...) AND entity_history_id IN (... n ...)} on a 4-col
   * PK. The per-column cartesian product is n² — at size=500 that's 250,000, far exceeding both
   * v1's MAX_IN_LIST_SKIP_SCAN_SIZE (50k) and v2's WHERE_OPTIMIZER_V2_CARTESIAN_BOUND (same 50k by
   * default). V2 detects this during extraction and drops the trailing entity_history_id slot
   * before emitting ranges; v1 builds both slot lists then flips forced-range-scan. Observe the
   * compile-time delta.
   */
  @Benchmark
  ...
  public static void main(String[] args) throws Exception {
    ...
    if (args.length > 0 && "prof".equals(args[0])) {
      builder =
        builder.include(".*orChain").param("size", "500").param("flag", "v2").warmupIterations(2)
          .warmupTime(org.openjdk.jmh.runner.options.TimeValue.seconds(3)).measurementIterations(3)
