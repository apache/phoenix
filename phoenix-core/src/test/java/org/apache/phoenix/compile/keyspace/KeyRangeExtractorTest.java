...
public class KeyRangeExtractorTest {
  ...
  public void adjacentRangesAreCoalesced() {
    ...
    KeySpaceList list =
      KeySpaceList.of(ks(2, range("1", true, "5", false)), ks(2, range("5", true, "9", false)));
  ...
  public void cartesianBoundTruncatesTrailingSlots() {
    ...
    KeySpaceList list =
      KeySpaceList.of(ks(3, pt("a"), pt("1")), ks(3, pt("a"), pt("2")), ks(3, pt("a"), pt("3")));
  ...
  public void multiDimDivergenceProducesPerSlotProjection() {
    ...
    KeySpaceList list = KeySpaceList.of(ks(2, pt("a"), pt("1")), ks(2, pt("b"), pt("2")));
    Result r = KeyRangeExtractor.extract(list, 2, 50);
    assertEquals(2, r.ranges.size());
  ...
  public void multiSpaceTrailingRangesPreservedUnderRelaxedBound() {
    KeySpaceList list = KeySpaceList.of(ks(3, pt("a"), pt("1")), ks(3, pt("a"), pt("2")));
    Result r = KeyRangeExtractor.extract(list, 3, 50);
  ...
  /**
   * Build a minimal {@link org.apache.phoenix.schema.RowKeySchema} with {@code n} fixed- width ASC
   * fields of {@code fieldLen} bytes each. Used to exercise the schema-based compound-emission
   * path.
   */
  ...
  /**
   * Regression for SkipScanQueryIT.testPreSplitCompositeFixedKey: compound emission is UNSAFE when
   * any space has a non-single-key dim followed by another constrained dim in the compound window.
   * For a query like {@code key_1 in [000,200) AND key_2 in
   * [aabb,aadd)} the byte interval {@code [000aabb, 200aadd)} is lex-wider than the conjunction
   * (rows like {@code (100, aaaa)} lie in the compound but don't match key_2's range). The
   * extractor must route to {@code emitV1Projection} which emits per-column slots with a
   * SkipScanFilter enforcing both constraints per-row.
   */
  @Test
  public void compoundIsUnsafeWhenLeadingRangeFollowedByTrailingRange() {
    ...
    KeyRange key2Range =
      KeyRange.getKeyRange(Bytes.toBytes("aab"), true, Bytes.toBytes("aad"), false);
    ...
    assertEquals("Range+range compound must fall back to per-column projection", 2,
      r.ranges.size());
    assertTrue("Per-column projection for range+range must force useSkipScan", r.useSkipScan);
  }

  /**
   * Regression for SkipScanQueryIT.testNullInfiniteLoop: {@code COL1 in [A,B] AND
   * COL2 = v} must emit a SkipScanFilter. The compound byte interval {@code [A+v, B+v]} includes
   * rows with any COL2 value in the middle of COL1's range, so without a filter, rows with COL2 !=
   * v slip through. V2 previously emitted no filter; this test asserts a filter is now required.
   */
  @Test
  ...
  /**
   * Regression for SkipScanQueryIT.testOrWithMixedOrderPKs: when compound emission runs for an
   * all-single-key shape on a DESC var-length leading PK with trailing EVERYTHING dims, the
   * trailing DESC separator byte from {@code ScanUtil.getMinKey/getMaxKey} must be stripped.
   * Otherwise the emitted point bytes are 2 bytes ({@code \xCD\xFF}) when the stored rows have PK
   * bytes {@code \xCD\xFF\x??} — the SkipScanFilter compares against the full row bytes and the
   * extra trailing separator causes the filter to miss all matching rows (scan returns 0 rows).
   * <p>
   * This test doesn't assert the stripping directly (that's a byte-level detail of compound
   * emission); the integration test {@code SkipScanQueryIT.testOrWithMixedOrderPKs} verifies the
   * end-to-end result. Here we verify the structural precondition: the extractor emits a single
   * compound slot with multiple point ranges when given an all-single-key OR chain.
   */
  @Test
  public void allSingleKeyCompoundOnLeadingDimEmitsPointKeyRanges() {
    ...
    KeySpaceList list = KeySpaceList.of(KeySpace.single(0, p1, 2), KeySpace.single(0, p2, 2),
    ...
    assertEquals("Single compound slot expected for all-single-key points", 1, r.ranges.size());
   ...
   * <p>
   * The compound emission path splits the trailing pinned dims (c, d) off the compound window
   * because b's range has an unbounded upper side. The resulting compound [aaabbb, ∞) is lex-wider
   * than the conjunction — rows with leading bytes inside the compound but whose c/d don't equal
   * the pinned values slip through unless a SkipScanFilter enforces per-row equality. Because the
   * coalesced compound has only one range, {@code coalesced.size() > 1} is false; the extractor
   * must still force {@code useSkipScan = true} when trailing pinned slots were split off.
   */
  @Test
  public void unboundedRangeWithTrailingPinnedForcesSkipScan() {
    ...
    assertTrue("Unbounded range with trailing pinned dims must force SkipScanFilter to enforce "
      + "per-row equality on split-off pinned slots", r.useSkipScan);
  }

  /**
   * Regression for VarBinaryEncoded1IT: when the emitV1Projection path has three constrained slots
   * where the middle slot is a non-point range AND the trailing slot is IS_NOT_NULL_RANGE, the
   * start/stop rows alone can't enforce the trailing IS_NOT_NULL — rows with a null PK3 sneak
   * through. Must force {@code useSkipScan = true} so
   * {@link org.apache.phoenix.filter.SkipScanFilter} rejects them per row.
   * <p>
   * Pattern: {@code PK1 = v AND PK2 BETWEEN a AND b AND PK3 IS NOT NULL} on a 3-col
   * VARBINARY_ENCODED PK.
   */
  @Test
  ...
  /**
   * Build a 2-field schema: (ASC BIGINT fixed 8 bytes, DESC DECIMAL variable-width). Mirrors
   * SortOrderIT.testSkipScanCompare's t_null_DECIMAL_DESC shape.
   */
  ...
  /**
   * Regression for SortOrderIT.testSkipScanCompare: when the compound window spans fields whose
   * {@link org.apache.phoenix.util.ScanUtil#getComparator} differs (e.g. ASC fixed-width leading +
   * DESC variable-width trailing), the extractor must fall back to per-column projection. Otherwise
   * the single compound slot is walked by {@link org.apache.phoenix.filter.SkipScanFilter} using
   * only the leading field's comparator, which silently mismatches DESC-variable-width bytes in the
   * trailing dim and misses rows whose trailing DESC values have a different byte length than the
   * slot's upper-bound bytes.
   */
  @Test
  public void compoundWithMixedComparatorsFallsBackToPerSlot() {
    ...
    byte[] k2_10_desc = org.apache.phoenix.schema.types.PDecimal.INSTANCE
      .toBytes(new java.math.BigDecimal("1.0"), org.apache.phoenix.schema.SortOrder.DESC);
    ...
    KeySpaceList list = KeySpaceList.of(KeySpace.of(new KeyRange[] { k1Eq2, k2RangeDesc }),
      KeySpace.of(new KeyRange[] { k1Eq4, k2RangeDesc }));
    Result r = KeyRangeExtractor.extract(list, 2, 50000, 0, schema);
    assertEquals("Expected per-column projection (2 slots) due to mixed comparators", 2,
      r.ranges.size());
