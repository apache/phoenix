...
public class ScannerLeaseRenewalTest extends BaseConnectionlessQueryTest {
  ...
  public void testRenewLeaseTaskBehavior() throws Exception {
    ...
    LinkedBlockingQueue<WeakReference<PhoenixConnection>> connectionsQueue =
    ...
    RenewLeaseOnlyTableIterator itr = new RenewLeaseOnlyTableIterator(numLeaseRenewals,
      skipRenewLeaseCount, failToAcquireLockAt, -1);
    ...
    assertTrue(connectionsQueue.size() == 1);
    assertEquals(CLOSED, itr.getLastRenewLeaseStatus()); // scanner closed and removed from the
                                                         // queue

    pconn.close();
    task.run();
    assertTrue(scannerQueue.size() == 0);
    assertTrue("Closing the connection should have removed it from the queue",
      connectionsQueue.size() == 0);
  }
  ...
  public void testRenewLeaseTaskBehaviorOnError() throws Exception {
    ...
    RenewLeaseOnlyTableIterator itr = new RenewLeaseOnlyTableIterator(numLeaseRenewals,
      thresholdNotReachedCount, lockNotAcquiredAt, failLeaseRenewalAt);
    LinkedBlockingQueue<WeakReference<TableResultIterator>> scannerQueue = pconn.getScanners();
    scannerQueue.add(new WeakReference<TableResultIterator>(itr));
    ...
    task.run();
    assertTrue(scannerQueue.size() == 0);
    assertTrue(connectionsQueue.size() == 0); // there was only one connection in the
                                              // connectionsQueue and it wasn't added back because
                                              // of error
