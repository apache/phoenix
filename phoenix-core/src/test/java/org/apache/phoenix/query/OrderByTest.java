...
public class OrderByTest extends BaseConnectionlessQueryTest {
  ...
  public void testSortOrderForSingleDescVarLengthCol() throws SQLException {
    ...
    Iterator<Pair<byte[], List<Cell>>> dataIterator =
    ...
    assertEquals("ab", Bytes.toString(
      SortOrder.invert(first.getRowArray(), first.getRowOffset(), first.getRowLength() - 1)));
    Cell second = kvs.get(1);
    assertEquals("a", Bytes.toString(
      SortOrder.invert(second.getRowArray(), second.getRowOffset(), second.getRowLength() - 1)));
  }
  ...
  public void testSortOrderForLeadingDescVarLengthColWithNullFollowing() throws SQLException {
    ...
    conn.createStatement()
      .execute("CREATE TABLE t (k1 VARCHAR, k2 VARCHAR, CONSTRAINT pk PRIMARY KEY (k1 DESC,k2))");
    conn.createStatement().execute("UPSERT INTO t VALUES ('a')");
    conn.createStatement().execute("UPSERT INTO t VALUES ('ab')");
  ...
  public void testSortOrderForLeadingDescVarLengthColWithNonNullFollowing() throws SQLException {
    ...
    conn.createStatement().execute(
      "CREATE TABLE t (k1 VARCHAR, k2 VARCHAR NOT NULL, CONSTRAINT pk PRIMARY KEY (k1 DESC,k2))");
    ...
    Cell first = kvs.get(0);
    assertEquals("ab",
      Bytes.toString(SortOrder.invert(first.getRowArray(), first.getRowOffset(), 2)));
    Cell second = kvs.get(1);
    assertEquals("a",
      Bytes.toString(SortOrder.invert(second.getRowArray(), second.getRowOffset(), 1)));
  }
