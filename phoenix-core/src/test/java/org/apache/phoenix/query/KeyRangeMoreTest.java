...
public class KeyRangeMoreTest extends TestCase {
  ...
  private void doTestListIntersectWithOneResultRange(int start1, int end1, int step1, int start2,
    int end2, int step2, boolean addEmptyRange) throws Exception {
    List<KeyRange> rowKeyRanges1 = new ArrayList<KeyRange>();
    List<KeyRange> rowKeyRanges2 = new ArrayList<KeyRange>();
    for (int i = start1; i <= end1; i++) {
      rowKeyRanges1.add(PInteger.INSTANCE.getKeyRange(PInteger.INSTANCE.toBytes(i), true,
        PInteger.INSTANCE.toBytes(i + step1), true, SortOrder.ASC));
    ...
    for (int i = start2; i <= end2; i++) {
      rowKeyRanges2.add(PInteger.INSTANCE.getKeyRange(PInteger.INSTANCE.toBytes(i), true,
        PInteger.INSTANCE.toBytes(i + step2), true, SortOrder.ASC));
    }
    if (addEmptyRange) {
    ...
    List<KeyRange> expected =
      Arrays.asList(KeyRange.getKeyRange(PInteger.INSTANCE.toBytes(maxStart), true,
        PInteger.INSTANCE.toBytes(minEnd), true));
  ...
  private void doTestListIntersectWithMultiResultRange(int start1, int count1, int step1,
    int start2, int count2, int step2, boolean addEmptyRange) throws Exception {
    List<KeyRange> rowKeyRanges1 = new ArrayList<KeyRange>();
    List<KeyRange> rowKeyRanges2 = new ArrayList<KeyRange>();
    for (int i = 1; i <= count1; i++) {
      rowKeyRanges1.add(
        PInteger.INSTANCE.getKeyRange(PInteger.INSTANCE.toBytes(start1 + (i - 1) * (step1 + 1)),
          true, PInteger.INSTANCE.toBytes(start1 + i * (step1 + 1) - 1), true, SortOrder.ASC));
    ...
    for (int i = 1; i <= count2; i++) {
      rowKeyRanges2.add(
        PInteger.INSTANCE.getKeyRange(PInteger.INSTANCE.toBytes(start2 + (i - 1) * (step2 + 1)),
          true, PInteger.INSTANCE.toBytes(start2 + i * (step2 + 1) - 1), true, SortOrder.ASC));
    }
    if (addEmptyRange) {
      rowKeyRanges2.add(KeyRange.EMPTY_RANGE);
    }
  ...
  public void testListIntersectForPoint() throws Exception {
    for (boolean addEmptyRange : new boolean[] { true, false }) {
      ...
      for (int i = 0; i <= 300; i += 2) {
        rowKeyRanges1.add(KeyRange.getKeyRange(PInteger.INSTANCE.toBytes(i)));
      }
      if (addEmptyRange) {
        rowKeyRanges1.add(KeyRange.EMPTY_RANGE);
      }
      for (int i = 0; i <= 300; i += 3) {
        rowKeyRanges2.add(KeyRange.getKeyRange(PInteger.INSTANCE.toBytes(i)));
      }
      if (addEmptyRange) {
        rowKeyRanges2.add(KeyRange.EMPTY_RANGE);
      }
      ...
      for (int i = 0; i <= 300; i += 6) {
        expected.add(KeyRange.getKeyRange(PInteger.INSTANCE.toBytes(i)));
      }
      listIntersectAndAssert(rowKeyRanges1, rowKeyRanges2, expected);
    }
  }
  ...
  public void testListIntersectForBoundary() throws Exception {
    ...
    for (int i = 0; i <= 100;) {
      rowKeyRanges2.add(PInteger.INSTANCE.getKeyRange(PInteger.INSTANCE.toBytes(i), true,
        PInteger.INSTANCE.toBytes(i + 2), true, SortOrder.ASC));
      i += 4;
    }
    ...
    listIntersectAndAssert(Arrays.asList(KeyRange.EMPTY_RANGE),
      Arrays.asList(KeyRange.EVERYTHING_RANGE), Arrays.asList(KeyRange.EMPTY_RANGE));

    rowKeyRanges1 = createKeyRangeList(Arrays.asList(2, 5, 8, Integer.MAX_VALUE),
      Arrays.asList(true, true, true, false));
    rowKeyRanges2 =
      createKeyRangeList(Arrays.asList(Integer.MIN_VALUE, 4, 7, 10, 13, 14, 19, Integer.MAX_VALUE),
        Arrays.asList(false, true, true, true, true, true, true, false));
    expected = createKeyRangeList(Arrays.asList(2, 4, 8, 10, 13, 14, 19, Integer.MAX_VALUE),
      Arrays.asList(true, true, true, true, true, true, true, false));
    listIntersectAndAssert(rowKeyRanges1, rowKeyRanges2, expected);

    rowKeyRanges1 =
      createKeyRangeList(Arrays.asList(3, 5, 5, 6), Arrays.asList(true, false, true, false));
    rowKeyRanges2 =
      createKeyRangeList(Arrays.asList(3, 5, 6, 7), Arrays.asList(true, true, true, true));
    expected = createKeyRangeList(Arrays.asList(3, 5), Arrays.asList(true, true));
    listIntersectAndAssert(rowKeyRanges1, rowKeyRanges2, expected);
  }

  @Test
  public void testKeyRangeCompareUpperRange() throws Exception {
    List<KeyRange> rowKeyRanges1 = createKeyRangeListWithFixedLowerRange(Arrays
      .asList(Integer.MAX_VALUE, Integer.MAX_VALUE, 10000, 1001, 1000, 1000, 1000, 1000, 1000),
      Arrays.asList(false, false, true, true, true, true, false, true, false));
    List<KeyRange> rowKeyRanges2 = createKeyRangeListWithFixedLowerRange(Arrays
      .asList(Integer.MAX_VALUE, 10000, Integer.MAX_VALUE, 1000, 1001, 1000, 1000, 1000, 1000),
  ...
  private static List<KeyRange> createKeyRangeListWithFixedLowerRange(List<Integer> keys,
  ...
  private static List<KeyRange> createKeyRangeList(List<Integer> keys,
    ...
    for (int i = 0; i < size; i++) {
      byte[] startKey = keys.get(2 * i).equals(Integer.MIN_VALUE)
        ? KeyRange.UNBOUND
        : PInteger.INSTANCE.toBytes(keys.get(2 * i));
      byte[] endKey = keys.get(2 * i + 1).equals(Integer.MAX_VALUE)
        ? KeyRange.UNBOUND
        : PInteger.INSTANCE.toBytes(keys.get(2 * i + 1));
      keyRangeList.add(PInteger.INSTANCE.getKeyRange(startKey, boundaryConditions.get(2 * i),
        endKey, boundaryConditions.get(2 * i + 1), SortOrder.ASC));
    }

    return keyRangeList;
  }

  private static void listIntersectAndAssert(List<KeyRange> rowKeyRanges1,
    List<KeyRange> rowKeyRanges2, List<KeyRange> expected) {
    for (int i = 0; i < 200; i++) {
      List<KeyRange> result = KeyRange.intersect(rowKeyRanges1, rowKeyRanges2);
