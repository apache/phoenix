...
public class ConnectionQueryServicesImplTest {
    ...
    private HTableDescriptor sysMutexTableDesc;
    private HColumnDescriptor defaultColFamDesc;
    ...
    public void reset() throws IOException {
        ...
        sysMutexTableDesc = new HTableDescriptor(TableName.valueOf(SYSTEM_MUTEX_NAME));
        defaultColFamDesc = new HColumnDescriptor(SYSTEM_MUTEX_FAMILY_NAME_BYTES);
    ...
    public void testSysMutexCheckModifiesTTLWhenWrong() throws Exception {
        // Set the wrong TTL
        defaultColFamDesc.setTimeToLive(HConstants.FOREVER);
        sysMutexTableDesc.addFamily(defaultColFamDesc);

        assertTrue(mockCqs.checkIfSysMutexExistsAndModifyTTLIfRequired(mockAdmin));
        verify(mockAdmin, Mockito.times(1))
                .modifyTable(sysMutexTableDesc.getTableName(), sysMutexTableDesc);
        assertEquals(TTL_FOR_MUTEX,
                sysMutexTableDesc.getFamily(SYSTEM_MUTEX_FAMILY_NAME_BYTES).getTimeToLive());
    }

    @Test
    public void testSysMutexCheckDoesNotModifyTableDescWhenTTLCorrect() throws Exception {
        // Set the correct TTL
        defaultColFamDesc.setTimeToLive(TTL_FOR_MUTEX);
        sysMutexTableDesc.addFamily(defaultColFamDesc);

        assertTrue(mockCqs.checkIfSysMutexExistsAndModifyTTLIfRequired(mockAdmin));
        verify(mockAdmin, Mockito.times(0))
                .modifyTable(sysMutexTableDesc.getTableName(), sysMutexTableDesc);
        assertEquals(TTL_FOR_MUTEX,
                sysMutexTableDesc.getFamily(SYSTEM_MUTEX_FAMILY_NAME_BYTES).getTimeToLive());
    }
