 ...
 * info, see: https://issues.apache.org/jira/browse/PHOENIX-6818 A generator for
 * OracleUpperTable.java. This generator creates an OracleUpperTable for each of a number of
 * {@link UpperExpr PL/SQL expressions}, which simply tabulates the these differences, allowing them
 * to be compensated for.
 * <p>
 * May be run as a JUnit test or as a stand-alone Java application. Run the output in Oracle to
 * generate the source for OracleUpperTable.java.
 * @see OracleUpper
...
public class OracleUpperTableGeneratorTest extends TestCase {
   ...
   * </pre>
   *
   * Each value is a PL/SQL upper case expression that may return different results than Java's
   * String.toUpperCase method for the given language.
   */
  private enum UpperExpr {
     ...
     * @param langCode ISO code for the language to use, as in <code> str.toUpperCase(new
   ...
   * OracleUpperTable value for each {@code UpperExpr}. Each table is created by comparing the
   * result of {@link String#toUpperCase(Locale)} against a {@link UpperExpr#getSql(char) PL/SQL
   * expression}. The table contains all deviations from Oracle for each character in a
   * {@link #charsToTest given set} that we know are fussy.
   */
  ...
  /**
   * Both {@code str} and {@code args} will be {@link #sqlEscape(String) sql escaped}, and then
   * {@code str} will be {@link String#format(String, Object...) formatted} using {@code args}.
   */
