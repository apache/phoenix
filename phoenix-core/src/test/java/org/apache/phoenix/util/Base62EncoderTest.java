...
public class Base62EncoderTest {
  ...
  public final void testPowersBase62() {
    ...
    long input = 0l;
    // test 0
    assertEquals("Base 62 encoded value for " + input + "  is incorrect ", "0",
      Base62Encoder.toString(input));
    StringBuilder expectedValBuilder = new StringBuilder("1");
    for (int i = 0; i < pow62.length; ++i) {
      input = pow62[i];
      assertEquals("Base 62 encoded value for " + input + "  is incorrect ",
        expectedValBuilder.toString(), Base62Encoder.toString(input));
      assertEquals("Base 62 encoded value for " + input + "  is incorrect ",
        "-" + expectedValBuilder.toString(), Base62Encoder.toString(-input));
