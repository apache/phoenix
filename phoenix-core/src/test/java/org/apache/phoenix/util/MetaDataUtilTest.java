...
public class MetaDataUtilTest {
  ...
  public void setupMockCellBuilder() {
    Mockito.when(mockBuilder.setRow(Mockito.any(byte[].class), Mockito.anyInt(), Mockito.anyInt()))
      .thenReturn(mockBuilder);
    Mockito
      .when(mockBuilder.setFamily(Mockito.any(byte[].class), Mockito.anyInt(), Mockito.anyInt()))
      .thenReturn(mockBuilder);
    Mockito
      .when(mockBuilder.setQualifier(Mockito.any(byte[].class), Mockito.anyInt(), Mockito.anyInt()))
      .thenReturn(mockBuilder);
    Mockito
      .when(mockBuilder.setValue(Mockito.any(byte[].class), Mockito.anyInt(), Mockito.anyInt()))
      .thenReturn(mockBuilder);
    Mockito.when(mockBuilder.setTimestamp(Mockito.anyLong())).thenReturn(mockBuilder);
    Mockito.when(mockBuilder.setType(Mockito.any(Cell.Type.class))).thenReturn(mockBuilder);
    Mockito.when(mockBuilder.setTags(Mockito.any(byte[].class))).thenReturn(mockBuilder);
  ...
  public void testCompatibilityNewerClient() {
    MetaDataUtil.ClientServerCompatibility compatibility1 =
      MetaDataUtil.areClientAndServerCompatible(VersionUtil.encodeVersion(3, 1, 10), 4, 0);
    ...
    MetaDataUtil.ClientServerCompatibility compatibility2 =
      MetaDataUtil.areClientAndServerCompatible(VersionUtil.encodeVersion(3, 1, 10), 3, 2);
  ...
  public void testCompatibilityMismatchedMajorVersions() {
    MetaDataUtil.ClientServerCompatibility compatibility =
      MetaDataUtil.areClientAndServerCompatible(VersionUtil.encodeVersion(3, 1, 10), 2, 0);
    assertFalse(compatibility.getIsCompatible());
    assertEquals(compatibility.getErrorCode(),
  ...
  public void testGetMutationKeyValue() throws Exception {
    ...
    KeyValue kv =
      builder.buildPut(wrap(ROW), wrap(TABLE_FAMILY_BYTES), wrap(QUALIFIER), wrap(ORIGINAL_VALUE));
    Put put = new Put(ROW);
    ...
    assertEquals(
      "Value returned doesn't match stored value for " + builder.getClass().getName() + "!", 0,
  ...
  public void testEncodeDecode() {
    ...
    boolean isTableNamespaceMappingEnabled =
      MetaDataUtil.decodeTableNamespaceMappingEnabled(version);
  ...
  public void testConditionallyAddTagsToPutCells() {
    List<Tag> tags =
      TagUtil.asList(VIEW_MODIFIED_PROPERTY_BYTES, 0, VIEW_MODIFIED_PROPERTY_BYTES.length);
    ...
    KeyValue kv = builder.buildPut(wrap(ROW), wrap(TABLE_FAMILY_BYTES),
      wrap(UPDATE_CACHE_FREQUENCY_BYTES), wrap(PLong.INSTANCE.toBytes(0)));
    Put put = new Put(ROW);
    KeyValueBuilder.addQuietly(put, kv);
    ...
    assertTrue(
      Bytes.compareTo(expectedTag.getValueArray(), TagUtil.concatTags(EMPTY_BYTE_ARRAY, cell))
          == 0);
    assertTrue(
    ...
    assertTrue(Bytes.compareTo(actualTag.getValueArray(), actualTag.getValueOffset(),
      actualTag.getValueLength(), expectedTag.getValueArray(), expectedTag.getValueOffset(),
  ...
  public void testGetLastDDLTimestampUpdate() throws Exception {
    ...
    Put p =
      MetaDataUtil.getLastDDLTimestampUpdate(tableHeaderRowKey, clientTimestamp, serverTimestamp);
    assertNotNull(p);
    ...
    assertEquals(serverTimestamp,
      PLong.INSTANCE.getCodec().decodeLong(CellUtil.cloneValue(c), 0, SortOrder.ASC));
  }
