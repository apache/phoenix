...
public class PhoenixRuntimeTest extends BaseConnectionlessQueryTest {
  ...
  public void testParseArguments_MinimalCase() {
    PhoenixRuntime.ExecutionCommand execCmd =
      PhoenixRuntime.ExecutionCommand.parseArgs(new String[] { "localhost", "test.csv" });

    assertEquals("localhost", execCmd.getConnectionString());

    assertEquals(ImmutableList.of("test.csv"), execCmd.getInputFiles());
    ...
    assertEquals(CSVCommonsLoader.DEFAULT_ARRAY_ELEMENT_SEPARATOR,
  ...
  public void testParseArguments_FullOption() {
    PhoenixRuntime.ExecutionCommand execCmd = PhoenixRuntime.ExecutionCommand.parseArgs(
      new String[] { "-t", "mytable", "myzkhost:2181", "--strict", "file1.sql", "test.csv",
  ...
  public void testGetPkColsEncodeDecode() throws Exception {
    ...
    String indexDDL =
      "CREATE INDEX i ON t (CREATED_DATE, PARENT_ID) INCLUDE (DATA_TYPE, OLDVAL_STRING, NEWVAL_STRING)";
    ...
    QueryPlan plan = conn.createStatement().unwrap(PhoenixStatement.class).optimizeQuery(
      "SELECT PARENT_ID FROM T WHERE CREATED_DATE > CURRENT_DATE()-1 AND TENANT_ID = '111111111111111'");
    ...
    byte[] encodedValues =
      PhoenixRuntime.encodeColumnValues(conn, fullTableName, values, pkColumns);
    Object[] decodedValues =
      PhoenixRuntime.decodeColumnValues(conn, fullTableName, encodedValues, pkColumns);
    assertArrayEquals(values, decodedValues);

    plan = conn.createStatement().unwrap(PhoenixStatement.class)
      .optimizeQuery("SELECT /*+ NO_INDEX */ ENTITY_HISTORY_ID FROM T");
  ...
  public void testGetPkColsDataTypes() throws Exception {
    ...
    try {
      for (i = 0; i < size; i++) {
        ...
        if (
          pType.isArrayType() && PDataType.arrayBaseType(pType).isFixedWidth()
            && PDataType.arrayBaseType(pType).getByteSize() == null
        ) {
          // Need to treat array type whose base type is of fixed width whose byte size is not known
          // as a special case.
  ...
  public void testGetTenantIdExpression() throws Exception {
    Connection conn = DriverManager.getConnection(getUrl());
    Expression e1 =
      PhoenixRuntime.getTenantIdExpression(conn, PhoenixDatabaseMetaData.SYSTEM_STATS_NAME);
    assertNull(e1);
    Expression e2 =
      PhoenixRuntime.getTenantIdExpression(conn, PhoenixDatabaseMetaData.SYSTEM_CATALOG_NAME);
    assertNotNull(e2);

    Expression e3 =
      PhoenixRuntime.getTenantIdExpression(conn, PhoenixDatabaseMetaData.SYSTEM_SEQUENCE_NAME);
    assertNotNull(e3);
    ...
    conn.createStatement().execute(
      "CREATE TABLE A.BAR (k1 VARCHAR NOT NULL, k2 VARCHAR, CONSTRAINT PK PRIMARY KEY(K1,K2)) MULTI_TENANT=true");
    ...
    conn.createStatement().execute(
      "CREATE TABLE BAS (k1 VARCHAR NOT NULL, k2 VARCHAR, CONSTRAINT PK PRIMARY KEY(K1,K2)) MULTI_TENANT=true, SALT_BUCKETS=3");
  ...
  public void testCaseSensitiveTableNameWithCaseSensitiveSchema() throws Exception {
    ...
    String fullNameQuoted =
      quoteString(caseSensitiveSchemaName) + "." + quoteString(caseSensitiveTableName);
  ...
  private void getTableTester(String normalizedName, String sqlStatementName) throws SQLException {
    ...
    try {
      conn.createStatement()
        .execute("CREATE TABLE " + sqlStatementName + " (k VARCHAR PRIMARY KEY)");
