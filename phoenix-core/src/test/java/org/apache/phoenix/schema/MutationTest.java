...
public class MutationTest extends BaseConnectionlessQueryTest {
  ...
  private void testDurability(boolean disableWAL) throws Exception {
    try (Connection conn = DriverManager.getConnection(getUrl())) {
      ...
      conn.createStatement()
        .execute("CREATE TABLE t1 (k integer not null primary key, a.k varchar, b.k varchar) "
          + (disableWAL ? "DISABLE_WAL=true" : ""));
  ...
  public void testSizeConstraint() throws Exception {
    ...
    try {
      ...
      conn.createStatement()
        .execute("CREATE TABLE t1 (k1 char(" + maxLength1 + ") not null, k2 varchar(" + maxLength2
          + "), " + "v1 varchar(" + maxLength2 + "), v2 varbinary(" + maxLength2 + "), v3 binary("
          + maxLength2 + "), constraint pk primary key (k1, k2))");
      conn.createStatement().execute("UPSERT INTO t1 VALUES('a','" + value + "', '" + value + "','"
        + bvalue + "','" + bvalue + "')");
      try {
        conn.createStatement()
          .execute("UPSERT INTO t1(k1,v1) VALUES('" + validValue + "','" + value + "')");
        fail();
