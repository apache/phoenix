...
public class CoveredColumnIndexCodec extends BaseIndexCodec {
  ...
  /** Returns the update that should be made to the table */
  private IndexUpdate getIndexUpdateForGroup(ColumnGroup group, TableState state,
    ...
    try {
      Pair<CoveredDeleteScanner, IndexUpdate> stateInfo =
        ((LocalTableState) state).getIndexedColumnsTableState(refs, false, false, indexMetaData);
   ...
   * for a given index. index information
  ...
  /**
   * Get the next batch of primary table values for the given columns columns to match against
  ...
  private Pair<Integer, List<ColumnEntry>> getNextEntries(List<CoveredColumn> refs, Scanner kvs,
    ...
    for (CoveredColumn ref : refs) {
      ...
      Cell next = kvs.next();
      if (
        ref.matchesFamily(next.getFamilyArray(), next.getFamilyOffset(), next.getFamilyLength())
   ...
   * copy everything over once. primary key of the original row total number of bytes of all the
   * values that should be added to use when building the key
   */
  public static byte[] composeRowKey(byte[] pk, int length, List<ColumnEntry> values) {
    ...
    //
    // output = length of values + primary key + column entries + length of each column entry +
  ...
  /**
   * Essentially a short-cut from building a {@link Put}. row key timestamp of all the keyvalues
  ...
  /**
   * Read an integer from the preceding {@value Bytes#SIZEOF_INT} bytes array to read from start
   * point, backwards from which to read. For example, if specifying "25", we would try to read an
   * integer from 21 -> 25
  ...
  /**
   * Check to see if an row key just contains a list of null values. row key to examine
