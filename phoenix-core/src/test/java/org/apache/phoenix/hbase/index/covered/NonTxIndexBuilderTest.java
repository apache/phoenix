...
public class NonTxIndexBuilderTest extends BaseConnectionlessQueryTest {
  ...
  private static final String TEST_TABLE_DDL = "CREATE TABLE IF NOT EXISTS " + TEST_TABLE_STRING
  ...
  private static final String TEST_TABLE_INDEX_DDL = "CREATE INDEX IF NOT EXISTS "
    + TEST_TABLE_INDEX_STRING + " ON " + TEST_TABLE_STRING + " (SCORE DESC, ENTITY_ID DESC)";
  ...
  public void setup() throws Exception {
    ...
    Mockito.when(mockIndexMetaData.requiresPriorRowState((Mutation) Mockito.any()))
      .thenReturn(true);
  ...
  public void testGetMutableIndexUpdate() throws IOException {
    ...
    put.add(CellBuilderFactory.create(CellBuilderType.SHALLOW_COPY).setRow(put.getRow())
      .setFamily(FAM).setType(Cell.Type.Put).setQualifier(INDEXED_QUALIFIER).setTimestamp(2)
      .setValue(VALUE_2).build());
    MultiMutation mutation = new MultiMutation(new ImmutableBytesPtr(ROW));
    mutation.addAll(put);

    CachedLocalTable cachedLocalTable = CachedLocalTable.build(Collections.singletonList(mutation),
      this.mockIndexMetaData, this.indexBuilder.getEnv().getRegion());
   ...
   * and we rebuild the index starting from time t=2 There should be one index row version per data
   * row version.
   */
  @Test
  public void testRebuildMultipleVersionRow() throws IOException {
    ...
    Put put = new Put(ROW);
    put.add(CellBuilderFactory.create(CellBuilderType.SHALLOW_COPY).setRow(put.getRow())
      .setFamily(FAM).setType(Cell.Type.Put).setQualifier(INDEXED_QUALIFIER).setTimestamp(4)
      .setValue(VALUE_4).build());
    mutation.addAll(put);
    put = new Put(ROW);
    put.add(CellBuilderFactory.create(CellBuilderType.SHALLOW_COPY).setRow(put.getRow())
      .setFamily(FAM).setType(Cell.Type.Put).setQualifier(INDEXED_QUALIFIER).setTimestamp(3)
      .setValue(VALUE_3).build());
    mutation.addAll(put);
    put = new Put(ROW);
    put.add(CellBuilderFactory.create(CellBuilderType.SHALLOW_COPY).setRow(put.getRow())
      .setFamily(FAM).setType(Cell.Type.Put).setQualifier(INDEXED_QUALIFIER).setTimestamp(2)
    ...
    CachedLocalTable cachedLocalTable = CachedLocalTable.build(mutations, this.mockIndexMetaData,
  ...
  public void testManyVersions() throws IOException {
    ...
    Collection<Pair<Mutation, byte[]>> indexUpdates = Lists.newArrayList();
    for (Mutation m : IndexManagementUtil
      .flattenMutationsByTimestamp(Collections.singletonList(mutation))) {
      indexUpdates.addAll(indexBuilder.getIndexUpdate(m, mockIndexMetaData, cachedLocalTable));
    }
    ...
    Predicate<Pair<Mutation, byte[]>> hasCellPredicate = new Predicate<Pair<Mutation, byte[]>>() {
      ...
      public boolean apply(Pair<Mutation, byte[]> input) {
        ...
        if (mutationTs == mutation.getTimestamp()) {
          ...
          if (
            cellType == updateCell.getType()
              && Bytes.compareTo(fam, CellUtil.cloneFamily(updateCell)) == 0
              && Bytes.compareTo(qual, CellUtil.cloneQualifier(updateCell)) == 0
  ...
  private MultiMutation getMultipleVersionMutation(int versions) {
    ...
    for (int i = versions - 1; i >= 0; i--) {
      ...
      try {
        put.add(CellBuilderFactory.create(CellBuilderType.SHALLOW_COPY).setRow(put.getRow())
          .setFamily(FAM).setType(Cell.Type.Put).setQualifier(INDEXED_QUALIFIER).setTimestamp(i)
