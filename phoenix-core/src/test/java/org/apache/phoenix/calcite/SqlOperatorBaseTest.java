...
//      return;
  ...
  @Test public void testNotInOperator() {
    ...
    public void testOverlapsOperator() {
        ...
        tester.checkBoolean(
            ...
            Boolean.FALSE);
        tester.checkBoolean("(date '1-2-3', date '4-5-6') overlaps (date '2-2-3', date '3-4-5')",
            Boolean.TRUE);
        tester.checkNull(
  ...
  @Test public void testMultiplyOperator() {
    ...
    if (BUG_CHECKER_EXACT_REAL_FIXED) tester.checkScalarExact(
        "10.0 * 5.0",
        "DECIMAL(5, 2) NOT NULL",
        "50.00");
    tester.checkScalarExact(
  ...
  @Test public void testNotEqualsOperator() {
    ...
    if (ENABLE_CAST_NULL_TEST) tester.checkNull("'a'<>cast(null as varchar(1))");
  }
  ...
  @Test public void testPlusOperator() {
    if (!BUG_CALCITE_818_FIXED) return;
  ...
  @Test public void testPlusOperatorAny() {
    tester.setFor(SqlStdOperatorTable.PLUS);
    if (ENABLE_CAST_ANY_TEST) tester.checkScalar("1+CAST(2 AS ANY)", "3", "ANY NOT NULL");
  }
  ...
  @Test public void testPrefixMinusOperator() {
    ...
    if (ENABLE_CAST_NULL_TEST) tester.checkNull("-cast(null as integer)");
    if (ENABLE_CAST_NULL_TEST) tester.checkNull("-cast(null as tinyint)");
  }
  ...
  @Test public void testLikeEscape() {
    if (!ENABLE_LIKE_ESCAPE_TEST) return;
  ...
  @Test public void testLikeOperator() {
    tester.setFor(SqlStdOperatorTable.LIKE);
    if (ENABLE_NULL_STRING_TEST) tester.checkBoolean("''  like ''", Boolean.TRUE);
  ...
  @Test public void testPositionFunc() {
      if (!ENABLE_COMBINED_ARGUMENTS) return;
  ...
  @Test public void testCharacterLengthFunc() {
    if (!ENABLE_FUNCTION_USING_DIFFERENT_NAME) return;
  ...
  @Test public void testUpperFunc() {
      if (!BUG_CHECKER_STRING_FIXED) return;
  ...
  @Test public void testAbsFunc() {
    ...
    tester.checkScalarApprox(
        ...
        3.5,
        0);
    if (BUG_CHECKER_CAST_FIXED) tester.checkScalarApprox(
    ...
    if (ENABLE_TYPE_REAL_TEST) tester.checkScalarApprox(
        ...
        3.5,
        0);
    if (ENABLE_CAST_NULL_TEST) tester.checkNull("abs(cast(null as double))");
  }
  ...
  @Test public void testCurrentDateFunc() {
    ...
    if (BUG_CHECKER_CAST_FIXED) tester.checkScalar("CURRENT_DATE", DATE_PATTERN, "DATE NOT NULL");
    if (BUG_CHECKER_CAST_FIXED) tester.checkScalar(
  ...
  @Test public void testArrayValueConstructor() {
      if (!TODO_ARRAY_VALUE_CONSTRUCTOR_TEST_FINISHED) return;
  ...
  @Test public void testMapValueConstructor() {
      if (!ENABLE_TYPE_MAP_TEST) return;
  ...
  @Test public void testFloorFunc() {
    ...
    if (BUG_CHECKER_CAST_FIXED) tester.checkScalarApprox("floor(2.5e0)", "DOUBLE NOT NULL", 2, 0);
    if (ENABLE_TYPE_REAL_TEST) tester.checkScalarApprox("floor(cast(-1.2e0 as real))", "REAL NOT NULL", -2,
        0);
    ...
    if (ENABLE_CAST_NULL_TEST) tester.checkNull("floor(cast(null as decimal(2,0)))");
    if (ENABLE_CAST_NULL_TEST) tester.checkNull("floor(cast(null as real))");
  }
  ...
  @Test public void testCountFunc() {
    ...
    tester.checkAgg("COUNT(*)", stringValues, 3, 0.0);
    if (ENABLE_CAST_NULL_TEST) {
      tester.checkAgg("COUNT(x)", stringValues, 2, 0.0);
    }
    if (TODO_COUNT_DISTINCT_FINISHED) {
      tester.checkAgg("COUNT(DISTINCT x)", stringValues, 2, 0.0);
      tester.checkAgg("COUNT(DISTINCT 123)", stringValues, 1, 0.0);
    }
  }
  ...
  @Test public void testSumFunc() {
    ...
    tester.checkAgg("sum(DISTINCT CASE x WHEN 0 THEN NULL ELSE -1 END)", values,
        result, 0.0);
    tester.checkAgg("sum(DISTINCT x)", values, 2, 0.0);
  }
  ...
  private static boolean ENABLE_INTERVAL_TEST = false;
  private static boolean ENABLE_NULLIF_TEST = false;
  private static boolean ENABLE_NULL_STRING_TEST = false;
  private static boolean ENABLE_LIKE_ESCAPE_TEST = false;
  private static boolean ENABLE_CAST_ANY_TEST = false;
  private static boolean ENABLE_CAST_NULL_TEST = false;
  private static boolean ENABLE_CAST_STRING_TO_NUMERIC_TEST = false;
  private static boolean ENABLE_CAST_STRING_TO_VARSTRING_IGNORE_SPACE_TEST = false;
  private static boolean ENABLE_CAST_STRING_TO_TIME_TEST = false;
  private static boolean ENABLE_CAST_STRING_TO_BOOLEAN_TEST = false;
  private static boolean ENABLE_CAST_NUMBERIC_TO_STRING_TEST = false;

  private static boolean ENABLE_TYPE_REAL_TEST = false;
  private static boolean ENABLE_TYPE_MAP_TEST = false;

  private static boolean ENABLE_IS_DISTINCT_FROM_TEST = false;

  private static boolean BUG_CALCITE_818_FIXED = false;
  private static boolean BUG_CHECKER_CAST_FIXED = false;
  private static boolean BUG_CHECKER_STRING_FIXED = false;
  private static boolean BUG_CHECKER_EXACT_REAL_FIXED = false;

  // Phoenix only supports separate argument list
  private static boolean ENABLE_COMBINED_ARGUMENTS = false;

  private static boolean ENABLE_FUNCTION_USING_DIFFERENT_NAME = false;
  private static boolean ENABLE_NOT_SUPPORT_FUNCTION = false;

  private static boolean ENABLE_OPERATOR_FOR_STR = false;

  private static boolean TODO_CASE_RELATED_TEST_FINISHED = false;
  private static boolean TODO_ARRAY_VALUE_CONSTRUCTOR_TEST_FINISHED = false;
