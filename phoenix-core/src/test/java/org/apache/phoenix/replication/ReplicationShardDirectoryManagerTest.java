...
public class ReplicationShardDirectoryManagerTest {
  ...
  public void testGetShardDirectoryWithSpecificConditions() {
    ...
    assertEquals("Timestamp at start of min should map to shard 000", expectedPath1,
      shardPath1.toString());
    ...
    assertEquals("Timestamp at mid of min should map to shard 000", expectedPath2,
      shardPath2.toString());
    ...
    assertEquals("Timestamp at end of min should map to shard 000", expectedPath3,
      shardPath3.toString());
    ...
    assertEquals("Timestamp at 128th round should wrap to shard 000 due to modulo", expectedPath4,
      shardPath4.toString());
    ...
    assertEquals("Timestamp at second round should map to shard 001", expectedPath5,
      shardPath5.toString());
    ...
    assertEquals("Timestamp at 127th round should map to shard 127", expectedPath6,
      shardPath6.toString());
    ...
    assertEquals("Timestamp at 129th round should wrap to shard 001", expectedPath7,
      shardPath7.toString());
  }
    ...
    for (int day = 0; day < 2; day++) {
      for (int hour = 0; hour < 24; hour++) {
        for (int minute = 0; minute < 60; minute++) {
          long minuteStart = dayStart + (day * 24 * 60 * 60 * 1000L) + (hour * 60 * 60 * 1000L)
            + (minute * 60 * 1000L);
          ...
          int shardIndex = (int) (secondsSinceStartOfDay / 60) % 128; // 60-second rounds, 128
                                                                      // shards
          ...
          assertEquals(
            String.format("Start of minute %02d:%02d:%02d (day %d) should map to shard %03d", hour,
              minute, 0, day + 1, shardIndex),
          ...
          assertEquals(
            String.format("Mid of minute %02d:%02d:%02d (day %d) should map to shard %03d", hour,
              minute, randomSeconds, day + 1, shardIndex),
          ...
          assertEquals(
            String.format("End of minute %02d:%02d:%02d (day %d) should map to shard %03d", hour,
              minute, 59, day + 1, shardIndex),
  ...
  public void testGetNearestRoundStartTimestamp() {
    ...
    assertEquals("Multiple rounds later should return exact round start", expectedRoundStart,
      result7);
  ...
  public void testGetNearestRoundStartTimestampWithCustomConfiguration() {
    ...
    customConf
      .setInt(ReplicationShardDirectoryManager.PHOENIX_REPLICATION_ROUND_DURATION_SECONDS_KEY, 30);

    Path rootPath = new Path(testFolder.getRoot().getAbsolutePath());
    ReplicationShardDirectoryManager customManager =
      new ReplicationShardDirectoryManager(customConf, rootPath);
    ...
    assertEquals("With 30-second rounds, should round down to 30-second boundary", expected,
      result);
  ...
  public void testGetReplicationRoundFromStartTime() {
    ...
    assertEquals("Round start time should be validated and unchanged", exactRoundStart,
    ...
    assertEquals("Mid-round timestamp should round down to round start", expectedStart2,
    ...
    assertEquals("End of round timestamp should round down to round start", expectedStart3,
      result3.getStartTime());
    assertEquals("Round end time should be start time + round duration",
    ...
    assertEquals("Second round start time should be validated and unchanged", secondRoundStart,
      result4.getStartTime());
    assertEquals("Round end time should be start time + round duration",
    ...
    assertEquals("Mid-second round timestamp should round down to round start", expectedStart5,
      result5.getStartTime());
    assertEquals("Round end time should be start time + round duration",
    ...
    assertEquals("Multiple rounds later should be validated and unchanged", multipleRoundsLater,
      result6.getStartTime());
    assertEquals("Round end time should be start time + round duration",
    ...
    assertEquals("Just before round boundary should round down", expectedStart7,
      result7.getStartTime());
    assertEquals("Round end time should be start time + round duration",
    ...
    assertEquals("Just after round boundary should round down", expectedStart8,
      result8.getStartTime());
    assertEquals("Round end time should be start time + round duration",
    ...
    Assert.assertTrue("Current time should round down to nearest round",
      result10.getStartTime() <= currentTime);
    assertEquals("Round start should be a multiple of round duration", 0,
      (result10.getStartTime() % roundDurationMs));
    assertEquals("Round end time should be start time + round duration",
  ...
  public void testGetReplicationRoundFromStartTimeWithCustomConfiguration() {
    ...
    assertEquals("With 30-second rounds, should round down to 30-second boundary", expectedStart,
      result.getStartTime());
    assertEquals("Round end time should be start time + round duration",
    ...
    assertEquals("Exact 30-second boundary should be validated and unchanged", exactBoundary,
      result2.getStartTime());
    assertEquals("Round end time should be start time + round duration",
  ...
  public void testGetReplicationRoundFromStartTimeConsistency() {
    ...
    assertEquals("Same input should produce same start time", result1.getStartTime(),
      result2.getStartTime());
    assertEquals("Same input should produce same end time", result1.getEndTime(),
      result2.getEndTime());
    assertEquals("Same input should produce same start time", result1.getStartTime(),
      result3.getStartTime());
    assertEquals("Same input should produce same end time", result1.getEndTime(),
  ...
  public void testGetReplicationRoundFromEndTime() {
    ...
    assertEquals("Round end time should be validated and unchanged", exactRoundEnd,
      result1.getEndTime());
    ...
    assertEquals("Mid-round timestamp should round down to round start", expectedEnd2,
      result2.getEndTime());
    assertEquals("Round start time should be end time - round duration", expectedStart2,
      result2.getStartTime());
    ...
    assertEquals("End of first round should round down to round start", expectedEnd3,
      result3.getEndTime());
    assertEquals("Round start time should be end time - round duration", expectedStart3,
      result3.getStartTime());
    ...
    assertEquals("Second round start time should be validated and unchanged", startSecondRound,
      result4.getEndTime());
    assertEquals("Round start time should be end time - round duration",
    ...
    assertEquals("Mid-second round timestamp should round down to round start", expectedEnd5,
      result5.getEndTime());
    assertEquals("Round start time should be end time - round duration", expectedStart5,
      result5.getStartTime());
    ...
    assertEquals("Just before round boundary should round down", expectedEnd7,
      result7.getEndTime());
    assertEquals("Round start time should be end time - round duration", expectedStart7,
      result7.getStartTime());
    ...
    assertEquals("Round start time should be end time - round duration", expectedStart8,
      result8.getStartTime());
    ...
    Assert.assertTrue("Current time should round down to nearest round",
      result9.getEndTime() <= currentTime);
    assertEquals("Round end should be a multiple of round duration", 0,
      (result9.getEndTime() % roundDurationMs));
    assertEquals("Round start time should be end time - round duration",
  ...
  public void testGetReplicationRoundFromEndTimeWithCustomConfiguration() {
    ...
    assertEquals("With 30-second rounds, should round down to 30-second boundary", expectedEnd,
      result.getEndTime());
    assertEquals("Round start time should be end time - round duration", expectedStart,
      result.getStartTime());
  ...
  public void testDefaultAndCustomConfigurationValues() {
    ...
    ReplicationShardDirectoryManager defaultManager =
      new ReplicationShardDirectoryManager(conf, new Path(testFolder.getRoot().getAbsolutePath()));

    // Validate default values
    assertEquals("numShards should use default value when not configured",
      ReplicationShardDirectoryManager.DEFAULT_REPLICATION_NUM_SHARDS,
      defaultManager.getNumShards());
    assertEquals("replicationRoundDurationSeconds should use default value when not configured",
      ReplicationShardDirectoryManager.DEFAULT_REPLICATION_ROUND_DURATION_SECONDS,
      defaultManager.getReplicationRoundDurationSeconds());
    ...
    customConf.setInt(
      ReplicationShardDirectoryManager.PHOENIX_REPLICATION_ROUND_DURATION_SECONDS_KEY,
      customReplicationRoundDurationSeconds);

    ReplicationShardDirectoryManager customManager = new ReplicationShardDirectoryManager(
      customConf, new Path(testFolder.getRoot().getAbsolutePath()));

    // Validate custom values
    assertEquals("numShards should use custom value when configured", customNumShards,
      customManager.getNumShards());
  ...
  public void testGetAllShardPaths() {
    testGetAllShardPathsHelper(conf,
      ReplicationShardDirectoryManager.DEFAULT_REPLICATION_NUM_SHARDS);
  }
  ...
  public void testGetAllShardPathsWithCustomConfiguration() {
    ...
    Configuration customConf = HBaseConfiguration.create();
    customConf.setInt(ReplicationShardDirectoryManager.REPLICATION_NUM_SHARDS_KEY,
      customShardCount);
  ...
  private void testGetAllShardPathsHelper(final Configuration conf, final int expectedShardCount) {
    ...
    assertEquals("Should return exactly " + expectedShardCount + " shard paths", expectedShardCount,
      shardPaths.size());
    ...
    for (int i = 0; i < expectedShardCount; i++) {
      ...
      assertEquals("Shard " + i + " should have correct name", expectedShardName,
        shardPath.getName());
    }
  }
  ...
  public void testGetPreviousRound() {
    ...
    assertEquals("Previous round start time should be end time - round duration", expectedStart,
    ...
    assertEquals("Previous round start time should be end time - round duration", expectedStart2,
    ...
    assertEquals("Previous round start time should be end time - round duration", expectedStart3,
    ...
    assertEquals("Previous round end time should be start time of current round", expectedEnd4,
      previousRound4.getEndTime());
    assertEquals("Previous round start time should be end time - round duration", expectedStart4,
    ...
    assertEquals("Previous round duration should be 60 seconds", roundDurationMs,
  ...
  public void testGetNextRound() {
    ...
    assertEquals("Next round end time should be start time + round duration", expectedEnd,
    ...
    assertEquals("Next round end time should be start time + round duration", expectedEnd2,
    ...
    assertEquals("Next round end time should be start time + round duration", expectedEnd3,
    ...
    assertEquals("Next round start time should be end time of current round", expectedStart4,
      nextRound4.getStartTime());
    assertEquals("Next round end time should be start time + round duration", expectedEnd4,
    ...
    ReplicationRound originalRound =
      new ReplicationRound(dayStart + (3 * roundDurationMs), dayStart + (4 * roundDurationMs)); // 00:03:00
                                                                                                // to
                                                                                                // 00:04:00
