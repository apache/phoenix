...
public class ReplicationLogTrackerTest {
  ...
  @Test
  public void testMarkInProgressCollisionIncrementsMetricOnRenameFailure() throws IOException {
    // Initialize tracker
    tracker.init();
    ...
    // Force the claim rename to lose, as if another replayer claimed the same file first.
    Mockito.doReturn(false).when(mockFs).rename(Mockito.eq(originalFile), Mockito.any(Path.class));
  ...
  @Test
  public void testMarkInProgressNoCollisionOnRenameSuccess() throws IOException {
    // Initialize tracker
    tracker.init();
  ...
  public void testGetNewFilesWithStartAndEndRound() throws IOException {
    ...
    Path middleRoundFile2 =
      new Path(middleRoundShardDir, middleRoundStartTime + 30000 + "_rs4.plog");
    ...
    Path beforeStartRoundFile =
      new Path(beforeStartRoundShardDir, beforeStartRound.getStartTime() + "_rs0.plog");
    ...
    Path afterEndRoundFile =
      new Path(afterEndRoundShardDir, afterEndRound.getStartTime() + "_rs7.plog");
    ...
    assertEquals("Should return exactly 6 files from start to end rounds", expectedPaths.size(),
      actualPaths.size());
    assertEquals("File paths do not match", expectedPaths, actualPaths);
  ...
  public void testGetNewFilesWithSameStartAndEndRound() throws IOException {
    ...
    assertEquals("Should return exactly 3 files from the round", expectedPaths.size(),
      actualPaths.size());
    assertEquals("File paths do not match", expectedPaths, actualPaths);
  }
