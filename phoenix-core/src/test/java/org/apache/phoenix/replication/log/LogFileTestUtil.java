...
public interface LogFileTestUtil {
  ...
  static LogFile.Record newPutRecord(String table, long commitId, String rowKey, long ts,
    int numCols) {
    return new LogFileRecord().setMutation(newPut(rowKey, ts, numCols)).setHBaseTableName(table)
  ...
  static Put newPut(String rowKey, long ts, int numCols) {
    ...
    for (int i = 0; i < numCols; i++) {
      put.addColumn(Bytes.toBytes("col" + i), qualifier, ts, Bytes.toBytes("v" + i + "_" + rowKey));
    }
    return put;
  }
  ...
  static LogFile.Record newDeleteRecord(String table, long commitId, String rowKey, long ts,
    int numCols) {
    return new LogFileRecord().setMutation(newDelete(rowKey, ts, numCols)).setHBaseTableName(table)
      .setCommitId(commitId);
  }
  ...
  static LogFile.Record newDeleteColumnRecord(String table, long commitId, String rowKey, long ts,
    ...
    for (int i = 0; i < numCols; i++) {
      ...
      delete.add(CellBuilderFactory.create(CellBuilderType.SHALLOW_COPY)
        .setRow(Bytes.toBytes(rowKey)).setFamily(column).setQualifier(qualifier).setTimestamp(ts)
        .setType(Cell.Type.DeleteColumn).build());
    }
    LogFile.Record record =
      new LogFileRecord().setMutation(delete).setHBaseTableName(table).setCommitId(commitId);
    return record;
  }

  static LogFile.Record newDeleteFamilyRecord(String table, long commitId, String rowKey, long ts,
    int numCols) throws IOException {
    Delete delete = new Delete(Bytes.toBytes(rowKey));
    delete.setTimestamp(ts);
    for (int i = 0; i < numCols; i++) {
    ...
    public int read(long position, byte[] buffer, int offset, int length) throws IOException {
      ...
      if (offset < 0 || length < 0 || offset + length > buffer.length) {
        throw new IndexOutOfBoundsException(
          String.format("Offset %d or length %d is invalid for buffer of size %d", offset, length,
            buffer.length));
      }
      if (position < 0) {
    ...
    public void readFully(long position, byte[] buffer, int offset, int length) throws IOException {
      ...
      if (length > avail) {
        throw new EOFException(String.format(
          "Premature EOF from stream: expected %d bytes, but only %d available", length, avail));
      }
  ...
  static class SyncableByteArrayOutputStream extends OutputStream implements SyncableDataOutput {
