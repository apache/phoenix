...
public class ReplicationLogDiscoveryTest {
  ...
  private static final MetricsReplicationLogTracker metricsLogTracker =
  ...
  /**
   * Tests the start and stop lifecycle of ReplicationLogDiscovery. Validates scheduler
   * initialization, thread naming, and proper cleanup.
   */
  @Test
  ...
  public void testGetInProgressFileMaxRetries() {
    ...
    conf.setInt(ReplicationLogDiscovery.REPLICATION_IN_PROGRESS_FILE_MAX_RETRIES_KEY, 5);
    assertEquals("maxRetries should reflect configured value", 5,
      discovery.getInProgressFileMaxRetries());

    // Reset to verify it reads from conf each time
    conf.setInt(ReplicationLogDiscovery.REPLICATION_IN_PROGRESS_FILE_MAX_RETRIES_KEY, 3);
    assertEquals("maxRetries should reflect updated configured value", 3,
      discovery.getInProgressFileMaxRetries());
  }

  /**
   * Tests processRound with in-progress directory processing enabled. Validates that both new files
  ...
  public void testProcessRoundWithInProgressDirectoryProcessing() throws IOException {
    ...
    ReplicationRound differentRoundSameShard = new ReplicationRound(
      1704153600000L + (shardCount * 60 * 1000L), 1704153660000L + (shardCount * 60 * 1000L));
    ...
    ReplicationRound round0100 = new ReplicationRound(1704153660000L, 1704153720000L); // 00:01:00 -
                                                                                       // 00:02:00
    List<Path> round0100NewFiles = createNewFilesForRound(round0100, 2);
    ReplicationRound round0200 = new ReplicationRound(1704153720000L, 1704153780000L); // 00:02:00 -
                                                                                       // 00:03:00
    ...
    // 7. Ensure current round new files (3) are processed and in progress (4) are processed (Total
    // 7)
    List<Path> processedFiles = discovery.getProcessedFiles();
    assertEquals("Invalid number of files processed", 7, processedFiles.size());

    // Create set of expected files that should be processed (by prefix, since UUIDs are added
  ...
  /**
   * Tests processRound with in-progress directory processing disabled. Validates that only new
   * files for the current round are processed.
   */
  @Test
  public void testProcessRoundWithoutInProgressDirectoryProcessing() throws IOException {
    ...
    for (Path unexpectedFile : inProgressFiles0004) {
      ...
      assertFalse(
        "Should NOT have processed in-progress file from 00:00:04: " + unexpectedFile.getName(),
        actualProcessedFilePrefixes.contains(unexpectedPrefix));
    }
  ...
  /**
   * Tests the probability-based in-progress directory processing decision. Validates that the
   * actual probability matches the configured probability.
   */
  @Test
  public void testShouldProcessInProgressDirectory() {
    ...
    assertTrue(
      "Actual probability (" + actualProbability + "%) should be close to expected probability ("
        + expectedProbability + "%), variance: " + variance + "%",
      variance < 2.0);
  ...
  /**
   * Tests processing of new files for a specific round. Validates that only files belonging to the
   * current round are processed.
   */
  @Test
  public void testProcessNewFilesForRound() throws IOException {
    ...
    // Validate that in-progress files were NOT processed (processNewFilesForRound only processes
    // new files)
    for (Path unexpectedFile : inProgressFiles0004) {
      String unexpectedPrefix = extractPrefix(unexpectedFile.getName());
      assertFalse(
  ...
  public void testProcessNewFilesForRoundWithPartialFailure() throws IOException {
    ...
    // Mock processFile to throw exception for specific files (files 1 and 3) - using prefix
    // matching
    ...
    Mockito.verify(fileTracker, Mockito.never()).markCompleted(
      Mockito.argThat(path -> extractPrefix(path.getName()).equals(unexpectedPrefix1)));
    String unexpectedPrefix3 = newFilesForRound.get(3).getName().substring(0,
      newFilesForRound.get(3).getName().lastIndexOf("."));
    Mockito.verify(fileTracker, Mockito.never()).markCompleted(
      Mockito.argThat(path -> extractPrefix(path.getName()).equals(unexpectedPrefix3)));

    // Verify that markFailed was called for failed files
    Mockito.verify(fileTracker, Mockito.times(1))
      .markFailed(Mockito.argThat(path -> extractPrefix(path.getName()).equals(unexpectedPrefix1)));
    Mockito.verify(fileTracker, Mockito.times(1))
      .markFailed(Mockito.argThat(path -> extractPrefix(path.getName()).equals(unexpectedPrefix3)));

    // Verify that markFailed was NOT called for successfully processed files
    Mockito.verify(fileTracker, Mockito.never())
      .markFailed(Mockito.argThat(path -> extractPrefix(path.getName()).equals(expectedPrefix0)));
    Mockito.verify(fileTracker, Mockito.never())
      .markFailed(Mockito.argThat(path -> extractPrefix(path.getName()).equals(expectedPrefix2)));
    Mockito.verify(fileTracker, Mockito.never())
      .markFailed(Mockito.argThat(path -> extractPrefix(path.getName()).equals(expectedPrefix4)));
  }

  /**
   * Tests complete failure handling during new file processing. Validates that all failed files are
   * marked as failed with no completed files.
   */
  @Test
  public void testProcessNewFilesForRoundWithAllFailures() throws IOException {
    ...
    // Mock processFile to throw exception for all files (using prefix matching since files are
    ...
    for (Path failedFile : newFilesForRound) {
      String expectedPrefix =
        failedFile.getName().substring(0, failedFile.getName().lastIndexOf("."));
      Mockito.verify(fileTracker, Mockito.times(1))
        .markFailed(Mockito.argThat(path -> extractPrefix(path.getName()).equals(expectedPrefix)));
    }
  }

  /**
   * Tests processing of all files in the in-progress directory. Validates that only in-progress
   * files are processed, not new files.
   */
  @Test
  ...
  public void testProcessInProgressDirectoryWithIntermittentFailure() throws IOException {
    ...
    conf.setInt(ReplicationLogDiscovery.REPLICATION_IN_PROGRESS_FILE_MAX_RETRIES_KEY, 2);
    ...
    // Mock processFile to throw exception for specific files (files 1 and 3) only on first call,
    // succeed on retry
    ...
    // Inject a monotonically advancing clock so that the rename timestamp from markInProgress
    // is always strictly less than the threshold computed in the next loop iteration.
    // Without this, both can land on the same millisecond causing renameTs < threshold to be
    // false, which makes the retry file invisible to getOlderInProgressFiles.
    AtomicLong clock = new AtomicLong(EnvironmentEdgeManager.currentTime());
    EnvironmentEdgeManager.injectEdge(() -> clock.incrementAndGet());
  ...
  /**
   * Tests that with default maxRetries=1, files that fail processing are skipped for the rest of
   * the round and not retried. Verifies that successfully processed files are completed, while
   * failed files are only marked as failed without retry.
   */
  @Test
  public void testProcessInProgressDirectorySkipsFailedFiles() throws IOException {
  ...
  /**
   * Tests that when all in-progress files fail processing, the loop terminates without retrying any
   * file (default maxRetries=1). Verifies no infinite loop occurs and all files are marked as
   * failed.
   */
  @Test
  ...
  /**
   * Tests that the failure tracking is scoped per invocation of processInProgressDirectory. Files
   * that fail in the first invocation are skipped for that round, but get a fresh chance in the
  ...
  /**
   * Tests processing of in-progress directory when no files meet the rename timestamp criteria.
   * Validates that no files are processed when all files were recently renamed.
   */
  @Test
  public void testProcessInProgressDirectoryWithNoOldFiles() throws IOException {
    ...
    conf.setInt(ReplicationLogDiscovery.REPLICATION_IN_PROGRESS_FILE_MIN_AGE_SECONDS_KEY, 60);
    EnvironmentEdge edge = () -> currentTime;
    EnvironmentEdgeManager.injectEdge(edge);

    try {
      // Create files with recent rename timestamps (within the 60-second min age window)
      long recentRename1 = currentTime - 5000L; // renamed 5 seconds ago
      long recentRename2 = currentTime - 2000L; // renamed 2 seconds ago

      List<Path> recentFiles1 = createInProgressFiles(1704153600000L, 2, recentRename1);
      List<Path> recentFiles2 = createInProgressFiles(1704153600000L, 2, recentRename2);
      ...
      // Verify that no files were processed (all rename timestamps are too recent)
      assertEquals("Should not process any files when all files were recently renamed", 0,
        processedFiles.size());
  ...
  /**
   * Tests processing of in-progress directory with rename timestamp filtering. Validates that only
   * files whose rename timestamp is older than the configured minimum age are processed.
   */
  @Test
  public void testProcessInProgressDirectoryWithTimestampFiltering() throws IOException {
    ...
    try {
      ...
      long veryOldRename = currentTime - 120000L; // renamed 2 min ago - should be processed
      long oldRename = currentTime - 70000L; // renamed 70s ago - should be processed
      long recentRename = currentTime - 5000L; // renamed 5s ago - should NOT be processed
      long veryRecentRename = currentTime - 2000L; // renamed 2s ago - should NOT be processed

      List<Path> veryOldFiles = createInProgressFiles(1704153500000L, 1, veryOldRename);
      List<Path> oldFiles = createInProgressFiles(1704153500001L, 1, oldRename);
      List<Path> recentFiles = createInProgressFiles(1704153500002L, 1, recentRename);
      List<Path> veryRecentFiles = createInProgressFiles(1704153500003L, 1, veryRecentRename);
      ...
      assertEquals("Should process only old files based on rename timestamp filtering", 2,
        processedFiles.size());

      // Create set of expected processed files (by prefix, since UUIDs are updated during
      // markInProgress)
      Set<String> expectedProcessedFilePrefixes = new HashSet<>();
      ...
      for (Path file : recentFiles) {
        ...
        assertFalse("Recent files should not be processed due to rename timestamp filtering: "
          + file.getName(), actualProcessedFilePrefixes.contains(unexpectedPrefix));
      }
  ...
  public void testGetMinTimestampFromInProgressFilesSingleFile() throws IOException {
    ...
    assertEquals("Should return the timestamp of the single file", Long.valueOf(1704153600000L),
      result.get());
  ...
  public void testGetMinTimestampFromInProgressFilesMultipleFiles() throws IOException {
    ...
    assertEquals("Should return the minimum timestamp", Long.valueOf(1704153600000L), result.get());
  ...
  @Test
  public void testInitializeLastRoundInSync_MultipleInProgressFiles_NoNewFiles()
    throws IOException {
    // Create multiple in-progress files with timestamps
    Path inProgressFile1 = new Path(testFolderPath, "1704067200000_rs1.plog"); // 2024-01-01
                                                                               // 00:00:00
    Path inProgressFile2 = new Path(testFolderPath, "1704067260000_rs2.plog"); // 2024-01-01
                                                                               // 00:01:00
    Path inProgressFile3 = new Path(testFolderPath, "1704067320000_rs3.plog"); // 2024-01-01
                                                                               // 00:02:00
    ...
    long expectedEndTime =
      (1704067200000L / TimeUnit.MINUTES.toMillis(1)) * TimeUnit.MINUTES.toMillis(1); // Round down
                                                                                      // to nearest
                                                                                      // 60-second
                                                                                      // boundary
    long expectedStartTime = expectedEndTime - TimeUnit.MINUTES.toMillis(1); // Start time is 60
                                                                             // seconds less than
                                                                             // end time
    assertEquals("Should use minimum timestamp from in-progress files", expectedStartTime,
      result.getStartTime());
    assertEquals("End time must be rounded down to nearest 60-second boundary", expectedEndTime,
      result.getEndTime());
  }
  ...
  public void testInit_MultipleInProgressFiles_OneNewFile() throws IOException {
    // Create multiple in-progress files with timestamps
    Path inProgressFile1 = new Path(testFolderPath, "1704153600000_rs1.plog"); // 2024-01-02
                                                                               // 00:00:00
    Path inProgressFile2 = new Path(testFolderPath, "1704153660000_rs2.plog"); // 2024-01-02
                                                                               // 00:01:00
    Path inProgressFile3 = new Path(testFolderPath, "1704153720000_rs3.plog"); // 2024-01-02
                                                                               // 00:02:00
    ...
    long expectedEndTime =
      (1704153600000L / TimeUnit.MINUTES.toMillis(1)) * TimeUnit.MINUTES.toMillis(1); // Round down
                                                                                      // to nearest
                                                                                      // 60-second
                                                                                      // boundary
    long expectedStartTime = expectedEndTime - TimeUnit.MINUTES.toMillis(1); // Start time is 60
  ...
  public void testInit_MultipleInProgressFiles_MultipleNewFiles() throws IOException {
    // Create multiple in-progress files with timestamps
    Path inProgressFile1 = new Path(testFolderPath, "1704240000000_rs1.plog"); // 2024-01-03
                                                                               // 00:00:00
    Path inProgressFile2 = new Path(testFolderPath, "1704240060000_rs2.plog"); // 2024-01-03
                                                                               // 00:01:00
    Path inProgressFile3 = new Path(testFolderPath, "1704240120000_rs3.plog"); // 2024-01-03
                                                                               // 00:02:00
    ...
    long expectedEndTime =
      (1704240000000L / TimeUnit.MINUTES.toMillis(1)) * TimeUnit.MINUTES.toMillis(1); // Round down
                                                                                      // to nearest
                                                                                      // 60-second
                                                                                      // boundary
    long expectedStartTime = expectedEndTime - TimeUnit.MINUTES.toMillis(1); // Start time is 60
  ...
  public void testInit_OneInProgressFile_NoNewFiles() throws IOException {
    ...
    long expectedEndTime =
      (1704326400000L / TimeUnit.MINUTES.toMillis(1)) * TimeUnit.MINUTES.toMillis(1); // Round down
                                                                                      // to nearest
                                                                                      // 60-second
                                                                                      // boundary
    long expectedStartTime = expectedEndTime - TimeUnit.MINUTES.toMillis(1); // Start time is 60
                                                                             // seconds less than
                                                                             // end time
    assertEquals("Should use timestamp from single in-progress file", expectedStartTime,
      result.getStartTime());
    assertEquals("End time must be rounded down to nearest 60-second boundary", expectedEndTime,
      result.getEndTime());
  }
  ...
  public void testInit_OneInProgressFile_OneNewFile() throws IOException {
    ...
    long expectedEndTime =
      (1704412800000L / TimeUnit.MINUTES.toMillis(1)) * TimeUnit.MINUTES.toMillis(1); // Round down
                                                                                      // to nearest
                                                                                      // 60-second
                                                                                      // boundary
    long expectedStartTime = expectedEndTime - TimeUnit.MINUTES.toMillis(1); // Start time is 60
                                                                             // seconds less than
                                                                             // end time
    assertEquals("Should use timestamp from in-progress file", expectedStartTime,
      result.getStartTime());
    assertEquals("End time must be rounded down to nearest 60-second boundary", expectedEndTime,
      result.getEndTime());
  }
  ...
  public void testInit_OneInProgressFile_MultipleNewFiles() throws IOException {
    ...
    long expectedEndTime =
      (1704499200000L / TimeUnit.MINUTES.toMillis(1)) * TimeUnit.MINUTES.toMillis(1); // Round down
                                                                                      // to nearest
                                                                                      // 60-second
                                                                                      // boundary
    long expectedStartTime = expectedEndTime - TimeUnit.MINUTES.toMillis(1); // Start time is 60
  ...
  public void testInit_NoInProgressFiles_MultipleNewFiles() throws IOException {
    ...
    long expectedEndTime =
      (1704585600000L / TimeUnit.MINUTES.toMillis(1)) * TimeUnit.MINUTES.toMillis(1); // Round down
                                                                                      // to nearest
                                                                                      // 60-second
                                                                                      // boundary
    long expectedStartTime = expectedEndTime - TimeUnit.MINUTES.toMillis(1); // Start time is 60
                                                                             // seconds less than
                                                                             // end time
    assertEquals("Should use minimum timestamp from new files when no in-progress files",
      expectedStartTime, result.getStartTime());
    assertEquals("End time must be rounded down to nearest 60-second boundary", expectedEndTime,
      result.getEndTime());
  }
  ...
  public void testInit_NoInProgressFiles_OneNewFile() throws IOException {
    ...
    long expectedEndTime =
      (1704672000000L / TimeUnit.MINUTES.toMillis(1)) * TimeUnit.MINUTES.toMillis(1); // Round down
                                                                                      // to nearest
                                                                                      // 60-second
                                                                                      // boundary
    long expectedStartTime = expectedEndTime - TimeUnit.MINUTES.toMillis(1); // Start time is 60
                                                                             // seconds less than
                                                                             // end time
    assertEquals("Should use timestamp from single new file when no in-progress files",
      expectedStartTime, result.getStartTime());
    assertEquals("End time must be rounded down to nearest 60-second boundary", expectedEndTime,
      result.getEndTime());
  }
  ...
  public void testInit_NoInProgressFiles_NoNewFiles() throws IOException {
    ...
    try {
      ...
      long expectedEndTime =
        (1704758400000L / TimeUnit.MINUTES.toMillis(1)) * TimeUnit.MINUTES.toMillis(1); // Round
                                                                                        // down to
                                                                                        // nearest
                                                                                        // 60-second
                                                                                        // boundary
      long expectedStartTime = expectedEndTime - TimeUnit.MINUTES.toMillis(1); // Start time is 60
                                                                               // seconds less than
                                                                               // end time
      assertEquals("Should use custom time when no files are found", expectedStartTime,
        result.getStartTime());
      assertEquals("End time must be rounded down to nearest 60-second boundary", expectedEndTime,
        result.getEndTime());
    } finally {
  ...
  /**
   * Tests that files older than the configured minimum age are processed immediately without any
   * sleep. All files have timestamps well before the threshold.
   */
  @Test
  public void testProcessNewFilesForRoundWithOldFiles() throws IOException {
    long roundStart = 1704153600000L;
    long roundEnd = 1704153660000L;
    ReplicationRound replicationRound = new ReplicationRound(roundStart, roundEnd);

    // Inject a clock that makes all files old enough (current time = roundStart + 120s)
    long currentTime = roundStart + 120000L;
    EnvironmentEdgeManager.injectEdge(() -> currentTime);

    try {
      List<Path> newFiles = createNewFilesForRound(replicationRound, 3);
      discovery.processNewFilesForRound(replicationRound);

      List<Path> processedFiles = discovery.getProcessedFiles();
      assertEquals("All old files should be processed", 3, processedFiles.size());

      // Verify markInProgress was called for each file by prefix
      Mockito.verify(fileTracker, Mockito.times(3)).markInProgress(Mockito.any(Path.class));
      for (Path expectedFile : newFiles) {
        Mockito.verify(fileTracker, Mockito.times(1)).markInProgress(Mockito
          .argThat(path -> path.getName().startsWith(expectedFile.getName().split("\\.")[0])));
      }

      // Verify markCompleted was called for each file by prefix
      Mockito.verify(fileTracker, Mockito.times(3)).markCompleted(Mockito.any(Path.class));
      for (Path expectedFile : newFiles) {
        String expectedPrefix =
          expectedFile.getName().substring(0, expectedFile.getName().lastIndexOf("."));
      ...
      // Verify waitForFileAge was never called since all files were old enough
      assertEquals("Should never sleep when all files are old enough", 0,
        discovery.getSleepDurations().size());
    } finally {
      EnvironmentEdgeManager.reset();
    }
  }
  ...
  /**
   * Tests that when a mix of old and young files exist, only the old file is processed first. The
   * young file is deferred and processed only after waitForFileAge advances the clock past the
   * threshold. Verifies processing order and exact sleep duration.
   */
  @Test
  public void testProcessNewFilesForRoundWithMixedAgeFiles() throws IOException {
    long roundStart = 1704153600000L;
    long roundEnd = 1704153660000L;
    long minAgeMs = 69000L; // 60s + 15%
    ReplicationRound replicationRound = new ReplicationRound(roundStart, roundEnd);

    // Create files at roundStart (old) and roundStart + 50s (young at first)
    ReplicationShardDirectoryManager shardManager =
      fileTracker.getReplicationShardDirectoryManager();
    Path shardPath = shardManager.getShardDirectory(roundStart);
    localFs.mkdirs(shardPath);

    String oldFilePrefix = roundStart + "_rs-0";
    Path oldFile = new Path(shardPath, oldFilePrefix + ".plog");
    localFs.create(oldFile, true).close();
    long youngFileTimestamp = roundStart + 50000L;
    String youngFilePrefix = youngFileTimestamp + "_rs-1";
    Path youngFile = new Path(shardPath, youngFilePrefix + ".plog");
    localFs.create(youngFile, true).close();

    // Clock starts at roundStart + 80s:
    // oldFile age = 80s >= 69s (eligible)
    // youngFile age = 30s < 69s (too young)
    AtomicLong clock = new AtomicLong(roundStart + 80000L);
    EnvironmentEdgeManager.injectEdge(clock::get);
    // waitForFileAge will advance the clock by the sleep duration
    discovery.setTestClock(clock);

    try {
      discovery.processNewFilesForRound(replicationRound);

      List<Path> processedFiles = discovery.getProcessedFiles();
      assertEquals("Both files should be processed", 2, processedFiles.size());

      // Verify ordering: old file processed first, young file second
      assertEquals("Old file should be processed first", oldFilePrefix,
        extractPrefix(processedFiles.get(0).getName()));
      assertEquals("Young file should be processed second", youngFilePrefix,
        extractPrefix(processedFiles.get(1).getName()));

      // Verify waitForFileAge was called exactly once with the correct duration
      // Expected: youngFileTimestamp + minAge - clockAtSleepTime
      // clockAtSleepTime = roundStart + 80000 (old file was processed but clock doesn't advance)
      // sleepTime = (roundStart + 50000) + 69000 - (roundStart + 80000) = 39000ms
      List<Long> sleepDurations = discovery.getSleepDurations();
      assertEquals("Should have slept exactly once", 1, sleepDurations.size());
      long expectedSleepTime = youngFileTimestamp + minAgeMs - (roundStart + 80000L);
      assertEquals("Sleep duration should be exactly the time needed for young file to age",
        expectedSleepTime, sleepDurations.get(0).longValue());

      // Verify markInProgress was called for each file by prefix
      Mockito.verify(fileTracker, Mockito.times(2)).markInProgress(Mockito.any(Path.class));
      Mockito.verify(fileTracker, Mockito.times(1))
        .markInProgress(Mockito.argThat(path -> path.getName().startsWith(oldFilePrefix)));
      Mockito.verify(fileTracker, Mockito.times(1))
        .markInProgress(Mockito.argThat(path -> path.getName().startsWith(youngFilePrefix)));

      // Verify markCompleted was called for each file by prefix
      Mockito.verify(fileTracker, Mockito.times(2)).markCompleted(Mockito.any(Path.class));
      Mockito.verify(fileTracker, Mockito.times(1)).markCompleted(
        Mockito.argThat(path -> extractPrefix(path.getName()).equals(oldFilePrefix)));
      Mockito.verify(fileTracker, Mockito.times(1)).markCompleted(
        Mockito.argThat(path -> extractPrefix(path.getName()).equals(youngFilePrefix)));
    } finally {
      EnvironmentEdgeManager.reset();
    }
  }
  ...
  /**
   * Tests that when all files are too young, they are processed in batches as they age past the
   * threshold via waitForFileAge advancing the clock. Three files with staggered timestamps: file0
   * (earliest) becomes eligible after the first sleep, file1 and file2 (same later timestamp)
   * become eligible together after the second sleep. Verifies exact sleep durations.
   */
  @Test
  public void testProcessNewFilesForRoundAllTooYoung() throws IOException {
    long roundStart = 1704153600000L;
    long roundEnd = 1704153660000L;
    long minAgeMs = 69000L; // 60s + 15%
    ReplicationRound replicationRound = new ReplicationRound(roundStart, roundEnd);

    // Create 3 files: file0 at roundStart, file1 and file2 at roundStart + 5000
    ReplicationShardDirectoryManager shardManager =
      fileTracker.getReplicationShardDirectoryManager();
    Path shardPath = shardManager.getShardDirectory(roundStart);
    localFs.mkdirs(shardPath);

    long laterTimestamp = roundStart + 5000L;
    String file0Prefix = roundStart + "_rs-0";
    Path file0 = new Path(shardPath, file0Prefix + ".plog");
    localFs.create(file0, true).close();
    String file1Prefix = laterTimestamp + "_rs-1";
    Path file1 = new Path(shardPath, file1Prefix + ".plog");
    localFs.create(file1, true).close();
    String file2Prefix = laterTimestamp + "_rs-2";
    Path file2 = new Path(shardPath, file2Prefix + ".plog");
    localFs.create(file2, true).close();
    List<Path> newFiles = Arrays.asList(file0, file1, file2);

    // Clock at roundStart + 10s: all files too young
    // file0 age = 10s, file1/file2 age = 5s — all < 69s
    AtomicLong clock = new AtomicLong(roundStart + 10000L);
    EnvironmentEdgeManager.injectEdge(clock::get);
    discovery.setTestClock(clock);
    ...
    try {
      ...
      List<Path> processedFiles = discovery.getProcessedFiles();
      assertEquals("All 3 files should be processed after aging", 3, processedFiles.size());

      // Sleep 1: based on file0 (oldest young file)
      // sleepTime = roundStart + 69000 - (roundStart + 10000) = 59000ms
      // clock advances to roundStart + 69000
      // file0 age = 69000 >= 69000 (eligible), file1/file2 age = 64000 < 69000 (still young)
      // After processing file0:
      // Sleep 2: based on file1/file2 (oldest young = laterTimestamp)
      // sleepTime = (roundStart + 5000) + 69000 - (roundStart + 69000) = 5000ms
      // clock advances to roundStart + 74000
      // file1/file2 age = 69000 >= 69000 (eligible)
      List<Long> sleepDurations = discovery.getSleepDurations();
      assertEquals("Should have slept exactly twice", 2, sleepDurations.size());
      long expectedSleep1 = roundStart + minAgeMs - (roundStart + 10000L); // 59000
      assertEquals("First sleep should be based on the oldest young file (file0)", expectedSleep1,
        sleepDurations.get(0).longValue());
      long expectedSleep2 = laterTimestamp + minAgeMs - (roundStart + 69000L); // 5000
      assertEquals("Second sleep should be based on file1/file2 timestamp", expectedSleep2,
        sleepDurations.get(1).longValue());
  ...
  /**
   * Tests that the new file age buffer percentage configuration is respected.
   */
  @Test
  public void testNewFileMinAgeConfiguration() {
    // Default: 15%
    conf.setDouble(ReplicationLogDiscovery.REPLICATION_NEW_FILE_AGE_BUFFER_PERCENTAGE_KEY, 15.0);
    long expectedMinAge = 60000L + (long) (60000L * 15.0 / 100.0); // 69000ms
    assertEquals("Min age should be roundTime + 15%", expectedMinAge,
      discovery.getNewFileMinAgeMs());

    // Custom: 50%
    conf.setDouble(ReplicationLogDiscovery.REPLICATION_NEW_FILE_AGE_BUFFER_PERCENTAGE_KEY, 50.0);
    long expectedMinAge50 = 60000L + (long) (60000L * 50.0 / 100.0); // 90000ms
    assertEquals("Min age should be roundTime + 50%", expectedMinAge50,
      discovery.getNewFileMinAgeMs());

    // Zero: disabled
    conf.setDouble(ReplicationLogDiscovery.REPLICATION_NEW_FILE_AGE_BUFFER_PERCENTAGE_KEY, 0.0);
    assertEquals("Min age should be roundTime when buffer is 0%", 60000L,
      discovery.getNewFileMinAgeMs());
  }

  /**
   * Tests that processNewFilesForRound completes without error when no files exist for the round.
   */
  @Test
  public void testProcessNewFilesForRoundNoFiles() throws IOException {
    long roundStart = 1704153600000L;
    long roundEnd = 1704153660000L;
    ReplicationRound replicationRound = new ReplicationRound(roundStart, roundEnd);

    // Don't create any files - the round directory doesn't exist
    discovery.processNewFilesForRound(replicationRound);

    List<Path> processedFiles = discovery.getProcessedFiles();
    assertEquals("No files should be processed when no files exist", 0, processedFiles.size());
    Mockito.verify(fileTracker, Mockito.never()).markInProgress(Mockito.any(Path.class));
    Mockito.verify(fileTracker, Mockito.never()).markCompleted(Mockito.any(Path.class));
    assertEquals("Should never sleep when no files exist", 0, discovery.getSleepDurations().size());
  }

  private List<Path> createNewFilesForRound(ReplicationRound replicationRound, int fileCount)
    throws IOException {
    // Create files for multiple rounds in the same shard with each file at gap of 2 seconds
    Preconditions.checkArgument(fileCount
    ...
    for (int i = 0; i < fileCount; i++) {
      Path file =
        new Path(shardPath, replicationRound.getStartTime() + (1234L * i) + "_rs-" + i + ".plog");
  ...
  /**
   * Extracts the stable prefix (<timestamp>_<server>) from a filename. Works for all formats:
  ...
  private List<Path> createInProgressFiles(long timestamp, int count, long renameTimestamp)
    throws IOException {
    Path inProgressDir = fileTracker.getInProgressDirPath();
  ...
  public void testReplay() throws IOException {
    ...
    discovery
      .setLastRoundProcessed(new ReplicationRound(initialEndTime - roundTimeMills, initialEndTime));
    ...
    try {
      ...
      assertEquals("Should not process any rounds when not enough time has passed", 0,
    ...
    try {
      ...
      ReplicationRound expectedRound =
        new ReplicationRound(initialEndTime, initialEndTime + roundTimeMills);
      assertEquals("Processed round should match expected round", expectedRound,
        processedRounds.get(0));
      assertEquals("Last round processed should be updated", expectedRound,
    ...
    try {
      ...
      // Verify third round
      ReplicationRound expectedRound3 = new ReplicationRound(initialEndTime + (2 * roundTimeMills),
      ...
      assertEquals("Last round processed should be updated to third round", expectedRound3,
        discovery.getLastRoundProcessed());
    } finally {
      EnvironmentEdgeManager.reset();
    }
    ...
    ReplicationRound round2 =
      new ReplicationRound(initialEndTime + roundTimeMills, initialEndTime + (2 * roundTimeMills));
    ReplicationRound round3 = new ReplicationRound(initialEndTime + (2 * roundTimeMills),
      initialEndTime + (3 * roundTimeMills));
    ...
    try {
      ...
      assertEquals("Last round processed should be updated to first round only", expectedRound1,
        discovery.getLastRoundProcessed());
    } finally {
      EnvironmentEdgeManager.reset();
      // Reset the mock
    ...
    try {
      discovery.replay();
      List<ReplicationRound> processedRounds = discovery.getProcessedRounds();
      assertEquals("Should not process any rounds when 1ms before threshold", 0,
        processedRounds.size());
    } finally {
      EnvironmentEdgeManager.reset();
    }
  ...
  public void testGetNextRoundToProcess() {
    ...
    discovery.setLastRoundProcessed(
      new ReplicationRound(lastRoundEndTimestamp - roundTimeMills, lastRoundEndTimestamp));
    ...
    try {
      ...
      ReplicationRound expectedRound =
        new ReplicationRound(lastRoundEndTimestamp, lastRoundEndTimestamp + roundTimeMills);
    ...
    try {
      ...
      assertFalse("Should return empty when halfway through buffer time", result8.isPresent());
    } finally {
      EnvironmentEdgeManager.reset();
    }
  }
  ...
  /**
   * Testable implementation of ReplicationLogTracker for unit testing. Exposes protected methods
   * and provides minimal implementation for testing.
   */
  ...
  /**
   * Testable implementation of ReplicationLogDiscovery for unit testing. Tracks processed files and
   * rounds, and provides access to protected methods.
   */
  private static class TestableReplicationLogDiscovery extends ReplicationLogDiscovery {
    ...
    @Override
    public void waitForFileAge(long sleepTimeMs) {
      sleepDurations.add(sleepTimeMs);
      if (testClock != null) {
        testClock.addAndGet(sleepTimeMs);
      }
    }

    public void setTestClock(AtomicLong clock) {
      this.testClock = clock;
    }

    public List<Long> getSleepDurations() {
      return new ArrayList<>(sleepDurations);
    }
