  ...
  public void testChildrenListConstructors() throws Exception {
    ...
    for (int i = 0; i < types.length; i++) {
      try {
        ...
        if (
          !Modifier.isAbstract(expressionClass.getModifiers())
            && (ScalarFunction.class.isAssignableFrom(expressionClass))
            && (expressionClass != UDFExpression.class)
        ) {
          Method cloneMethod = expressionClass.getMethod("clone", List.class);
          assertNotNull(cloneMethod);
          // ScalarFunctions that implement clone(List<Expression>) don't need to implement a
          // constructor that takes a List<Expression>
          if (cloneMethod.getDeclaringClass() == ScalarFunction.class) {
            Constructor cons = expressionClass.getDeclaredConstructor(List.class);
            assertTrue("Constructor for " + expressionClass + " is not public",
              Modifier.isPublic(cons.getModifiers()));
          }
        }
      } catch (Exception e) {
        throw new RuntimeException("Unable to find required List<Expression> constructor "
          + types[i].getExpressionClass().getName(), e);
      }
    }
  }
