...
public class UpperFunctionTest {
  ...
  private static ImmutableMap<String, String> turkishLowerToUpperCaseMap =
    ImmutableMap.of("\u0069", "\u0130", "\u0131", "\u0049");

  private static ImmutableMap<String, String> anyLocaleLowerToUpperCaseMap =
    ImmutableMap.of("\u00df", "\u0053\u0053", "Fahrvergnügen", "FAHRVERGNÜGEN");
  ...
  private void testLowerToUpperCaseMap(Map<String, String> lowerToUpperMap, String locale)
    throws Exception {
    for (Map.Entry<String, String> lowerUpperPair : lowerToUpperMap.entrySet()) {
      ...
      assertEquals("Result of calling UpperFunction[ASC] on [" + lowerUpperPair.getKey() + "]["
        + locale + "] not as expected.", lowerUpperPair.getValue(), upperCaseResultAsc);
      assertEquals("Result of calling UpperFunction[DESC] on [" + lowerUpperPair.getKey() + "]["
        + locale + "] not as expected.", lowerUpperPair.getValue(), upperCaseResultDesc);
    }
  }

  private static String callFunction(String inputStr, String localeIsoCode, SortOrder sortOrder)
    throws Exception {
    ...
    List<Expression> expressions =
      Lists.newArrayList((Expression) inputStrLiteral, (Expression) localeIsoCodeLiteral);
    Expression upperFunction = new UpperFunction(expressions);
    ImmutableBytesWritable ptr = new ImmutableBytesWritable();
    boolean ret = upperFunction.evaluate(null, ptr);
    String result =
      ret ? (String) upperFunction.getDataType().toObject(ptr, upperFunction.getSortOrder()) : null;
    return result;
  }
}
