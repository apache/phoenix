...
public class CollationKeyFunctionTest {
  ...
  private static String[] chineseChars =
    new String[] { "\u963f", "\u55c4", "\u963e", "\u554a", "\u4ec8", "\u3d9a", "\u9f51" };

  private static Comparator<byte[]> collationKeyComparator =
    UnsignedBytes.lexicographicalComparator();

  private static Comparator<ByteArrayAndInteger> collationKeyAndIndexComparator =
    new Comparator<ByteArrayAndInteger>() {
      @Override
  ...
  public void testZhSort() throws Exception {
    testSortOrderNoEquals(chineseChars, "zh", Boolean.FALSE, null, null,
  ...
  public void testZhTwSort() throws Exception {
    testSortOrderNoEquals(chineseChars, "zh_TW", Boolean.FALSE, null, null,
      new Integer[] { 4, 3, 1, 5, 2, 0, 6 });
  }
  ...
  public void testUpperCaseCollationKeyBytes() throws Exception {
    testCollationKeysEqual(new String[] { "abcdef", "ABCDEF", "aBcDeF" }, "en", Boolean.TRUE, null,
      null);
  }
  ...
  public void testCollationKeyBytesForSecondaryStrength() throws Exception {
    ...
    testSortOrderNoEquals(new String[] { "b", "a", "ä" }, "en", Boolean.FALSE, Collator.SECONDARY,
      null, new Integer[] { 1, 2, 0 });
  }

  @Test
  public void testCollationKeyBytesForTertiaryStrength() throws Exception {
    // none of these are considered equivalent
    testSortOrderNoEquals(new String[] { "b", "a", "ä", "A" }, "en", Boolean.FALSE,
      Collator.TERTIARY, null, new Integer[] { 1, 3, 2, 0 });
  }

  /**
   * Just test that changing the decomposition mode works for basic sorting. TODO: Actually test for
   * the accented characters and languages where this actually matters.
   */
  @Test
  public void testCollationKeyBytesForFullDecomposition() throws Exception {
    testCollationKeysEqual(new String[] { "a", "A" }, "en", Boolean.FALSE, null,
      Collator.FULL_DECOMPOSITION);
  }

  /** HELPER METHODS **/
  private void testSortOrderNoEquals(String[] inputStrings, String locale,
    Boolean uppercaseCollator, Integer strength, Integer decomposition, Integer[] expectedOrder)
    throws Exception {
    List<ByteArrayAndInteger> sortedCollationKeysAndIndexes =
      calculateCollationKeys(inputStrings, locale, uppercaseCollator, strength, decomposition);
  ...
  private List<ByteArrayAndInteger> calculateCollationKeys(String[] inputStrings, String locale,
    ...
    for (int i = 0; i < inputStrings.length; i++) {
      byte[] thisCollationKeyBytes = callFunction(inputStrings[i], locale, upperCaseCollator,
        strength, decomposition, SortOrder.ASC);
      collationKeysAndIndexes.add(new ByteArrayAndInteger(thisCollationKeyBytes, i));
    }
    return collationKeysAndIndexes;
  }

  private void testCollationKeysEqual(String[] inputStrings, String locale,
    Boolean upperCaseCollator, Integer strength, Integer decomposition) throws Exception {
    ...
    for (int i = 0, j = 1; i < inputStrings.length && j < inputStrings.length; i++, j++) {
      ...
      if (!isPairEqual) {
        fail(
          String.format("Collation keys for inputStrings [%s] and [%s] ([%s], [%s]) were not equal",
  ...
  private void testCollationKeysNotEqual(String[] inputStrings,
    List<ByteArrayAndInteger> collationKeysAndIndexes) throws Exception {
    for (int i = 0; i < inputStrings.length; i++) {
      for (int j = i + 1; j < inputStrings.length; j++) {
        ...
        if (isPairEqual) {
          fail(
            String.format("Collation keys for inputStrings [%s] and [%s] ([%s], [%s]) were equal",
              inputStrings[i], inputStrings[j], Hex.encodeHexString(iByteArray),
              Hex.encodeHexString(jByteArray)));
        }
      }
  ...
  private static byte[] callFunction(String inputStr, String localeIsoCode,
    Boolean upperCaseCollator, Integer strength, Integer decomposition, SortOrder sortOrder)
    throws Exception {
    LiteralExpression inputStrLiteral, localeIsoCodeLiteral, upperCaseBooleanLiteral,
        strengthLiteral, decompositionLiteral;
    ...
    return callFunction(inputStrLiteral, localeIsoCodeLiteral, upperCaseBooleanLiteral,
      strengthLiteral, decompositionLiteral);

  }

  private static byte[] callFunction(LiteralExpression inputStrLiteral,
    LiteralExpression localeIsoCodeLiteral, LiteralExpression upperCaseBooleanLiteral,
    LiteralExpression strengthLiteral, LiteralExpression decompositionLiteral) throws Exception {
    List<Expression> expressions = Lists.newArrayList((Expression) inputStrLiteral,
      (Expression) localeIsoCodeLiteral, (Expression) upperCaseBooleanLiteral,
      (Expression) strengthLiteral, (Expression) decompositionLiteral);
    ...
    byte[] result = ret
      ? (byte[]) collationKeyFunction.getDataType().toObject(ptr,
