...
public class NullValueTest extends BaseConnectionlessQueryTest {
  ...
  public void testComparisonExpressionWithNullOperands() throws Exception {
    String[] query = { "SELECT 'a' >= ''", "SELECT '' < 'a'", "SELECT '' = ''" };
  ...
  public void testAndExpressionWithNullOperands() throws Exception {
    String[] query = { "SELECT 'b' >= 'a' and '' < 'b'", "SELECT 'b' >= '' and 'a' < 'b'",
      "SELECT 'a' >= 'b' and 'a' < ''", "SELECT '' >= 'a' and 'b' < 'a'",
      "SELECT 'a' >= '' and '' < 'a'" };
    Boolean[] result = { null, null, Boolean.FALSE, Boolean.FALSE, null };
    Properties props = PropertiesUtil.deepCopy(TEST_PROPERTIES);
    Connection conn = DriverManager.getConnection(getUrl(), props);
    try {
  ...
  public void testOrExpressionWithNullOperands() throws Exception {
    String[] query = { "SELECT 'b' >= 'a' or '' < 'b'", "SELECT 'b' >= '' or 'a' < 'b'",
      "SELECT 'a' >= 'b' or 'a' < ''", "SELECT '' >= 'a' or 'b' < 'a'",
      "SELECT 'a' >= '' or '' < 'a'" };
    Boolean[] result = { Boolean.TRUE, Boolean.TRUE, null, null, null };
    Properties props = PropertiesUtil.deepCopy(TEST_PROPERTIES);
    Connection conn = DriverManager.getConnection(getUrl(), props);
    try {
      for (int i = 0; i < query.length; i++) {
