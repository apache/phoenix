...
public class ILikeExpressionTest {
    private boolean testExpression (String value, String expression, SortOrder sortorder)
            throws SQLException {
    LiteralExpression v = new LiteralExpression.BuilderA().setValue(value).setDataType(PVarchar.INSTANCE).setSortOrder(sortorder).build();
    LiteralExpression p = new LiteralExpression.BuilderA().setValue(expression).setDataType(PVarchar.INSTANCE).setSortOrder(sortorder).build();
