...
public class ILikeExpressionTest {
    private boolean testExpression (String value, String expression, SortOrder sortorder)
            throws SQLException {
    LiteralExpression v = new LiteralExpression.Builder().setValue(value).setDataType(PVarchar.INSTANCE).setSortOrder(sortorder).build();
    LiteralExpression p = new LiteralExpression.Builder().setValue(expression).setDataType(PVarchar.INSTANCE).setSortOrder(sortorder).build();
