...
public class WALRecoveryRegionPostOpenIT extends BaseTest {
  ...
  private static volatile Multimap<HTableInterfaceReference, Mutation> tableReferenceToMutation =
    null;
    ...
    public void preBatchMutate(ObserverContext<RegionCoprocessorEnvironment> observerContext,
      ...
      if (
        observerContext.getEnvironment().getRegion().getRegionInfo().getTable().getNameAsString()
          .contains(INDEX_TABLE_NAME) && failIndexTableWrite
      ) {
  ...
  public void testRecoveryRegionPostOpen() throws Exception {
    ...
    try (Connection conn = driver.connect(url, props)) {
      ...
      PreparedStatement stmt =
        conn.prepareStatement("UPSERT INTO " + DATA_TABLE_NAME + " VALUES(?,?,?)");
      stmt.setString(1, "a");
      stmt.setString(2, "x");
      stmt.setString(3, "1");
      stmt.execute();
      ...
      // begin to kill data table regionServer,and data table region would move to the other
      // regionSever,
      ...
      HRegionServer liveRegionServer =
        miniHBaseCluster.getLiveRegionServerThreads().get(0).getRegionServer();

      // verify handleFailure is called.
      handleFailureCountDownLatch.await();
      assertTrue(handleFailureCalledCount == 1);
      Map<HTableInterfaceReference, Collection<Mutation>> tableReferenceToMutations =
        tableReferenceToMutation.asMap();
      assertEquals("tableReferenceToMutation.size()", 1, tableReferenceToMutations.size());
      Iterator<Map.Entry<HTableInterfaceReference, Collection<Mutation>>> iter =
        tableReferenceToMutations.entrySet().iterator();
  ...
  private ServerName getRegionServerName(MiniHBaseCluster miniHBaseCluster, String tableName)
    ...
    return miniHBaseCluster.getServerHoldingRegion(TableName.valueOf(tableName),
  ...
  private void moveIndexTableRegionIfSameRegionSErver(MiniHBaseCluster miniHBaseCluster)
    ...
    int indexTableRegionServerIndex =
      miniHBaseCluster.getServerWith(indexTableRegion.getRegionInfo().getRegionName());
  ...
  private void moveRegionAndWait(MiniHBaseCluster miniHBaseCluster, HRegion destRegion,
    ...
    getUtility().getHBaseAdmin().move(destRegion.getRegionInfo().getEncodedNameAsBytes(),
      Bytes.toBytes(destRegionServer.getServerName().getServerName()));
    while (true) {
      ServerName currentRegionServerName = master.getAssignmentManager().getRegionStates()
        .getRegionServerOfRegion(destRegion.getRegionInfo());
      if (
        currentRegionServerName != null
