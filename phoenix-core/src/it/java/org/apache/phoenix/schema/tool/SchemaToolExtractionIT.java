...
public class SchemaToolExtractionIT extends ParallelStatsEnabledIT {
  ...
  public void testCreateTableStatementLowerCase() throws Exception {
    ...
    queries.add(createTableStmt);
    String result =
      runSchemaExtractionTool("\"" + schemaName + "\"", "\"" + tableName + "\"", null, queries);
  ...
  public void testCreateIndexStatement() throws Exception {
    ...
    String createIndexStatement =
      "CREATE INDEX " + indexName + " ON " + pTableFullName + "(\"v1\" DESC) INCLUDE (v2)";
    // FIXME never verified
    String createIndexStatement1 =
      "CREATE INDEX " + indexName1 + " ON " + pTableFullName + "(v2 DESC) INCLUDE (\"v1\")";
  ...
  public void testDDLsWithDefaults() throws Exception {
    ...
    String createTableStatement =
      "CREATE TABLE " + pTableFullName + "(k VARCHAR NOT NULL PRIMARY KEY, v1 VARCHAR, v2 VARCHAR)";
    String createIndexStatement =
      "CREATE INDEX " + indexName + " ON " + pTableFullName + "(v1 DESC) INCLUDE (v2)" + properties;
    ...
    try (Connection conn = DriverManager.getConnection(getUrl(), props)) {
      ...
      SchemaExtractionProcessor schemaExtractionProcessor =
        new SchemaExtractionProcessor(null, config, pData, true);
      String tableDDL = schemaExtractionProcessor.process();
      assertTrue(tableDDL.contains("IMMUTABLE_STORAGE_SCHEME"));
      SchemaExtractionProcessor schemaExtractionProcessorIndex =
        new SchemaExtractionProcessor(null, config, pIndex, true);
  ...
  public void testCreateIndexStatementLowerCase() throws Exception {
    ...
    String pTableFullName = SchemaUtil.getEscapedTableName(schemaName, tableName);
    String createTableStatement = "CREATE TABLE " + pTableFullName
      + "(\"k\" VARCHAR NOT NULL PRIMARY KEY, \"a\".V1 VARCHAR, \"v2\" VARCHAR)" + properties;
    String createIndexStatement = "CREATE INDEX " + indexName + " ON " + pTableFullName
      + "(\"a\".V1 DESC, \"k\") INCLUDE (\"v2\")";
    List<String> queries = new ArrayList<String>() {
    };
    queries.add(createTableStatement);
    queries.add(createIndexStatement);
  ...
  public void testCreateIndexStatementLowerCaseCombined() throws Exception {
    ...
    String pTableFullName = SchemaUtil.getEscapedTableName(schemaName, tableName);
    String createTableStatement = "CREATE TABLE " + pTableFullName
      + "(ID varchar primary key, \"number\" integer, \"currency\" decimal(6,2), lista varchar[])"
      + properties;
    String createIndexStatement = "CREATE INDEX " + indexName + " ON " + pTableFullName
      + "(\"number\" * \"currency\", ID) INCLUDE (LISTA)";
    List<String> queries = new ArrayList<String>() {
    };
    queries.add(createTableStatement);
    queries.add(createIndexStatement);
  ...
  public void testCreateViewStatement() throws Exception {
    ...
    String createTableStmt = "CREATE TABLE " + pTableFullName + "(k BIGINT NOT NULL PRIMARY KEY, "
      + "v1 VARCHAR, v2 VARCHAR)" + properties;
    ...
    String createView = "CREATE VIEW " + viewFullName + "(id1 BIGINT, id2 BIGINT NOT NULL, "
      + "id3 VARCHAR NOT NULL CONSTRAINT PKVIEW PRIMARY KEY (id2, id3 DESC)) " + "AS SELECT * FROM "
      + pTableFullName;
  ...
  public void testCreateViewStatementLowerCase() throws Exception {
    ...
    String pTableFullName = SchemaUtil.getEscapedTableName(schemaName, tableName);
    String createTableStmt = "CREATE TABLE " + pTableFullName
      + "(\"k\" BIGINT NOT NULL PRIMARY KEY, " + "\"a\".V1 VARCHAR, v2 VARCHAR)" + properties;
    ...
    queries.add(createView);
    String result =
      runSchemaExtractionTool("\"" + schemaName + "\"", "\"" + viewName + "\"", null, queries);
  ...
  public void testCreateViewIndexStatement() throws Exception {
    ...
    String createIndexStatement =
      "CREATE INDEX " + indexName + " ON " + childviewName + "(id2, id1) INCLUDE (v1)";
    List<String> queries = new ArrayList<String>() {
    };
    queries.add(createTableStmt);
    queries.add(createView);
    ...
    queries.add(createIndexStatement);
    String expected =
      "CREATE INDEX %s ON " + childviewName + "(ID2, ID1, K, ID3 DESC) INCLUDE (V1)";
    String result = runSchemaExtractionTool(schemaName, indexName, null, queries);
  ...
  private void compareOrdinalPositions(String table, String newTable) throws SQLException {
    String ordinalQuery = "SELECT COLUMN_NAME, " + "ORDINAL_POSITION FROM SYSTEM.CATALOG"
    ...
    try (Connection conn = DriverManager.getConnection(getUrl(), props)) {
      ...
      while (rs.next()) {
        Assert.assertEquals(ordinalMap.get(rs.getString(1)).intValue(), rs.getInt(2));
      }
    }
  }
  ...
  public void testCreateTableWithPKConstraint() throws Exception {
    ...
    String schemaName = generateUniqueName();
    String pTableFullName = SchemaUtil.getQualifiedTableName(schemaName, tableName);
    String query = "create table " + pTableFullName + "(a_char CHAR(15) NOT NULL, "
  ...
  public void testCreateTableWithArrayColumn() throws Exception {
    ...
    String query = "create table " + pTableFullName + "(a_char CHAR(15) NOT NULL, "
      + "b_char CHAR(10) NOT NULL, " + "c_var_array VARCHAR ARRAY, "
  ...
  public void testCreateTableWithDefaultCFProperties() throws Exception {
    ...
    String schemaName = generateUniqueName();
    String properties =
      "KEEP_DELETED_CELLS=TRUE, TTL=1209600, IMMUTABLE_STORAGE_SCHEME='ONE_CELL_PER_COLUMN', "
        + "REPLICATION_SCOPE=1, DEFAULT_COLUMN_FAMILY='cv', SALT_BUCKETS=16, MULTI_TENANT=true, TIME_TEST='72HOURS'";
    String pTableFullName = SchemaUtil.getQualifiedTableName(schemaName, tableName);
    String query = "create table " + pTableFullName + "(a_char CHAR(15) NOT NULL, "
      + "b_char CHAR(10) NOT NULL, " + "\"av\".\"_\" CHAR(1), " + "\"bv\".\"_\" CHAR(1), "
      + "\"cv\".\"_\" CHAR(1), "
  ...
  public void testCreateTableWithCFProperties() throws Exception {
    String tableName = generateUniqueName();
    String schemaName = generateUniqueName();
    String properties = "\"av\".VERSIONS=2, \"bv\".VERSIONS=2, " + "DATA_BLOCK_ENCODING='DIFF', "
  ...
  public void testCreateTableWithMultipleCF() throws Exception {
    ...
    String schemaName = generateUniqueName();
    String properties =
      "\"av\".VERSIONS=2, \"bv\".VERSIONS=3, " + "\"cv\".VERSIONS=4, DATA_BLOCK_ENCODING='DIFF', "
        + "IMMUTABLE_STORAGE_SCHEME='ONE_CELL_PER_COLUMN', SALT_BUCKETS=16, MULTI_TENANT=true";
    String pTableFullName = SchemaUtil.getQualifiedTableName(schemaName, tableName);
    final String query = "create table " + pTableFullName + "(a_char CHAR(15) NOT NULL, "
      + "b_char CHAR(10) NOT NULL, " + "\"av\".\"_\" CHAR(1), " + "\"bv\".\"_\" CHAR(1), "
      + "\"cv\".\"_\" CHAR(1), "
      + "\"dv\".\"_\" CHAR(1) CONSTRAINT PK PRIMARY KEY (a_char, b_char)) " + properties;
    List<String> queries = new ArrayList<String>() {
  ...
  public void testCreateTableWithMultipleCFProperties() throws Exception {
    ...
    String properties = "\"av\".DATA_BLOCK_ENCODING='DIFF', \"bv\".DATA_BLOCK_ENCODING='DIFF', "
      + "\"cv\".DATA_BLOCK_ENCODING='DIFF', " + "IMMUTABLE_STORAGE_SCHEME='ONE_CELL_PER_COLUMN', "
      + "SALT_BUCKETS=16, MULTI_TENANT=true, BLOOMFITER='ROW'";
    String simplifiedProperties =
      "DATA_BLOCK_ENCODING='DIFF', " + "IMMUTABLE_STORAGE_SCHEME='ONE_CELL_PER_COLUMN', "
        + "SALT_BUCKETS=16, MULTI_TENANT=true, BLOOMFITER='ROW'";
    String query =
      "create table " + schemaName + ".\"" + tableName + "\"" + "(a_char CHAR(15) NOT NULL, "
        + "b_char CHAR(10) NOT NULL, " + "\"av\".\"_\" CHAR(1), " + "\"bv\".\"_\" CHAR(1), "
        + "\"cv\".\"_\" CHAR(1) CONSTRAINT PK PRIMARY KEY (a_char, b_char)) " + properties;
    List<String> queries = new ArrayList<String>() {
    };
  ...
  public void testColumnAndPKOrdering_nonView() throws Exception {
    ...
    String table =
      "CREATE TABLE MY_SCHEMA.MY_SAMPLE_DATA_TABLE(" + "ORGANIZATION_ID CHAR(15) NOT NULL,"
        + " SOME_ID_COLUMN CHAR(3) NOT NULL," + " SOME_ID_COLUMN_2 CHAR(15) NOT NULL,"
        + " CREATED_DATE DATE NOT NULL," + " SOME_ID_COLUMN_3 CHAR(15) NOT NULL,"
        + " SOME_ID_COLUMN_4 CHAR(15)," + " CREATED_BY_ID VARCHAR," + " VALUE_FIELD VARCHAR"
    ...
    queries.add(table);
    queries.add(index);
    String result = runSchemaExtractionTool("MY_SCHEMA", "MY_DATA_TABLE_INDEX", null, queries);
  ...
  public void testCreateIndexStatementWithColumnFamily() throws Exception {
    ...
    String createIndexStmt =
      "CREATE INDEX " + indexName + " ON " + pTableFullName + "(\"av\".\"_\")";
    List<String> queries = new ArrayList<String>() {
    };
    queries.add(createTableStmt);
