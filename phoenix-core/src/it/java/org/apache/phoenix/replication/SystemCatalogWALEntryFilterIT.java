...
public class SystemCatalogWALEntryFilterIT extends ParallelStatsDisabledIT {
  ...
  private static final String CREATE_NONTENANT_VIEW_SQL =
    "CREATE VIEW IF NOT EXISTS  " + SCHEMA_NAME + "." + NONTENANT_VIEW_NAME + "(" + VIEW_COLUMN_NAME
      + " varchar)  AS SELECT * FROM " + TestUtil.ENTITY_HISTORY_TABLE_NAME
      + " WHERE OLD_VALUE like 'E%'";

  private static final String DROP_TENANT_VIEW_SQL =
    "DROP VIEW IF EXISTS " + SCHEMA_NAME + "." + TENANT_VIEW_NAME;
  private static final String DROP_NONTENANT_VIEW_SQL =
    "DROP VIEW IF EXISTS " + SCHEMA_NAME + "." + NONTENANT_VIEW_NAME;
  ...
  public static synchronized void setup() throws Exception {
    ...
    try (java.sql.Connection connection =
      ...
      catalogTable =
        PhoenixRuntime.getTable(connection, PhoenixDatabaseMetaData.SYSTEM_CATALOG_NAME);
      childLinkTable =
        PhoenixRuntime.getTable(connection, PhoenixDatabaseMetaData.SYSTEM_CHILD_LINK_NAME);
      walKeyCatalog = new WALKeyImpl(REGION,
        TableName.valueOf(PhoenixDatabaseMetaData.SYSTEM_CATALOG_NAME), 0, 0, uuid);
      walKeyChildLink = new WALKeyImpl(REGION,
        TableName.valueOf(PhoenixDatabaseMetaData.SYSTEM_CHILD_LINK_NAME), 0, 0, uuid);
    }
    ;
  ...
  public void testOtherTablesAutoPass() throws Exception {
    ...
    WAL.Entry entry =
      new WAL.Entry(new WALKeyImpl(REGION, TableName.valueOf(TestUtil.ENTITY_HISTORY_TABLE_NAME),
  ...
  public void testSystemCatalogWALEntryFilter() throws Exception {
    ...
    Assert.assertEquals(
      tenantEntryCatalog.getEdit().size() + nonTenantEntryCatalog.getEdit().size(),
  ...
  public void testSystemChildLinkWALEntryFilter() throws Exception {
    ...
    Get nonTenantGetChildLink =
      getGetChildLink(childLinkTable, DEFAULT_TENANT_BYTES, NONTENANT_VIEW_NAME);
    ...
    Assert.assertEquals(
      tenantEntryChildLink.getEdit().size() + nonTenantEntryChildLink.getEdit().size(),
      comboEntry.getEdit().size());
   ...
   * 1. Filtered for non-tenant views. 2. Not filtered for tenant views.
   */
  @Test
  public void testDeleteMarkerForParentChildLink() throws Exception {
    ...
    WAL.Entry childLinkEntry = getEntry(systemChildLinkTableName, new Scan(), false);
    ...
    // Chain the system catalog WAL entry filter to ChainWALEntryFilter
    ChainWALEntryFilter chainWALEntryFilter = new ChainWALEntryFilter(filter);
    childLinkEntry = getEntry(systemChildLinkTableName, new Scan(), false);
    ...
    Assert.assertTrue(tenantDeleteCountAfterFilter == tenantDeleteCountBeforeFilter
      && nonTenantDeleteCountAfterFilter == 0);
  ...
  private boolean isTenantOwnedCell(Cell cell, String tenantId) {
    ...
    boolean isChildLinkForTenantId = row.contains(tenantId)
      && CellUtil.matchingQualifier(cell, PhoenixDatabaseMetaData.LINK_TYPE_BYTES);
  ...
  /**
   * Asserts and returns cell count in the WAL.Entry. if tenantOwned is true, tenant owned cell
   * count is returned else non-tenant cell count.
  ...
  /**
   * Returns delete family cell count in the WAL.Entry. if tenantOwned is true, tenant owned cell
   * count is returned else non-tenant cell count.
   * @Param entry {@link WAL.Entry}
   * @Param tenantOwned {@link Boolean}
   */
    ...
    try (Connection conn = ConnectionFactory.createConnection(getUtility().getConfiguration())) {
      ...
      if (result != null) {
        ...
        Assert.assertNotNull(
          String.format("Didn't retrieve any cells from table %s", tableName.getNameAsString()),
          cellList);
        for (Cell c : cellList) {
          edit.add(c);
        }
