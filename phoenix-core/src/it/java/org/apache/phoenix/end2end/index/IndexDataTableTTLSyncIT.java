 ...
 * table and a global index stay aligned under compaction (Point 2).
 * <p>
 * The scenario is the RCA divergence: a covered column ({@code covcol}) is written once, then never
 * re-written by later partial "touch" upserts. On the index side {@code covcol} is rebuilt at every
 * touch's {@code batchTimestamp}; on the data side it would otherwise keep its original timestamp,
 * and a later major compaction opens a {@code >ttl} gap on only the data side, dropping {@code covcol}
 * there while the index keeps it.
 * <p>
 * The Point 2 rewrite re-injects {@code covcol} (and any indexed column) into the touch's data Put at
...
public class IndexDataTableTTLSyncIT extends BaseTest {
  ...
  public void testBaseTableCoveredColumnResync() throws Exception {
    ...
    try (Connection conn = DriverManager.getConnection(getUrl())) {
      ...
      // Data and index agree on covcol (Point 3 masked the internal scan; Point 2 kept them aligned).
  ...
  private void flushAndMajorCompact(Connection conn, String tableName) throws Exception {
