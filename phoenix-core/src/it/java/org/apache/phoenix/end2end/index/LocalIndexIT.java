...
public class LocalIndexIT extends BaseLocalIndexIT {
  ...
  public void testPhoenixRowTimestamp() throws Exception {
    ...
    try (Connection conn = DriverManager.getConnection(getUrl())) {
      ...
      conn.createStatement().execute("CREATE LOCAL INDEX " + indexName + " on " + tableName
        + " (val1, PHOENIX_ROW_TIMESTAMP()) ");

      String timeZoneID = Calendar.getInstance().getTimeZone().getID();
      // Write a query to get the row with val2 = 'bc' with a time range query
      String query = "SELECT  val1, val2, PHOENIX_ROW_TIMESTAMP() from " + tableName
      ...
      Thread.sleep(1);
      conn.createStatement()
        .execute("upsert into " + tableName + " values ('c', 'bc', 'ccc', 'cccc')");
      conn.commit();
      ...
      query = "SELECT  val1, val2, PHOENIX_ROW_TIMESTAMP() from " + tableName
      ...
      conn.createStatement().execute("CREATE LOCAL INDEX " + indexName + " on " + tableName
        + " (PHOENIX_ROW_TIMESTAMP()) " + "include (val1, val2, val3) ");
      // Add one more row
      // Sleep 1ms to get a different row timestamps
      Thread.sleep(1);
      conn.createStatement()
        .execute("upsert into " + tableName + " values ('d', 'ad', 'def', 'defg')");
      conn.commit();
  ...
  public void testSelectFromIndexWithUncoveredArrayIndex() throws Exception {
    ...
    conn.createStatement()
      .execute("CREATE TABLE " + tableName + " (pk INTEGER PRIMARY KEY, v1 FLOAT, v2 FLOAT[])");
    ...
    ResultSet rs =
      conn.createStatement().executeQuery("SELECT v2[1] FROM " + tableName + " WHERE v1 < 3");
    rs.next();
    ...
    conn.createStatement()
      .execute("CREATE LOCAL INDEX " + indexName + " ON " + tableName + "(v2[2])");
  ...
  public void testSelectFromIndexWithAdditionalWhereClause() throws Exception {
    ...
    conn.createStatement().execute(
      "CREATE TABLE " + tableName + " (pk INTEGER PRIMARY KEY, v1 FLOAT, v2 FLOAT, v3 INTEGER)");
    ...
    conn.createStatement().execute("DROP INDEX " + indexName + " ON " + tableName);
    conn.createStatement()
      .execute("CREATE LOCAL INDEX " + indexName + " ON " + tableName + "(v1) INCLUDE (v3)");
    testExtraWhere(conn, tableName);
    ...
    conn.createStatement().execute("DROP INDEX " + indexName + " ON " + tableName);
    conn.createStatement()
      .execute("CREATE LOCAL INDEX " + indexName + " ON " + tableName + "(v1) INCLUDE (v2,v3)");
    testExtraWhere(conn, tableName);
  }

  private void testExtraWhere(Connection conn, String tableName) throws SQLException {
    ResultSet rs = conn.createStatement()
      .executeQuery("SELECT COUNT(*) FROM " + tableName + " WHERE v1 < 3 AND v2 < 4");
    rs.next();
    assertEquals(1, rs.getInt(1));
    rs.close();

    rs = conn.createStatement()
      .executeQuery("SELECT COUNT(*) FROM " + tableName + " WHERE v1 < 3 AND v3 < 5");
    rs.next();
    assertEquals(1, rs.getInt(1));
    rs.close();

    rs = conn.createStatement()
      .executeQuery("SELECT COUNT(*) FROM " + tableName + " WHERE v1 < 10 AND v2 < 0 AND v3 < 0");
    rs.next();
    assertEquals(0, rs.getInt(1));
    rs.close();

    rs = conn.createStatement()
      .executeQuery("SELECT COUNT(*) FROM " + tableName + " WHERE v1 <= 2 AND v2 > 0 AND v3 < 5");
    rs.next();
    assertEquals(1, rs.getInt(1));
    rs.close();

    rs = conn.createStatement()
      .executeQuery("SELECT pk FROM " + tableName + " WHERE v1 > 3 AND v2 > 0 AND v3 > 6");
    rs.next();
    assertEquals(4, rs.getInt(1));
    rs.close();

    rs = conn.createStatement()
      .executeQuery("SELECT v1 FROM " + tableName + " WHERE v1 > 3 AND v2 > 0 AND v3 > 6");
    rs.next();
    assertEquals(5, rs.getInt(1));
    rs.close();

    rs = conn.createStatement()
      .executeQuery("SELECT pk FROM " + tableName + " WHERE (v1,v2) IN ((1,5),(4,5))");
    rs.next();
    assertEquals(3, rs.getInt(1));
    rs.close();

    rs = conn.createStatement()
      .executeQuery("SELECT v3 FROM " + tableName + " WHERE (v1,v2) IN ((1,5),(4,5))");
    rs.next();
    assertEquals(6, rs.getInt(1));
    rs.close();

    rs = conn.createStatement()
      .executeQuery("SELECT * FROM " + tableName + " WHERE v1 > 0 AND v3 > 5 LIMIT 2");
    assertTrue(rs.next());
    assertTrue(rs.next());
    assertFalse(rs.next());
    rs.close();
    ...
    rs = conn.createStatement()
      .executeQuery("SELECT * FROM " + tableName + " WHERE v1 > 0 AND v3 > 5 LIMIT 1");
    assertTrue(rs.next());
    assertFalse(rs.next());
    rs.close();

    rs = conn.createStatement()
      .executeQuery("SELECT * FROM " + tableName + " WHERE v3 > 5 ORDER BY v1 LIMIT 2");
    assertTrue(rs.next());
    assertTrue(rs.next());
    assertFalse(rs.next());
    rs.close();
  ...
  public void testLocalIndexConsistency() throws Exception {
    ...
    conn.createStatement()
      .execute("CREATE TABLE " + tableName + " (pk INTEGER PRIMARY KEY, v1 FLOAT) SPLIT ON (4000)");
    conn.createStatement().execute("CREATE LOCAL INDEX " + indexName + " ON " + tableName + "(v1)");
    conn.createStatement().execute("UPSERT INTO " + tableName + " VALUES(rand() * 8000, rand())");

    for (int i = 0; i < 16; i++) {
      conn.createStatement()
        .execute("UPSERT INTO " + tableName + " SELECT rand() * 8000, rand() FROM " + tableName);
  ...
  public void testLocalIndexConsistencyWithGlobalMix() throws Exception {
    ...
    conn.createStatement().execute("CREATE TABLE " + tableName
      + " (pk INTEGER PRIMARY KEY, v1 FLOAT, v2 FLOAT, v3 FLOAT, v4 FLOAT) SPLIT ON (4000)");

    int idx = 1;
    for (String indexName : localIndexNames) {
      conn.createStatement()
        .execute("CREATE LOCAL INDEX " + indexName + " ON " + tableName + "(v" + idx++ + ")");
    }
    for (String indexName : globalIndexNames) {
      conn.createStatement()
        .execute("CREATE INDEX " + indexName + " ON " + tableName + "(v" + idx++ + ")");
    }
    conn.createStatement().execute("UPSERT INTO " + tableName + " VALUES(rand() * 8000, rand())");
  ...
  private int getCountViaIndex(Connection conn, String tableName, String indexName)
    throws SQLException {
    String hint = indexName == null ? "NO_INDEX" : "INDEX(" + tableName + " " + indexName + ")";
    try (ResultSet rs = conn.createStatement()
      .executeQuery("SELECT /*+ " + hint + " */ COUNT(*) FROM " + tableName)) {
      rs.next();
      return rs.getInt(1);
    }
  }
  ...
  public void testUseUncoveredLocalIndexWithPrefix() throws Exception {
    ...
    assertPlan(conn, "SELECT * FROM " + tableName + " WHERE pk1 = 3 AND pk2 = 4")
      ...
      .keyRanges("[3,4]").indexRule(OptimizerReasons.RULE_NON_LOCAL_PREFERRED).indexRejectedCount(1)
  ...
  public void testUseUncoveredLocalIndexWithSplitPrefix() throws Exception {
    ...
    // + " (pk1 INTEGER NOT NULL, pk2 INTEGER NOT NULL, pk3 INTEGER NOT NULL, v1 FLOAT, v2 FLOAT, V3
    // INTEGER CONSTRAINT pk PRIMARY KEY(pk1,pk2,pk3))");
    conn.createStatement()
  ...
  public void testLocalIndexRoundTrip() throws Exception {
    ...
    PTable localIndex =
      conn1.unwrap(PhoenixConnection.class).getTable(new PTableKey(null, indexTableName));
    ...
    String createTable = "CREATE TABLE IF NOT EXISTS " + tableName2
    ...
    conn1.createStatement().execute("CREATE local INDEX IF NOT EXISTS " + indexName2 + " on "
      + tableName2 + "(\"HOUR\"(user_time))");
    conn1.createStatement()
      .execute("upsert into " + tableName2 + " values(TO_TIME('2005-10-01 14:03:22.559'), 'foo')");
    conn1.commit();
  ...
  public void testLocalIndexCreationWithSplitsShouldFail() throws Exception {
    ...
    try {
      conn1.createStatement().execute(
        "CREATE LOCAL INDEX " + indexName + " ON " + tableName + "(v1)" + " split on (1,2,3)");
  ...
  public void testLocalIndexCreationWithSaltingShouldFail() throws Exception {
    ...
    try {
      conn1.createStatement().execute(
        "CREATE LOCAL INDEX " + indexName + " ON " + tableName + "(v1)" + " salt_buckets=16");
  ...
  public void testLocalIndexTableRegionSplitPolicyAndSplitKeys() throws Exception {
    ...
    TableDescriptor htd = admin.getDescriptor(TableName.valueOf(indexPhysicalTableName));
    assertEquals(IndexRegionSplitPolicy.class.getName(),
      htd.getValue(TableDescriptorBuilder.SPLIT_POLICY));
    try (org.apache.hadoop.hbase.client.Connection c =
      ConnectionFactory.createConnection(admin.getConfiguration())) {
      try (RegionLocator userTable = c.getRegionLocator(
        SchemaUtil.getPhysicalTableName(tableName.getBytes(), isNamespaceMapped))) {
        try (
          RegionLocator indxTable = c.getRegionLocator(TableName.valueOf(indexPhysicalTableName))) {
  ...
  public void testDropLocalIndexTable() throws Exception {
    ...
    String sequenceName =
      getViewIndexSequenceName(PNameFactory.newName(tableName), null, isNamespaceMapped);
    String sequenceSchemaName =
      getViewIndexSequenceSchemaName(PNameFactory.newName(tableName), isNamespaceMapped);
  ...
  public void testPutsToLocalIndexTable() throws Exception {
    ...
    assertEquals(4, rs.getInt(1));
    Admin admin = driver.getConnectionQueryServices(getUrl(), TestUtil.TEST_PROPERTIES).getAdmin();
    Table indexTable = admin.getConnection().getTable(TableName.valueOf(indexPhysicalTableName));
    Pair<byte[][], byte[][]> startEndKeys = admin.getConnection()
      .getRegionLocator(TableName.valueOf(indexPhysicalTableName)).getStartEndKeys();
  ...
  public void testLocalIndexScanJoinColumnsFromDataTable() throws Exception {
    ...
    try {
      ...
      assertEquals(BaseScannerRegionObserverConstants.KEY_ORDERED_GROUP_BY_EXPRESSIONS,
  ...
  public void testDropLocalIndexShouldDeleteDataFromLocalIndexTable() throws Exception {
    ...
    try {
      ...
      Pair<byte[][], byte[][]> startEndKeys =
        admin.getConnection().getRegionLocator(TableName.valueOf(tableName)).getStartEndKeys();
      byte[][] startKeys = startEndKeys.getFirst();
      byte[][] endKeys = startEndKeys.getSecond();
  ...
  public void testLocalIndexScanWithInList() throws Exception {
    ...
    try {
      ...
      conn1.commit();
      conn1.createStatement()
        .execute("CREATE LOCAL INDEX " + indexName + " ON " + tableName + "(v1) include (k3)");
  ...
  public void testLocalIndexAutomaticRepair() throws Exception {
    ...
    try (Table metaTable = conn.getQueryServices().getTable(TableName.META_TABLE_NAME.getName());
      ...
      for (int i = 0; i < 2000; i++) {
        statement
          .execute("upsert into " + tableName + "  values(" + i + ",'fn" + i + "','ln" + i + "')");
      }
      conn.commit();
      ResultSet rs = statement.executeQuery("SELECT COUNT(*) FROM " + indexName);
      assertTrue(rs.next());
  ...
  public void testLocalGlobalIndexMix() throws Exception {
    ...
    conn1.commit();
    ResultSet rs =
      conn1.createStatement().executeQuery("SELECT COUNT(*) FROM " + tableName + " WHERE v1 = 'c'");
    assertTrue(rs.next());
    assertEquals(1, rs.getInt(1));
    rs =
      conn1.createStatement().executeQuery("SELECT COUNT(*) FROM " + tableName + " WHERE v2 = '2'");
    assertTrue(rs.next());
    assertEquals(1, rs.getInt(1));
    conn1.close();
  }
  ...
  public void testLocalIndexSelfJoin() throws Exception {
    ...
    conn1.commit();
    conn1.createStatement()
      .execute("CREATE LOCAL INDEX " + indexName + " ON " + tableName + "(postal_code)");
    ResultSet rs = conn1.createStatement().executeQuery("SELECT * from " + tableName + " c1, "
      + tableName + " c2 where c1.customer_id=c2.customer_id and c2.postal_code='560103'");
    assertTrue(rs.next());
    conn1.close();
  }

  private void copyLocalIndexHFiles(Configuration conf, RegionInfo fromRegion, RegionInfo toRegion,
    boolean move) throws IOException {
    Path root = CommonFSUtils.getRootDir(conf);

    Path seondRegion = new Path(CommonFSUtils.getTableDir(root, fromRegion.getTable())
      + Path.SEPARATOR + fromRegion.getEncodedName() + Path.SEPARATOR + "L#0/");
    Path hfilePath =
      CommonFSUtils.getCurrentFileSystem(conf).listFiles(seondRegion, true).next().getPath();
    Path firstRegionPath = new Path(CommonFSUtils.getTableDir(root, toRegion.getTable())
      + Path.SEPARATOR + toRegion.getEncodedName() + Path.SEPARATOR + "L#0/");
    FileSystem currentFileSystem = CommonFSUtils.getCurrentFileSystem(conf);
    assertTrue(
      FileUtil.copy(currentFileSystem, hfilePath, currentFileSystem, firstRegionPath, move, conf));
  }
  ...
  public void testLocalIndexForMultiTenantTable() throws Exception {
    ...
    try {
      ...
      ResultSet rs =
        conn1.createStatement().executeQuery("SELECT * FROM " + tableName + " WHERE v1 = 'x'");
      assertTrue(rs.next());
      assertEquals("b", rs.getString("T_ID"));
  ...
  public void testEstimatesWithLocalIndexes() throws Exception {
    ...
    try (Connection conn = DriverManager.getConnection(getUrl())) {
      ...
      conn.createStatement().execute("CREATE TABLE " + tableName
        + " (k INTEGER PRIMARY KEY, a bigint, b bigint)" + " GUIDE_POSTS_WIDTH=" + guidePostWidth);
      ...
      conn.commit();
      conn.createStatement()
        .execute("CREATE LOCAL INDEX " + indexName + " ON " + tableName + " (a) INCLUDE (b) ");
    ...
    try (Connection conn = DriverManager.getConnection(getUrl())) {
      ...
      assertTrue("Index " + indexName + " should have been used",
        rs.unwrap(PhoenixResultSet.class).getStatement().getQueryPlan().getTableRef().getTable()
          .getName().getString().equals(indexName));
