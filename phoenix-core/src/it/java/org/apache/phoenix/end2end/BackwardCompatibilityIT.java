...
public class BackwardCompatibilityIT {
  ...
  public void testViewIndexIdCreatedWithOldClient() throws Exception {
    ...
    try (org.apache.hadoop.hbase.client.Connection conn = hbaseTestUtil.getConnection();
      Admin admin = conn.getAdmin()) {
      HTableDescriptor tableDescriptor =
        admin.getTableDescriptor(TableName.valueOf(PhoenixDatabaseMetaData.SYSTEM_CATALOG_NAME));
      assertFalse(
  ...
  private boolean isClientCompatibleForOrderedGroupByQuery() {
    String[] clientVersion = compatibleClientVersion.split("\\.");
    int majorVersion = Integer.parseInt(clientVersion[0]);
    int minorVersion = Integer.parseInt(clientVersion[1]);
    int patchVersion = Integer.parseInt(clientVersion[2].split("-")[0]);
    if (majorVersion > 5) {
      return true;
    }
    if (majorVersion < 5) {
