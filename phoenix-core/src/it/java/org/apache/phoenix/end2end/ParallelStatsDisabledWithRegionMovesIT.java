...
/**
 * Base class for tests whose methods run in parallel with 1. Statistics enabled on server side
 * (QueryServices#STATS_COLLECTION_ENABLED is true) 2. Guide Post Width for all relevant tables is
 * 0. Stats are disabled at table level.
 * <p>
 * See {@link org.apache.phoenix.schema.stats.NoOpStatsCollectorIT} for tests that disable stats
 * collection from server side.
 * <p>
 * You must create unique names using {@link #generateUniqueName()} for each table and sequence used
...
public abstract class ParallelStatsDisabledWithRegionMovesIT extends BaseTest {
  ...
  protected static void moveRegionsOfTable(String tableName) throws IOException {
    try (AsyncConnection asyncConnection =
      ConnectionFactory.createAsyncConnection(getUtility().getConfiguration()).get()) {
      AsyncAdmin admin = asyncConnection.getAdmin();
      List<ServerName> servers = new ArrayList<>(admin.getRegionServers().get());
      ...
      regionsOnServer1.forEach(regionInfo -> {
        if (regionInfo.getTable().equals(TableName.valueOf(tableName))) {
          try {
            for (int i = 0; i < 2; i++) {
              RegionStatesCount regionStatesCount = admin.getClusterMetrics().get()
              ...
              if (
                ...
                admin.move(regionInfo.getEncodedNameAsBytes(), server2).get(3, TimeUnit.SECONDS);
                break;
              } else {
          ...
          try {
            for (int i = 0; i < 2; i++) {
              ...
              if (
                ...
                admin.move(regionInfo.getEncodedNameAsBytes(), server1).get(3, TimeUnit.SECONDS);
  ...
  protected static void splitAllRegionsOfTable(String tableName, int splitAtRow) {
    try (AsyncConnection asyncConnection =
      ConnectionFactory.createAsyncConnection(getUtility().getConfiguration()).get()) {
      AsyncAdmin admin = asyncConnection.getAdmin();
      List<RegionInfo> regionsOfTable = admin.getRegions(TableName.valueOf(tableName)).get();
          ...
          try {
            for (int i = 0; i < 5; i++) {
              ...
              if (
                ...
                try (Table table = utility.getConnection().getTable(TableName.valueOf(tableName))) {
                  try (ResultScanner resultScanner = table.getScanner(new Scan())) {
                    ...
                    for (int rowCount = 0; rowCount < splitAtRow; rowCount++) {
                      ...
                      if (result == null) {
                        LOGGER.info(
                          "Table {} has only {} rows, splitting" + " at row number {} not possible",
                          tableName, rowCount, splitAtRow);
                        return;
                      }
                    }
                    splitPoint =
                      result == null ? null : ByteUtil.closestPossibleRowAfter(result.getRow());
                  }
                }
                LOGGER.info("Splitting region {}", regionInfo.getRegionNameAsString());
                admin.flushRegion(regionInfo.getRegionName()).get(3, TimeUnit.SECONDS);
                admin.splitRegion(regionInfo.getRegionName(), splitPoint).get(4, TimeUnit.SECONDS);
                break;
              } else {
                LOGGER.info("Table {} has some region(s) in RIT or not online", tableName);
                Thread.sleep(1000);
