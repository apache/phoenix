...
//    static HBaseTestingUtility testUtil;
    ...
    public void testCreateIndex() throws Throwable {
        ...
        grantPermissions(regularUser1.getShortName(), "SYSTEM", Permission.Action.READ, Permission.Action.EXEC);
        grantPermissions(regularUser1.getShortName(), Collections.singleton("SYSTEM:CATALOG"), Permission.Action.EXEC, Permission.Action.WRITE, Permission.Action.READ);
        ...
        List<UserPermission> perms = AccessControlClient.getUserPermissions(getUtility().getConnection(), SYSTEM_CATALOG_NAME, regularUser1.getShortName());
        for (UserPermission perm : perms) {
            Assert.assertEquals(perm.getUser(), regularUser1.getShortName() );
            Permission.Action[] actions = { Permission.Action.READ, Permission.Action.WRITE, Permission.Action.EXEC };
