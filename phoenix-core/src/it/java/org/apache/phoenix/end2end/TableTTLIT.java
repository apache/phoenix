...
public class TableTTLIT extends BaseTest {
  ...
  public static synchronized Collection<Object[]> data() {
    return Arrays.asList(new Object[][] { { false, false, KeepDeletedCells.FALSE, 1, 100 },
      { false, false, KeepDeletedCells.TRUE, 5, 50 }, { false, false, KeepDeletedCells.TTL, 1, 25 },
      { true, false, KeepDeletedCells.FALSE, 5, 50 }, { true, false, KeepDeletedCells.TRUE, 1, 25 },
  ...
  public void testMaskingAndCompaction() throws Exception {
    ...
    try (Connection conn = DriverManager.getConnection(getUrl())) {
      ...
      conn.createStatement().execute(
        "Alter Table " + noCompactTableName + " set \"phoenix.table.ttl.enabled\" = false");
      conn.commit();
  ...
  public void testMinorCompactionShouldNotRetainCellsWhenMaxLookbackIsDisabled() throws Exception {
    try (Connection conn = DriverManager.getConnection(getUrl())) {
      ...
      conn.createStatement()
        .execute("Alter Table " + tableName + " set \"phoenix.max.lookback.age.seconds\" = 0");
      conn.commit();
      ...
      int retainedCellCount =
        (MAX_COLUMN_INDEX + 1) * versions + (multiCF ? rowUpdateCounter - versions : 0);
      assertEquals(retainedCellCount,
