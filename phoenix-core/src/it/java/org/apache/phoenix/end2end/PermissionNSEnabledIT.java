...
public class PermissionNSEnabledIT extends BasePermissionsIT {
  ...
  private AccessTestAction createMappedView(final String schemaName, final String tableName)
    throws SQLException {
    return new AccessTestAction() {
      ...
      public Object run() throws Exception {
        try (Connection conn = getConnection(); Statement stmt = conn.createStatement();) {
          String viewStmtSQL =
            "CREATE VIEW \"" + schemaName + "\".\"" + tableName + "\" ( PK varchar primary key)";
  ...
  public void testSchemaPermissions() throws Throwable {
    ...
    superUser1.runAs(new PrivilegedExceptionAction<Void>() {
      ...
      public Void run() throws Exception {
        try {
          grantPermissions(regularUser1.getShortName(), Permission.Action.ADMIN);
        } catch (Throwable e) {
  ...
  public void testConnectionCreationFailsWhenNoExecPermsOnSystemCatalog() throws Throwable {
    ...
    superUser1.runAs((PrivilegedExceptionAction<Object>) () -> {
      TableName systemCatalogTableName = TableName.valueOf(SchemaUtil
        .getPhysicalHBaseTableName(SYSTEM_SCHEMA_NAME, SYSTEM_CATALOG_TABLE, true).getString());
      try {
        // Revoke Exec permissions for SYSTEM CATALOG for the unprivileged user
        revokePermissions(unprivilegedUser.getShortName(),
          Collections.singleton(systemCatalogTableName.getNameAsString()), Permission.Action.EXEC);
      } catch (Throwable t) {
    ...
    superUser1.runAs(new PrivilegedExceptionAction<Object>() {
      ...
      public Object run() throws Exception {
        try {
          ...
          revokePermissions(unprivilegedUser.getShortName(),
            Collections.singleton(systemChildLink.getNameAsString()), Permission.Action.EXEC);
