...
public class PhoenixTTLToolIT extends ParallelStatsDisabledIT {
  ...
  private final String VIEW_DDL_WITH_ID_PREFIX_AND_TTL =
    "CREATE VIEW %s (" + "PK1 BIGINT PRIMARY KEY,A BIGINT, B BIGINT, C BIGINT, D BIGINT)"
  ...
  private void verifyNumberOfRowsFromHBaseLevel(String tableName, String regrex, int expectedRows)
    ...
    try (Table table = driver.getConnectionQueryServices(getUrl(), TestUtil.TEST_PROPERTIES)
      ...
      Filter filter = new RowFilter(CompareOperator.EQUAL, new RegexStringComparator(regrex));
      Scan scan = new Scan();
  ...
  /*
   * BaseMultiTenantTable GlobalView1 with TTL(1 ms) Index1 Index2 Creating 2 tenantViews and
   * Upserting data. After running the MR job, it should delete all data.
   */
  @Test
  public void testTenantViewOnGlobalViewWithMoreThanOneIndex() throws Exception {
    ...
    try (Connection globalConn = DriverManager.getConnection(getUrl());
      ...
      globalConn.createStatement().execute(String.format(VIEW_DDL_WITH_ID_PREFIX_AND_TTL,
        globalViewName, baseTableFullName, VIEW_PREFIX1, PHOENIX_TTL_EXPIRE_IN_A_SECOND));

      globalConn.createStatement()
        .execute(String.format(VIEW_INDEX_DDL, indexTable1, globalViewName, "A,B"));
      globalConn.createStatement()
        .execute(String.format(VIEW_INDEX_DDL, indexTable2, globalViewName, "C,D"));

      tenant1Connection.setAutoCommit(true);
      tenant2Connection.setAutoCommit(true);

      tenant1Connection.createStatement()
        .execute(String.format(TENANT_VIEW_DDL, tenantView1, globalViewName));
      tenant2Connection.createStatement()
        .execute(String.format(TENANT_VIEW_DDL, tenantView2, globalViewName));

      tenant1Connection.createStatement()
        .execute(String.format(UPSERT_TO_LEAF_VIEW_QUERY, tenantView1));
      verifyNumberOfRows(baseTableFullName, tenant1, 1, globalConn);
      tenant2Connection.createStatement()
        .execute(String.format(UPSERT_TO_LEAF_VIEW_QUERY, tenantView2));
  ...
  /*
   * BaseMultiTenantTable GlobalView1 with TTL(1 ms) GlobalView2 with TTL(1 DAY) Index1 Index2
   * Index3 Index4 Upserting data to both global views and run the MR job. It should only delete
   * GlobalView1 data not remove GlobalView2 data.
   */
  @Test
  public void testGlobalViewWithMoreThanOneIndex() throws Exception {
    ...
    try (Connection globalConn = DriverManager.getConnection(getUrl());
      ...
      globalConn.createStatement().execute(String.format(VIEW_DDL_WITH_ID_PREFIX_AND_TTL,
        globalViewName1, baseTableFullName, VIEW_PREFIX1, PHOENIX_TTL_EXPIRE_IN_A_SECOND));
      globalConn.createStatement().execute(String.format(VIEW_DDL_WITH_ID_PREFIX_AND_TTL,
      ...
      tenant1Connection.createStatement()
        .execute(String.format(UPSERT_TO_GLOBAL_VIEW_QUERY, globalViewName1));
      tenant1Connection.createStatement()
        .execute(String.format(UPSERT_TO_GLOBAL_VIEW_QUERY, globalViewName2));
      tenant2Connection.createStatement()
        .execute(String.format(UPSERT_TO_GLOBAL_VIEW_QUERY, globalViewName1));
      tenant2Connection.createStatement()
        .execute(String.format(UPSERT_TO_GLOBAL_VIEW_QUERY, globalViewName2));
  ...
  /*
   * BaseMultiTenantTable GlobalView1 with TTL(1 ms) GlobalView2 with TTL(1 DAY) Index1 Index2
   * Index3 Index4 TenantView1 TenantView2 Upserting data to both global views, and run the MR job.
  ...
  public void testTenantViewCase() throws Exception {
    ...
    try (Connection globalConn = DriverManager.getConnection(getUrl());
      ...
      globalConn.createStatement()
        .execute(String.format(ddl, globalViewName1, VIEW_PREFIX1, PHOENIX_TTL_EXPIRE_IN_A_SECOND));
      globalConn.createStatement()
        .execute(String.format(ddl, globalViewName2, VIEW_PREFIX2, PHOENIX_TTL_EXPIRE_IN_A_DAY));
      ...
      tenant1Connection.createStatement()
        .execute(String.format(ddl, tenantViewName1, globalViewName1));
      tenant1Connection.createStatement()
        .execute(String.format(ddl, tenantViewName2, globalViewName2));
      ...
      tenant1Connection.createStatement()
        .execute(String.format(UPSERT_TO_LEAF_VIEW_QUERY, tenantViewName1));
      tenant1Connection.createStatement()
        .execute(String.format(UPSERT_TO_LEAF_VIEW_QUERY, tenantViewName2));
      tenant2Connection.createStatement()
        .execute(String.format(UPSERT_TO_LEAF_VIEW_QUERY, tenantViewName1));
      tenant2Connection.createStatement()
        .execute(String.format(UPSERT_TO_LEAF_VIEW_QUERY, tenantViewName2));
  ...
  /*
   * BaseMultiTenantTable GlobalView1 with TTL(1 ms) GlobalView2 with TTL(1 DAY) Upserting data to
   * both global views, and run the MR job. It should only delete GlobalView1 data not remove
   * GlobalView2 data.
   */
  @Test
  ...
  /*
   * BaseTable GlobalView1 with TTL(1 ms) GlobalView2 with TTL(1 DAY) Upserting data to both global
   * views, and run the MR job. It should only delete GlobalView1 data not remove GlobalView2 data.
   */
  @Test
  public void testGlobalViewOnNonMultiTenantTable() throws Exception {
    ...
    try (Connection globalConn = DriverManager.getConnection(getUrl())) {
      String ddl =
        "CREATE TABLE " + baseTableFullName + " (ID CHAR(10) NOT NULL PRIMARY KEY, NUM BIGINT)";
      globalConn.createStatement().execute(ddl);

      ddl = "CREATE VIEW %s (PK1 BIGINT PRIMARY KEY, " + "A BIGINT, B BIGINT, C BIGINT, D BIGINT)"
        + " AS SELECT * FROM " + baseTableFullName + " WHERE ID ='%s' TTL = %d";
  ...
  /*
   * BaseTable GlobalView1 with TTL(1 ms) GlobalView2 with TTL(1 DAY) Index1 Index2 Index3 Index4
   * Upserting data to both global views, and run the MR job. It should only delete GlobalView1 data
   * not remove GlobalView2 data.
   */
  @Test
  public void testGlobalViewOnNonMultiTenantTableWithIndex() throws Exception {
    ...
    try (Connection globalConn = DriverManager.getConnection(getUrl())) {
      ...
      ddl = "CREATE VIEW %s (PK2 BIGINT PRIMARY KEY, " + "A BIGINT, B BIGINT, C BIGINT, D BIGINT)"
        + " AS SELECT * FROM " + baseTableFullName + " WHERE PK1=%d TTL = %d";

      globalConn.createStatement()
        .execute(String.format(ddl, globalViewName1, 1, PHOENIX_TTL_EXPIRE_IN_A_SECOND));
      globalConn.createStatement()
        .execute(String.format(ddl, globalViewName2, 2, PHOENIX_TTL_EXPIRE_IN_A_DAY));

      ddl = "CREATE INDEX %s ON %s(%s)";
      globalConn.createStatement()
        .execute(String.format(ddl, indexTable1, globalViewName1, "A,ID,B"));
      globalConn.createStatement()
        .execute(String.format(ddl, indexTable2, globalViewName1, "C,ID,D"));
      globalConn.createStatement()
        .execute(String.format(ddl, indexTable3, globalViewName2, "A,ID,B"));
      globalConn.createStatement()
        .execute(String.format(ddl, indexTable4, globalViewName2, "C,ID,D"));
  ...
  /*
   * BaseMultiTenantTable GlobalView1 TenantView1 TenantView2
   */
  @Test
  public void testDeleteByViewAndTenant() throws Exception {
    ...
    try (Connection globalConn = DriverManager.getConnection(getUrl());
      ...
      ddl = "CREATE VIEW %s (E BIGINT, F BIGINT) AS SELECT * FROM %s " + "WHERE ID = '%s' TTL = %d";
      tenant1Connection.createStatement().execute(String.format(ddl, tenantViewName1,
        globalViewName1, VIEW_PREFIX1, PHOENIX_TTL_EXPIRE_IN_A_SECOND));
      tenant1Connection.createStatement().execute(String.format(ddl, tenantViewName2,
        globalViewName1, VIEW_PREFIX2, PHOENIX_TTL_EXPIRE_IN_A_SECOND));
      ...
      phoenixTtlTool.setConf(conf);
      int status =
        phoenixTtlTool.run(new String[] { "-runfg", "-v", tenantViewName2, "-i", tenant1 });
      assertEquals(0, status);
  ...
  /*
   * BaseMultiTenantTable GlobalView1 GlobalView1 TenantView1 TenantView2 TenantView1 TenantView2
   */
  @Test
  public void testDeleteByTenant() throws Exception {
    ...
    try (Connection globalConn = DriverManager.getConnection(getUrl());
      ...
      tenant1Connection.createStatement().execute(String.format(ddl, tenantViewName1,
        globalViewName1, VIEW_PREFIX1, PHOENIX_TTL_EXPIRE_IN_A_SECOND));
      tenant1Connection.createStatement().execute(String.format(ddl, tenantViewName2,
        globalViewName2, VIEW_PREFIX2, PHOENIX_TTL_EXPIRE_IN_A_SECOND));

      tenant2Connection.createStatement().execute(String.format(ddl, tenantViewName3,
        globalViewName1, VIEW_PREFIX1, PHOENIX_TTL_EXPIRE_IN_A_SECOND));
      tenant2Connection.createStatement().execute(String.format(ddl, tenantViewName4,
        globalViewName2, VIEW_PREFIX2, PHOENIX_TTL_EXPIRE_IN_A_SECOND));
      ...
      tenant1Connection.createStatement()
        .execute(String.format(UPSERT_TO_LEAF_VIEW_QUERY, tenantViewName2));
      tenant2Connection.createStatement()
        .execute(String.format(UPSERT_TO_LEAF_VIEW_QUERY, tenantViewName3));
      tenant2Connection.createStatement()
        .execute(String.format(UPSERT_TO_LEAF_VIEW_QUERY, tenantViewName4));
  ...
  public void testDeleteByViewName() throws Exception {
    ...
    try (Connection globalConn = DriverManager.getConnection(getUrl());
      ...
      globalConn.createStatement()
        .execute(String.format(ddl, globalViewName1, 1, PHOENIX_TTL_EXPIRE_IN_A_SECOND));
      globalConn.createStatement()
        .execute(String.format(ddl, globalViewName2, 2, PHOENIX_TTL_EXPIRE_IN_A_SECOND));

      ddl = "CREATE VIEW %s (E BIGINT, F BIGINT) AS SELECT * FROM %s WHERE ID = '%s'";
      tenant1Connection.createStatement()
        .execute(String.format(ddl, tenantViewName1, globalViewName1, VIEW_PREFIX1));
      tenant1Connection.createStatement()
        .execute(String.format(ddl, tenantViewName2, globalViewName2, VIEW_PREFIX2));

      tenant2Connection.createStatement()
        .execute(String.format(ddl, tenantViewName3, globalViewName1, VIEW_PREFIX1));
      tenant2Connection.createStatement()
        .execute(String.format(ddl, tenantViewName4, globalViewName2, VIEW_PREFIX2));
  ...
  /*
   * BaseTable GlobalView1 with TTL MiddleLevelView1 with TTL(1 ms) MiddleLevelView2 with TTL(1 DAY)
   * LeafView1 LeafView2 Upserting data to both leafView, and run the MR job. It should only delete
   * MiddleLevelView1 data not remove MiddleLevelView2 data.
   */
  @Test
  public void testCleanMoreThanThreeLevelViewCase() throws Exception {
    ...
    try (Connection globalConn = DriverManager.getConnection(getUrl())) {
      ...
      String globalViewDdl = "CREATE VIEW %s (PK1 BIGINT PRIMARY KEY, " + "A BIGINT, B BIGINT)"
        + " AS SELECT * FROM " + baseTableFullName;

      globalConn.createStatement().execute(String.format(globalViewDdl, globalViewName));

      String middleLevelViewDdl =
        "CREATE VIEW %s (C BIGINT, D BIGINT)" + " AS SELECT * FROM %s WHERE ID ='%s' TTL = %d";

      globalConn.createStatement().execute(String.format(middleLevelViewDdl, middleLevelViewName1,
        globalViewName, VIEW_PREFIX1, PHOENIX_TTL_EXPIRE_IN_A_SECOND));
      globalConn.createStatement().execute(String.format(middleLevelViewDdl, middleLevelViewName2,
        globalViewName, VIEW_PREFIX2, PHOENIX_TTL_EXPIRE_IN_A_DAY));

      String leafViewDdl = "CREATE VIEW %s (E BIGINT, F BIGINT)" + " AS SELECT * FROM %s";

      globalConn.createStatement()
        .execute(String.format(leafViewDdl, leafViewName1, middleLevelViewName1));
      globalConn.createStatement()
        .execute(String.format(leafViewDdl, leafViewName2, middleLevelViewName2));

      globalConn.setAutoCommit(true);

      globalConn.createStatement().execute(String.format(UPSERT_TO_LEAF_VIEW_QUERY, leafViewName1));
      globalConn.createStatement().execute(String.format(UPSERT_TO_LEAF_VIEW_QUERY, leafViewName2));
