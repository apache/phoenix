...
public class SystemTablesUpgradeIT extends BaseTest {
  ...
  private static class PhoenixUpgradeCountingServices extends ConnectionQueryServicesImpl {
    public PhoenixUpgradeCountingServices(QueryServices services, ConnectionInfo connectionInfo,
      Properties info) {
  ...
  public static class PhoenixUpgradeCountingDriver extends PhoenixTestDriver {
    ...
    public synchronized ConnectionQueryServices getConnectionQueryServices(String url,
      ...
      if (cqs == null) {
        cqs = new PhoenixUpgradeCountingServices(
          new QueryServicesTestImpl(getDefaultProps(), overrideProps),
          ConnectionInfo.create(url, null, null), info);
        cqs.init(url, info);
  ...
  public void testUpgradeOnlyHappensOnce() throws Exception {
    ...
    try (Admin admin = services.getAdmin()) {
      ...
      assertEquals(SystemTaskSplitPolicy.class.getName(), taskSplitPolicy);
