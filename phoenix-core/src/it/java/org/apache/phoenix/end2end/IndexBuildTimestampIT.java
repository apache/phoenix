...
public class IndexBuildTimestampIT extends BaseTest {
          ...
          for (boolean view : Booleans) {
            for (boolean snapshot : Booleans) {
              for (String transactionProvider : new String[] { "TEPHRA", "OMID", null }) {
                ...
                list.add(
                  new Object[] { transactionProvider, mutable, localIndex, async, view, snapshot });
              }
            }
          }
        }
  ...
  private void populateTable(String tableName, MyClock clock1, MyClock clock2) throws Exception {
    ...
    conn.createStatement()
      .execute("upsert into " + tableName + " values ('aaa', 'abc', current_date())");
    conn.commit();

    EnvironmentEdgeManager.injectEdge(clock2);
    conn.createStatement()
      .execute("upsert into " + tableName + " values ('bbb', 'bcd', current_date())");
    conn.commit();
    conn.close();
  ...
  public void testCellTimestamp() throws Exception {
    ...
    try {
      ...
      String selectSql =
        String.format("SELECT * FROM %s WHERE val = 'abc'", (view ? viewName : dataTableName));
      conn = DriverManager.getConnection(getUrl());
      // assert we are pulling from index table
      assertExplainPlan(conn, localIndex, selectSql, dataTableName,
        (view && !localIndex ? "_IDX_" + dataTableName : indexName));
