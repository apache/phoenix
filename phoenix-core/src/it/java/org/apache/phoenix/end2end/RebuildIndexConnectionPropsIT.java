...
public class RebuildIndexConnectionPropsIT extends BaseTest {
  ...
  public void testRebuildIndexConnectionProperties() throws Exception {
    try (PhoenixConnection rebuildIndexConnection = MetaDataRegionObserver
      .getRebuildIndexConnection(hbaseTestUtil.getMiniHBaseCluster().getConfiguration())) {
      try (PhoenixConnection regularConnection =
        ...
        assertTrue(rebuildUrl.contains(MetaDataRegionObserver.REBUILD_INDEX_APPEND_TO_URL_STRING));
        ...
        assertEquals(Long.toString(QueryServicesOptions.DEFAULT_INDEX_REBUILD_QUERY_TIMEOUT),
          rebuildQueryServicesConfig.get(QueryServices.THREAD_TIMEOUT_MS_ATTRIB));
        assertEquals(
          Long.toString(QueryServicesOptions.DEFAULT_INDEX_REBUILD_CLIENT_SCANNER_TIMEOUT),
        ...
        Connection rebuildIndexHConnection =
          (Connection) Whitebox.getInternalState(rebuildQueryServices, "connection");
        Connection regularHConnection = (Connection) Whitebox
          .getInternalState(regularConnection.getQueryServices(), "connection");
