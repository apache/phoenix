...
public class PartialIndexRebuilderIT extends BaseTest {
  ...
  public static synchronized void doSetup() throws Exception {
    ...
    indexRebuildTaskRegionEnvironment = getUtility()
      ...
      .getRegions(PhoenixDatabaseMetaData.SYSTEM_CATALOG_HBASE_TABLE_NAME).get(0)
      .getCoprocessorHost().findCoprocessorEnvironment(MetaDataRegionObserver.class.getName());
  ...
  private static void runIndexRebuilder(List<String> tables)
    ...
    BuildIndexScheduleTask task =
      new MetaDataRegionObserver.BuildIndexScheduleTask(indexRebuildTaskRegionEnvironment, tables);
    task.run();
  }
  ...
  private static void mutateRandomly(final String fullTableName, final int nThreads,
    final int nRows, final int nIndexValues, final int batchSize, final CountDownLatch doneSignal) {
        ...
        public void run() {
          try {
            ...
            for (int i = 0; i < 3000; i++) {
              ...
              conn.createStatement().execute("UPSERT INTO " + fullTableName + " VALUES (" + pk
                + ", 0, " + (isNull ? null : randInt) + ")");
              if ((i % batchSize) == 0) {
                conn.commit();
              }
            }
            ...
            for (int i = 0; i < 3000; i++) {
              ...
              conn.createStatement()
                .execute("DELETE FROM " + fullTableName + " WHERE k1= " + pk + " AND k2=0");
              if (i % batchSize == 0) {
                conn.commit();
              }
            }
            ...
            for (int i = 0; i < 3000; i++) {
              ...
              conn.createStatement().execute(
                "UPSERT INTO " + fullTableName + " VALUES (" + pk + ", 0, " + randInt + ")");
              if ((i % batchSize) == 0) {
                conn.commit();
              }
            }
  ...
  public void testConcurrentUpsertsWithRebuild() throws Throwable {
    ...
    conn.createStatement().execute("CREATE TABLE " + fullTableName
      + "(k1 INTEGER NOT NULL, k2 INTEGER NOT NULL, v1 INTEGER, CONSTRAINT pk PRIMARY KEY (k1,k2)) STORE_NULLS=true, VERSIONS=1");
    ...
    IndexUtil.updateIndexState(fullIndexName, EnvironmentEdgeManager.currentTimeMillis(), metaTable,
      PIndexState.DISABLE);
  ...
  private static boolean hasInactiveIndex(PMetaData metaCache, PTableKey key)
  ...
  private static boolean hasDisabledIndex(PMetaData metaCache, PTableKey key)
    throws TableNotFoundException {
    return hasIndexWithState(metaCache, key, PIndexState.DISABLE);
  }

  private static boolean hasIndexWithState(PMetaData metaCache, PTableKey key,
    PIndexState expectedState) throws TableNotFoundException {
    PTable table = metaCache.getTableRef(key).getTable();
    for (PTable index : table.getIndexes()) {
  ...
  private static boolean mutateRandomly(Connection conn, String fullTableName, int nRows,
    ...
    for (int i = 0; i < 10000; i++) {
      ...
      conn.createStatement()
        .execute("UPSERT INTO " + fullTableName + " VALUES(" + pk + "," + v1 + "," + v2 + ")");
      if (i % batchSize == 0) {
        conn.commit();
        if (checkForInactive) {
  ...
  public void testDeleteAndUpsertAfterFailure() throws Throwable {
    ...
    try (Connection conn = DriverManager.getConnection(getUrl())) {
      conn.createStatement().execute("CREATE TABLE " + fullTableName
        + "(k INTEGER PRIMARY KEY, v1 INTEGER, v2 INTEGER) COLUMN_ENCODED_BYTES = 0, STORE_NULLS=true");
      conn.createStatement()
        .execute("CREATE INDEX " + indexName + " ON " + fullTableName + " (v1) INCLUDE (v2)");
  ...
  public void testMultiVersionsAfterFailure() throws Throwable {
    ...
    try (Connection conn = DriverManager.getConnection(getUrl())) {
      conn.createStatement().execute("CREATE TABLE " + fullTableName
        + "(k VARCHAR PRIMARY KEY, v VARCHAR) COLUMN_ENCODED_BYTES = 0, STORE_NULLS=true");
  ...
  private static void waitForIndexState(Connection conn, String fullTableName, String fullIndexName,
  ...
  public void testMultiValuesAtSameTS() throws Throwable {
    ...
    try (Connection conn = DriverManager.getConnection(getUrl())) {
      ...
      Table metaTable = conn.unwrap(PhoenixConnection.class).getQueryServices()
        .getTable(PhoenixDatabaseMetaData.SYSTEM_CATALOG_NAME_BYTES);
      IndexUtil.updateIndexState(fullIndexName, clock.currentTime(), metaTable,
        PIndexState.DISABLE);
  ...
  public void testTimeBatchesInCoprocessorRequired() throws Throwable {
    ...
    try (Connection conn = DriverManager.getConnection(getUrl())) {
      PMetaData metaCache = conn.unwrap(PhoenixConnection.class).getMetaDataCache();
      conn.createStatement().execute("CREATE TABLE " + fullTableName
        + "(k VARCHAR PRIMARY KEY, v1 VARCHAR, v2 VARCHAR) COLUMN_ENCODED_BYTES = 0, STORE_NULLS=true");
      conn.createStatement()
        .execute("CREATE INDEX " + indexName + " ON " + fullTableName + " (v1, v2)");
  ...
  public void testUpperBoundSetOnRebuild() throws Throwable {
    ...
    try (Connection conn = DriverManager.getConnection(getUrl())) {
      ...
      conn.createStatement().execute("UPSERT INTO " + fullTableName + " VALUES('a','a', '0')");
      conn.commit();
      // Set clock forward in time past the "overlap" amount we wait for index maintenance to kick
      // in
  ...
  public void testMultiValuesWhenDisableAndInactive() throws Throwable {
    ...
    try (Connection conn = DriverManager.getConnection(getUrl())) {
      PMetaData metaCache = conn.unwrap(PhoenixConnection.class).getMetaDataCache();
      conn.createStatement().execute("CREATE TABLE " + fullTableName
        + "(k VARCHAR PRIMARY KEY, v1 VARCHAR, v2 VARCHAR, v3 VARCHAR) COLUMN_ENCODED_BYTES = 0, STORE_NULLS=true");
      conn.createStatement()
        .execute("CREATE INDEX " + indexName + " ON " + fullTableName + " (v1, v2) INCLUDE (v3)");
      conn.createStatement().execute("UPSERT INTO " + fullTableName + " VALUES('a','a','0','x')");
      conn.commit();
      try (Table metaTable = conn.unwrap(PhoenixConnection.class).getQueryServices()
        .getTable(PhoenixDatabaseMetaData.SYSTEM_CATALOG_NAME_BYTES)) {
        // By using an INDEX_DISABLE_TIMESTAMP of 0, we prevent the partial index rebuilder from
        // triggering
        IndexUtil.updateIndexState(fullIndexName, 0L, metaTable, PIndexState.DISABLE);
        long disableTime = clock.currentTime();
        // Set some values while index disabled
        conn.createStatement()
          .execute("UPSERT INTO " + fullTableName + " VALUES('b','bb', '11','yy')");
        conn.commit();
        assertTrue(hasDisabledIndex(metaCache, key));
        conn.createStatement()
          .execute("UPSERT INTO " + fullTableName + " VALUES('a','ccc','222','zzz')");
        conn.commit();
        conn.createStatement()
          .execute("UPSERT INTO " + fullTableName + " VALUES('a','dddd','3333','zzzz')");
        conn.commit();
      ...
      conn.createStatement()
        .execute("UPSERT INTO " + fullTableName + " VALUES('a','eeeee','44444','zzzzz')");
      conn.commit();
      conn.createStatement()
        .execute("UPSERT INTO " + fullTableName + " VALUES('a','fffff','55555','zzzzzz')");
      conn.commit();
      clock.time += WAIT_AFTER_DISABLED;
  ...
  private void testIndexWriteFailureDuringRebuild(PIndexState indexStateOnFailure)
    throws Throwable {
    String schemaName = generateUniqueName();
    String tableName = generateUniqueName();
    String indexName = generateUniqueName();
    ...
    try (Connection conn = DriverManager.getConnection(getUrl())) {
      PMetaData metaCache = conn.unwrap(PhoenixConnection.class).getMetaDataCache();
      conn.createStatement().execute("CREATE TABLE " + fullTableName
        + "(k VARCHAR PRIMARY KEY, v1 VARCHAR, v2 VARCHAR, v3 VARCHAR) COLUMN_ENCODED_BYTES = 0, DISABLE_INDEX_ON_WRITE_FAILURE = "
      ...
      IndexUtil.updateIndexState(fullIndexName,
        indexStateOnFailure == PIndexState.DISABLE ? disableTime : -disableTime, metaTable,
        indexStateOnFailure);
      ...
      waitForIndexState(conn, fullTableName, fullIndexName,
        indexStateOnFailure == PIndexState.DISABLE ? PIndexState.INACTIVE : PIndexState.ACTIVE);
      clock.time += WAIT_AFTER_DISABLED;
      ...
      conn.createStatement()
        .execute("UPSERT INTO " + fullTableName + " VALUES('dddd','dddd','3333')");
      try {
        conn.commit();
        fail();
      ...
      assertEquals(indexStateOnFailure == PIndexState.DISABLE ? fullTableName : fullIndexName,
  ...
  public void testPendingDisable() throws Throwable {
    ...
    try (Connection conn = DriverManager.getConnection(getUrl())) {
      conn.createStatement().execute("CREATE TABLE " + fullTableName
        + "(k VARCHAR PRIMARY KEY, v1 VARCHAR, v2 VARCHAR, v3 VARCHAR) COLUMN_ENCODED_BYTES = 0, DISABLE_INDEX_ON_WRITE_FAILURE = TRUE");
      clock.time += 100;
      conn.createStatement()
        .execute("CREATE INDEX " + indexName + " ON " + fullTableName + " (v1, v2)");
      clock.time += 100;
  ...
  public void testIndexFailureWithinRSDoesnotDisablesIndex() throws Throwable {
    ...
    try (Connection conn = DriverManager.getConnection(getUrl())) {
      try {
        conn.createStatement().execute("CREATE TABLE " + fullTableName
          + "(k VARCHAR PRIMARY KEY, v1 VARCHAR, v2 VARCHAR, v3 VARCHAR) DISABLE_INDEX_ON_WRITE_FAILURE = TRUE");
        conn.createStatement()
          .execute("CREATE INDEX " + indexName + " ON " + fullTableName + " (v1, v2)");
        conn.createStatement()
          .execute("UPSERT INTO " + fullTableName + " VALUES('a','a','0', 't')");
        conn.commit();
        // Simulate write failure
        TestUtil.addCoprocessor(conn, fullIndexName, WriteFailingRegionObserver.class);
        ...
        assertFalse(
          TestUtil.checkIndexState(conn, fullIndexName, PIndexState.PENDING_ACTIVE, null));
      } finally {
        TestUtil.removeCoprocessor(conn, fullIndexName, WriteFailingRegionObserver.class);
      }
    }
  ...
  public void testPendingDisableWithDisableCountTs() throws Throwable {
    ...
    final String fullTableName = SchemaUtil.getTableName(schemaName, tableName);
    final String fullIndexName = SchemaUtil.getTableName(schemaName, indexName);
    final MyClock clock = new MyClock(EnvironmentEdgeManager.currentTimeMillis());
    Properties props = PropertiesUtil.deepCopy(TEST_PROPERTIES);

    try (Connection conn = DriverManager.getConnection(getUrl(), props)) {
      conn.createStatement()
        .execute(String.format("CREATE TABLE %s (k VARCHAR PRIMARY KEY, v1 VARCHAR, v2 VARCHAR, "
          + "v3 VARCHAR, v4 VARCHAR) COLUMN_ENCODED_BYTES = 0, "
          + "DISABLE_INDEX_ON_WRITE_FAILURE = TRUE", fullTableName));
      EnvironmentEdgeManager.injectEdge(clock);
      clock.addTime(100);
      conn.createStatement()
        .execute(String.format("CREATE INDEX %s ON %s (v1, v2)", indexName, fullTableName));
      clock.addTime(100);
      conn.createStatement().execute(
        String.format("UPSERT INTO %s VALUES('k01', 'v01', 'v02', 'v03', 'v04')", fullTableName));
      conn.commit();
      clock.addTime(100);

      try (Table systemCatalog = conn.unwrap(PhoenixConnection.class).getQueryServices()
        .getTable(PhoenixDatabaseMetaData.SYSTEM_CATALOG_NAME_BYTES)) {
        IndexUtil.updateIndexState(fullIndexName, clock.currentTime(), systemCatalog,
          PIndexState.PENDING_DISABLE);
      }
      ...
      PhoenixStatement stmt = conn.createStatement().unwrap(PhoenixStatement.class);
      ResultSet rs =
        stmt.executeQuery(String.format("SELECT V2 FROM %s WHERE V1 = 'v01'", fullTableName));
      assertTrue(rs.next());
