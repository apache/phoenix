...
public class DropMetadataIT extends ParallelStatsDisabledIT {
  ...
  public void testDropIndexTableHasSameNameWithDataTable() {
    ...
    try (Connection conn = DriverManager.getConnection(getUrl())) {
      String createTable =
        "CREATE TABLE " + tableName + "  (id varchar not null primary key, col integer)";
  ...
  public void testDropViewKeepsHTable() throws Exception {
    ...
    try {
      TableDescriptorBuilder builder =
        TableDescriptorBuilder.newBuilder(TableName.valueOf(hbaseNativeBytes));
      ColumnFamilyDescriptor columnDescriptor = ColumnFamilyDescriptorBuilder
        .newBuilder(FAMILY_NAME).setKeepDeletedCells(KeepDeletedCells.TRUE).build();
    ...
    conn.createStatement()
      .execute("create view " + hbaseNativeViewName + "   (uint_key unsigned_int not null,"
