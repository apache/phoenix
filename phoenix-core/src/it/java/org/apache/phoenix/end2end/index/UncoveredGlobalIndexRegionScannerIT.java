...
public class UncoveredGlobalIndexRegionScannerIT extends BaseTest {
  ...
  public void testDDLWithPhoenixRowTimestamp() throws Exception {
    try (Connection conn = DriverManager.getConnection(getUrl())) {
      ...
      conn.createStatement()
        .execute("create table " + dataTableName + " (id varchar(10) not null primary key)");
  ...
  private void testUncoveredGlobalIndex(boolean salted) throws Exception {
    String dataTableName = generateUniqueName();
    populateTable(dataTableName, salted); // with two rows ('a', 'ab', 'abc', 'abcd') and ('b',
                                          // 'bc', 'bcd', 'bcde')
    try (Connection conn = DriverManager.getConnection(getUrl())) {
      String indexTableName = generateUniqueName();
      conn.createStatement().execute(
      ...
      // Verify that with index hint, we will read from the index table even though val3 is not
      // included by the index table
      String selectSql =
        "SELECT /*+ INDEX(" + dataTableName + " " + indexTableName + ")*/ val3,val2 from "
      ...
      indexTableName = generateUniqueName();
      conn.createStatement()
        .execute("CREATE INDEX " + indexTableName + " on " + dataTableName + " (val1)");
      conn.commit();
  ...
  public void testSkipScanFilter() throws Exception {
    try (Connection conn = DriverManager.getConnection(getUrl())) {
      ...
      String indexTableName = generateUniqueName();
      conn.createStatement()
        .execute("CREATE INDEX " + indexTableName + " ON " + dataTableName + "(v1) include (v2)");
  ...
  public void testCount() throws Exception {
    try (Connection conn = DriverManager.getConnection(getUrl())) {
      ...
      conn.createStatement()
        .execute("CREATE TABLE " + dataTableName
          + "(k1 BIGINT NOT NULL, k2 BIGINT NOT NULL, v1 INTEGER, " + "v2 INTEGER, v3 BIGINT "
          + "CONSTRAINT pk PRIMARY KEY (k1,k2)) " + " VERSIONS=1, IMMUTABLE_ROWS=TRUE");
      String indexTableName = generateUniqueName();
      conn.createStatement()
        .execute("CREATE INDEX " + indexTableName + " ON " + dataTableName + "(v1) include (v2)");
