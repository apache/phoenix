...
public abstract class ConcurrentMutationsExtendedIndexIT extends ParallelStatsDisabledIT {
  ...
  public void testConcurrentUpsertsWithTableSplits() throws Exception {
    ...
    if (eventual) {
      try {
        verifyRandomIndexes(allIndexes, schemaName, tableName, conn, nRows, false);
    ...
    try {
      verifyRandomIndexes(allIndexes, schemaName, tableName, conn, nRows, true);
    } finally {
      dumpTableRows(conn, "SYSTEM.CDC_STREAM");
      dumpTableRows(conn, "SYSTEM.IDX_CDC_TRACKER");
    }
    // conn.createStatement()
    // .execute("DROP TABLE " + "\"" + schemaName + "\".\"" + tableName + "\"" + " CASCADE");
  }

  private static void verifyRandomIndexes(List<String> allIndexes, String schemaName,
    String tableName, Connection conn, int nRows, boolean dumpTables) throws Exception {
  ...
  public void testConcurrentUpsertsWithTableSplitsMerges() throws Exception {
    ...
    assertTrue("Ran out of time", doneSignal.await(800, TimeUnit.SECONDS));
    LOGGER.info(
      "Total upsert time in ms : " + (EnvironmentEdgeManager.currentTimeMillis() - startTime));
