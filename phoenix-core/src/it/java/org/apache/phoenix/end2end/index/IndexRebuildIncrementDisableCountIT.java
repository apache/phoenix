...
public class IndexRebuildIncrementDisableCountIT extends BaseTest {
  ...
  static long getPendingDisableCount(PhoenixConnection conn, String indexTableName) {
    ...
    try {
      pendingDisableCountResult = conn.getQueryServices()
  ...
  private static void checkIndexPendingDisableCount(final PhoenixConnection conn,
    ...
    Runnable runnable = new Runnable() {
      ...
      public void run() {
        try {
          while (!TestUtil.checkIndexState(conn, indexTableName, PIndexState.ACTIVE, 0L)) {
            long count = getPendingDisableCount(conn, indexTableName);
            if (count > 0) {
              indexState = new String(pendingDisableCountResult.getValue(TABLE_FAMILY_BYTES,
  ...
  private static void mutateRandomly(Connection conn, String tableName, int maxOrgId) {
    try {
      ...
      for (int i = 0; i < 10000; i++) {
        stmt.executeUpdate("UPSERT INTO " + tableName + " VALUES('" + getRandomOrgId(maxOrgId)
          + "'," + i + "," + (i + 1) + "," + (i + 2) + ")");
      }
      conn.commit();
    } catch (Exception e) {
  ...
  private static MutationCode updateIndexState(PhoenixConnection phoenixConn, String fullIndexName,
    PIndexState state) throws Throwable {
    Table metaTable =
      phoenixConn.getQueryServices().getTable(PhoenixDatabaseMetaData.SYSTEM_CATALOG_NAME_BYTES);
  ...
  public void checkIndexPendingDisableResetCounter() throws Throwable {
    ...
    assertTrue(
      "Pending disable count should reset when index moves from ACTIVE to PENDING_DISABLE ",
      getPendingDisableCount(phoenixConn, fullIndexName) == 0);
    IndexUtil.incrementCounterForIndex(phoenixConn, fullIndexName,
      MetaDataRegionObserver.PENDING_DISABLE_INACTIVE_STATE_COUNT);
    ...
    assertTrue(
      "Pending disable count should reset when index moves from ACTIVE to PENDING_DISABLE ",
      getPendingDisableCount(phoenixConn, fullIndexName)
          == MetaDataRegionObserver.PENDING_DISABLE_INACTIVE_STATE_COUNT);
  }
