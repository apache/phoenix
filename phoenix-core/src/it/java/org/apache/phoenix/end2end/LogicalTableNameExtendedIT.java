...
public class LogicalTableNameExtendedIT extends LogicalTableNameBaseIT {
    ...
    try (Connection conn = getConnection(propsNamespace)) {
      try (Connection conn2 = getConnection(propsNamespace)) {
        ...
        ResultSet rs = conn2.createStatement().executeQuery(
          "SELECT * FROM " + (schemaName + "." + view2IndexName1) + " WHERE \":PK1\"='PK10'");
  ...
  private void test_bothTableAndIndexHaveDifferentNames(Connection conn, Connection conn2,
    ...
    try (Connection conn = getConnection(propsNamespace)) {
      try (Connection conn2 = getConnection(propsNamespace)) {
        ...
        try (Admin admin = conn.unwrap(PhoenixConnection.class).getQueryServices().getAdmin()) {
          ...
          rs = conn.createStatement()
            .executeQuery("SELECT * FROM " + fullIndexName + " WHERE \":PK1\"='PK30'");
          assertEquals(false, rs.next());
          try (Table htable = conn.unwrap(PhoenixConnection.class).getQueryServices()
            .getTable(Bytes.toBytes(fullNewTableHName))) {
          ...
          assertEquals(false,
            admin.tableExists(TableName.valueOf((schemaName + ":NEW_IDXTBL_" + indexName))));
        }
      }
    }
  }
    ...
    try (Connection conn = getConnection(propsNamespace)) {
      try (Connection conn2 = getConnection(propsNamespace)) {
        ...
        conn2.createStatement().execute(
          "ALTER TABLE " + fullTableName + " ADD new_column_1 VARCHAR(64) CASCADE INDEX ALL");
        ...
        rs = conn.createStatement().executeQuery(
          "SELECT NEW_COLUMN_1 FROM " + fullTableName + " WHERE NEW_COLUMN_1 IS NOT NULL");
        assertEquals(true, rs.next());
        assertEquals(false, rs.next());

        // Drop column, check is that there are no exceptions
        conn.createStatement()
          .execute("ALTER TABLE " + fullTableName + " DROP COLUMN NEW_COLUMN_1");
      }
    }
  }
  ...
  public void testHint() throws Exception {
    ...
    try (Connection conn = getConnection(propsNamespace)) {
      ...
      String indexSelect =
        "SELECT /*+ INDEX(" + tableName + " " + indexName2 + ")*/ V1,V2,V3 FROM " + tableName;
  ...
  public void testUpdatePhysicalTableName_viewIndexSequence() throws Exception {
    ...
    try (Connection conn = getConnection(propsNamespace)) {
      ...
      String indexDDL = "CREATE INDEX IF NOT EXISTS " + viewIndexName2 + " ON " + fullViewName
      ...
      validateIndex(conn, fullViewIndex1Name, true, expected);
      rs = conn.createStatement().executeQuery(
        "SELECT * FROM " + fullViewIndex2Name + " WHERE \"0:VIEW_COL1\"='VIEW_COL1_10'");
      assertEquals(true, rs.next());
      ...
      conn.createStatement().execute("CREATE VIEW " + fullViewName2
        + "  (VIEW_COL1 VARCHAR, VIEW_COL2 VARCHAR) AS SELECT * FROM " + fullTableName);
      conn.createStatement().execute("CREATE INDEX IF NOT EXISTS " + view2IndexName1 + " ON "
        + fullViewName2 + " (VIEW_COL1) include (VIEW_COL2)");
      populateView(conn, fullViewName2, 20, 1);
      rs = conn.createStatement().executeQuery(
        "SELECT * FROM " + fullView2Index1Name + " WHERE \"0:VIEW_COL1\"='VIEW_COL1_10'");
      assertEquals(true, rs.next());
      assertEquals("VIEW_COL1_10", rs.getString(1));
      assertEquals("PK10", rs.getString(2));
      assertEquals("VIEW_COL2_10", rs.getString(3));
