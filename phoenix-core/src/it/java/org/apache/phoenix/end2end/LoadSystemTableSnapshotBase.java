...
/**
 * This is a not a standard IT. It is starting point for writing ITs that load specific tables from
 * a snapshot. Tests based on this IT are meant for debugging specific problems where HBase table
 * snapshots are available for replication, and are not meant to be part of the standard test suite
 * (or even being committed to the ASF branches)
 */

// TODO:- Snapshot here is storing integers as TTL Value and Phoenix Level TTL is Long, need to work
// on this.
public abstract class LoadSystemTableSnapshotBase extends BaseTest {
  ...
  public static synchronized void setupCluster(boolean createBlockUpgradeMutex, String tarName, String snapshotDir, HashMap<String, String> snapshotsToLoad, String nameSpaceMapping) throws Exception {
    ...
    for (Entry<String, String> snapshot : SNAPSHOTS_TO_LOAD.entrySet()) {
      String snapshotLoc =
        new File(folderUrl.getFile()).getAbsolutePath() + "/" + snapshot.getKey();
    ...
    if (createBlockUpgradeMutex) {
      try {
        ...
        try {
          mutexTableName = SchemaUtil.getPhysicalTableName(SYSTEM_MUTEX_NAME,
            new ReadOnlyProps(serverProps.entrySet().iterator()));
  ...
  private static void importSnapshot(String key, String value, String loc) throws IOException {
    ...
    int importExitCode = exportTool.run(new String[] { "-snapshot", key, "-copy-from",
      hdfsImportPath.toUri().toString(), "-copy-to", rootDir });
