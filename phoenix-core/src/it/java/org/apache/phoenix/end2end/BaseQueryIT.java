...
public abstract class BaseQueryIT extends ParallelStatsDisabledIT {
  ...
  protected static final String[] GLOBAL_INDEX_DDLS = new String[] {
    ...
    "CREATE INDEX IF NOT EXISTS %s ON %s (a_integer) INCLUDE (" + "    A_STRING, "
      + "    B_STRING, " + "    A_DATE)",
    NO_INDEX };
  protected static final String[] LOCAL_INDEX_DDLS = new String[] {
    "CREATE LOCAL INDEX %s ON %s (a_integer DESC) INCLUDE (" + "    A_STRING, " + "    B_STRING, "
      + "    A_DATE)",
    "CREATE LOCAL INDEX %s ON %s (a_integer, a_string) INCLUDE (" + "    B_STRING, "
      + "    A_DATE)",
    "CREATE LOCAL INDEX %s ON %s (a_integer) INCLUDE (" + "    A_STRING, " + "    B_STRING, "
      + "    A_DATE)" };
  ...
  public static synchronized void initTables(String idxDdl, boolean columnEncoded,
    ...
    try {
      tableName = initATableValues(generateUniqueName(), tenantId, getDefaultSplits(tenantId),
    ...
    if (idxDdl.length() > 0) {
      ...
      try (Connection conn = DriverManager.getConnection(getUrl(), props)) {
        ...
        try (Connection conn = DriverManager.getConnection(getUrl(), props)) {
          ...
          LOGGER.error("Exception while creating index during second retry: " + indexDDL, ex);
          throw ex;
        }
      } catch (Exception e) {
