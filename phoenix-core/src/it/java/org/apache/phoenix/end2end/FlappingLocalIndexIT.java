...
public class FlappingLocalIndexIT extends BaseLocalIndexIT {
  ...
  public void testScanWhenATableHasMultipleLocalIndexes() throws Exception {
    ...
    try {
      ...
      conn1.createStatement()
        .execute("CREATE LOCAL INDEX " + indexName + "2 ON " + tableName + "(k3)");
      conn1.commit();
  ...
  public void testLocalIndexScan() throws Exception {
    ...
    try {
      ...
      int trimmedRegionLocations =
        admin.getConfiguration().getInt(QueryServices.MAX_REGION_LOCATIONS_SIZE_EXPLAIN_PLAN, -1);

      String query = "SELECT * FROM " + tableName + " where v1 like 'a%'";

      String explainPlanOutput = QueryUtil
        .getExplainPlan(conn1.createStatement().executeQuery("EXPLAIN WITH REGIONS " + query));
      ...
      assertEquals(" [1,'a'] - [1,'b']", explainPlanAttributes.getKeyRanges());
      ...
      assertEquals(trimmedRegionLocations, explainPlanAttributes.getRegionLocations().size());
      ...
      assertEquals(indexTableName + "(" + indexPhysicalTableName + ")",
        explainPlanAttributes.getTableName());
      assertEquals(" [1,'a']", explainPlanAttributes.getKeyRanges());
      assertEquals("SERVER FILTER BY FIRST KEY ONLY", explainPlanAttributes.getServerWhereFilter());
      assertEquals("CLIENT MERGE SORT", explainPlanAttributes.getClientSortAlgo());
  ...
  @Test
  public void testBuildingLocalCoveredIndexShouldHandleNoSuchColumnFamilyException()
    throws Exception {
    testBuildingLocalIndexShouldHandleNoSuchColumnFamilyException(true);
  }

  private void testBuildingLocalIndexShouldHandleNoSuchColumnFamilyException(boolean coveredIndex)
    throws Exception {
    String tableName = schemaName + "." + generateUniqueName();
    String indexName = "IDX_" + generateUniqueName();
    String indexTableName = schemaName + "." + indexName;
    ...
    tableDescriptor = TableDescriptorBuilder.newBuilder(tableDescriptor)
      .setCoprocessor(
        CoprocessorDescriptorBuilder.newBuilder(DeleyOpenRegionObserver.class.getName())
    ...
    conn1.createStatement().execute("CREATE LOCAL INDEX " + indexName + " ON " + tableName + "(k3)"
