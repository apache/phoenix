 ...
 * that ResultSet.getString(columnName) works correctly for both the original column name and
...
public class ViewIndexColumnNameGetterIT extends ParallelStatsDisabledIT {
    ...
    /**
     * Tests basic view index with view constant column retrieval by name.
     * This is the core scenario reported in PHOENIX-6644.
     */
    @Test
    public void testViewIndexColumnNameGetter() throws Exception {
        try (Connection conn = DriverManager.getConnection(getUrl())) {
            ...
            conn.createStatement().execute("CREATE VIEW " + viewName +
                    " AS SELECT * FROM " + tableName + " WHERE v1 = 'a'");

            conn.createStatement().execute("CREATE INDEX " + indexName +
                    " ON " + viewName + " (v2) INCLUDE (v3)");
            ...
            ResultSet rs = conn.createStatement().executeQuery(
                    "SELECT v1, v3 FROM " + viewName + " WHERE v2 = 'ab'");
    ...
    public void testMultipleViewConstantColumns() throws Exception {
        try (Connection conn = DriverManager.getConnection(getUrl())) {
            ...
            conn.createStatement().execute("CREATE VIEW " + viewName +
                    " AS SELECT * FROM " + tableName + " WHERE v1 = 'a' AND v2 = 'b'");

            conn.createStatement().execute("CREATE INDEX " + indexName +
                    " ON " + viewName + " (v3) INCLUDE (v4)");
            ...
            ResultSet rs = conn.createStatement().executeQuery(
                    "SELECT v1, v2, v4 FROM " + viewName + " WHERE v3 = 'c'");
    ...
    public void testViewIndexWithSelectStar() throws Exception {
        try (Connection conn = DriverManager.getConnection(getUrl())) {
            ...
            conn.createStatement().execute("CREATE VIEW " + viewName +
                    " AS SELECT * FROM " + tableName + " WHERE v1 = 'xyz'");

            conn.createStatement().execute("CREATE INDEX " + indexName +
                    " ON " + viewName + " (v2) INCLUDE (v3)");

            conn.createStatement().execute("UPSERT INTO " + viewName +
                    " VALUES (1, 'xyz', 'indexed', 'included')");
            conn.commit();

            ResultSet rs = conn.createStatement().executeQuery(
                    "SELECT * FROM " + viewName + " WHERE v2 = 'indexed'");
    ...
    public void testPreparedStatementWithViewIndex() throws Exception {
        try (Connection conn = DriverManager.getConnection(getUrl())) {
            ...
            conn.createStatement().execute("CREATE VIEW " + viewName +
                    " AS SELECT * FROM " + tableName + " WHERE v1 = 'constant'");

            conn.createStatement().execute("CREATE INDEX " + indexName +
                    " ON " + viewName + " (v2) INCLUDE (v3)");

            conn.createStatement().execute("UPSERT INTO " + viewName +
                    " VALUES (1, 'constant', 'value1', 'data1')");
            conn.createStatement().execute("UPSERT INTO " + viewName +
                    " VALUES (2, 'constant', 'value2', 'data2')");
            conn.commit();
    ...
    public void testSubqueryWithViewIndex() throws Exception {
        try (Connection conn = DriverManager.getConnection(getUrl())) {
            ...
            conn.createStatement().execute("CREATE VIEW " + viewName +
                    " AS SELECT * FROM " + tableName + " WHERE v1 = 'subquery_test'");

            conn.createStatement().execute("CREATE INDEX " + indexName +
                    " ON " + viewName + " (v2) INCLUDE (v3)");

            conn.createStatement().execute("UPSERT INTO " + viewName +
                    " VALUES (1, 'subquery_test', 'indexed', 'result')");
            conn.commit();

            // Query using subquery with view index
            String subquery = "SELECT v1, v3 FROM " + viewName + " WHERE v2 = 'indexed'";
            ResultSet rs = conn.createStatement().executeQuery(
                    "SELECT * FROM (" + subquery + ") sub WHERE sub.v1 = 'subquery_test'");
    ...
    public void testRegularIndexNotAffected() throws Exception {
        try (Connection conn = DriverManager.getConnection(getUrl())) {
            ...
            conn.createStatement().execute("CREATE INDEX " + indexName +
                    " ON " + tableName + " (col1) INCLUDE (col2)");
            ...
            ResultSet rs = conn.createStatement().executeQuery(
                    "SELECT col1, col2 FROM " + tableName + " WHERE col1 = 'value1'");
