...
public class ConcurrentMutationsExtendedIT extends ParallelStatsDisabledIT {
  ...
  static long verifyIndexTable(String tableName, String indexName, Connection conn)
    throws Exception {
    // This checks the state of every raw index row without rebuilding any row
    IndexTool indexTool = IndexToolIT.runIndexTool(false, "", tableName, indexName, null, 0,
    ...
    indexTool = IndexToolIT.runIndexTool(false, "", tableName, indexName, null, 0,
      IndexTool.IndexVerifyType.ONLY);
    System.out.println(indexTool.getJob().getCounters());
    ...
    assertEquals(0, indexTool.getJob().getCounters()
      .findCounter(BEFORE_REBUILD_BEYOND_MAXLOOKBACK_INVALID_INDEX_ROW_COUNT).getValue());
    // The index scrutiny run will trigger index repair on all unverified rows and they rows will be
    // made verified
  ...
  public void testConcurrentDeletesAndUpsertValues() throws Exception {
    ...
    TestUtil.addCoprocessor(conn, tableName, DelayingRegionObserver.class);
    conn.createStatement().execute("CREATE INDEX " + indexName + " ON " + tableName + "(v1)");
    conn.createStatement().execute("CREATE INDEX " + singleCellindexName + " ON " + tableName
  ...
  public void testConcurrentUpserts() throws Exception {
    ...
    conn.createStatement()
      .execute("CREATE INDEX " + indexName + " ON " + tableName + "(v1) INCLUDE(v2, v3)");
