...
/**
 * Disabling this test as this works on TTL being set on View which is removed and will be added in
 * future. TODO:- To enable this test after re-enabling TTL for view for more info check :-
 * PHOENIX-6978
 */
@Ignore
...
public class DefaultPhoenixMultiViewListProviderIT extends ParallelStatsDisabledIT {
  ...
  private final String VIEW_DDL = "CREATE VIEW %s ("
    + "PK1 BIGINT PRIMARY KEY,A BIGINT, B BIGINT, C BIGINT, D BIGINT)" + " AS SELECT * FROM %s ";
  private final String VIEW_DDL_WITH_ID_PREFIX_AND_TTL = VIEW_DDL + " TTL = 1000";
  ...
  public void testGetPhoenixMultiViewList() throws Exception {
    ...
    /*
     * Case 2 : BaseMultiTenantTable GlobalView1 with TTL(1 ms) Index1 Index2 TenantView1,
     * TenantView2
     */
    try (Connection globalConn = DriverManager.getConnection(url);
      Connection tenant1Connection = PhoenixMultiInputUtil.buildTenantConnection(url, tenant1);
      Connection tenant2Connection = PhoenixMultiInputUtil.buildTenantConnection(url, tenant2)) {

      globalConn.createStatement().execute(String.format(BASE_TABLE_DDL, baseTableFullName));
      globalConn.createStatement().execute(
        String.format(VIEW_DDL_WITH_ID_PREFIX_AND_TTL, globalViewName1, baseTableFullName));

      globalConn.createStatement()
        .execute(String.format(VIEW_INDEX_DDL, indexTable1, globalViewName1, "A,B"));
      globalConn.createStatement()
        .execute(String.format(VIEW_INDEX_DDL, indexTable2, globalViewName1, "C,D"));

      tenant1Connection.createStatement()
        .execute(String.format(TENANT_VIEW_DDL, tenantViewName1, globalViewName1));
      tenant2Connection.createStatement()
        .execute(String.format(TENANT_VIEW_DDL, tenantViewName2, globalViewName1));
    }
    ...
    /*
     * Case 3: globalView2 without TTL BaseMultiTenantTable GlobalView1 with TTL(1 ms) GlobalView2
     * without TTL Index1 Index2 Index3 Index4 TenantView1, TenantView2
     */
    try (Connection globalConn = DriverManager.getConnection(url)) {
      globalConn.createStatement()
        .execute(String.format(VIEW_DDL, globalViewName2, baseTableFullName));
      globalConn.createStatement()
        .execute(String.format(VIEW_INDEX_DDL, indexTable3, globalViewName2, "A,B"));
      globalConn.createStatement()
        .execute(String.format(VIEW_INDEX_DDL, indexTable4, globalViewName2, "C,D"));
    }
    result = defaultPhoenixMultiViewListProvider.getPhoenixMultiViewList(cloneConfig);
    assertEquals(3, result.size());

    /*
     * Case 4: adding tenant3 and tenant4 with TTL BaseMultiTenantTable GlobalView1 with TTL(1 ms)
     * GlobalView2 without TTL Index1 Index2 Index3 Index4 TenantView1, TenantView2 TenantView3 with
     * TTL, TenantView4 without TTL
     */

    try (Connection tenant1Connection = PhoenixMultiInputUtil.buildTenantConnection(url, tenant1);
      Connection tenant2Connection = PhoenixMultiInputUtil.buildTenantConnection(url, tenant2)) {
      tenant1Connection.createStatement()
        .execute(String.format(TENANT_VIEW_DDL_WITH_TTL, tenantViewName3, globalViewName2));
      tenant2Connection.createStatement()
        .execute(String.format(TENANT_VIEW_DDL, tenantViewName4, globalViewName2));
    }
    result = defaultPhoenixMultiViewListProvider.getPhoenixMultiViewList(cloneConfig);
    ...
    /*
     * Deleting tenant1 with tenantViewName1 will NOT add any deletion job to the list because the
     * parent global view has TTL value.
     */
    cloneConfig.set(PhoenixConfigurationUtil.MAPREDUCE_TENANT_ID, tenant1);
    ...
    /*
     * Without tenant id, it will not add the job to the list even the tenant view name is provided.
     */
    cloneConfig = PropertiesUtil.cloneConfig(config);
    cloneConfig.set(MAPREDUCE_MULTI_INPUT_QUERY_BATCH_SIZE, "2");
    cloneConfig.set(PhoenixConfigurationUtil.MAPREDUCE_PHOENIX_TTL_DELETE_JOB_PER_VIEW,
    ...
    /*
     * tenant id + tenant view name will add 3 job to the list. 1 for data table and 2 for the index
     * table.
     */
    cloneConfig.set(PhoenixConfigurationUtil.MAPREDUCE_TENANT_ID, tenant1);
    cloneConfig.set(PhoenixConfigurationUtil.MAPREDUCE_PHOENIX_TTL_DELETE_JOB_PER_VIEW,
    ...
    /*
     * tenant id + tenant view name will NOT add ot the list because tenantViewName4 did NOT have
     * TTL value.
     */
