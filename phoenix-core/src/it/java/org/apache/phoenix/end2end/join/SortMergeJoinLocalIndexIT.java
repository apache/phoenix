...
public class SortMergeJoinLocalIndexIT extends SortMergeJoinIT {
  ...
  protected void assertSkipMergeOptimizationPlan(Connection conn, String query) throws Exception {
    ...
    assertPlan(conn, query).abstractExplainPlan("SORT-MERGE-JOIN (LEFT)").scanType("RANGE SCAN")
  ...
  protected void assertSelfJoinPlan(Connection conn, String query) throws Exception {
    String item = getTableName(conn, JOIN_ITEM_TABLE_FULL_NAME);
    String itemIndex = SchemaUtil.getTableName(getSchemaName(), JOIN_ITEM_INDEX);
    assertPlan(conn, query).abstractExplainPlan("SORT-MERGE-JOIN (INNER)").scanType("RANGE SCAN")
      .table(itemIndex + "(" + item + ")").keyRanges(" [1]")
      .serverWhereFilter("SERVER FILTER BY FIRST KEY ONLY").serverSortedBy("[\"I1.:item_id\"]")
      .clientSortAlgo("CLIENT MERGE SORT").sortMergeSkipMerge(false).rhs().scanType("RANGE SCAN")
      .table(itemIndex + "(" + item + ")").keyRanges(" [1]")
  ...
  protected void assertSetMaxRowsPlan(Connection conn, String query, int queryIndex)
    ...
    assertPlan(attributes).abstractExplainPlan("SORT-MERGE-JOIN (INNER)").scanType("RANGE SCAN")
      .table(itemIndex + "(" + item + ")").keyRanges(" [1]")
      .serverWhereFilter("SERVER FILTER BY FIRST KEY ONLY").serverSortedBy("[\"I.:item_id\"]")
      .clientSortAlgo("CLIENT MERGE SORT").sortMergeSkipMerge(false).clientRowLimit(4).rhs()
