...
public class QueryTimeoutIT extends BaseTest {
  ...
  public void testSetRPCTimeOnConnection() throws Exception {
    ...
    overriddenProps.put(QueryServices.EXTRA_JDBC_ARGUMENTS_ATTRIB,
      QueryServicesOptions.DEFAULT_EXTRA_JDBC_ARGUMENTS);
  ...
  public void testQueryTimeout() throws Exception {
    ...
    conn.createStatement()
      .execute("CREATE TABLE " + tableName + "(k BIGINT PRIMARY KEY, v VARCHAR)");
    ...
    try {
      ...
      rs.next();
      fail("Total time of query was " + (System.currentTimeMillis() - startTime)
        + " ms, but expected to be greater than 1000");
    } catch (SQLTimeoutException e) {
      long elapsedTimeMillis = System.currentTimeMillis() - startTime;
      assertEquals(SQLExceptionCode.OPERATION_TIMED_OUT.getErrorCode(), e.getErrorCode());
      assertTrue("Total time of query was " + elapsedTimeMillis
        + " ms, but expected to be greater or equal to 1000", elapsedTimeMillis >= 1000);
    }
    conn.close();
  }
