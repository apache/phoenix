...
/**
 * Tests to ensure we perform READ_COMMITTED scan i.e. after the scanner is opened, any updated data
 * should not be visible to the scanner until it is closed. The ongoing scanner should only be able
 * to cover the region state that was present before the scanner was opened. HBase provides this
 * guarantee by using region level MVCC.
 */
...
public class ScanUncommittedWithRegionMovesIT extends ParallelStatsDisabledIT {
  ...
  private static class TestScanningResultPostDummyResultCaller
    ...
    public void postDummyProcess() {
      if (
        hasTestStarted && (countOfDummyResults++ % 5) == 0
  ...
  private static void moveRegionsOfTable(String tableName) throws IOException {
    ...
    regionsOnServer1.forEach(regionInfo -> {
      if (regionInfo.getTable().equals(TableName.valueOf(tableName))) {
        try {
          admin.move(regionInfo.getEncodedNameAsBytes(), Bytes.toBytes(server2.getServerName()));
        } catch (IOException e) {
          throw new RuntimeException(e);
        }
      }
    ...
    regionsOnServer2.forEach(regionInfo -> {
      if (regionInfo.getTable().equals(TableName.valueOf(tableName))) {
        try {
          admin.move(regionInfo.getEncodedNameAsBytes(), Bytes.toBytes(server1.getServerName()));
        } catch (IOException e) {
          throw new RuntimeException(e);
        }
      }
  ...
  public void testCoveredIndex() throws Exception {
    ...
    try (Connection conn = DriverManager.getConnection(getUrl())) {
      ...
      selectSql = "SELECT  val2, val3 from " + dataTableName + " WHERE val1 IS NOT NULL";
  ...
  private static void populateTableWithAdditionalRows(String dataTableName, Connection conn)
    throws SQLException {
    conn.createStatement().execute(
      "upsert into " + dataTableName + " (id, val1, val2, val3) values ('c', 'cd','cde', 'cdef')");
    conn.createStatement().execute(
      "upsert into " + dataTableName + " (id, val1, val2, val3) values ('d', 'de','de1', 'de11')");
    conn.createStatement().execute(
      "upsert into " + dataTableName + " (id, val1, val2, val3) values ('e', 'ef','ef1', 'ef11')");
    conn.createStatement().execute(
      "upsert into " + dataTableName + " (id, val1, val2, val3) values ('f', 'fg','fg1', 'fg11')");
    conn.createStatement().execute(
      "upsert into " + dataTableName + " (id, val1, val2, val3) values ('g', 'gh','gh1', 'gh11')");
    conn.createStatement().execute(
      "upsert into " + dataTableName + " (id, val1, val2, val3) values ('h', 'hi','hi1', 'hi11')");
    conn.createStatement().execute(
      "upsert into " + dataTableName + " (id, val1, val2, val3) values ('i', 'ij','ij1', 'ij11')");
    conn.createStatement().execute(
      "upsert into " + dataTableName + " (id, val1, val2, val3) values ('j', 'jk','jk1', 'jk11')");
    conn.createStatement().execute(
      "upsert into " + dataTableName + " (id, val1, val2, val3) values ('k', 'kl','kl1', 'kl11')");
  }

  private static void assertRowsWithNewScanner(String dataTableName, String indexTableName,
    ResultSet rs, boolean moveRegions) throws SQLException, IOException {
    assertTrue(rs.next());
    assertEquals("abc", rs.getString(1));
    assertEquals("abcd", rs.getString(2));
    if (moveRegions) {
