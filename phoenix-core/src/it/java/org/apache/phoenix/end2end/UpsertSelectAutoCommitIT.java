...
public class UpsertSelectAutoCommitIT extends ParallelStatsDisabledIT {
  ...
  @Parameters(name = "UpsertSelectAutoCommitIT_allowServerSideMutations={0}") // name is used by
                                                                              // failsafe as file
                                                                              // name in reports
  ...
  public void testAutoCommitUpsertSelect() throws Exception {
    ...
    PreparedStatement stmt =
      conn.prepareStatement("upsert into " + atable + "(" + "    ORGANIZATION_ID, "
  ...
  public void testDynamicUpsertSelect() throws Exception {
    ...
    String dataTableDDL = "CREATE TABLE IF NOT EXISTS " + tableName2 + "("
      + "ORGANIZATION_ID CHAR(15) NOT NULL, " + "PLINY_ID CHAR(15) NOT NULL, "
      + "CREATED_DATE DATE NOT NULL, " + "TEXT VARCHAR, " + "CONSTRAINT PK PRIMARY KEY " + "("
      + "ORGANIZATION_ID, " + "PLINY_ID, " + "CREATED_DATE" + ")" + ")";
  ...
  public void testUpsertSelectDoesntSeeUpsertedData() throws Exception {
    ...
    conn.createStatement()
      .execute("UPSERT INTO " + tableName + " VALUES (NEXT VALUE FOR " + tableName + "_seq, 1)");
  ...
  public void testMaxMutationSize() throws Exception {
    ...
    try (Statement stmt = connection.createStatement()) {
      ...
      stmt.execute("UPSERT INTO " + fullTableName + " VALUES (NEXT VALUE FOR " + fullTableName
        + "_seq, rand(), rand())");
    }
    try (Statement stmt = connection.createStatement()) {
      for (int i = 0; i < 16; i++) {
        stmt.execute("UPSERT INTO " + fullTableName + " SELECT NEXT VALUE FOR " + fullTableName
          + "_seq, rand(), rand() FROM " + fullTableName);
      }
    }
    connection.close();
  }
  ...
  public void testRowCountWithNoAutoCommitOnUpsertSelect() throws Exception {
    ...
    conn.createStatement()
      .execute("CREATE TABLE " + tableName + " (pk INTEGER PRIMARY KEY, val INTEGER)");
    ...
    for (int i = 0; i < 6; i++) {
      ...
      int upsertCount = stmt.executeUpdate("UPSERT INTO " + tableName + " SELECT NEXT VALUE FOR "
