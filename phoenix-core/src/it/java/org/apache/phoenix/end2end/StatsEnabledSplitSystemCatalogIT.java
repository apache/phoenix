...
public class StatsEnabledSplitSystemCatalogIT extends BaseTest {
  ...
  public static Collection<Object[]> data() {
    return TestUtil
      .filterTxParamData(Arrays.asList(new Object[][] { { "TEPHRA" }, { "OMID" }, { null } }), 0);
  }
  ...
  /**
   * Salted tests must be in their own test file to ensure that the underlying table is dropped.
   * Otherwise, the splits may not be performed. TODO: we should throw in that case
   */
  @Test
  ...
  public void testUpdatableOnUpdatableView() throws Exception {
    ...
    String ddl =
      "CREATE VIEW " + fullViewName2 + " AS SELECT * FROM " + fullViewName1 + " WHERE k3 = 2";
    ViewIT.testUpdatableView(fullTableName, fullViewName1, fullViewName2, ddl, null,
      tableDDLOptions);
    ...
    rs = conn.createStatement()
      .executeQuery("SELECT k1, k2, k3 FROM " + fullViewName2 + " WHERE k2 >= 120");
    assertTrue(rs.next());
    assertEquals(1, rs.getInt(1));
    ...
    try {
      conn.createStatement()
        .execute("UPSERT INTO " + fullViewName2 + "(k2,k3) select k2, 3 from " + fullViewName1);
      fail();
    } catch (SQLException e) {
      assertEquals(SQLExceptionCode.CANNOT_UPDATE_VIEW_COLUMN.getErrorCode(), e.getErrorCode());
    }
  ...
  private void testUpdatableViewWithIndex(Integer saltBuckets, boolean localIndex)
    throws Exception {
    ...
    Pair<String, Scan> pair =
      ViewIT.testUpdatableViewIndex(fullTableName, saltBuckets, localIndex, viewName);
  ...
  public void testReadOnlyOnReadOnlyView() throws Exception {
    ...
    ddl = "CREATE VIEW " + fullParentViewName + " (v2 VARCHAR) AS SELECT * FROM " + fullTableName
      + " WHERE k > 5";
    conn.createStatement().execute(ddl);
    ddl =
      "CREATE VIEW " + fullViewName + " AS SELECT * FROM " + fullParentViewName + " WHERE k < 9";
    conn.createStatement().execute(ddl);
