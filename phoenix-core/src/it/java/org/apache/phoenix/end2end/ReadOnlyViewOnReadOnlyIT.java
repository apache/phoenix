...
public class ReadOnlyViewOnReadOnlyIT extends BaseTenantSpecificViewIndexIT {
    ...
    public void testReadOnlyTenantViewOnReadOnly() throws Exception {
        try(Connection connection = DriverManager.getConnection(getUrl(), PropertiesUtil.deepCopy(TEST_PROPERTIES))){
            PhoenixMonitoredConnection conn = connection.unwrap(PhoenixMonitoredConnection.class);
            //base table
            ...
            try (PhoenixMonitoredConnection tenantConn = (PhoenixMonitoredConnection) getTenantConnection(TENANT_ID))
            {
