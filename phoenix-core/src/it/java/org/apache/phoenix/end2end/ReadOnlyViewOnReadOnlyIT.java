  ...
  public void testReadOnlyTenantViewOnReadOnly() throws Exception {
    ...
    try (Connection connection = DriverManager.getConnection(getUrl())) {
      ...
      conn.createStatement()
        .execute("CREATE TABLE IF NOT EXISTS  " + tableName + "  (" + " ID INTEGER NOT NULL,"
          + " COL1 INTEGER NOT NULL," + " COL2 bigint NOT NULL," + " CREATED_DATE DATE,"
          + " CREATION_TIME BIGINT," + " CONSTRAINT NAME_PK PRIMARY KEY (ID, COL1, COL2))"
      ...
      conn.createStatement().execute(
        "CREATE VIEW " + viewName + " (" + generateUniqueName() + " SMALLINT) as select * from "
          + tableName + " where id > 1 " + (false ? "TTL = 1000" : ""));
      ...
      try (Connection tenantConn = getTenantConnection(TENANT_ID)) {
        ...
        tenantStmt
          .execute("CREATE VIEW " + "TENANT_VIEW_" + viewName + " AS SELECT * FROM " + viewName);

        assertEquals(PTable.ViewType.READ_ONLY,
          PhoenixRuntime.getTable(conn, viewName).getViewType());
        assertEquals(PTable.ViewType.READ_ONLY,
          PhoenixRuntime.getTable(tenantConn, "TENANT_VIEW_" + viewName).getViewType());
      }
    }
  }
  ...
  public void testReadOnlyViewOnReadOnly() throws Exception {
    try (Connection connection = DriverManager.getConnection(getUrl())) {
      ...
      assertEquals(PTable.ViewType.READ_ONLY,
        PhoenixRuntime.getTable(conn, viewName).getViewType());
      assertEquals(PTable.ViewType.READ_ONLY,
        PhoenixRuntime.getTable(conn, childView).getViewType());
