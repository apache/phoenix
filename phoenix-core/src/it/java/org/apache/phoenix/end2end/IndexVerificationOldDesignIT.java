...
public class IndexVerificationOldDesignIT extends BaseTest {
  ...
  public static synchronized void setup() throws Exception {
    ...
    clientProps.put(QueryServices.INDEX_REGION_OBSERVER_ENABLED_ATTRIB, Boolean.toString(false));
  ...
  public void testIndexToolOnlyVerifyOption() throws Exception {
    ...
    try (Connection conn = DriverManager.getConnection(getUrl(), props)) {
      conn.createStatement().execute("CREATE TABLE " + dataTableFullName
        + " (ID INTEGER NOT NULL PRIMARY KEY, NAME VARCHAR, CODE VARCHAR) COLUMN_ENCODED_BYTES=0, TTL="
        + ttl);
      conn.createStatement().execute(String.format("CREATE INDEX %s ON %s (NAME) INCLUDE (CODE)",
      ...
      assertEquals(1, indexTool.getJob().getCounters()
        .findCounter(BEFORE_REBUILD_INVALID_INDEX_ROW_COUNT).getValue());
      assertEquals(5, indexTool.getJob().getCounters()
        .findCounter(BEFORE_REBUILD_VALID_INDEX_ROW_COUNT).getValue());
      assertEquals(0, indexTool.getJob().getCounters()
        .findCounter(BEFORE_REBUILD_MISSING_INDEX_ROW_COUNT).getValue());
  ...
  public void testIndexToolOnlyVerifyOption_viewIndex() throws Exception {
    ...
    try (Connection conn = DriverManager.getConnection(getUrl(), props)) {
      ...
      conn.createStatement()
        .execute("CREATE VIEW " + fullViewName + " AS SELECT * FROM " + dataTableFullName);
      conn.createStatement().execute(
        String.format("CREATE INDEX %s ON %s (NAME) INCLUDE (CODE)", indexTableName, fullViewName));

      upsertValidRows(conn, fullViewName);

      IndexToolIT.runIndexTool(false, schemaName, viewName, indexTableName, null, 0,
        IndexTool.IndexVerifyType.ONLY);

      conn.createStatement()
        .execute("upsert into " + indexTableFullName + " values ('Phoenix5', 6,'G')");
      conn.createStatement()
        .execute("delete from " + indexTableFullName + " where \"0:CODE\" = 'D'");
      conn.commit();
