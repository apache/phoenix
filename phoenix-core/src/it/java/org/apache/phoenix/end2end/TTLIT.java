...
public class TTLIT extends ParallelStatsDisabledIT {
  ...
  /**
   * BUILD methods to create Table/Index/View/ViewIndexes with or without TTL as table props at
   * different levels
   */
  ...
  private SchemaBuilder createTenantViewOnTable(boolean addTTL, boolean addIndex) {
    return createTenantViewOnTableOrGlobalView(createTableWithTTL(false, false, null), addTTL,
      addIndex, true);
  }

  private SchemaBuilder createTenantViewOnGlobalView(boolean addTTL, boolean addIndex) {
    return createTenantViewOnTableOrGlobalView(createGlobalViewOnTable(false, false), addTTL,
      addIndex, true);
  }

  private SchemaBuilder createTenantViewOnTableOrGlobalView(SchemaBuilder schemaBuilder,
    boolean addTTL, boolean addIndex, boolean addPK) {
    SchemaBuilder.TenantViewOptions tenantViewOptions =
  ...
  private void assertTTLForGivenPTable(Connection connection, PTable table, int ttl)
    throws SQLException, IOException {
    LiteralTTLExpression expected = new LiteralTTLExpression(ttl);
    TestUtil.assertTTLValue(connection, table, expected);
  }

  private void assertTTLForGivenEntity(Connection connection, String entityName, int ttl)
    throws SQLException, IOException {
    PTable pTable = PhoenixRuntime.getTable(connection, entityName);
    LiteralTTLExpression expected = new LiteralTTLExpression(ttl);
    TestUtil.assertTTLValue(connection, pTable, expected);
  }

  private void assertTTLForIndexName(Connection connection, String indexName, int ttl)
    throws SQLException, IOException {
    if (!indexName.equals(SKIP_ASSERT)) {
      PTable index = PhoenixRuntime.getTable(connection, indexName);
      LiteralTTLExpression expected = new LiteralTTLExpression(ttl);
      TestUtil.assertTTLValue(connection, index, expected);
    }
  }

  private void assertTTLForIndexFromParentPTable(Connection connection, String baseEntity, int ttl)
    throws SQLException, IOException {
  ...
  public void testTTLAtTableLevelWithIndex() throws Exception {
    ...
    try (Connection globalConnection = DriverManager.getConnection(getUrl())) {
      ...
      assertTTLForIndexFromParentPTable(globalConnection, schemaBuilder.getEntityTableName(),
  ...
  /**
   * Test TTL defined at Table level for 2 level View Hierarchy, TTL value should be passed down to
   * view at Global Level as well as Child Views of Global View.
   */
  ...
  public void testTTLInHierarchyDefinedAtTableLevelWithIndex() throws Exception {
    ...
    SchemaBuilder schemaBuilder = createTableWithTTL(true, true, null);
    // Create Global View on top of table without defining TTL but don't extend PK as we are
    // building index on table.
    schemaBuilder = createGlobalViewOnTable(schemaBuilder, false, false, false);
    // Create Tenant View on top of Global View without defining TTL but don't extend PK as we are
    // building index on table.
  ...
  public void testTTLInHierarchyDefinedAtTableLevelWithIndexAtTenantLevel() throws Exception {
    ...
    String globalView = schemaBuilder.getEntityGlobalViewName();
    String childView = schemaBuilder.getEntityTenantViewName();
    String childViewIndexName =
      isMultiTenant ? schemaBuilder.getEntityTenantViewIndexName() : SKIP_ASSERT;
  ...
  public void testTTLHierarchyDefinedAtGlobalViewLevel() throws Exception {
    ...
    try (Connection globalConnection = DriverManager.getConnection(getUrl());
      ...
      assertTTLForIndexFromParentPTable(globalConnection, globalViewName,
        DEFAULT_TEST_TTL_VALUE_AT_GLOBAL);
  ...
  @Test
  public void testSettingAndAlteringTTLInHierarchyAboveOfTenantViewWithTTLDefined()
    throws Exception {
    SchemaBuilder schemaBuilder = createTenantViewOnGlobalView(true, isMultiTenant);
    schemaBuilder.build();
  ...
  public void testAlteringTTLFromLevel1ToLevel2() throws Exception {
    ...
    try (Connection globalConnection = DriverManager.getConnection(getUrl());
      ...
      assertTTLForIndexName(tenantConnection, tenantViewIndexName,
        DEFAULT_TEST_TTL_VALUE_AT_GLOBAL);
      assertTTLForIndexFromParentPTable(tenantConnection, tenantViewName,
        DEFAULT_TEST_TTL_VALUE_AT_GLOBAL);
