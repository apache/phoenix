...
public class IndexRebuildTaskIT extends BaseTest {
    ...
    try {
      ...
      for (int i = 0; i < numOfValues; i++) {
        tenantConn.createStatement().execute(String.format(
          "UPSERT INTO %s VALUES('%s', '%s', '%s')", viewName, String.valueOf(i), "y", "z"));
      }
      tenantConn.commit();
      ...
      // If I Alter to DISABLE, it fails to in Index tool while setting state to active due to
      // Invalid transition.
      tenantConn.createStatement()
        .execute(String.format("ALTER INDEX %s ON %s UNUSABLE", indexName, viewName));
      tenantConn.commit();
      ...
      Task.addTask(new SystemTaskParams.SystemTaskParamsBuilder()
        .setConn(conn.unwrap(PhoenixConnection.class)).setTaskType(PTable.TaskType.INDEX_REBUILD)
        .setTenantId(TENANT1).setSchemaName(null).setTableName(viewName)
        .setTaskStatus(PTable.TaskStatus.CREATED.toString()).setData(data).setPriority(null)
        .setStartTs(startTs).setEndTs(null).setAccessCheckEnabled(true).build());
      task.run();
      ...
      if (conn != null) {
        conn.createStatement().execute("DELETE " + " FROM "
          + PhoenixDatabaseMetaData.SYSTEM_TASK_NAME + " WHERE TABLE_NAME ='" + viewName + "'");
        conn.commit();
        conn.close();
      }
      if (tenantConn != null) {
  ...
  public static void waitForTaskState(Connection conn, PTable.TaskType taskType,
    String expectedTableName, PTable.TaskStatus expectedTaskStatus)
    ...
    do {
      Thread.sleep(2000);
      String stmt = "SELECT * " + " FROM " + PhoenixDatabaseMetaData.SYSTEM_TASK_NAME + " WHERE "
    ...
    fail(
      String.format("Ran out of time waiting for current task state %s to become %s. TaskData: %s",
        taskStatus, expectedTaskStatus, taskData));
  }
}
