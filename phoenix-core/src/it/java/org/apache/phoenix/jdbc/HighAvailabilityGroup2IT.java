...
public class HighAvailabilityGroup2IT extends HABaseIT {
  ...
  /**
   * Test that if STANDBY ZK cluster is down, the connect should work. This differs from
   * {@link #testCanConnectWhenStandbyHBaseClusterDown} because this stops the ZK cluster, not the
   * HBase cluster.
   */
  @Test
  ...
  /**
   * Test that if STANDBY HBase cluster is down, connect to new HA group should work. This test
   * covers only HBase cluster is down, and both ZK clusters are still healthy so clients will be
   * able to get latest clusters role record from both clusters. This tests a new HA group which
   * will get initialized during the STANDBY HBase cluster down time.
   */
  @Test
  ...
  /**
   * Test that if STANDBY cluster ZK service is down, connect to new HA group should work. This
   * differs from {@link #testCanConnectNewGroupWhenStandbyHBaseClusterDown} because this is testing
   * scenarios when STANDBY ZK cluster is down.
   */
  @Test
  ...
  /**
   * Test that client can not establish connection to when the ACTIVE ZK cluster is down, while
   * client can establish active connection after active ZK cluster restarts. This differs from the
   * {@link #testCanNotEstablishConnectionWhenActiveHBaseClusterDown()} because this is for ZK
   * cluster down while the other is for HBase cluster down.
   */
  @Test
  ...
  /**
   * Test that HA connection request will fall back to the first cluster when HA group fails to
   * initialize due to missing cluster role record (CRR).
   */
  @Test
  ...
  public void testPollerShouldBeRunning() throws Exception {
    ...
    assertTrue(haGroup2.getRoleRecord()
      .getRole(JDBCUtil.formatUrl(CLUSTERS.getMasterAddress1(), registryType)).isActive());
    assertEquals(haGroup2.getRoleRecord().getRole(
      JDBCUtil.formatUrl(CLUSTERS.getMasterAddress2(), registryType)), ClusterRole.STANDBY);

    // transit url1 to standby and refresh the roleRecord as well which should have started the
    // poller
    // as there is no ACTIVE role in the new record
    CLUSTERS.transitClusterRole(haGroup2, ClusterRole.STANDBY, ClusterRole.STANDBY);
    assertEquals(haGroup2.getRoleRecord().getRole(
      JDBCUtil.formatUrl(CLUSTERS.getMasterAddress1(), registryType)), ClusterRole.STANDBY);
    assertEquals(haGroup2.getRoleRecord().getRole(
      JDBCUtil.formatUrl(CLUSTERS.getMasterAddress2(), registryType)), ClusterRole.STANDBY);
    ...
    assertEquals(haGroup2.getRoleRecord()
      .getRole(JDBCUtil.formatUrl(CLUSTERS.getMasterAddress1(), registryType)), ClusterRole.ACTIVE);
    assertEquals(haGroup2.getRoleRecord().getRole(
      JDBCUtil.formatUrl(CLUSTERS.getMasterAddress2(), registryType)), ClusterRole.STANDBY);
