...
public class HighAvailabilityGroupIT extends HABaseIT {
  ...
  /**
   * Test HAGroup.get() method to get same HAGroups if we have different principals only and
   * different HAGroups if anything else in the key changes
   */
  @Test
  public void testGetWithDifferentPrincipals() throws Exception {
    // Client will get same HAGroup if we have difference in principal only but JDBCURLs should have
    // different
    // principals
    ...
    try {
      ...
      // URLs we are getting for haGroup2 should have newer principal i.e. Current HAURLInfo should
      // have new
    ...
    // Client will get a different HighAvailabilityGroup when group name is different and with same
    // principal as
    // default HAGroup
    String haGroupName3 = testName.getMethodName() + RandomStringUtils.randomAlphabetic(3);
    CLUSTERS.initClusterRole(haGroupName3, HighAvailabilityPolicy.FAILOVER);
    ...
    try {
      ...
      // URLs we are getting for haGroup4 should have newer principal i.e. Current HAURLInfo should
      // have new
      // principal instead default PRINCIPAL
  ...
  /**
   * Test that client can get an HA group when ACTIVE HBase cluster is down. NOTE: we can not test
   * with existing HA group because {@link HighAvailabilityGroup#get} would get the cached object,
   * which has also been initialized. The reason this works is because getting an HA group depends
   * on doing rpc calls to both clusters to get the ClusterRoleRecord. If one cluster fails, the
   * other cluster should be able to return clusterRoleRecord and start the HA group.
   */
  @Test
  ...
  /**
   * Test that client can not get an HA group when both HBase clusters are down. NOTE: we can not
   * test with existing HA group because {@link HighAvailabilityGroup#get} would get the cached
   * object, which has also been initialized. What if two HBase clusters instead of two ZK clusters
   * are down? We may still get a new HA group because creating and initializing HA group do not set
   * up the HBase connection, which should indeed fail when ACTIVE HBase cluster is down.
   */
  @Test
  ...
  public void testConnectToOneCluster() throws SQLException {
    ...
    final String randomHostUrl =
      String.format("%s:%d", RandomStringUtils.randomAlphabetic(4), RandomUtils.nextInt(0, 65536));
    try {
  ...
  /**
   * Test that it can connect to a given cluster in this HA group after ZK service restarts. Method
   * {@link HighAvailabilityGroup#connectToOneCluster(String, Properties, HAURLInfo)} is used by
   * Phoenix HA framework to connect to one specific HBase cluster in this HA group. The cluster may
   * not necessarily be in ACTIVE role. For example, parallel HA connection needs to connect to both
   * clusters. This tests that it can connect to a specific ZK cluster after ZK restarts.
   */
  @Test
  public void testConnectToOneClusterAfterZKRestart() throws Exception {
    ...
    try (Connection conn =
      haGroup.connectToOneCluster(jdbcUrlToCluster1, clientProperties, haURLInfo)) {
  ...
  public void testIsConnectionActive() throws Exception {
    PhoenixConnection conn1 =
      haGroup.connectToOneCluster(CLUSTERS.getJdbcUrl1(haGroup), clientProperties, haURLInfo);
    assertTrue(haGroup.isActive(conn1));
    PhoenixConnection conn2 =
      haGroup.connectToOneCluster(CLUSTERS.getJdbcUrl2(haGroup), clientProperties, haURLInfo);
  ...
  public void testNodeChange() throws Exception {
    assertEquals(ClusterRole.ACTIVE,
      haGroup.getRoleRecord().getRole(CLUSTERS.getURL(1, registryType)));
    assertEquals(ClusterRole.STANDBY,
      haGroup.getRoleRecord().getRole(CLUSTERS.getURL(2, registryType)));
  ...
  private void assertJDBCUrlForGivenHAGroup(HighAvailabilityGroup haGroup, HAURLInfo haURLInfo,
    String testPrincipal) {
    String testUrl1 = testPrincipal == null
      ? CLUSTERS.getJdbcUrl1(haGroup)
      : CLUSTERS.getJdbcUrl1(haGroup, testPrincipal);
    String testUrl2 = testPrincipal == null
      ? CLUSTERS.getJdbcUrl2(haGroup)
      : CLUSTERS.getJdbcUrl2(haGroup, testPrincipal);
    String testHAUrl =
      testPrincipal == null ? CLUSTERS.getJdbcHAUrl() : CLUSTERS.getJdbcHAUrl(testPrincipal);
  ...
  private String getJdbcHAUrlForGivenHAGroup(HAURLInfo haURLInfo) {
    return HighAvailabilityGroup.getJDBCHAUrl(CLUSTERS.getMasterAddress1(),
      CLUSTERS.getMasterAddress2(), haURLInfo);
  }
}
