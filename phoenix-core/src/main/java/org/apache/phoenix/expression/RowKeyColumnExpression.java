...
public class RowKeyColumnExpression extends ColumnExpression {
  ...
  private RowKeyColumnExpression(PDatum datum, RowKeyValueAccessor accessor, PDataType fromType,
    String name) {
    super(datum);
  ...
  /**
   * Used to set an offset to be skipped from the start of a the row key. Used by local indexing to
   * skip the region start key bytes.
  ...
  /**
   * Since we may never have encountered a key value column of interest, but the expression may
   * evaluate to true just based on the row key columns, we need to do a final evaluation. An
   * example of when this would be required is: SELECT a FROM t WHERE a = 5 OR b = 2 in the case
   * where a is a PK column, b is a KV column and no b KV is found.
   */
  @Override
