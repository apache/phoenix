...
public class ArrayInsertUniqueFunction extends UpsertFunctionExpression {
	...
	public ArrayInsertUniqueFunction() {
	}

	public ArrayInsertUniqueFunction(List<Expression> children) {
		super(children);
	}

    @Override
    public List<Pair<String, byte[]>> getAttributes() {
        List<Pair<String, byte[]>> attributes = new ArrayList<Pair<String, byte[]>>();

        attributes.add(new Pair(ArrayUpdateObserver.ARRAY_INSERT_UNIQUE_ATTRIBUTE, getIdentifier()));
        return attributes;
    }

    @Override
    public boolean evaluate(Tuple tuple, ImmutableBytesWritable ptr) {
        return children.get(0).evaluate(tuple, ptr);
    }
