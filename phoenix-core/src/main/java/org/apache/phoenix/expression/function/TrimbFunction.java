...
@BuiltInFunction(name = TrimbFunction.NAME, args = {
        @Argument(allowedTypes = { PBinary.class, PVarbinary.class }),
        @Argument(allowedTypes = { PVarbinary.class }) }) public class TrimbFunction
    ...
    @Override public boolean evaluate(Tuple tuple, ImmutableBytesWritable ptr) {
        ...
        Set<Byte> trimBytesSet = new HashSet<>(trimBytes.length);
    ...
    @Override public PDataType getDataType() {
        return getDataBytesExpression().getDataType().isFixedWidth() ?
                getDataBytesExpression().getDataType() :
                PVarbinary.INSTANCE;
    }
    ...
    @Override public OrderPreserving preservesOrder() {
