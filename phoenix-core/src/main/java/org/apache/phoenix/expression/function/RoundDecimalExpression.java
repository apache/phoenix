...
public class RoundDecimalExpression extends ScalarFunction {
  ...
  public static Expression create(Expression expr, int scale) throws SQLException {
    ...
    Expression scaleExpr =
      LiteralExpression.newConstant(scale, PInteger.INSTANCE, Determinism.ALWAYS);
  ...
  public static Expression create(List<Expression> exprs) throws SQLException {
    ...
    if (exprs.size() == 1) {
      Expression scaleExpr =
        LiteralExpression.newConstant(0, PInteger.INSTANCE, Determinism.ALWAYS);
  ...
  public RoundDecimalExpression(List<Expression> children) {
    ...
    if (scaleValue != null) {
      ...
      throw new IllegalDataException("Invalid second argument for scale: " + scaleValue
        + ". The scale must be between 0 and " + PDataType.MAX_PRECISION + " inclusive.");
    }
  }
  ...
  public boolean evaluate(Tuple tuple, ImmutableBytesWritable ptr) {
    ...
    if (childExpr.evaluate(tuple, ptr)) {
      ...
      BigDecimal value = (BigDecimal) PDecimal.INSTANCE.toObject(ptr, childExpr.getDataType(),
  ...
  public KeyPart newKeyPart(final KeyPart childPart) {
    return new KeyPart() {
      private final Set<Expression> extractNodes =
        new LinkedHashSet<>(Collections.<Expression> singleton(RoundDecimalExpression.this));
      ...
      public KeyRange getKeyRange(CompareFilter.CompareOp op, Expression rhs) {
        final BigDecimal rhsDecimal =
          (BigDecimal) PDecimal.INSTANCE.toObject(evaluateExpression(rhs));
      ...
      /**
       * Produces a the given decimal rounded to this rounding expression's scale. If the decimal
       * requires more scale precision to produce than this expression has, as in ROUND(?, 2) &gt;
       * 2.0098974, it ensures that the decimal is rounded such that the given operator will still
  ...
  /**
   * Finds the Decimal KeyRange that will produce the given result when fed into this rounding
   * expression. For example, a ROUND expression with scale 2 will produce the result "2.05" with
   * any decimal in the range [2.045, 2.0545). The result must be pre-rounded to within this
   * rounding expression's scale.
  ...
  /**
   * Determines whether this rounding expression's scale has enough precision to produce the minimum
   * precision for the input decimal. In other words, determines whether the given decimal can be
   * rounded to this scale without losing ordering information. For example, an expression with a
   * scale of 2 has enough precision to produce "2.3", "2.71" and "2.100000", but does not have
   * enough precision to produce "2.001"
  ...
  /**
   * Returns the given decimal rounded to this rounding expression's scale. For example, with scale
   * 2 the decimal "2.453" would be rounded to either 2.45 or 2.46 depending on the rounding mode,
   * while "2.38" and "2.7" would be unchanged.
  ...
  /**
   * Produces a value half of a "step" back in this expression's rounding scale. For example with a
   * scale of 2, "2.5" would be stepped back to "2.495".
   */
  ...
  /**
   * Produces a value half of a "step" forward in this expression's rounding scale. For example with
   * a scale of 2, "2.5" would be stepped forward to "2.505".
   */
  ...
  /**
   * Produces a value one "step" back in this expression's rounding scale. For example with a scale
   * of 2, "2.5" would be stepped back to "2.49".
   */
  ...
  /**
   * Produces a value one "step" forward in this expression's rounding scale. For example with a
   * scale of 2, "2.5" would be stepped forward to "2.51".
   */
