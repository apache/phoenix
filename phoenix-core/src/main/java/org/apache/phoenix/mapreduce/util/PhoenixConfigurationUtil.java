...
/**
 * A utility class to set properties on the {#link Configuration} instance. Used as part of Map
 * Reduce job configuration.
 */
public final class PhoenixConfigurationUtil {
  ...
  public static final String MAPREDUCE_SELECT_COLUMN_VALUE_PREFIX =
    "phoenix.mr.select.column.value";

  public static final String MAPREDUCE_SELECT_COLUMN_COUNT = "phoenix.mr.select.column.count";

  public static final String MAPREDUCE_UPSERT_COLUMN_VALUE_PREFIX =
    "phoenix.mr.upsert.column.value";
  ...
  public static final String MAPREDUCE_MULTI_INPUT_MAPPER_SPLIT_SIZE =
    "phoenix.mapreduce.multi.input.split.size";

  public static final String MAPREDUCE_MULTI_INPUT_QUERY_BATCH_SIZE =
    "phoenix.mapreduce.multi.input.batch.size";

  // phoenix ttl data deletion job for a specific view
  public static final String MAPREDUCE_PHOENIX_TTL_DELETE_JOB_PER_VIEW =
    "phoenix.mapreduce.phoenix_ttl.per_view";

  // phoenix ttl data deletion job for all views.
  public static final String MAPREDUCE_PHOENIX_TTL_DELETE_JOB_ALL_VIEWS =
    "phoenix.mapreduce.phoenix_ttl.all";

  // provide an absolute path to inject your multi input logic
  public static final String MAPREDUCE_MULTI_INPUT_STRATEGY_CLAZZ =
    "phoenix.mapreduce.multi.input.strategy.path";

  // provide an absolute path to inject your multi split logic
  public static final String MAPREDUCE_MULTI_INPUT_SPLIT_STRATEGY_CLAZZ =
    "phoenix.mapreduce.multi.split.strategy.path";
  ...
  // provide control to whether or not handle mapreduce snapshot restore and cleanup operations
  // which
  ...
  public static void setInputTableConditions(final Configuration configuration,
  ...
  private static void setValues(final Configuration configuration, final String[] columns,
    final String VALUE_COUNT, final String VALUE_NAME) {
    Preconditions.checkNotNull(configuration);
  ...
  private static List<String> getValues(final Configuration configuration, final String VALUE_COUNT,
  ...
  public static void setSelectColumnNames(final Configuration configuration,
    final String[] columns) {
    setValues(configuration, columns, MAPREDUCE_SELECT_COLUMN_COUNT,
      MAPREDUCE_SELECT_COLUMN_VALUE_PREFIX);
  }

  public static List<String> getSelectColumnNames(final Configuration configuration) {
    return getValues(configuration, MAPREDUCE_SELECT_COLUMN_COUNT,
      MAPREDUCE_SELECT_COLUMN_VALUE_PREFIX);
  }

  public static void setInputClass(final Configuration configuration,
  ...
  public static void setPhysicalTableName(final Configuration configuration,
  ...
  public static void setUpsertColumnNames(final Configuration configuration,
    final String[] columns) {
    setValues(configuration, columns, MAPREDUCE_UPSERT_COLUMN_COUNT,
      MAPREDUCE_UPSERT_COLUMN_VALUE_PREFIX);
  }

  public static void setSnapshotNameKey(final Configuration configuration,
   ...
   */
  @Deprecated
  public static void setInputCluster(final Configuration configuration, final String quorum) {
    Preconditions.checkNotNull(configuration);
   ...
   */
  @Deprecated
  public static void setOutputCluster(final Configuration configuration, final String quorum) {
    Preconditions.checkNotNull(configuration);
   ...
   */
  public static void setInputClusterUrl(final Configuration configuration, final String url) {
    Preconditions.checkNotNull(configuration);
   ...
   */
  public static void setOutputClusterUrl(final Configuration configuration, final String url) {
    Preconditions.checkNotNull(configuration);
  ...
  public static List<ColumnInfo> getUpsertColumnMetadataList(final Configuration configuration)
    throws SQLException {
    Preconditions.checkNotNull(configuration);
    ...
    try (final Connection connection = ConnectionUtil.getOutputConnection(configuration)) {
      ...
      if (!upsertColumnList.isEmpty()) {
        LOGGER.info(String.format(
          "UseUpsertColumns=%s, upsertColumnList.size()=%s," + " upsertColumnList=%s ",
          !upsertColumnList.isEmpty(), upsertColumnList.size(),
          Joiner.on(",").join(upsertColumnList)));
      }
      columnMetadataList =
        PhoenixRuntime.generateColumnInfo(connection, tableName, upsertColumnList);
  ...
  public static void setMultiInputMapperSplitSize(Configuration configuration,
    final int splitSize) {
    Preconditions.checkNotNull(configuration);
    configuration.set(MAPREDUCE_MULTI_INPUT_MAPPER_SPLIT_SIZE, String.valueOf(splitSize));
  }

  public static void setMultiViewQueryMoreSplitSize(Configuration configuration,
    final int batchSize) {
    Preconditions.checkNotNull(configuration);
  ...
  public static List<ColumnInfo> getSelectColumnMetadataList(final Configuration configuration)
    throws SQLException {
    Preconditions.checkNotNull(configuration);
    List<ColumnInfo> columnMetadataList = null;
    columnMetadataList = ColumnInfoToStringEncoderDecoder.decode(configuration);
  ...
  private static List<String> getSelectColumnList(final Configuration configuration) {
    List<String> selectColumnList = PhoenixConfigurationUtil.getSelectColumnNames(configuration);
    if (!selectColumnList.isEmpty()) {
      LOGGER.info(
        String.format("UseSelectColumns=%s, selectColumnList.size()=%s, " + "selectColumnList=%s ",
          !selectColumnList.isEmpty(), selectColumnList.size(),
          Joiner.on(",").join(selectColumnList)));
    }
  ...
  public static int getSelectColumnsCount(Configuration configuration, String tableName)
    throws SQLException {
    Preconditions.checkNotNull(configuration);
    final String schemaTp = configuration.get(SCHEMA_TYPE);
  ...
  /**
   * Returns the Phoenix JDBC URL a Phoenix MapReduce job will read from. If
   * MAPREDUCE_INPUT_CLUSTER_URL is not set, then it returns the value of "jdbc:phoenix"
   * @return URL string
   */
  ...
  /**
   * Returns the Phoenix JDBC URL a Phoenix MapReduce job will write to. If
   * MAPREDUCE_OUTPUT_CLUSTER_URL is not set, then it returns the value of "jdbc:phoenix"
   * @return URL string
   */
  ...
  /**
   * Returns the value of HConstants.ZOOKEEPER_QUORUM. For tests only
   * @return ZooKeeper quorum string if defined, null otherwise
   */
  @Deprecated
  ...
  public static void setIndexMaintainers(final Configuration configuration,
    ...
    configuration.set(INDEX_MAINTAINERS,
      Bytes.toString(Base64.getEncoder().encode(indexMetaDataPtr.get())));
  }
  ...
  public static void setIndexVerifyType(Configuration configuration,
  ...
  public static IndexTool.IndexVerifyType getDisableLoggingVerifyType(Configuration configuration) {
    Preconditions.checkNotNull(configuration);
    String value =
      configuration.get(DISABLE_LOGGING_TYPE, IndexTool.IndexVerifyType.NONE.getValue());
    return IndexTool.IndexVerifyType.fromValue(value);
  }
  ...
  public static boolean isMRSnapshotManagedExternally(final Configuration configuration) {
    Preconditions.checkNotNull(configuration);
    boolean isSnapshotRestoreManagedExternally = configuration
      .getBoolean(MAPREDUCE_EXTERNAL_SNAPSHOT_RESTORE, DEFAULT_MAPREDUCE_EXTERNAL_SNAPSHOT_RESTORE);
