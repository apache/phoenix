...
public class OrphanViewTool extends Configured implements Tool {
  ...
  private void removeLink(PhoenixConnection phoenixConnection, Key src, Key dst,
    ...
    String deleteQuery = "DELETE FROM "
      ...
      + TABLE_SCHEM
      + (src.getSchemaName() == null ? " IS NULL " : " = '" + src.getSchemaName() + "'") + " AND "
      + TABLE_NAME + " = '" + src.getTableName() + "' AND " + COLUMN_NAME
      + (dst.getTenantId() == null ? " IS NULL" : " = '" + dst.getTenantId() + "'") + " AND "
      + COLUMN_FAMILY + " = '"
      + (dst.getSchemaName() == null
        ? dst.getTableName()
  ...
  private void forcefullyDropView(PhoenixConnection phoenixConnection, Key key) throws Exception {
    String deleteRowsFromCatalog = "DELETE FROM " + SYSTEM_CATALOG_NAME + " WHERE " + TENANT_ID
      ...
      + TABLE_SCHEM
      + (key.getSchemaName() == null ? " IS NULL " : " = '" + key.getSchemaName() + "'") + " AND "
      + TABLE_NAME + " = '" + key.getTableName() + "'";
    String deleteRowsFromChildLink = "DELETE FROM " + SYSTEM_CHILD_LINK_NAME + " WHERE "
      + COLUMN_NAME + (key.getTenantId() == null ? " IS NULL" : " = '" + key.getTenantId() + "'")
      + " AND " + COLUMN_FAMILY + " = '"
