...
public class DefaultPhoenixMultiViewListProvider implements PhoenixMultiViewListProvider {
  ...
  public List<ViewInfoWritable> getPhoenixMultiViewList(Configuration configuration) {
    boolean isFetchAll =
      configuration.get(PhoenixConfigurationUtil.MAPREDUCE_PHOENIX_TTL_DELETE_JOB_ALL_VIEWS)
          != null;
        ...
        do {
          ...
          while (viewRs.next()) {
            ...
            if (!isParentHasTTL(connection, tenantId, fullTableName)) {
              addingViewIndexToTheFinalList(connection, tenantId, fullTableName, viewTtlValue,
                viewInfoWritables);
            }
          }
          if (isQueryMore) {
  ...
  private boolean isParentHasTTL(PhoenixConnection connection, String tenantId,
    String fullTableName) {
    boolean skip = false;
    try {
      PTable pTable = PhoenixRuntime.getTable(connection, tenantId, fullTableName);
      PTable parentTable =
        PhoenixRuntime.getTable(connection, null, pTable.getParentName().toString());
      if (parentTable.getType() == PTableType.VIEW && parentTable.getPhoenixTTL() > 0) {
        /*
         * if the current view parent already has a TTL value, we want to skip the current view
         * cleanup job because we want to run the cleanup job for at the GlobalView level instead of
         * running multi-jobs at the LeafView level for the better performance. BaseTable
         * GlobalView(has TTL) LeafView1, LeafView2, LeafView3....
         */
        skip = true;
      }
    } catch (Exception e) {
      skip = true;
      LOGGER
        .error(String.format("Had an issue to process the view: %s, " + "tenantId: see error %s ",
          fullTableName, tenantId, e.getMessage()));
    }
    return skip;
  }

  private void addingViewIndexToTheFinalList(PhoenixConnection connection, String tenantId,
    String fullTableName, long viewTtlValue, List<ViewInfoWritable> viewInfoWritables)
    throws Exception {
    PTable pTable = PhoenixRuntime.getTable(connection, tenantId, fullTableName);
    ViewInfoWritable viewInfoTracker = new ViewInfoTracker(tenantId, fullTableName, viewTtlValue,
      pTable.getPhysicalName().getString(), false
    ...
    for (PTable viewIndexTable : allIndexesOnView) {
      ...
      ViewInfoWritable viewInfoTrackerForIndexEntry =
        new ViewInfoTracker(tenantId, fullTableName, viewTtlValue, indexName, true
