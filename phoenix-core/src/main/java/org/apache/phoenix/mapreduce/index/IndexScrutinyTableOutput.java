...
public class IndexScrutinyTableOutput {
  ...
  public static final String OUTPUT_TABLE_BEYOND_LOOKBACK_DDL = "" + "ALTER TABLE "
   ...
   * row data from the output table. The queries contain the dynamic columns which are equivalent to
   * the original source/target table columns
   */
  public static final String OUTPUT_METADATA_TABLE_NAME = "PHOENIX_INDEX_SCRUTINY_METADATA";
  public static final String OUTPUT_METADATA_DDL = "CREATE TABLE IF NOT EXISTS "
    ...
    + "    TARGET_DYNAMIC_COLS VARCHAR,\n" + "    INVALID_ROWS_QUERY_ALL VARCHAR,\n" + // stored sql
                                                                                       ...
                                                                                       // the output
                                                                                       // table
    "    INVALID_ROWS_QUERY_MISSING_TARGET VARCHAR,\n" + // stored sql query to fetch all the
                                                         // invalid rows which are missing a target
                                                         // row
    "    INVALID_ROWS_QUERY_BAD_COVERED_COL_VAL VARCHAR,\n" + // stored sql query to fetch all the
                                                              // invalid rows which have bad covered
                                                              // column values
    "    INVALID_ROWS_QUERY_BEYOND_MAX_LOOKBACK VARCHAR,\n" + // stored sql query to fetch all the
                                                              // potentially invalid rows which are
                                                              // before max lookback age
  ...
  public static final String UPSERT_METADATA_SQL =
    "UPSERT INTO " + OUTPUT_METADATA_TABLE_NAME + " VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";

  /**
   * Gets the parameterized upsert sql to the output table Used by the scrutiny MR job to write its
   * results
  ...
  public static String constructOutputTableUpsert(List<String> sourceDynamicCols,
    ...
    List<String> upsertCols = Lists
      .newArrayList(Iterables.concat(outputTableColumns, sourceDynamicCols, targetDynamicCols));
    String upsertStmt = QueryUtil
      .constructUpsertStatement(IndexScrutinyTableOutput.OUTPUT_TABLE_NAME, upsertCols, null);
    return upsertStmt;
  }
  ...
  public static String getSqlQueryBeyondMaxLookback(Connection conn,
    ...
    String inClause = " IN " + QueryUtil.constructParameterizedInClause(getPkCols().size() + 2, 1);
  ...
  public static ResultSet queryMetadata(Connection conn, List<String> selectCols,
    String qSourceTableName, String qTargetTableName, long scrutinyTimeMillis) throws SQLException {
  ...
  public static ResultSet queryAllLatestMetadata(Connection conn, String qSourceTableName,
    ...
    String sql =
      "SELECT MAX(" + SCRUTINY_EXECUTE_TIME_COL_NAME + ") " + "FROM " + OUTPUT_METADATA_TABLE_NAME
  ...
  public static void writeJobResults(Connection conn, String[] cmdLineArgs, List<Job> completedJobs)
    ...
    for (Job job : completedJobs) {
      ...
      pStmt.setString(index++,
        getSqlQueryMissingTargetRows(conn, columnNames, scrutinyExecuteTime));
      pStmt.setString(index++, getSqlQueryBadCoveredColVal(conn, columnNames, scrutinyExecuteTime));
      pStmt.setString(index++,
        getSqlQueryBeyondMaxLookback(conn, columnNames, scrutinyExecuteTime));
      pStmt.setLong(index++,
  ...
  static String constructMetadataParamQuery(List<String> metadataSelectCols) {
    ...
    String query = QueryUtil.constructSelectStatement(OUTPUT_METADATA_TABLE_NAME,
      metadataSelectCols, pkColsCsv, null, true);
  ...
  private static String bindPkCols(SourceTargetColumnNames columnNames, long scrutinyTimeMillis,
    String paramQuery) {
    paramQuery =
      paramQuery.replaceFirst("\\?", "'" + columnNames.getQualifiedSourceTableName() + "'");
    paramQuery =
      paramQuery.replaceFirst("\\?", "'" + columnNames.getQualifiedTargetTableName() + "'");
  ...
  private static String getHasTargetRowQuery(Connection conn, SourceTargetColumnNames columnNames,
    ...
    String whereQuery = constructOutputTableQuery(conn, columnNames,
      getPksCsv() + ", " + SchemaUtil.getEscapedFullColumnName("HAS_TARGET_ROW"));
    String inClause = " IN " + QueryUtil.constructParameterizedInClause(getPkCols().size() + 1, 1);
    String paramQuery = whereQuery + inClause;
    paramQuery = bindPkCols(columnNames, scrutinyTimeMillis, paramQuery);
    return paramQuery;
  }
  ...
  private static String constructOutputTableQuery(Connection connection,
    ...
    List<String> selectCols = Lists.newArrayList(Iterables.concat(outputTableColumns,
      columnNames.getUnqualifiedSourceColNames(), columnNames.getUnqualifiedTargetColNames()));
    String dynamicCols = Joiner.on(",").join(
      Iterables.concat(columnNames.getSourceDynamicCols(), columnNames.getTargetDynamicCols()));
