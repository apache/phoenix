...
public class TupleProjectionPlan extends DelegateQueryPlan {
  ...
  private List<OrderBy> convertInputOrderBys(QueryPlan targetQueryPlan) throws SQLException {
    ...
    for (OrderBy inputOrderBy : inputOrderBys) {
      OrderBy newOrderBy = this.convertSingleInputOrderBy(selectColumnExpressionToIndex,
        selectColumnExpressions, inputOrderBy);
  ...
  private OrderBy convertSingleInputOrderBy(Map<Expression, Integer> selectColumnExpressionToIndex,
    Expression[] selectColumnExpressions, OrderBy inputOrderBy) throws SQLException {

    OrderPreservingTracker orderPreservingTracker =
      new OrderPreservingTracker(this.statementContext, GroupBy.EMPTY_GROUP_BY, Ordering.UNORDERED,
