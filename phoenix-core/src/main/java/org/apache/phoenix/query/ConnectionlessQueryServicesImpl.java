...
public class ConnectionlessQueryServicesImpl extends DelegateQueryServices implements ConnectionQueryServices  {
    ...
    public PMetaData addColumn(PName tenantId, String tableName, List<PColumn> columns, long tableTimeStamp,
            long tableSeqNum, boolean isImmutableRows, boolean isWalDisabled, boolean isMultitenant) throws SQLException {
        return metaData = metaData.addColumn(tenantId, tableName, columns, tableTimeStamp, tableSeqNum, isImmutableRows, isWalDisabled, isMultitenant);
    }
    ...
    private static List<HRegionLocation> generateRegionLocations(byte[] physicalName, byte[][] splits) {
        ...
        for (byte[] split : splits) {
            regions.add(new HRegionLocation(
                    new HRegionInfo(physicalName, startKey, split),
                    LOCALHOST, -1));
            startKey = split;
        }
        regions.add(new HRegionLocation(
                new HRegionInfo(physicalName, startKey, HConstants.EMPTY_END_ROW),
                LOCALHOST, -1));
        return regions;
    }
