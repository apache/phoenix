...
/**
 * Class, copied for the most part from HRegion.getRowLockInternal implementation that manages
 * reentrant row locks based on the row key. Phoenix needs to manage it's own locking due to
...
public class LockManager {
  ...
  /**
   * Unlock the row. We need this stateless way of unlocking because we have no means of passing the
   * RowLock instances between coprocessor calls (see HBASE-18482). Once we have that, we can have
   * the caller collect RowLock instances and free when needed.
  ...
  class RowLockContext {
    ...
    public String toString() {
      return "RowLockContext{" + "row=" + rowKey + ", readWriteLock=" + reentrantLock + ", count="
