...
public interface IndexCodec {
   ...
   * </pre>
   *
   * To then cleanup this entry, you would just return 'v1,v2,v3', 'myIndex'. the current state of
   * the table that needs to be cleaned up. Generally, you only care about the latest column values,
   * for each column you are indexing for each index table.
   * @param context        TODO
   ...
   * right one). the current state of the table that needs to an index update Generally, you only
   * care about the latest column values, for each column you are indexing for each index table.
   * @param context        TODO
   * @param regionStartKey TODO
   * @param regionEndKey   TODO
   ...
   * and further. mutation that should be indexed.
