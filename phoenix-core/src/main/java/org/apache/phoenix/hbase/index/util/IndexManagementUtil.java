...
public class IndexManagementUtil {
  ...
  private static final String INDEX_HLOG_READER_CLASS_NAME =
  ...
  public static boolean isWALEditCodecSet(Configuration conf) {
    ...
    if (
      INDEX_WAL_EDIT_CODEC_CLASS_NAME.equals(conf.get(WALCellCodec.WAL_CELL_CODEC_CLASS_KEY, null))
    ) {
  ...
  public static void ensureMutableIndexingCorrectlyConfigured(Configuration conf)
    ...
    try {
      ...
      throw new IllegalStateException(codecClass + " is not installed, but " + indexLogReaderName
  ...
  public static ValueGetter createGetterFromScanner(CoveredDeleteScanner scanner,
    byte[] currentRow) {
   ...
   * assumes that for any index, there are going to small number of columns, versus the number of
  ...
  public static boolean updateMatchesColumns(Collection<KeyValue> update,
    ...
    outer: for (KeyValue kv : update) {
      for (ColumnReference ref : columns) {
        if (
          ref.matchesFamily(kv.getFamilyArray(), kv.getFamilyOffset(), kv.getFamilyLength())
   ...
   * <p>
   * This employs the same logic as {@link #updateMatchesColumns(Collection, List)}, but is flips
   * the iteration logic to search columns before kvs.
   */
  public static boolean columnMatchesUpdate(List<ColumnReference> columns,
    Collection<KeyValue> update) {
    boolean matches = false;
  ...
  public static Scan
    newLocalStateScan(List<? extends Iterable<? extends ColumnReference>> refsArray) {
    return newLocalStateScan(null, refsArray);
  }

  public static Scan newLocalStateScan(Scan scan,
    List<? extends Iterable<? extends ColumnReference>> refsArray) {
    Scan s = scan;
    if (scan == null) {
      s = new Scan();
    }
  ...
  /**
   * Propagate the given failure as a generic {@link IOException}, if it isn't already reason
   * indexing failed. If ,<tt>null</tt>, throws a {@link NullPointerException}, which should unload
   * the coprocessor.
   */
  public static void rethrowIndexingException(Throwable e) throws IOException {
    try {
      ...
      LOGGER
        .info("Rethrowing " + e1 + " as a " + IndexBuildingFailureException.class.getSimpleName());
   ...
   */
  public static void createTimestampBatchesFromKeyValues(Collection<KeyValue> kvs,
