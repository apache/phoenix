...
>>>>>>> Revert "Initialial functional index impl"
...
=======
	...
	ColumnDef(ColumnName columnDefName, String sqlTypeName, boolean isArray,
			Integer arrSize, Boolean isNull, Integer maxLength, Integer scale,
			boolean isPK, SortOrder sortOrder, Expression expression) {
		try {
			Preconditions.checkNotNull(sortOrder);
			PDataType localType = null;
			this.columnDefName = columnDefName;
			this.isArray = isArray;
			// TODO : Add correctness check for arrSize. Should this be ignored
			// as in postgres
			// Also add what is the limit that we would support. Are we going to
			// support a
			// fixed size or like postgres allow infinite. May be the data types
			// max limit can
			// be used for the array size (May be too big)
			if (this.isArray) {
				localType = sqlTypeName == null ? null : PDataType
						.fromTypeId(PDataType.sqlArrayType(SchemaUtil
								.normalizeIdentifier(sqlTypeName)));
				this.dataType = sqlTypeName == null ? null : PDataType
						.fromSqlTypeName(SchemaUtil
								.normalizeIdentifier(sqlTypeName));
				this.arrSize = arrSize; // Can only be non negative based on
										// parsing
				if (this.dataType == PDataType.VARBINARY) {
					throw new SQLExceptionInfo.Builder(
							SQLExceptionCode.VARBINARY_ARRAY_NOT_SUPPORTED)
			...
			if (this.dataType == PDataType.CHAR) {
				...
				if (maxLength != null) {
					...
					// points as possible.
					scale = scale == null ? PDataType.DEFAULT_SCALE
	...
	ColumnDef(ColumnName columnDefName, String sqlTypeName, Boolean isNull,
			Integer maxLength, Integer scale, boolean isPK,
			SortOrder sortOrder, Expression expression) {
		this(columnDefName, sqlTypeName, false, 0, isNull, maxLength, scale,
				isPK, sortOrder, expression);
	}
