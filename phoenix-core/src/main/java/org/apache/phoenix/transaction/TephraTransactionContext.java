...
public class TephraTransactionContext implements PhoenixTransactionContext {
  ...
  public long getTransactionId() {
    ...
    return tx == null ? HConstants.LATEST_TIMESTAMP : tx.getTransactionId(); // First write pointer
                                                                             // - won't change with
                                                                             // checkpointing
  }
  ...
  @Override
  public PhoenixTransactionContext newTransactionContext(PhoenixTransactionContext context,
    boolean subTask) {
  ...
  public Table getTransactionalTable(Table htable, boolean isConflictFree) {
    TransactionAwareHTable transactionAwareHTable = new TransactionAwareHTable(htable,
      isConflictFree ? TxConstants.ConflictDetection.NONE : TxConstants.ConflictDetection.ROW);
    this.addTransactionAware(transactionAwareHTable);
    return transactionAwareHTable;
  }

  @Override
  public Table getTransactionalTableWriter(PhoenixConnection connection, PTable table, Table htable,
    boolean isIndex) throws SQLException {
    ...
    if (isIndex) {
      transactionAwareHTable =
        new TransactionAwareHTable(htable, TxConstants.ConflictDetection.NONE);
  ...
  /**
   * Wraps Tephra data table HTables to catch when a rollback occurs so that index Delete mutations
   * can be generated and applied (as opposed to storing them in the Tephra change set). This
   * technique allows the Tephra API to be used directly with HBase APIs and Phoenix APIs since we
   * can detect the rollback as a callback when the Tephra rollback is called.
   */
  private static class RollbackHookHTableWrapper extends DelegateHTable {
     ...
     * opportunity to attach our index meta data to the mutations such that we can also undo the
     * index mutations.
     */
    @Override
    public void delete(List<Delete> deletes) throws IOException {
      ...
      try {
        ...
        if (table.getIndexMaintainers(indexMetaDataPtr, connection)) {
          cache = IndexMetaDataCacheClient.setMetaDataOnMutations(connection, table, deletes,
            indexMetaDataPtr);
        }
        ...
        if (!indexes.isEmpty()) {
          PhoenixTxIndexMutationGenerator generator = PhoenixTxIndexMutationGenerator
            .newGenerator(connection, table, indexes, deletes.get(0).getAttributesMap());
          Collection<Pair<Mutation, byte[]>> indexUpdates =
            generator.getIndexUpdates(delegate, deletes.iterator());
