...
public interface PhoenixTransactionContext {
  public static PhoenixTransactionContext NULL_CONTEXT = new PhoenixTransactionContext() {
    ...
    @Override
    public Table getTransactionalTableWriter(PhoenixConnection connection, PTable table,
      Table htable, boolean isIndex) {
      return null;
    }
  };
  ...
  /**
   * Commit DDL to guarantee that no transaction started before create index and committed
   * afterwards, as explained in [PHOENIX-2478], [TEPHRA-157] and [OMID-56].
  ...
  /**
   * Returns transaction write pointer. After checkpoint the write pointer is different than the
   * initial one
   */
  public long getWritePointer();
