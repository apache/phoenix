...
/**
 * Global root cache for the server. Each tenant is managed as a child tenant cache of this one.
 * Queries not associated with a particular tenant use this as their tenant cache.
 * @since 0.1
 */
public class GlobalCache extends TenantCacheImpl {
  ...
  private final ConcurrentMap<ImmutableBytesWritable, TenantCache> perTenantCacheMap =
    new ConcurrentHashMap<ImmutableBytesWritable, TenantCache>();
  ...
  public long clearTenantCache() {
    ...
    if (unfreedBytes != 0 && LOGGER.isDebugEnabled()) {
      LOGGER.debug(
        "Found " + (getMemoryManager().getMaxMemory() - getMemoryManager().getAvailableMemory())
    ...
    for (Map.Entry<ImmutableBytesWritable, TenantCache> entry : perTenantCacheMap.entrySet()) {
      ...
      long unfreedTenantBytes =
        cache.getMemoryManager().getMaxMemory() - cache.getMemoryManager().getAvailableMemory();
    ...
    if (result == null) {
      synchronized (this) {
        ...
        if (result == null) {
          ...
          metaDataCache = result = CacheBuilder.newBuilder().maximumWeight(maxSize)
            ...
            .weigher(new Weigher<ImmutableBytesPtr, PMetaDataEntity>() {
              ...
              public int weigh(ImmutableBytesPtr key, PMetaDataEntity table) {
                return SizedUtil.IMMUTABLE_BYTES_PTR_SIZE + key.getLength()
  ...
  public static TenantCache getTenantCache(RegionCoprocessorEnvironment env,
    ...
    TenantCache tenantCache =
      tenantId == null ? globalCache : globalCache.getChildTenantCache(tenantId);
    return tenantCache;
  }
  ...
  public TenantCache getChildTenantCache(ImmutableBytesPtr tenantId) {
    ...
    if (tenantCache == null) {
      ...
      int maxServerCachePersistenceTimeToLive =
        config.getInt(QueryServices.MAX_SERVER_CACHE_PERSISTENCE_TIME_TO_LIVE_MS_ATTRIB,
          QueryServicesOptions.DEFAULT_MAX_SERVER_CACHE_PERSISTENCE_TIME_TO_LIVE_MS);
      TenantCacheImpl newTenantCache =
        new TenantCacheImpl(new ChildMemoryManager(getMemoryManager(), maxTenantMemoryPerc),
