...
public abstract class UncoveredIndexRegionScanner extends BaseRegionScanner {
  ...
  public UncoveredIndexRegionScanner(final RegionScanner innerScanner, final Region region,
    ...
    byte[] pageSizeFromScan = scan.getAttribute(BaseScannerRegionObserver.INDEX_PAGE_ROWS);
  ...
  protected boolean scanIndexTableRows(List<Cell> result, final long startTime,
    final byte[] actualStartKey, final int offset) throws IOException {
  ...
  private boolean getNextCoveredIndexRow(List<Cell> result) {
    if (indexRowIterator.hasNext()) {
      ...
      try {
        Result dataRow = dataRows
          .get(new ImmutableBytesPtr(indexToDataRowKeyMap.get(CellUtil.cloneRow(indexRow.get(0)))));
        if (dataRow != null) {
