  ...
  public static PTable addViewInfoToPColumnsIfNeeded(PTable view) throws SQLException {
    ...
    try (PhoenixConnection conn = getConnectionlessConnection()) {
      ...
      ExpressionCompiler expressionCompiler =
        new CreateTableCompiler.ColumnTrackingExpressionCompiler(context,
      ...
      for (PColumn column : PTableImpl.getColumnsToClone(view)) {
        ...
        if (
          ...
          result.add(new PColumnImpl(column, viewColumnConstantsToBe[column.getPosition()],
            isViewReferenced));
        }
      ...
      return PTableImpl.builderWithColumns(view, result).build();
    }
  }

  private static PhoenixConnection getConnectionlessConnection() throws SQLException {
    return DriverManager.getConnection(JDBC_PROTOCOL + JDBC_PROTOCOL_SEPARATOR + CONNECTIONLESS)
