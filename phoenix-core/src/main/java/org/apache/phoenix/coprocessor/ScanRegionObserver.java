...
/**
 * Wraps the scan performing a non aggregate query to prevent needless retries if a Phoenix bug is
 * encountered from our custom filter expression evaluation. Unfortunately, until HBASE-7481 gets
 * fixed, there's no way to do this from our custom filters.
 * @since 0.1
 */
public class ScanRegionObserver extends BaseScannerRegionObserver implements RegionCoprocessor {
  ...
  /**
   * In case we are supporting exposing dynamic columns for wildcard queries, which is based on the
   * client-side config
   * {@link org.apache.phoenix.query.QueryServices#WILDCARD_QUERY_DYNAMIC_COLS_ATTRIB}, we
   * previously set attributes on the Put mutations where the key is the column family and the value
   * is the serialized list of dynamic columns. Here we iterate over all Put mutations and add
   * metadata for the list of dynamic columns for each column family in its own cell under reserved
   * qualifiers. See PHOENIX-374
    ...
    for (int i = 0; i < miniBatchOp.size(); i++) {
      ...
      if (
        ...
        if (LOGGER.isDebugEnabled()) {
          LOGGER.debug(
            "Adding dynamic column metadata for table: " + tableName + ". Put :" + m.toString());
        }
        ...
        for (byte[] fam : famCellMap.keySet()) {
          ...
          List<PTableProtos.PColumn> dynColsInThisFam =
            DynamicColumnMetaDataProtos.DynamicColumnMetaData.parseFrom(serializedDynColsList)
          ...
          for (PTableProtos.PColumn dynColProto : dynColsInThisFam) {
            ...
            dynColShadowCellsPut.addColumn(fam,
              getQualifierForDynamicColumnMetaDataCell(dynColProto), dynColProto.toByteArray());
          }
        }
      }
      if (dynColShadowCellsPut != null) {
  ...
  /**
   * We store the metadata for each dynamic cell in a separate cell in the same column family. The
   * column qualifier for this cell is: {@link ScanRegionObserver#DYN_COLS_METADATA_CELL_QUALIFIER}
   * concatenated with the qualifier of the actual dynamic column
