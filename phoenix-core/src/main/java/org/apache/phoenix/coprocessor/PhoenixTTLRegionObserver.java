...
public class PhoenixTTLRegionObserver extends BaseScannerRegionObserver
  ...
  protected RegionScanner doPostScannerOpen(final ObserverContext<RegionCoprocessorEnvironment> c,
    ...
    if (ScanUtil.isMaskTTLExpiredRows(scan) && ScanUtil.isDeleteTTLExpiredRows(scan)) {
      ...
      scan.setAttribute(PhoenixTTLRegionScanner.MASK_PHOENIX_TTL_EXPIRED_REQUEST_ID_ATTR, Bytes
        .toBytes(String.format("DELETE-EXPIRED-%d", metricSource.getDeleteExpiredRequestCount())));
    }
    LOG.trace(String.format(
      "********** PHOENIX-TTL: PhoenixTTLRegionObserver::postScannerOpen TTL for table = "
        + "[%s], scan = [%s], PHOENIX_TTL = %d ***************, "
        + "numMaskExpiredRequestCount=%d, " + "numDeleteExpiredRequestCount=%d",
  ...
  private static class PhoenixTTLRegionScanner extends BaseRegionScanner {
    ...
    public void close() throws IOException {
      if (!reported) {
        LOG.debug(
          String.format("PHOENIX-TTL-SCAN-STATS-ON-CLOSE: " + "request-id:[%s,%s] = [%d, %d, %d]",
            this.requestId, Bytes.toString(scanTableName), this.numRowsScanned, this.numRowsExpired,
            this.numRowsDeleted));
        reported = true;
      }
      scanner.close();
    ...
    private boolean doNext(List<Cell> result, boolean raw) throws IOException {
      try {
        ...
        do {
          ...
          /**
           * Note : That both MaskIfExpiredRequest and DeleteIfExpiredRequest cannot be set at the
           * same time. Case : MaskIfExpiredRequest, If row not expired then return.
           */
          numRowsScanned++;
          ...
          /**
           * Case : DeleteIfExpiredRequest, If deleted then return. So that it will count towards
           * the aggregate deleted count.
           */
    ...
    private boolean deleteRowIfExpired(List<Cell> cellList) throws IOException {
      ...
      if (isRowExpired) {
        ...
        LOG.trace(String.format(
          ...
          Bytes.toString(rowKey), Bytes.toString(scanTableName),
          maxTimestamp != HConstants.LATEST_TIMESTAMP, now, now - ttl, ts));
    ...
    public RegionScanner getNewRegionScanner(Scan scan) throws IOException {
      return new PhoenixTTLRegionScanner(env, scan,
        ((BaseRegionScanner) delegate).getNewRegionScanner(scan));
    }
  }
}
