...
abstract public class BaseScannerRegionObserver extends CompatBaseScannerRegionObserver {
  ...
  private static void throwIfScanOutOfRegion(Scan scan, Region region)
    ...
    if (isLocalIndex) {
      ...
      byte[] expectedUpperRegionKey = scan.getAttribute(EXPECTED_UPPER_REGION_KEY) == null
        ? scan.getStopRow()
  ...
  /*
   * We want to override the store scanner so that we can read "past" a delete marker on an SCN /
   * lookback query to see the underlying edit. This was possible in HBase 1.x, but not possible
   * after the interface changes in HBase 2.0. HBASE-24321 in HBase 2.3 gave us this ability back,
   * but we need to use it through a compatibility shim so we can compile against 2.1 and 2.2. When
   * 2.3 is the minimum supported HBase version, the shim can be retired and the logic moved into
   * the real coproc. We also need to override the flush compaction coproc hooks in order to
   * implement max lookback age to keep versions from being purged. Because the required APIs aren't
   * present in HBase 2.1 and 2.2, we override in the 2.3 version of CompatBaseScannerRegionObserver
   * and no-op in the other versions.
   */
