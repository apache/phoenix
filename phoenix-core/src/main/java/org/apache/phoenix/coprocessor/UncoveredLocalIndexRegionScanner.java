...
public class UncoveredLocalIndexRegionScanner extends UncoveredIndexRegionScanner {
  ...
  public UncoveredLocalIndexRegionScanner(final RegionScanner innerScanner, final Region region,
    ...
    try (RegionScanner regionScanner = region.getScanner(dataScan)) {
      ...
      do {
        ...
        if (!row.isEmpty()) {
          ...
          dataRows.put(new ImmutableBytesPtr(CellUtil.cloneRow(firstCell)), Result.create(row));
      ...
      if (state == State.SCANNING_DATA_INTERRUPTED) {
        LOGGER.info(
          "Data table scan is interrupted in " + "UncoveredLocalIndexRegionScanner for region "
  ...
  @Override
  protected boolean scanIndexTableRows(List<Cell> result, final long startTime) throws IOException {
