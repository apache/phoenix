...
public abstract class GlobalIndexRegionScanner extends BaseRegionScanner {
  ...
  public static final String INDEX_VERIFY_ROW_COUNTS_PER_TASK_CONF_KEY =
    "index.verify.row.count.per.task";
  ...
  public GlobalIndexRegionScanner(final RegionScanner innerScanner, final Region region,
    ...
    if (verify) {
      ...
      byte[] scanParamShouldLogBeyondMaxLookbackInvalidRows = scan.getAttribute(
        BaseScannerRegionObserver.INDEX_REBUILD_DISABLE_LOGGING_BEYOND_MAXLOOKBACK_AGE);
  ...
  private void logMismatch(Mutation expected, Mutation actual, int iteration,
    ...
    for (List<Cell> cells : expected.getFamilyCellMap().values()) {
      ...
      for (Cell expectedCell : cells) {
        ...
        if (actualCell == null || !CellUtil.matchingType(expectedCell, actualCell)) {
          byte[] dataKey = indexMaintainer
            .buildDataRowKey(new ImmutableBytesWritable(expected.getRow()), viewConstants);
  ...
  private static Put prepareIndexPutForRebuid(IndexMaintainer indexMaintainer,
   ...
   * only for global indexes. pendingMutations is a sorted list of data table mutations that are
   * used to replay index table mutations. This list is sorted in ascending order by the tuple of
   * row key, timestamp and mutation type where delete comes after put.
   */
  ...
  protected RegionScanner getLocalScanner() throws IOException {
    ...
    if (minTimestamp != 0) {
      ...
      try (RegionScanner scanner =
        new PagedRegionScanner(region, region.getScanner(incrScan), incrScan)) {
