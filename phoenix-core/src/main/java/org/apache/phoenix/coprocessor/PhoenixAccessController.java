...
public class PhoenixAccessController extends BaseMetaDataEndpointObserver {
  ...
  public void preCreateTable(ObserverContext<PhoenixMetaDataControllerEnvironment> ctx,
    ...
    if (tableType != PTableType.VIEW) {
      ...
      for (byte[] familyName : familySet) {
        tableDescBuilder
          .addColumnFamily(ColumnFamilyDescriptorBuilder.newBuilder(familyName).build());
      }
      ...
      for (TableName index : indexes) {
        ...
        if (permissionForUser != null) {
          ...
          if (!requireAccess.isEmpty()) {
            for (UserPermission userPermission : permissionForUser) {
              accessExists.addAll(Arrays.asList(getPermissionFromUP(userPermission).getActions()));
            }
          }
        } else {
        ...
        if (userPermissions != null) {
          for (UserPermission userPermission : userPermissions) {
            ...
            List<UserPermission> permsToTable =
              getPermissionForUser(permissionsOnTheTable, getUserFromUP(userPermission));
            ...
            if (permsToTable != null) {
              ...
              for (UserPermission permToTable : permsToTable) {
                accessExists.addAll(Arrays.asList(getPermissionFromUP(permToTable).getActions()));
              }
            }
            if (!requireAccess.isEmpty()) {
              ...
              handleRequireAccessOnDependentTable(request, getUserFromUP(userPermission), toTable,
  ...
  private boolean hasAccess(List<UserPermission> perms, TableName table, Permission.Action action,
    ...
    if (perms != null) {
      if (hbaseAccessControllerEnabled && authorizeUserTable(accessChecker, user, table, action)) {
        return true;
      }
      ...
      if (permissionsForUser != null) {
        for (UserPermission permissionForUser : permissionsForUser) {
          if (getPermissionFromUP(permissionForUser).implies(action)) {
            return true;
          }
        }
      }
