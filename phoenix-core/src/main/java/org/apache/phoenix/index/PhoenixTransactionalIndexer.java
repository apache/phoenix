...
/**
 * Do all the work of managing local index updates for a transactional table from a single
 * coprocessor. Since the transaction manager essentially time orders writes through conflict
 * detection, the logic to maintain a secondary index is quite a bit simpler than the non
 * transactional case. For example, there's no need to muck with the WAL, as failure scenarios are
 * handled by aborting the transaction.
 */
public class PhoenixTransactionalIndexer implements RegionObserver, RegionCoprocessor {
  ...
  public void start(CoprocessorEnvironment e) throws IOException {
    ...
    this.writer =
      new IndexWriter(IndexWriter.getCommitter(indexWriterEnv, ParallelWriterIndexCommitter.class),
  ...
  private static Iterator<Mutation>
    getMutationIterator(final MiniBatchOperationInProgress<Mutation> miniBatchOp) {
  ...
  public void preBatchMutate(ObserverContext<RegionCoprocessorEnvironment> c,
    ...
    try (TraceScope scope = Trace.startSpan("Starting to build index updates")) {
      ...
      PhoenixTxIndexMutationGenerator generator = new PhoenixTxIndexMutationGenerator(
        env.getConfiguration(), indexMetaData, env.getRegionInfo().getTable().getName(),
