...
public class GlobalIndexChecker extends BaseScannerRegionObserver implements RegionCoprocessor {
  ...
  private class GlobalIndexScanner extends BaseRegionScanner {
    ...
    private void deleteRowIfAgedEnough(byte[] indexRowKey, long ts, boolean specific)
      throws IOException {
    ...
    private void repairIndexRows(byte[] indexRowKey, long ts, List<Cell> row) throws IOException {
      if (buildIndexScanForDataTable == null) {
        ...
        byte[] dataTableName =
          scan.getAttribute(BaseScannerRegionObserver.PHYSICAL_DATA_TABLE_NAME);
        ...
        buildIndexScanForDataTable.setAttribute(BaseScannerRegionObserver.UNGROUPED_AGG,
          TRUE_BYTES);
        ...
        buildIndexScanForDataTable.setAttribute(BaseScannerRegionObserver.REBUILD_INDEXES,
          TRUE_BYTES);
        buildIndexScanForDataTable
          .setAttribute(BaseScannerRegionObserver.SKIP_REGION_BOUNDARY_CHECK, Bytes.toBytes(true));
        buildIndexScanForDataTable
          .setAttribute(BaseScannerRegionObserver.EMPTY_COLUMN_QUALIFIER_NAME, emptyCQ);
      ...
      buildIndexScanForDataTable.setAttribute(BaseScannerRegionObserver.INDEX_ROW_KEY, indexRowKey);
      Result result = null;
      ...
      // timestamp is
      // lower than than the timestamp of the unverified index row (ts) and the index row that is
      // built from
      ...
      do {
        ...
        RegionScanner singleRowScanner =
          ((BaseRegionScanner) delegate).getNewRegionScanner(singleRowIndexScan);
        row.clear();
    ...
    /**
     * An index row is composed of cells with the same timestamp. However, if there are multiple
     * versions of an index row, HBase can return an index row with cells from multiple versions,
     * and thus it can return cells with different timestamps. This happens if the version of the
     * row we are reading does not have a value (i.e., effectively has null value) for a column
     * whereas an older version has a value for the column. In this case, we need to remove the
     * older cells for correctness.
     */
    ...
    private boolean verifyRowAndRemoveEmptyColumn(List<Cell> cellList) throws IOException {
      ...
      while (cellIterator.hasNext()) {
        ...
        if (isEmptyColumn(cell)) {
          ...
          // Empty column is not supposed to be returned to the client except it is the only column
          // included
          // in the scan
          if (cellListSize > 1) {
            cellIterator.remove();
