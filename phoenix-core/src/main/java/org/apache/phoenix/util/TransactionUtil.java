...
public class TransactionUtil {
  ...
  private static Cell newDeleteFamilyMarker(byte[] row, byte[] family, long timestamp) {
    return CellUtil.createCell(row, family, FAMILY_DELETE_MARKER, timestamp,
      KeyValue.Type.Put.getCode(), HConstants.EMPTY_BYTE_ARRAY);
  }

  private static Cell newDeleteColumnMarker(byte[] row, byte[] family, byte[] qualifier,
    long timestamp) {
    return CellUtil.createCell(row, family, qualifier, timestamp, KeyValue.Type.Put.getCode(),
  ...
  public static long getResolvedTimestamp(PhoenixConnection connection, boolean isTransactional,
    long defaultResolvedTimestamp) {
    MutationState mutationState = connection.getMutationState();
    Long scn = connection.getSCN();
    return scn != null ? scn
      : (isTransactional && mutationState.isTransactionStarted())
  ...
  public static Long getTableTimestamp(PhoenixConnection connection, boolean transactional,
    TransactionFactory.Provider provider) throws SQLException {
    Long timestamp = null;
    if (!transactional) {
      return timestamp;
    }
      ...
      for (Map.Entry<byte[], List<Cell>> entry : mutation.getFamilyCellMap().entrySet()) {
        ...
        if (familyCells.size() == 1) {
          if (CellUtil.isDeleteFamily(familyCells.get(0))) {
            ...
            deleteMarker.add(newDeleteFamilyMarker(deleteMarker.getRow(), family,
          ...
          for (Cell cell : familyCells) {
            if (CellUtil.isDeleteColumns(cell)) {
              ...
              deleteMarker.add(newDeleteColumnMarker(deleteMarker.getRow(), family,
