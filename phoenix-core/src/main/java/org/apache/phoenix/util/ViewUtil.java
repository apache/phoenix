...
public class ViewUtil {
  ...
  private static TableViewFinderResult findImmediateRelatedViews(Table sysCatOrsysChildLink,
    ...
    SingleColumnValueFilter linkFilter = new SingleColumnValueFilter(TABLE_FAMILY_BYTES,
      LINK_TYPE_BYTES, CompareFilter.CompareOp.EQUAL, linkType.getSerializedValueAsByteArray());
  ...
  public static boolean hasChildViews(Table sysCatOrsysChildLink, byte[] tenantId,
    ...
    SingleColumnValueFilter linkFilter =
      new SingleColumnValueFilter(TABLE_FAMILY_BYTES, LINK_TYPE_BYTES,
        CompareFilter.CompareOp.EQUAL, LinkType.CHILD_TABLE.getSerializedValueAsByteArray()) {
   ...
   * {@code parent->child } entry in SYSTEM.CHILD_LINK/SYSTEM.CATALOG (as a child) but for whom the
  ...
  public static void dropChildViews(RegionCoprocessorEnvironment env, byte[] tenantIdBytes,
    ...
    try {
      hTable = ServerUtil.getHTableForCoprocessorScan(env,
        SchemaUtil.getPhysicalTableName(sysCatOrSysChildLink, env.getConfiguration()));
    } catch (Exception e) {
    ...
    try {
      childViewsResult = findImmediateRelatedViews(hTable, tenantIdBytes, schemaName,
        tableOrViewName, LinkType.CHILD_TABLE, HConstants.LATEST_TIMESTAMP);
    } finally {
      hTable.close();
    }
    ...
    for (TableInfo viewInfo : childViewsResult.getLinks()) {
      ...
      if (logger.isDebugEnabled()) {
        logger.debug("dropChildViews : " + Bytes.toString(schemaName) + "."
          + Bytes.toString(tableOrViewName) + " -> " + Bytes.toString(viewSchemaName) + "."
          + Bytes.toString(viewName) + "with tenant id :" + Bytes.toString(viewTenantId));
      }
      Properties props = new Properties();
      PTable view = null;
      ...
      try (PhoenixConnection connection = QueryUtil
        ...
        try {
          ...
          logger.info("Found an expected orphan parent->child link keyed by the parent."
            + " Parent Tenant Id: '" + Bytes.toString(tenantIdBytes) + "'. Parent Schema Name: '"
            + Bytes.toString(schemaName) + "'. Parent Table/View Name: '"
            + Bytes.toString(tableOrViewName)
            + "'. Will attempt to drop this child view with ViewInfo: '" + viewInfo + "'.");
        }
        if (view != null) {
        ...
        try {
          client
            .dropTable(new DropTableStatement(viewTableName, PTableType.VIEW, true, true, true));
  ...
  /**
   * Inherit all columns from the parent unless its an excluded column if the same columns is
   * present in the parent and child (for table metadata created before PHOENIX-3534) we chose the
  ...
  public static PTable addDerivedColumnsFromParent(PhoenixConnection connection, PTable view,
    ...
    int baseTableColumnCount = isDiverged
    ...
    PTable pTable = PTableImpl.builderWithColumns(view, columnsToAdd)
      ...
      .setBaseColumnCount(baseTableColumnCount).setTimeStamp(maxTableTimestamp)
    ...
    for (Mutation m : tableMetaData) {
      if (m instanceof Put) {
        MetaDataUtil.conditionallyAddTagsToPutCells((Put) m,
          ...
          parentUpdateCacheFreqBytes, MetaDataEndpointImpl.VIEW_MODIFIED_PROPERTY_BYTES);
        MetaDataUtil.conditionallyAddTagsToPutCells((Put) m,
          PhoenixDatabaseMetaData.TABLE_FAMILY_BYTES,
          PhoenixDatabaseMetaData.USE_STATS_FOR_PARALLELIZATION_BYTES, extendedCellBuilder,
          parentUseStatsForParallelizationBytes, MetaDataEndpointImpl.VIEW_MODIFIED_PROPERTY_BYTES);
        MetaDataUtil.conditionallyAddTagsToPutCells((Put) m,
