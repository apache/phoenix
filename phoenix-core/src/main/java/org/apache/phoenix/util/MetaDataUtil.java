...
public class MetaDataUtil {
  ...
  public static void deleteViewIndexSequences(PhoenixConnection connection, PName name,
    ...
    connection.createStatement()
      .executeUpdate("DELETE FROM " + PhoenixDatabaseMetaData.SYSTEM_SEQUENCE + " WHERE "
        + PhoenixDatabaseMetaData.SEQUENCE_SCHEMA
        + (schemaName.length() > 0 ? "='" + schemaName + "'" : " IS NULL") + " AND "
        + PhoenixDatabaseMetaData.SEQUENCE_NAME + " = '" + sequenceName + "'");
  }
  ...
  public static boolean tableRegionsOnline(Configuration conf, PTable table) {
    try (ClusterConnection hcon = (ClusterConnection) ConnectionFactory.createConnection(conf)) {
      ...
      for (HRegionLocation loc : locations) {
        try {
          ...
          org.apache.hadoop.hbase.shaded.protobuf.ProtobufUtil.getRegionInfo(controller, admin,
            loc.getRegion().getRegionName());
        } catch (RemoteException e) {
          LOGGER.debug(
            "Cannot get region " + loc.getRegion().getEncodedName() + " info due to error:" + e);
          return false;
        }
      }
