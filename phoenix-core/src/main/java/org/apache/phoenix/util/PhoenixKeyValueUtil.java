...
/**
 * Utilities for KeyValue. Where there's duplication with KeyValue methods, these avoid creating new
 * objects when not necessary (primary preventing byte array copying).
 * @since 0.1
 */
public class PhoenixKeyValueUtil {
  ...
  public static Cell newKeyValue(byte[] key, byte[] cf, byte[] cq, long ts, byte[] value,
    ...
    return CellBuilderFactory.create(CellBuilderType.DEEP_COPY).setRow(key).setFamily(cf)
      .setQualifier(cq).setTimestamp(ts).setType(Type.Put).setValue(value, valueOffset, valueLength)
      .build();
  }

  public static Cell newKeyValue(ImmutableBytesWritable key, byte[] cf, byte[] cq, long ts,
    ...
    return CellBuilderFactory.create(CellBuilderType.DEEP_COPY)
      ...
      .setTimestamp(ts).setType(Type.Put).setValue(value, valueOffset, valueLength).build();
  }

  public static Cell newKeyValue(byte[] key, int keyOffset, int keyLength, byte[] cf, byte[] cq,
    long ts, byte[] value, int valueOffset, int valueLength) {
    return CellBuilderFactory.create(CellBuilderType.DEEP_COPY).setRow(key, keyOffset, keyLength)
      .setFamily(cf).setQualifier(cq).setTimestamp(ts).setType(Type.Put)
      .setValue(value, valueOffset, valueLength).build();
  }

  public static Cell newKeyValue(byte[] key, int keyOffset, int keyLength, byte[] cf, int cfOffset,
    int cfLength, byte[] cq, int cqOffset, int cqLength, long ts, byte[] value, int valueOffset,
    int valueLength, Type type) {
    return CellBuilderFactory.create(CellBuilderType.DEEP_COPY).setRow(key, keyOffset, keyLength)
      .setFamily(cf, cfOffset, cfLength).setQualifier(cq, cqOffset, cqLength).setTimestamp(ts)
   ...
   */
  public static Cell getColumnLatest(KeyValueBuilder kvBuilder, List<Cell> kvs, byte[] family,
    byte[] qualifier) {
    if (kvs.size() == 0) {
      return null;
    }
   ...
   * @param kvBuilder TODO
   */
  public static Cell getColumnLatest(KeyValueBuilder kvBuilder, Cell[] kvs, byte[] family,
    byte[] qualifier) {
    if (kvs.length == 0) {
      return null;
    }
  ...
  /*
   * Special comparator, *only* works for binary search. We make the following assumption: 1. All
   * KVs compared have the same row key 2. For each (rowkey, family, qualifier) there is at most one
   * version 3. Current JDKs only uses the search term on the right side #1 allows us to avoid row
   * key comparisons altogether. #2 allows for exact matches #3 lets us save instanceof checks, and
   * allows to inline the search term in the comparator
   */
  ...
  /**
   * Replace non-KeyValue cells with their KeyValue copies This ensures that all Cells are copied
   * on-heap, but will do extra work for any non-KeyValue on-heap cells
  ...
  /**
   * Copy all Off-Heap cells to KeyValues The input list is modified.
  ...
  private static long
    calculateRowMutationSize(Entry<ImmutableBytesPtr, RowMutationState> rowEntry) {
