...
public class IndexUtil {
    ...
    public static ScanProjector getScanProjector(Scan scan, ColumnReference[] dataColumns) {
        if (dataColumns != null && dataColumns.length != 0) {
            ...
            return new ScanProjector(keyValueSchema, keyValueColumns);
        }
        return null;
    }
    
    public static void wrapResultUsingOffset(List<Cell> result, final int offset,
            ColumnReference[] dataColumns, ScanProjector scanProjector, HRegion dataRegion,
        ...
        if (scanProjector != null) {
            ...
            byte[] value =
                    scanProjector.getSchema().toBytes(joinTuple, scanProjector.getExpressions(),
                        scanProjector.getValueBitSet(), ptr);
            KeyValue keyValue =
                    KeyValueUtil.newKeyValue(firstCell.getRowArray(),firstCell.getRowOffset(),firstCell.getRowLength(), ScanProjector.VALUE_COLUMN_FAMILY,
                        ScanProjector.VALUE_COLUMN_QUALIFIER, firstCell.getTimestamp(), value, 0, value.length);
            result.add(keyValue);
        }
