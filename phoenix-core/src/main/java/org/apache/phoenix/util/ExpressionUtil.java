...
public class ExpressionUtil {
  ...
  public static LiteralExpression getConstantExpression(Expression expression,
    ImmutableBytesWritable ptr) throws SQLException {
    Object value = null;
    PDataType type = expression.getDataType();
    if (expression.evaluate(null, ptr) && ptr.getLength() != 0) {
      value = type.toObject(ptr.get(), ptr.getOffset(), ptr.getLength(), type,
  ...
  public static boolean isPkPositionChanging(TableRef tableRef,
    ...
    for (int i = 0; i < tableRef.getTable().getPKColumns().size(); i++) {
      ...
      if (
        source == null
          || !source.equals(new ColumnRef(tableRef, column.getPosition()).newColumnExpression())
      ) {
        return true;
      }
    }
  ...
  /**
   * check the whereExpression to see if the columnExpression is constant. eg. for
   * {@code "where a = 3 and b > 9" }, a is constant,but b is not.
   */
  public static boolean isColumnExpressionConstant(ColumnExpression columnExpression,
  ...
  private static class IsColumnConstantExpressionVisitor
    ...
    private void checkConstantValue(Expression rhsExpression) {
      ...
      if (
        !isExpressionEquals(this.isNullExpressionVisited ? null : this.firstRhsConstantExpression,
          rhsExpression)
      ) {
        rhsConstantCount++;
        return;
  ...
  private static class IsCompositeLiteralExpressionVisitor
    ...
    public Boolean defaultReturn(Expression expression, List<Boolean> childResultValues) {
      if (
        !ExpressionUtil.isContantForStatement(expression)
   ...
   * </pre>
   */
  public static Pair<OrderBy, Integer> getOrderByFromTable(TableRef tableRef,
    PhoenixConnection phoenixConnection, boolean orderByReverse) throws SQLException {
  ...
  /**
   * Infer OrderBys from the rowkey columns of {@link PTable}. The second part of the return pair is
   * the rowkey column offset we must skip when we create OrderBys, because for table with
   * salted/multiTenant/viewIndexId, some leading rowkey columns should be skipped.
   */
  public static Pair<OrderBy, Integer> getOrderByFromTableByRowKeyColumn(PTable table,
    PhoenixConnection phoenixConnection, boolean orderByReverse) {
    ...
    List<RowKeyColumnExpression> rowKeyColumnExpressions =
      rowKeyColumnExpressionsAndRowKeyColumnOffset.getFirst();
  ...
  /**
   * For projected table may be there is no rowkey columns, so we should move forward to inspect
   * {@link ProjectedColumn} to check if the source column is rowkey column. The second part of the
   * return pair is the rowkey column offset we must skip when we create OrderBys, because for table
   * with salted/multiTenant/viewIndexId, some leading rowkey columns should be skipped.
   */
  public static Pair<OrderBy, Integer> getOrderByFromProjectedTable(TableRef projectedTableRef,
    PhoenixConnection phoenixConnection, boolean orderByReverse) throws SQLException {
    ...
    for (PColumn column : projectedTable.getColumns()) {
      ...
      if (sourceRowKeyColumnIndex >= 0) {
        ...
        sourceRowKeyColumnIndexToProjectedColumnRef.put(Integer.valueOf(sourceRowKeyColumnIndex),
          projectedColumnRef);
      }
    }
    ...
    final int sourceRowKeyColumnOffset =
      getRowKeyColumnOffset(sourceTableRef.getTable(), phoenixConnection);
    ...
    for (Entry<Integer, ColumnRef> entry : sourceRowKeyColumnIndexToProjectedColumnRef.entrySet()) {
      ...
      OrderByExpression orderByExpression = OrderByExpression
        .convertExpressionToOrderByExpression(projectedValueColumnExpression, orderByReverse);
  ...
  /**
   * Create {@link RowKeyColumnExpression} from {@link PTable}. The second part of the return pair
   * is the rowkey column offset we must skip when we create OrderBys, because for table with
   * salted/multiTenant/viewIndexId, some leading rowkey columns should be skipped.
   */
  public static Pair<List<RowKeyColumnExpression>, Integer>
    getRowKeyColumnExpressionsFromTable(PTable table, PhoenixConnection phoenixConnection) {
    int pkPositionOffset = getRowKeyColumnOffset(table, phoenixConnection);
    List<PColumn> pkColumns = table.getPKColumns();
    if (pkPositionOffset >= pkColumns.size()) {
      return new Pair<List<RowKeyColumnExpression>, Integer>(
        Collections.<RowKeyColumnExpression> emptyList(), 0);
    }
    List<RowKeyColumnExpression> rowKeyColumnExpressions =
      new ArrayList<RowKeyColumnExpression>(pkColumns.size() - pkPositionOffset);
    ...
    return new Pair<List<RowKeyColumnExpression>, Integer>(rowKeyColumnExpressions,
      pkPositionOffset);
  }

  /**
   * Create OrderByExpression by RowKeyColumnExpression,isNullsLast is the default value
   * "false",isAscending is based on {@link Expression#getSortOrder()}. If orderByReverse is true,
   * reverse the isNullsLast and isAscending.
   */
  public static OrderBy convertRowKeyColumnExpressionsToOrderBy(
    List<RowKeyColumnExpression> rowKeyColumnExpressions, boolean orderByReverse) {
    return convertRowKeyColumnExpressionsToOrderBy(rowKeyColumnExpressions,
      Collections.<Info> emptyList(), orderByReverse);
  }

  /**
   * Create OrderByExpression by RowKeyColumnExpression, if the orderPreservingTrackInfos is not
   * null, use isNullsLast and isAscending from orderPreservingTrackInfos. If orderByReverse is
   * true, reverse the isNullsLast and isAscending.
   */
  public static OrderBy convertRowKeyColumnExpressionsToOrderBy(
    List<RowKeyColumnExpression> rowKeyColumnExpressions, List<Info> orderPreservingTrackInfos,
    boolean orderByReverse) {
    if (rowKeyColumnExpressions.isEmpty()) {
      return OrderBy.EMPTY_ORDER_BY;
    }
    List<OrderByExpression> orderByExpressions =
      new ArrayList<OrderByExpression>(rowKeyColumnExpressions.size());
    ...
    if (orderPreservingTrackInfos != null && orderPreservingTrackInfos.size() > 0) {
      if (orderPreservingTrackInfos.size() != rowKeyColumnExpressions.size()) {
        throw new IllegalStateException("orderPreservingTrackInfos.size():["
          + orderPreservingTrackInfos.size() + "] should equals rowKeyColumnExpressions.size():["
          + rowKeyColumnExpressions.size() + "]!");
      }
    ...
    for (RowKeyColumnExpression rowKeyColumnExpression : rowKeyColumnExpressions) {
      ...
      OrderByExpression orderByExpression = OrderByExpression.convertExpressionToOrderByExpression(
        rowKeyColumnExpression, orderPreservingTrackInfo, orderByReverse);
      orderByExpressions.add(orderByExpression);
    }
  ...
  /**
   * Convert the GroupBy to OrderBy, expressions in GroupBy should be converted to
  ...
  public static OrderBy convertGroupByToOrderBy(GroupBy groupBy, boolean orderByReverse) {
    ...
    List<RowKeyColumnExpression> rowKeyColumnExpressions =
      convertGroupByToRowKeyColumnExpressions(groupBy);
    ...
    return convertRowKeyColumnExpressionsToOrderBy(rowKeyColumnExpressions,
      orderPreservingTrackInfos, orderByReverse);
  }

  /**
   * Convert the expressions in GroupBy to {@link RowKeyColumnExpression}, the convert logic is same
   * as {@link ExpressionCompiler#wrapGroupByExpression}.
   */
  public static List<RowKeyColumnExpression>
    convertGroupByToRowKeyColumnExpressions(GroupBy groupBy) {
    if (groupBy.isEmpty()) {
      return Collections.<RowKeyColumnExpression> emptyList();
    }
    List<Expression> groupByExpressions = groupBy.getExpressions();
    List<RowKeyColumnExpression> rowKeyColumnExpressions =
      new ArrayList<RowKeyColumnExpression>(groupByExpressions.size());
    int columnIndex = 0;
  ...
  /**
   * Convert the expressions in GroupBy to {@link RowKeyColumnExpression}, a typical case is in
   * {@link ExpressionCompiler#wrapGroupByExpression}.
   */
  public static RowKeyColumnExpression convertGroupByExpressionToRowKeyColumnExpression(
    GroupBy groupBy, Expression originalExpression, int groupByColumnIndex) {
    RowKeyValueAccessor rowKeyValueAccessor =
      new RowKeyValueAccessor(groupBy.getKeyExpressions(), groupByColumnIndex);
    return new RowKeyColumnExpression(originalExpression, rowKeyValueAccessor,
