...
/**
 * This filter overrides the behavior of delegate so that we do not jump to the next row if the row
 * is unverified and doesn't match the filter since it is possible that a previous verified version
 * of the same row could match the filter and thus should be included in the results. For tables
 * using encoded columns, the empty column is the first column the filter processes, so we can check
 * whether it is verified or not. If no encoding is used, the empty column is the last column to be
 * processed by the filter, so we have to wait to determine whether the row is verified or not.
 */
public class UnverifiedRowFilter extends DelegateFilter {
  ...
  public ReturnCode filterCell(final Cell cell) throws IOException {
    ...
    if (ret == ReturnCode.NEXT_ROW || ret == ReturnCode.INCLUDE_AND_SEEK_NEXT_ROW) {
