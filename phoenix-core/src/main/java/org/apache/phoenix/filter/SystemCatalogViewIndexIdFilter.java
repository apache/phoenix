...
public class SystemCatalogViewIndexIdFilter extends FilterBase implements Writable {
  ...
  public void filterRowCells(List<Cell> kvs) throws IOException {
    Cell viewIndexIdCell = PhoenixKeyValueUtil.getColumnLatest(GenericKeyValueBuilder.INSTANCE, kvs,
      DEFAULT_COLUMN_FAMILY_BYTES, VIEW_INDEX_ID_BYTES);

    /*
     * We retrieve the VIEW_INDEX_ID cell from SMALLINT to BIGINT or BIGINT to SMALLINT if and only
     * if VIEW_INDEX_ID is included as part of the projected column. This is combination of diff
     * client created view index looks like: client VIEW_INDEX_ID(Cell number of bytes)
     * VIEW_INDEX_ID_DATA_TYPE pre-4.15 2 bytes NULL post-4.15[config smallint] 2 bytes 5(smallint)
     * post-4.15[config bigint] 8 bytes -5(bigint)
     */
    if (viewIndexIdCell != null) {
      int type = NULL_DATA_TYPE_VALUE;
      Cell viewIndexIdDataTypeCell =
        PhoenixKeyValueUtil.getColumnLatest(GenericKeyValueBuilder.INSTANCE, kvs,
          DEFAULT_COLUMN_FAMILY_BYTES, VIEW_INDEX_ID_DATA_TYPE_BYTES);
      if (viewIndexIdDataTypeCell != null) {
        type = (Integer) PInteger.INSTANCE.toObject(viewIndexIdDataTypeCell.getValueArray(),
  ...
  public static SystemCatalogViewIndexIdFilter parseFrom(final byte[] pbBytes)
    throws DeserializationException {
    try {
      SystemCatalogViewIndexIdFilter writable = (SystemCatalogViewIndexIdFilter) Writables
        .getWritable(pbBytes, new SystemCatalogViewIndexIdFilter());
      return writable;
    } catch (IOException e) {
