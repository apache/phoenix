...
public class ZKConnectionInfo extends ConnectionInfo {
  ...
  private ZKConnectionInfo(boolean isConnectionless, String principal, String keytab, User user,
    String zkHosts, Integer zkPort, String zkRootNode) {
    super(isConnectionless, principal, keytab, user);
    this.zkPort = zkPort;
  ...
  public ReadOnlyProps asProps() {
    ...
    if (!DO_NOT_SET_REGISTRY) {
      connectionProps.put(CLIENT_CONNECTION_REGISTRY_IMPL_CONF_KEY, ZK_REGISTRY_NAME);
    }
    if (getZkHosts() != null) {
    ...
    return connectionProps.isEmpty()
  ...
  @Override
  public String toUrl() {
    return PhoenixRuntime.JDBC_PROTOCOL_ZK + PhoenixRuntime.JDBC_PROTOCOL_SEPARATOR + toString();
  }

  @Override
  public ConnectionInfo withPrincipal(String principal) {
    return new ZKConnectionInfo(isConnectionless, principal, keytab, user, zkHosts, zkPort,
      zkRootNode);
  }
  ...
  protected static class Builder extends ConnectionInfo.Builder {
    ...
    /**
     * Detect url with quorum:1,quorum:2 as HBase does not handle different port numbers for
     * different quorum hostnames.
     */
    ...
    private void parse() throws SQLException {
      ...
      boolean first = true;
      while (
        tokenizer.hasMoreTokens() && !(token = tokenizer.nextToken()).equals(TERMINATOR)
