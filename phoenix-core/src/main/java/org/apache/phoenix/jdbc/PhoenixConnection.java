...
public class PhoenixConnection implements Connection, MetaDataMutated, SQLCloseable {
  ...
  private PhoenixConnection(ConnectionQueryServices services, String url, Properties info,
    ...
    long maxSizeBytes =
      this.services.getProps().getLong(QueryServices.MAX_MUTATION_SIZE_BYTES_ATTRIB,
  ...
  public PTable getTable(PTableKey key) throws TableNotFoundException {
    return getTableRef(key).getTable();
  }
  ...
  private void checkOpen() throws SQLException {
    if (isClosed || isClosing) {
      throw new SQLExceptionInfo.Builder(SQLExceptionCode.CONNECTION_CLOSED).build()
        .buildException();
    }
  }
