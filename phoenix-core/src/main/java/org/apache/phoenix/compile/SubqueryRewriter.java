...
/**
 * Class for rewriting where-clause sub-queries into join queries. If the where-clause sub-query is
 * one of those top-node conditions (being the only condition node or direct descendant of AND
 * nodes), we convert the sub-query directly into semi-joins, anti-joins or inner-joins, and
 * meanwhile remove the original condition node from the where clause. Otherwise, we convert the
 * sub-query into left-joins and change the original condition node into a null test of a join table
 * field (ONE if matched, NULL if not matched).
 */
public class SubqueryRewriter extends ParseNodeRewriter {
  ...
  protected ParseNode leaveCompoundNode(CompoundParseNode node, List<ParseNode> children,
    ParseNodeRewriter.CompoundNodeFactory factory) {
    if (topNode == node) {
      topNode = null;
    }
   ...
   */
  @Override
  public ParseNode visitLeave(InParseNode inParseNode, List<ParseNode> childParseNodes)
    throws SQLException {
    ...
    if (whereParseNodeAfterExtract == subquerySelectStatementToUse.getWhere()) {
      ...
      newSubquerySelectAliasedNodes =
        Lists.<AliasedNode> newArrayListWithExpectedSize(oldSubqueryAliasedNodes.size() + 1);

      newSubquerySelectAliasedNodes
        .add(NODE_FACTORY.aliasedNode(ParseNodeFactory.createTempAlias(), LiteralParseNode.ONE));
      this.addNewAliasedNodes(newSubquerySelectAliasedNodes, oldSubqueryAliasedNodes);
      subquerySelectStatementToUse =
        NODE_FACTORY.select(subquerySelectStatementToUse, !inParseNode.isSubqueryDistinct(),
          newSubquerySelectAliasedNodes, whereParseNodeAfterExtract);
    } else {
      /**
      ...
      newSubquerySelectAliasedNodes = Lists.<AliasedNode> newArrayListWithExpectedSize(
        oldSubqueryAliasedNodes.size() + 1 + extractedAdditionalSelectAliasNodes.size());
      ...
      if (!isAggregate) {
        ...
        /**
         * If exists AggregateFunction,we must add the correlated join condition to both the groupBy
         * clause and select lists of the subquery.
         */
        List<ParseNode> newGroupByParseNodes = this.createNewGroupByParseNodes(
          extractedAdditionalSelectAliasNodes, subquerySelectStatementToUse);

        subquerySelectStatementToUse =
          NODE_FACTORY.select(subquerySelectStatementToUse, !inParseNode.isSubqueryDistinct(),
            newSubquerySelectAliasedNodes, whereParseNodeAfterExtract, newGroupByParseNodes, true);
      }
    }

    ParseNode joinOnConditionParseNode =
      getJoinConditionNodeForInSubquery(childParseNodes.get(0), newSubquerySelectAliasedNodes,
        subqueryTableTempAlias, extractedJoinConditionParseNode, extractedSelectAliasNodeCount);
    DerivedTableNode subqueryDerivedTableNode =
      NODE_FACTORY.derivedTable(subqueryTableTempAlias, subquerySelectStatementToUse);
    JoinType joinType =
      isTopNode ? (inParseNode.isNegate() ? JoinType.Anti : JoinType.Semi) : JoinType.Left;
    ParseNode resultWhereParseNode = isTopNode
      ? null
      : NODE_FACTORY.isNull(NODE_FACTORY.column(NODE_FACTORY.table(null, subqueryTableTempAlias),
   ...
   */
  @Override
  public ParseNode visitLeave(ExistsParseNode existsParseNode, List<ParseNode> childParseNodes)
    throws SQLException {
    ...
    if (whereParseNodeAfterExtract == subquerySelectStatementToUse.getWhere()) {
       ...
       */
      subquerySelectStatementToUse = NODE_FACTORY.select(subquerySelectStatementToUse,
      ...
      return super.visitLeave(existsParseNode,
    ...
    List<AliasedNode> newSubquerySelectAliasedNodes =
      Lists.newArrayListWithExpectedSize(extractedAdditionalSelectAliasNodes.size() + 1);
    /**
    ...
    boolean isAggregate = subquerySelectStatementToUse.isAggregate();
    if (!isAggregate) {
      subquerySelectStatementToUse = NODE_FACTORY.select(subquerySelectStatementToUse, true,
        newSubquerySelectAliasedNodes, whereParseNodeAfterExtract);
    } else {
      /**
       * If exists AggregateFunction,we must add the correlated join condition to both the groupBy
    ...
    DerivedTableNode subqueryDerivedTableNode =
      NODE_FACTORY.derivedTable(subqueryTableTempAlias, subquerySelectStatementToUse);
    JoinType joinType =
      isTopNode ? (existsParseNode.isNegate() ? JoinType.Anti : JoinType.Semi) : JoinType.Left;
    ParseNode resultWhereParseNode = isTopNode
      ? null
      : NODE_FACTORY.isNull(NODE_FACTORY.column(NODE_FACTORY.table(null, subqueryTableTempAlias),
  ...
  public ParseNode visitLeave(ComparisonParseNode node, List<ParseNode> l) throws SQLException {
    ...
    if (!isAggregate) {
      ...
      List<ParseNode> groupbyNodes =
        this.createNewGroupByParseNodes(additionalSelectNodes, subquery);
      subquery = NODE_FACTORY.select(subquery, subquery.isDistinct(), selectNodes, where,
        groupbyNodes, true);
    }
  ...
  protected List<ParseNode> leaveArrayComparisonNode(ParseNode node, List<ParseNode> l)
    throws SQLException {
    boolean isTopNode = topNode == node;
    if (isTopNode) {
      topNode = null;
    ...
    if (aliasedNodes.size() == 1) {
      rhsNode = derivedTableAlias == null
        ? aliasedNodes.get(0).getNode()
        : NODE_FACTORY.column(NODE_FACTORY.table(null, derivedTableAlias),
          aliasedNodes.get(0).getAlias(), null);
    } else {
      List<ParseNode> nodes = Lists.<ParseNode> newArrayListWithExpectedSize(aliasedNodes.size());
      for (AliasedNode aliasedNode : aliasedNodes) {
        nodes.add(derivedTableAlias == null
          ? aliasedNode.getNode()
    ...
    if (!isNonGroupByAggregate) {
      rhsNode = NODE_FACTORY.function(DistinctValueAggregateFunction.NAME,
    ...
    if (derivedTableAlias == null) {
      ...
      List<ParseNode> derivedTableGroupBy =
        Lists.newArrayListWithExpectedSize(subquery.getGroupBy().size() + groupbyNodes.size());
      derivedTableGroupBy.addAll(groupbyNodes);
      derivedTableGroupBy.addAll(subquery.getGroupBy());
      List<AliasedNode> derivedTableSelect =
        Lists.newArrayListWithExpectedSize(aliasedNodes.size() + selectNodes.size() - 1);
    ...
    firstChild = NODE_FACTORY.column(NODE_FACTORY.table(null, rhsTableAlias),
  ...
  private SelectStatement fixSubqueryStatement(SelectStatement select) {
    ...
    return NODE_FACTORY.select(
      NODE_FACTORY.derivedTable(ParseNodeFactory.createTempAlias(), select),
  ...
  /**
   * Create new {@link AliasedNode}s by every {@link ParseNode} in subquerySelectAliasedNodes and
   * generate new aliases by {@link ParseNodeFactory#createTempAlias}. and generate new Aliases for
   * subquerySelectAliasedNodes,
   */
  private List<AliasedNode> createNewAliasedNodes(List<AliasedNode> subquerySelectAliasedNodes) {
    List<AliasedNode> newAliasedNodes =
      Lists.<AliasedNode> newArrayListWithExpectedSize(subquerySelectAliasedNodes.size());
  ...
  /**
   * Add every {@link ParseNode} in oldSelectAliasedNodes to newSelectAliasedNodes and generate new
   * aliases by {@link ParseNodeFactory#createTempAlias}.
   */
  private void addNewAliasedNodes(List<AliasedNode> newSelectAliasedNodes,
    List<AliasedNode> oldSelectAliasedNodes) {
    for (int index = 0; index < oldSelectAliasedNodes.size(); index++) {
      AliasedNode oldSelectAliasedNode = oldSelectAliasedNodes.get(index);
      newSelectAliasedNodes.add(NODE_FACTORY.aliasedNode(ParseNodeFactory.createTempAlias(),
   ...
   */
  private ParseNode getJoinConditionNodeForInSubquery(ParseNode lhsParseNode,
    List<AliasedNode> rhsSubquerySelectAliasedNodes, String rhsSubqueryTableAlias,
    ParseNode extractedJoinConditionParseNode, int extractedSelectAliasNodeCount)
    throws SQLException {
    ...
    if (
      ...
      throw new SQLExceptionInfo.Builder(
        SQLExceptionCode.SUBQUERY_RETURNS_DIFFERENT_NUMBER_OF_FIELDS).build().buildException();
    }

    int count = lhsParseNodes.size();
    TableName rhsSubqueryTableName = NODE_FACTORY.table(null, rhsSubqueryTableAlias);
    List<ParseNode> joinEqualParseNodes =
      Lists.newArrayListWithExpectedSize(count + (extractedJoinConditionParseNode == null ? 0 : 1));
    for (int index = 0; index < count; index++) {
      /**
       * The +1 is to skip the first {@link LiteralParseNode#ONE}
       */
      ParseNode rhsNode = NODE_FACTORY.column(rhsSubqueryTableName,
        rhsSubquerySelectAliasedNodes.get(index + 1).getAlias(), null);
    ...
    return joinEqualParseNodes.size() == 1
      ? joinEqualParseNodes.get(0)
      : NODE_FACTORY.and(joinEqualParseNodes);
  }

  /**
   * Combine every {@link ParseNode} in extractedAdditionalSelectAliasNodes and GroupBy clause of
   * the subquerySelectStatementToUse to get new GroupBy ParseNodes.
   */
  private List<ParseNode> createNewGroupByParseNodes(
    List<AliasedNode> extractedAdditionalSelectAliasNodes,
    SelectStatement subquerySelectStatementToUse) {
    List<ParseNode> newGroupByParseNodes =
      Lists.newArrayListWithExpectedSize(extractedAdditionalSelectAliasNodes.size()
  ...
  private static class JoinConditionExtractor extends AndRewriterBooleanParseNodeVisitor {
    ...
    protected ParseNode leaveBooleanNode(ParseNode node, List<ParseNode> l) throws SQLException {
      ...
      if (
        type != ColumnResolveVisitor.ColumnResolveType.NONE
          && type != ColumnResolveVisitor.ColumnResolveType.LOCAL
      ) throw new SQLFeatureNotSupportedException(
        "Does not support non-standard or non-equi correlated-subquery conditions.");
    ...
    @Override
    public ParseNode visitLeave(ComparisonParseNode node, List<ParseNode> l) throws SQLException {
      if (node.getFilterOp() != CompareFilter.CompareOp.EQUAL) return leaveBooleanNode(node, l);
      ...
      if (
        (lhsType == ColumnResolveVisitor.ColumnResolveType.NONE
          || lhsType == ColumnResolveVisitor.ColumnResolveType.LOCAL)
          && (rhsType == ColumnResolveVisitor.ColumnResolveType.NONE
      ...
      if (
        lhsType == ColumnResolveVisitor.ColumnResolveType.OUTER
          && rhsType == ColumnResolveVisitor.ColumnResolveType.LOCAL
      ) {
        String alias = ParseNodeFactory.createTempAlias();
        this.additionalSubselectSelectAliasedNodes
