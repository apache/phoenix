...
public class OrderPreservingTracker {
  ...
  public static class Info {
    ...
    public Info(int rowKeyColumnPosition, int rowKeySlotSpan, OrderPreserving orderPreserving,
  ...
  public OrderPreservingTracker(StatementContext context, GroupBy groupBy, Ordering ordering,
    int nNodes) throws SQLException {
    this(context, groupBy, ordering, nNodes, null, null, null);
  }

  public OrderPreservingTracker(StatementContext context, GroupBy groupBy, Ordering ordering,
    int nNodes, List<OrderBy> inputOrderBys, QueryPlan innerQueryPlan, Expression whereExpression)
    throws SQLException {
    ...
    for (OrderBy inputOrderBy : this.inputOrderBys) {
      this.trackOrderByContexts
        .add(new TrackOrderByContext(isOrderPreserving, nNodes, inputOrderBy));
    }
  }

  /**
   * Infer input OrderBys, if the innerQueryPlan is null, we make the OrderBys from the pk columns
   * of {@link PTable}.
   */
  private void getInputOrderBys(QueryPlan innerQueryPlan, GroupBy groupBy,
    ...
    if (!groupBy.isEmpty()) {
      this.inputOrderBys =
        Collections.singletonList(ExpressionUtil.convertGroupByToOrderBy(groupBy, false));
      return;
    }
  ...
  private class TrackOrderByContext {
    ...
    public TrackOrderByContext(boolean isOrderPreserving, int orderByNodeCount,
      OrderBy inputOrderBy) {
      this.isOrderPreserving = isOrderPreserving;
      this.trackOrderPreservingExpressionVisitor =
        new TrackOrderPreservingExpressionVisitor(inputOrderBy);
    ...
    public void track(List<TrackOrderByCell> trackOrderByCells) {
      for (TrackOrderByCell trackOrderByCell : trackOrderByCells) {
        doTrack(trackOrderByCell.expression, trackOrderByCell.isAscending,
    ...
    public List<Info> getOrderPreservingTrackInfos() {
      ...
      return ImmutableList
        .copyOf(this.orderPreservingTrackInfos.subList(0, orderPreservingColumnCountToUse));
    }
    ...
    public boolean isOrderPreserving() {
      ...
      for (int i = 0; i < orderPreservingTrackInfos.size(); i++) {
        ...
        isOrderPreserving = isOrderPreserving && entry.orderPreserving != OrderPreserving.NO
          && prevOrderPreserving == OrderPreserving.YES
          && (pos == prevPos || pos - prevSlotSpan == prevPos
  ...
  /**
   * Determines if an expression is held constant. Only works for columns in the PK currently, as we
   * only track whether PK columns are held constant.
   */
  private static class IsConstantVisitor extends StatelessTraverseAllExpressionVisitor<Boolean> {
    ...
    public Boolean defaultReturn(Expression node, List<Boolean> returnValues) {
      if (
        !ExpressionUtil.isContantForStatement(node)
  ...
  /**
   * Visitor used to determine if order is preserved across a list of expressions (GROUP BY or ORDER
   * BY expressions)
   */
  private static class TrackOrderPreservingExpressionVisitor
    ...
    private Map<Expression,
      Pair<Integer, OrderByExpression>> expressionToPositionAndOrderByExpression;

    public TrackOrderPreservingExpressionVisitor(OrderBy orderBy) {
      if (orderBy.isEmpty()) {
        this.expressionToPositionAndOrderByExpression =
          Collections.<Expression, Pair<Integer, OrderByExpression>> emptyMap();
        return;
      }
      List<OrderByExpression> orderByExpressions = orderBy.getOrderByExpressions();
      this.expressionToPositionAndOrderByExpression =
        new HashMap<Expression, Pair<Integer, OrderByExpression>>(orderByExpressions.size());
      int index = 0;
      for (OrderByExpression orderByExpression : orderByExpressions) {
        this.expressionToPositionAndOrderByExpression.put(orderByExpression.getExpression(),
    ...
    private Info match(Expression expression) {
      Pair<Integer, OrderByExpression> positionAndOrderByExpression =
        this.expressionToPositionAndOrderByExpression.get(expression);
      if (positionAndOrderByExpression == null) {
        return null;
      }
      return new Info(positionAndOrderByExpression.getFirst(),
    ...
    public Iterator<Expression> visitEnter(ScalarFunction node) {
      return node.preservesOrder() == OrderPreserving.NO
        ? Collections.<Expression> emptyIterator()
        : Iterators.singletonIterator(node.getChildren().get(node.getKeyFormationTraversalIndex()));
    }
    ...
    public Info visitLeave(ScalarFunction node, List<Info> l) {
      ...
      OrderPreserving orderPreserving = OrderPreserving.values()[Math
        .min(node.preservesOrder().ordinal(), info.orderPreserving.ordinal())];
      ...
      return new Info(info.pkPosition, info.slotSpan, orderPreserving,
        sortOrderIsSame ? info.ascending : !info.ascending, info.nullsLast);
    }
    ...
    public Info visitLeave(RowValueConstructorExpression node, List<Info> l) {
      ...
      return new Info(firstInfo.pkPosition, l.size(), lastInfo.orderPreserving, lastInfo.ascending,
        lastInfo.nullsLast);
    }
  }
}
