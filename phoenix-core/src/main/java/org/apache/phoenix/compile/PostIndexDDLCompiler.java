...
public class PostIndexDDLCompiler {
  ...
  public MutationPlan compile(final PTable indexTable) throws SQLException {
    ...
    updateStmtStr.append("UPSERT /*+ NO_INDEX */ INTO ")
      .append(schemaName.length() == 0 ? "" : '"' + schemaName + "\".").append('"')
      .append(tableName).append("\"(").append(indexColumns).append(") ");
    ...
    try (final PhoenixStatement statement = new PhoenixStatement(connection)) {
      DelegateMutationPlan delegate =
        new DelegateMutationPlan(statement.compileMutation(updateStmtStr.toString())) {
          @Override
