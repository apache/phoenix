...
/**
 * Class that compiles plan to update data values after a DDL command executes. TODO: get rid of
 * this ugly code and just go through the standard APIs. The only time we may still need this is to
 * manage updating the empty key value, as we sometimes need to "go back through time" to adjust
 * this.
 * @since 0.1
 */
public class PostDDLCompiler {
  ...
  public MutationPlan compile(final List<TableRef> tableRefs, final byte[] emptyCF,
    final List<byte[]> projectCFs, final List<PColumn> deleteList, final long timestamp)
    throws SQLException {
    PhoenixStatement statement = new PhoenixStatement(connection);
    final StatementContext context = new StatementContext(statement,
      new MultipleTableRefColumnResolver(tableRefs), scan, new SequenceManager(statement));
  ...
  private class PostDDLMutationPlan extends BaseMutationPlan {
    ...
    public PostDDLMutationPlan(StatementContext context, List<TableRef> tableRefs, long timestamp,
      byte[] emptyCF, List<PColumn> deleteList, List<byte[]> projectCFs) {
    ...
    public MutationState execute() throws SQLException {
      ...
      try {
        ...
        /*
         * Handles: 1) deletion of all rows for a DROP TABLE and subsequently deletion of all rows
         * for a DROP INDEX; 2) deletion of all column values for a ALTER TABLE DROP COLUMN 3)
         * updating the necessary rows to have an empty KV 4) updating table stats
         */
        ...
        for (final TableRef tableRef : tableRefs) {
          ...
          long ts = timestamp;
          // FIXME: DDL operations aren't transactional, so we're basing the timestamp on a server
          // timestamp.
          // Not sure what the fix should be. We don't need conflict detection nor filtering of
          ...
          try {
            if (deleteList != null) {
              if (deleteList.isEmpty()) {
                ...
                // In the case of a row deletion, add index metadata so mutable secondary indexing
                // works
                /*
                 * TODO: we currently manually run a scan to delete the index data here
                 * ImmutableBytesWritable ptr = context.getTempPtr();
                 * tableRef.getTable().getIndexMaintainers(ptr); if (ptr.getLength() > 0) {
                 * IndexMetaDataCacheClient client = new IndexMetaDataCacheClient(connection,
                 * tableRef); cache = client.addIndexMetadataCache(context.getScanRanges(), ptr);
                 * byte[] uuidValue = cache.getId(); scan.setAttribute(PhoenixIndexCodec.INDEX_UUID,
                 * uuidValue); }
                 */
              } else {
            ...
            // Need to project all column families into the scan, since we haven't yet created our
            // empty key value
            RowProjector projector = ProjectionCompiler.compile(context, SelectStatement.COUNT_ONE,
