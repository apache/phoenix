...
public class UpdateStatisticsTool extends Configured implements Tool {
  ...
  private static final Option SNAPSHOT_NAME_OPTION =
    new Option("s", "snapshot", true, "HBase Snapshot Name");
  private static final Option RESTORE_DIR_OPTION =
    new Option("d", "restore-dir", true, "Restore Directory for HBase snapshot");
  private static final Option JOB_PRIORITY_OPTION =
    new Option("p", "job-priority", true, "Define job priority from 0(highest) to 4");
  ...
  /**
   * Run any tasks before the MR job is launched Currently being used for snapshot creation
   */
  private void preJobTask() throws Exception {
    ...
    try (final Connection conn = ConnectionUtil.getInputConnection(getConf())) {
      ...
      boolean namespaceMapping =
        getConf().getBoolean(IS_NAMESPACE_MAPPING_ENABLED, DEFAULT_IS_NAMESPACE_MAPPING_ENABLED);
      String physicalTableName = SchemaUtil
        .getPhysicalTableName(tableName.getBytes(StandardCharsets.UTF_8), namespaceMapping)
        .getNameAsString();
  ...
  CommandLine parseOptions(String[] args) {
    ...
    if (!cmdLine.hasOption(TABLE_NAME_OPTION.getOpt())) {
      throw new IllegalStateException(
        TABLE_NAME_OPTION.getLongOpt() + " is a mandatory " + "parameter");
    }

    if (
      cmdLine.hasOption(MANAGE_SNAPSHOT_OPTION.getOpt())
