...
public enum PTableType {
  ...
  private static final Map<String, PTableType> FROM_VALUE =
    Maps.newHashMapWithExpectedSize(PTableType.values().length);
    ...
    for (PTableType type : PTableType.values()) {
      if (FROM_VALUE.put(type.getValue().getString(), type) != null) {
        throw new IllegalStateException(
          "Duplicate PTableType value of " + type.getValue().getString() + " is not allowed");
      }
    }
  }
  ...
  public static PTableType fromSerializedValue(String serializedValue) {
    ...
    throw new IllegalArgumentException(
      "Unable to PTableType enum for serialized value of '" + serializedValue + "'");
  }
  ...
  public static PTableType fromSerializedValue(byte serializedByte) {
    ...
    throw new IllegalArgumentException(
      "Unable to PTableType enum for serialized value of '" + (char) serializedByte + "'");
  }
}
