...
public class StatisticsUtils {
    ...
    public static byte[] getCFFromRowKey(byte[] table, byte[] row) {
        // Move over the the sepeartor byte that would be written after the table name
        int startOff = indexOf(row, table) + (table.length) + 1;
        int endOff = startOff;
        while (endOff < row.length) {
    ...
    public static byte[] copyRow(KeyValue kv) {
        return Arrays.copyOfRange(kv.getBuffer(), kv.getRowOffset(), kv.getRowOffset() + kv.getRowLength());
    }
    
    /**
     * Copied from org.apache.hadoop.hbase.util.Bytes.java
     * @param array
     * @param target
     * @return
     */
