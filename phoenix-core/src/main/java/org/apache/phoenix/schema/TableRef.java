...
public class TableRef {
  ...
  private static TableRef createEmptyTableRef() {
    try {
      return new TableRef(new PTableImpl.Builder().setIndexes(Collections.emptyList())
  ...
  public TableRef(TableRef tableRef) {
    this(tableRef.alias, tableRef.table, tableRef.upperBoundTimeStamp, tableRef.lowerBoundTimeStamp,
  ...
  public TableRef(String alias, PTable table, long upperBoundTimeStamp, long lowerBoundTimeStamp,
    ...
    this.upperBoundTimeStamp =
      table.getUpdateCacheFrequency() != 0 ? QueryConstants.UNSET_TIMESTAMP : upperBoundTimeStamp;
  ...
  public String getColumnDisplayName(ColumnRef ref, boolean cfCaseSensitive,
    ...
    if (
      (table.getType() == PTableType.PROJECTED
      ...
      String dataFamilyName =
        isIndex ? IndexUtil.getDataColumnFamilyName(name) : column.getFamilyName().getString();
      cf = (table.getIndexType() == IndexType.LOCAL
        ? IndexUtil.getActualColumnFamilyName(defaultFamilyName)
        : defaultFamilyName).equals(dataFamilyName) ? null : dataFamilyName;
      cq = isIndex ? IndexUtil.getDataColumnName(name) : name;
    }
