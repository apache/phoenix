...
public class Task {
  ...
  private static void mutateSystemTaskTable(PhoenixConnection conn, PreparedStatement stmt,
  ...
  private static List<Mutation> getMutationsForSystemTaskTable(PhoenixConnection conn,
    PreparedStatement stmt, boolean accessCheckEnabled) throws IOException {
    // we need to mutate SYSTEM.TASK with HBase/login user if access is enabled.
    if (accessCheckEnabled) {
  ...
  private static List<Mutation> executeStatementAndGetTaskMutations(PhoenixConnection conn,
  ...
  private static PreparedStatement setValuesToAddTaskPS(PreparedStatement stmt,
    PTable.TaskType taskType, String tenantId, String schemaName, String tableName,
    String taskStatus, String data, Integer priority, Timestamp startTs, Timestamp endTs)
    throws SQLException {
  ...
  /**
   * Execute and commit upsert query on SYSTEM.TASK This method should be used only from server
   * side. Client should use {@link #getMutationsForAddTask(SystemTaskParams)} instead of direct
   * upsert commit.
   * @param systemTaskParams Task params with various task related arguments
   * @throws IOException If something goes wrong while preparing mutations or committing
   *                     transactions
   */
  ...
  private static PreparedStatement addTaskAndGetStatement(SystemTaskParams systemTaskParams,
    PhoenixConnection connection, boolean shouldCommit) throws IOException {
    PreparedStatement stmt;
    try {
      stmt = connection.prepareStatement("UPSERT INTO " + PhoenixDatabaseMetaData.SYSTEM_TASK_NAME
        ...
        + ", " + PhoenixDatabaseMetaData.TASK_STATUS + ", " + PhoenixDatabaseMetaData.TASK_PRIORITY
        + ", " + PhoenixDatabaseMetaData.TASK_TS + ", " + PhoenixDatabaseMetaData.TASK_END_TS + ", "
        + PhoenixDatabaseMetaData.TASK_DATA + " ) VALUES(?,?,?,?,?,?,?,?,?)");
      stmt = setValuesToAddTaskPS(stmt, systemTaskParams.getTaskType(),
        systemTaskParams.getTenantId(), systemTaskParams.getSchemaName(),
        systemTaskParams.getTableName(), systemTaskParams.getTaskStatus(),
        systemTaskParams.getData(), systemTaskParams.getPriority(), systemTaskParams.getStartTs(),
        systemTaskParams.getEndTs());
      LOGGER.info(
        "Adding task type: {} , tableName: {} , taskStatus: {}" + " , startTs: {} , endTs: {}",
        systemTaskParams.getTaskType(), systemTaskParams.getTableName(),
    ...
    // mutations
    if (shouldCommit) {
      mutateSystemTaskTable(connection, stmt, systemTaskParams.isAccessCheckEnabled());
    }
    return stmt;
  }

  public static List<Mutation> getMutationsForAddTask(SystemTaskParams systemTaskParams)
    ...
    try (PhoenixConnection newConnection = QueryUtil
      .getConnectionOnServer(curConn.getClientInfo(), conf).unwrap(PhoenixConnection.class)) {
      PreparedStatement statement = addTaskAndGetStatement(systemTaskParams, newConnection, false);
      return getMutationsForSystemTaskTable(newConnection, statement,
   ...
   * @param callable   used to invoke the coprocessor endpoint to upsert records in SYSTEM.TASK
  ...
  public static MetaDataMutationResult taskMetaDataCoprocessorExec(
    ...
    final Batch.Call<TaskMetaDataService, MetaDataResponse> callable) throws SQLException {
    TableName tableName = SchemaUtil.getPhysicalName(PhoenixDatabaseMetaData.SYSTEM_TASK_NAME_BYTES,
      connection.getQueryServices().getProps());
    try (Table table = connection.getQueryServices().getTable(tableName.getName())) {
      final Map<byte[], MetaDataResponse> results =
        table.coprocessorService(TaskMetaDataService.class, rowKey, rowKey, callable);
  ...
  public static void deleteTask(PhoenixConnection conn, PTable.TaskType taskType, Timestamp ts,
    String tenantId, String schemaName, String tableName, boolean accessCheckEnabled)
    throws IOException {
    PreparedStatement stmt = null;
    try {
      stmt = conn.prepareStatement("DELETE FROM " + PhoenixDatabaseMetaData.SYSTEM_TASK_NAME
        + " WHERE " + PhoenixDatabaseMetaData.TASK_TYPE + " = ? AND "
        + PhoenixDatabaseMetaData.TASK_TS + " = ? AND " + PhoenixDatabaseMetaData.TENANT_ID
  ...
  public static List<TaskRecord> queryTaskTable(Connection connection, Timestamp ts, String schema,
    String tableName, PTable.TaskType taskType, String tenantId, String indexName)
    throws SQLException {
    String taskQuery = "SELECT " + PhoenixDatabaseMetaData.TASK_TS + ", "
      + PhoenixDatabaseMetaData.TENANT_ID + ", " + PhoenixDatabaseMetaData.TABLE_SCHEM + ", "
      + PhoenixDatabaseMetaData.TABLE_NAME + ", " + PhoenixDatabaseMetaData.TASK_STATUS + ", "
      + PhoenixDatabaseMetaData.TASK_TYPE + ", " + PhoenixDatabaseMetaData.TASK_PRIORITY + ", "
      + PhoenixDatabaseMetaData.TASK_DATA + " FROM " + PhoenixDatabaseMetaData.SYSTEM_TASK_NAME;
    taskQuery += " WHERE " + PhoenixDatabaseMetaData.TABLE_NAME + " ='" + tableName + "' AND "
    ...
    if (ts != null) {
      // Adding TASK_TS to the where clause did not work. It returns empty when directly querying
      // with the timestamp.
  ...
  public static TaskRecord parseResult(ResultSet rs) throws SQLException {
    ...
    taskRecord.setTaskType(
      PTable.TaskType.fromSerializedValue(rs.getByte(PhoenixDatabaseMetaData.TASK_TYPE)));
