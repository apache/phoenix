...
public class StatisticsScanner implements InternalScanner {
  ...
  public void close() throws IOException {
    ...
    try {
      ...
      // TODO : here the tenant id and schema are considered to be null.
      byte[] tableKey = SchemaUtil.getTableKey(HConstants.EMPTY_BYTE_ARRAY,
                HConstants.EMPTY_BYTE_ARRAY, region.getTable().getName());
      stats.updateStats(
          new StatisticsWriter(tableKey, Bytes.toBytes(region
              .getRegionNameAsString())), this.tracker, family);
    } catch (IOException e) {
