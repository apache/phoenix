...
public class SaltingUtil {
  ...
  public static final PColumnImpl SALTING_COLUMN = new PColumnImpl(
    PNameFactory.newName(SALTING_COLUMN_NAME), null, PBinary.INSTANCE, 1, 0, false, 0,
    SortOrder.getDefault(), 0, null, false, null, false, false, null, HConstants.LATEST_TIMESTAMP);
  public static final RowKeySchema VAR_BINARY_SALTED_SCHEMA =
    new RowKeySchemaBuilder(2).addField(SALTING_COLUMN, false, SortOrder.getDefault())
  ...
  public static List<KeyRange> generateAllSaltingRanges(int bucketNum) {
    ...
    for (int i = 0; i < bucketNum; i++) {
      ...
      allRanges.add(
        SALTING_COLUMN.getDataType().getKeyRange(saltByte, true, saltByte, true, SortOrder.ASC));
    }
    return allRanges;
  }
  ...
  public static void addRegionStartKeyToScanStartAndStopRows(byte[] startKey, byte[] endKey,
    ...
    System.arraycopy(scan.getStartRow(), 0, newStartRow, prefixBytes.length,
    ...
    if (scan.getStopRow().length != 0) {
      ...
      System.arraycopy(scan.getStopRow(), 0, newStopRow, prefixBytes.length,
