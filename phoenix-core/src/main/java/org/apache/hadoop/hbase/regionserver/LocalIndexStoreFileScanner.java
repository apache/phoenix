...
public class LocalIndexStoreFileScanner extends StoreFileScanner{
    ...
    public LocalIndexStoreFileScanner(Reader reader, HFileScanner hfs, boolean useMVCC,
        ...
        this.reader = ((IndexHalfStoreFileReader)reader);
    ...
    public Cell next() throws IOException {
        ...
        if (next!=null && (reader.isTop() || changeBottomKeys)) {
            next = getChangedKey(next, changeBottomKeys);
        } 
        return next;
    }
    ...
    public Cell peek() {
        ...
        if (peek != null && (reader.isTop() || changeBottomKeys)) {
            peek = getChangedKey(peek, changeBottomKeys);
        } 
        return peek;
    }
    ...
    public boolean seekToLastRow() throws IOException {
        ...
        while(super.peek()!=null && isSatisfiedMidKeyCondition(super.peek())) {
            seekToLastRow = seekToPreviousRow(super.peek());
        }
    ...
    private KeyValue getKeyPresentInHFiles(byte[] key) {
        ...
        if (keyValue.getValueLength() ==0
            ...
            // TODO check if the row part includes the index id then then suffix index id to the
            // region start key in hfile to seek to exact index data.
            KeyValue createFirstDeleteFamilyOnRow =
                    KeyValueUtil.createFirstDeleteFamilyOnRow(reader.getRegionStartKeyInHFile(), keyValue.getFamily());
            return createFirstDeleteFamilyOnRow;
        }

        short length = (short) (keyValue.getRowLength() - reader.getSplitRow().length + reader.getOffset());
        byte[] replacedKey =
